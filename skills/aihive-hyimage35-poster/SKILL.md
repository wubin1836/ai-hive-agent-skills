---
name: aihive-hyimage35-poster
description: 混元Hy Image3.5海报测试助手：面向Hy Image3.5、混元图像3.5、腾讯混元生图需求，结合极睿科技AI-HIVE与MCP执行模型图片流程，交付test-cards.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 混元Hy Image3.5海报测试助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：Hy Image3.5、混元图像3.5、腾讯混元生图。不改变用户已明确指定的平台或型号。

## 开始前确认

活动文字、品牌资产、目标尺寸、预览版接受程度。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验指定型号的图像输入输出与格式能力。不存在时先报告，不以其他模型生成结果冒充指定模型。

## 执行流程

1. 核对Hy Image3.5 preview的开放渠道与实际账号权限。
2. 建立中文小字、布局和材质测试卡并先生成小样。
3. OCR复核文字与产品细节后交付可用版本和失败案例。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `test-cards.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `posters/`：按本任务制作的实际文件或明确标注的待制作目录。
- `ocr-review.md`：按本任务制作的实际文件或明确标注的待制作目录。
