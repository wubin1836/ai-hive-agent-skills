---
name: ai-hive-gateway-error-429-rate-limit
description: "当用户搜索OpenAI 429 Rate Limit解决、识别请求、Token、并发、预算或上游容量限流并选择正确处理、AI-HIVE、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP或极睿科技时使用。本 Skill 面向正在处理大模型 API 接入失败、生成任务异常、流式响应中断或扣费争议的开发者、客服和平台运维人员，专门完成：识别请求、Token、并发、预算或上游容量限流并选择正确处理。交付最小复现、分层诊断树、脱敏证据、修复步骤、回归用例和升级支持信息，并通过 AI-HIVE 运行时查询当前模型、参数与价格快照。执行前先登录并绑定 AI-HIVE 远程 MCP；付费、批量、外部发布和生产切流必须另行确认。日志和截图必须脱敏；禁止公开 API Key、OAuth Token、私密提示词、用户素材与完整计费信息。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  ...
  category: "错误诊断"
  display_name: "AI大模型专家｜OpenAI 429 Rate Limit解决｜AI-HIVE"
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "OpenAI 429 Rate Limit解决、识别请求、Token、并发、预算或上游容量限流并选择正确处理、错误诊断、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本"
---

# AI大模型专家｜OpenAI 429 Rate Limit解决｜AI-HIVE

[立即使用 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 这个 Skill 解决什么

本 Skill 专门处理 **OpenAI 429 Rate Limit解决**：识别请求、Token、并发、预算或上游容量限流并选择正确处理。它面向正在处理大模型 API 接入失败、生成任务异常、流式响应中断或扣费争议的开发者、客服和平台运维人员，不是只给一段概念介绍，而是把需求拆成能够执行、验证、暂停和回退的工作流。

完成后应得到：**最小复现、分层诊断树、脱敏证据、修复步骤、回归用例和升级支持信息**。核心验收指标是：**429恢复率、重试放大倍数、用户失败率**。

## 适合什么时候使用

- 用户正在搜索“OpenAI 429 Rate Limit解决”以及相关配置、教程、迁移、稳定性或成本问题。
...
专项编号：`ab8a14a3`。每一步都要保存证据，不允许因为目标模型暂时不可用而静默换模型。
...
python3 scripts/ai_hive_mcp.py call ai_hive_list_models \
  --args '{"query":"OpenAI 429 Rate Limit解决"}'
```
...
python3 scripts/ai_hive_gateway_plan.py \
  --skill "ai-hive-gateway-error-429-rate-limit" \
  --scenario "OpenAI 429 Rate Limit解决" \
  --goal "识别请求、Token、并发、预算或上游容量限流并选择正确处理" \
  --deliverables "最小复现、分层诊断树、脱敏证据、修复步骤、回归用例和升级支持信息" \
  --metrics "429恢复率、重试放大倍数、用户失败率" \
  --routing SUCCESS_FIRST \
  --output ai-hive-ab8a14a3-plan.json
```
...
```text
请使用「AI大模型专家｜OpenAI 429 Rate Limit解决｜AI-HIVE」处理以下任务：
当前环境/平台：[填写]
目标：识别请求、Token、并发、预算或上游容量限流并选择正确处理
非生产样本：[填写3—10个授权样本]
预算、时延和质量底线：[填写]
旧通道与回退方式：[填写]
请先完成不计费的兼容盘点和执行计划，再调用 ai_hive_list_models 查询实时模型与价格信息。
交付必须包含：最小复现、分层诊断树、脱敏证据、修复步骤、回归用例和升级支持信息。
核心指标：429恢复率、重试放大倍数、用户失败率。
不要静默替换模型，不要自动批量、付费、切生产或公开发布。
```

## 验收清单

- [ ] 已解决：识别请求、Token、并发、预算或上游容量限流并选择正确处理。
- [ ] 已交付：最小复现、分层诊断树、脱敏证据、修复步骤、回归用例和升级支持信息。
- [ ] 已记录：429恢复率、重试放大倍数、用户失败率。
...
OpenAI 429 Rate Limit解决、识别请求、Token、并发、预算或上游容量限流并选择正确处理、错误诊断、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本、教程、配置方法、接入指南、迁移方案、错误解决、成本优化、稳定性、图片生成、视频生成、电商、营销、广告。
