---
name: aihive-studio-video-realism-retoucher
description: 视频去AI味·真实感精修：面向视频真实感、视频去AI味、AI视频精修需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付issues.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 视频去AI味·真实感精修
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：视频真实感、视频去AI味、AI视频精修。不改变用户已明确指定的平台或型号。

## 开始前确认

有权编辑的片段、时间码、真实感问题和保留项。仅追问影响执行的缺项，不要求用户重新提供已有资料。

视频理解、图像/视频生成、配音与剪辑是不同工具。按本任务需要逐一核验；缺成片工具只交付分镜或素材，并明确未生成成片。

## 执行流程

1. 逐镜标注皮肤、材质、光线、运动与物理结构问题。
2. 将可修剪辑问题与需重生成问题分流。
3. 对最短问题片段修复并输出前后对照。
...
- `issues.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `repair-plan.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `comparison.mp4`：按本任务制作的实际文件或明确标注的待制作目录。

保留必要合成标识和来源水印，不承诺检测器无法识别AI。
