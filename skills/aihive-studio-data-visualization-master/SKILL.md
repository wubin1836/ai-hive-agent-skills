---
name: aihive-studio-data-visualization-master
description: 数据可视化全域大师：面向数据可视化、数据图表、可视化报告需求，结合极睿科技AI-HIVE与MCP执行分析流程，交付clean-data.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 数据可视化全域大师
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 分析
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：数据可视化、数据图表、可视化报告。不改变用户已明确指定的平台或型号。

## 开始前确认

数据文件、字段含义、单位、关注问题和图表用途。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 检查缺失、重复、异常及时间口径。
2. 按问题选择图形并实际计算统计量。
3. 生成可追溯图表与数据副本并校对坐标和单位。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `clean-data.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `charts/`：按本任务制作的实际文件或明确标注的待制作目录。
- `methodology.md`：按本任务制作的实际文件或明确标注的待制作目录。
