...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "制作高质量精准文字图片，主体准确，商业信息清晰", "keywords": "精准文字图片、图片文字、精准排版、海报文字、中文文字生成、文字准确、GPT Image 2、Image2、Nano Banana、AI图片、AIGC图片", "model": "public_model_gpt_image_2", "name": "exact-text-image", "rule": "optional", "search": "通用图片能力和商业场景搜索入口，底层调用 AI Hive 裸接口", "title": "精准文字图片"}')
