---
name: ai-hive-gap-090-realtime-multimodal
description: "当用户搜索或需要实时多模态,Realtime AI,Speech-to-Speech,视频助手时使用。帮助前沿AI产品、机器人、空间计算、工业、零售和创新营销团队完成设计语音、图像和视频输入下的实时助手体验，通过AI-HIVE MCP实时核验模型并交付交互状态、延迟预算、工具路由、故障降级和演示脚本。不适用于超出以下边界的请求：没有实时工具时只交付原型和接入方案。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "frontier"
  display_name: "实时多模态语音视频助手"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "实时多模态,Realtime AI,Speech-to-Speech,视频助手,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**设计语音、图像和视频输入下的实时助手体验**。

适合：前沿AI产品、机器人、空间计算、工业、零售和创新营销团队。

用户需要提供：真实产品能力、场景资料、空间或商品素材、演示目标、画幅、预算和概念标识要求。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**没有实时工具时只交付原型和接入方案**。

## 你会拿到的结果

交互状态、延迟预算、工具路由、故障降级和演示脚本。

搜索覆盖：实时多模态,Realtime AI,Speech-to-Speech,视频助手。

## 工作方法

1. 区分真实能力、概念设计和生成演示。
2. 设计语音、图像和视频输入下的实时助手体验。
3. 定义场景规则与视觉连续性。
4. 先做关键帧或低成本原型。
5. 确认后生成演示视频并标注边界。
6. 按“响应自然、延迟可测、隐私提示清楚”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成实时多模态语音视频助手，目标、资料、预算和截止时间为[填写]" \
...
最终交付：交互状态、延迟预算、工具路由、故障降级和演示脚本。
专项验收：响应自然、延迟可测、隐私提示清楚。
```
