---
name: aihive-studio-product-on-model-locale-studio
description: AI多地区模特上身图工作室：面向AI模特上身图、跨境模特图、服装试穿图需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付garment-lock.json等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: AI多地区模特上身图工作室
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：AI模特上身图、跨境模特图、服装试穿图。不改变用户已明确指定的平台或型号。

## 开始前确认

服装产品图、授权模特参考、尺码、目标市场。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验图像生成或编辑、参考图输入、尺寸与下载工具。准确文字、透明通道、二维码要用真实文件工具另行检查。

## 执行流程

1. 锁定服装版型、花色、扣位和不能修改的商标。
2. 根据使用场景选模特与背景而不推断敏感身份。
3. 先做单款试穿预览检查遮挡再批量出图。
...
- `garment-lock.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `model-brief.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `tryon-images/`：按本任务制作的实际文件或明确标注的待制作目录。
