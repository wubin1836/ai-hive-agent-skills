---
name: aihive-studio-crm
description: CRM客户档案补全与商机洞察：面向CRM补全、客户档案补全、商机洞察需求，结合极睿科技AI-HIVE与MCP执行运营流程，交付crm-clean.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: CRM客户档案补全与商机洞察
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 运营
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：CRM补全、客户档案补全、商机洞察。不改变用户已明确指定的平台或型号。

## 开始前确认

授权导出的CRM、去重规则、公开公司资料和业务目标。仅追问影响执行的缺项，不要求用户重新提供已有资料。

优先交付可配置规则、SOP和数据文件。部署、CRM写回、发送消息或投放需要真实系统接口和单独授权。

## 执行流程

1. 标准化公司名与域名并保留合并前ID。
2. 仅用合法来源补业务字段，冲突字段待复核。
3. 形成商机假设与写回预览，获授权后才更新系统。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `crm-clean.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `enrichment-evidence.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `writeback-preview.csv`：按本任务制作的实际文件或明确标注的待制作目录。
