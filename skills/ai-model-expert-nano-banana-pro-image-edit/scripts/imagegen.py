...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana Pro 制作高完成度图片编辑，主体准确，构图清晰", "keywords": "Nano Banana Pro、NanoBanana Pro、NanoBananaPro、香蕉 Pro、Gemini 图片、图片编辑、AI修图、改图、P图、局部修改、图片重绘、照片编辑、AI图片、AIGC图片", "model": "public_model_nano_banana_pro", "name": "nano-banana-pro-image-edit", "rule": "image", "search": "固定调用 Nano Banana Pro 的图片编辑裸接口能力", "title": "Nano Banana Pro 图片编辑"}')
