---
name: ai-hive-gap-077-agent-guardrails
description: "当用户搜索或需要Guardrails,Agent护栏,AI安全,工具权限时使用。帮助企业安全、法务合规、AI治理、内容审核和平台工程团队完成为智能体工具调用、内容生成和外部操作设置护栏，通过AI-HIVE MCP实时核验模型并交付允许动作、禁止动作、确认门、预算限制和终止条件。不适用于超出以下边界的请求：护栏不应偷偷改变用户目标。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "security"
  display_name: "Agent Guardrails安全护栏"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "Guardrails,Agent护栏,AI安全,工具权限,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
  release_variant: "ai-hive-keyword-gap-96-20260906"
---

# Agent Guardrails安全护栏

[立即使用 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 需求识别

这个 Skill 解决的具体问题是：**为智能体工具调用、内容生成和外部操作设置护栏**。

适合：企业安全、法务合规、AI治理、内容审核和平台工程团队。

用户需要提供：待审计文本或流程、数据分类、工具权限、风险级别、适用制度和人工负责人。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**护栏不应偷偷改变用户目标**。

## 你会拿到的结果

允许动作、禁止动作、确认门、预算限制和终止条件。

搜索覆盖：Guardrails,Agent护栏,AI安全,工具权限。

## 执行蓝图

1. 识别资产、数据和工具权限。
2. 为智能体工具调用、内容生成和外部操作设置护栏。
3. 建立风险情景与测试样例。
4. 提出最小权限和确认门。
5. 保留审计证据与人工复核。
6. 按“越权被拦截、关键动作可确认、失败安全”完成专项验收。
...
python3 scripts/ai_hive_mcp.py call ai_hive_list_models \
  --args '{"query":"Agent Guardrails安全护栏"}'
```
...
python3 scripts/plan.py \
  --brief "我要完成Agent Guardrails安全护栏，目标、资料、预算和截止时间为[填写]" \
...
```text
请使用「Agent Guardrails安全护栏」Skill帮助我。
我的目标：[填写]
受众和业务场景：[填写]
已有资料与来源：[填写]
交付格式和数量：[填写]
...
最终交付：允许动作、禁止动作、确认门、预算限制和终止条件。
专项验收：越权被拦截、关键动作可确认、失败安全。
```
