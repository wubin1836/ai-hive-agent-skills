---
name: aihive-studio-ip-character-consistency-studio
description: 角色一致性形象包：面向角色一致性、IP形象包、人物一致性需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付character-bible.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 角色一致性形象包
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：角色一致性、IP形象包、人物一致性。不改变用户已明确指定的平台或型号。

## 开始前确认

原创或已授权角色、三视图参考、服装和使用场景。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验图像生成或编辑、参考图输入、尺寸与下载工具。准确文字、透明通道、二维码要用真实文件工具另行检查。

## 执行流程

1. 建立角色脸部、比例、配色和标记点档案。
2. 为不同表情与动作编制变化表。
3. 固定参考图逐批生成并做一致性对照。
...
- `pose-matrix.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `character-pack/`：按本任务制作的实际文件或明确标注的待制作目录。
