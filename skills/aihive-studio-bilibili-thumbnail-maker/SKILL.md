---
name: aihive-studio-bilibili-thumbnail-maker
description: B站视频封面制作器：面向B站封面、哔哩哔哩封面需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付thumbnail-brief.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: B站视频封面制作器
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：B站封面、哔哩哔哩封面。不改变用户已明确指定的平台或型号。

## 开始前确认

视频主题、主体素材、标题、系列风格和封面尺寸。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
- `thumbnail-brief.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `thumbnails/`：按本任务制作的实际文件或明确标注的待制作目录。
- `mobile-preview/`：按本任务制作的实际文件或明确标注的待制作目录。

封面与视频实际内容对应，图片不冒充未发生事件。
