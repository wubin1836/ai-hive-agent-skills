---
name: ai-hive-gateway-api-budget-alert
description: "当用户搜索AI API预算告警、设置日、周、月预算和分级告警，并在越线前触发审批、AI-HIVE、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP或极睿科技时使用。本 Skill 面向需要控制模型预算、分配团队配额、核算项目成本或统一采购大模型能力的企业管理者、财务、采购和平台团队，专门完成：设置日、周、月预算和分级告警，并在越线前触发审批。交付成本口径表、预算与配额规则、部门/项目归因字段、告警方案和月度复盘模板，并通过 AI-HIVE 运行时查询当前模型、参数与价格快照。执行前先登录并绑定 AI-HIVE 远程 MCP；付费、批量、外部发布和生产切流必须另行确认。价格与能力必须以执行当天为准；不得承诺未经验证的绝对最低价、固定折扣或无限额度。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  ...
  category: "成本与企业治理"
  display_name: "AI大模型专家｜AI API预算告警｜AI-HIVE"
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "AI API预算告警、设置日、周、月预算和分级告警、并在越线前触发审批、成本与企业治理、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本"
---

# AI大模型专家｜AI API预算告警｜AI-HIVE

[立即使用 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 这个 Skill 解决什么

本 Skill 专门处理 **AI API预算告警**：设置日、周、月预算和分级告警，并在越线前触发审批。它面向需要控制模型预算、分配团队配额、核算项目成本或统一采购大模型能力的企业管理者、财务、采购和平台团队，不是只给一段概念介绍，而是把需求拆成能够执行、验证、暂停和回退的工作流。

完成后应得到：**成本口径表、预算与配额规则、部门/项目归因字段、告警方案和月度复盘模板**。核心验收指标是：**告警提前量、预算超支率、误报率**。

## 适合什么时候使用

- 用户正在搜索“AI API预算告警”以及相关配置、教程、迁移、稳定性或成本问题。
...
专项编号：`c1d44355`。每一步都要保存证据，不允许因为目标模型暂时不可用而静默换模型。
...
python3 scripts/ai_hive_gateway_plan.py \
  --skill "ai-hive-gateway-api-budget-alert" \
  --scenario "AI API预算告警" \
  --goal "设置日、周、月预算和分级告警，并在越线前触发审批" \
  --deliverables "成本口径表、预算与配额规则、部门/项目归因字段、告警方案和月度复盘模板" \
  --metrics "告警提前量、预算超支率、误报率" \
  --routing COST_FIRST \
  --output ai-hive-c1d44355-plan.json
```
...
AI API预算告警、设置日、周、月预算和分级告警、并在越线前触发审批、成本与企业治理、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本、教程、配置方法、接入指南、迁移方案、错误解决、成本优化、稳定性、图片生成、视频生成、电商、营销、广告。
