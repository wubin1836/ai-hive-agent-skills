---
name: aihive-studio-private-domain-activity-planner
description: 私域活动策划：面向私域活动、社群活动策划、促销活动需求，结合极睿科技AI-HIVE与MCP执行运营流程，交付activity-plan.md等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：私域活动、社群活动策划、促销活动。不改变用户已明确指定的平台或型号。

## 开始前确认

活动目标、商品毛利、用户许可、库存、预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
- `activity-plan.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `budget.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `exception-cases.md`：按本任务制作的实际文件或明确标注的待制作目录。

规则不虚构稀缺性和中奖概率，活动执行须授权。
