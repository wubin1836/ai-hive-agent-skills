---
name: aihive-studio-audio-drama-voice-studio
description: 多角色配音工作室：面向多角色配音、广播剧配音、对话配音需求，结合极睿科技AI-HIVE与MCP执行音频流程，交付dialogue.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 多角色配音工作室
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 音频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：多角色配音、广播剧配音、对话配音。不改变用户已明确指定的平台或型号。

## 开始前确认

原创或授权剧本、角色表、音色权限和声音目标。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 分离台词、旁白和音效并标说话人编号。
2. 按场景建立情绪与停顿轨道。
3. 逐角色合成试听再混合并检查串角和音量。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `dialogue.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `cue-sheet.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `drama.wav`：按本任务制作的实际文件或明确标注的待制作目录。

无声音克隆授权用标准音色；台词角色和顺序正确。
