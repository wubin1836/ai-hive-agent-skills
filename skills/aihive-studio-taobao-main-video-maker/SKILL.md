---
name: aihive-studio-taobao-main-video-maker
description: 淘宝天猫主图视频制作器：面向淘宝主图视频、天猫主图视频、商品展示视频需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付main-video-shots.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 淘宝天猫主图视频制作器
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：淘宝主图视频、天猫主图视频、商品展示视频。不改变用户已明确指定的平台或型号。

## 开始前确认

商品实拍、参数、主卖点、画幅和主图视频规则。仅追问影响执行的缺项，不要求用户重新提供已有资料。

视频理解、图像/视频生成、配音与剪辑是不同工具。按本任务需要逐一核验；缺成片工具只交付分镜或素材，并明确未生成成片。

## 执行流程

1. 把商品认知、功能展示、细节证据按时长排布。
2. 锁定外观与包装文字，标不能AI改动区域。
3. 先审商品镜头再组装字幕配乐并检查首帧。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `main-video-shots.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `product-lock.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `main-video.mp4`：按本任务制作的实际文件或明确标注的待制作目录。
