---
name: aihive-muse-spark-13-project
description: Muse Spark长任务交付助手：面向Muse长任务、Spark智能体、Meta Agent需求，结合极睿科技AI-HIVE与MCP执行模型工作流流程，交付project-plan.json等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Muse Spark长任务交付助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型工作流
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：Muse长任务、Spark智能体、Meta Agent。不改变用户已明确指定的平台或型号。

## 开始前确认

目标、可访问文件、验收标准、检查点和预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

模型负责规划或生成，实际文件和外部操作由用户授权的宿主工具执行。模型标题不赋予浏览器、邮件或桌面权限。

## 执行流程

1. 建立文件证据索引并拆分可验收子任务。
2. 由实际可用模型规划且用宿主工具实施，记录任务依赖和未决项。
3. 每个检查点复核产物后续跑，不因长上下文省略人工确认。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `project-plan.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `checkpoints.jsonl`：按本任务制作的实际文件或明确标注的待制作目录。
- `deliverables/`：按本任务制作的实际文件或明确标注的待制作目录。
