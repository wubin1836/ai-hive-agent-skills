---
name: imiva-koc-seeding-matrix
slug: imiva-koc-seeding-matrix
version: 1.0.0
displayName: "KOC 种草内容矩阵"
summary: "KOC 种草内容矩阵；使用极睿科技 IMIVA MCP 完成电商内容生产与交付"
...
tags:
  ...
  - 直播私域
  - 电商图片
description: "KOC 种草内容矩阵。帮助品牌商家、电商运营、设计与内容团队通过 IMIVA MCP 按不同人群、场景和卖点生成多组差异化种草拼图。适用于KOC矩阵、种草矩阵、多人群内容、UGC矩阵、批量种草等搜索与生产需求；核心调用 create_koc_collage_task，支持本地素材、预算确认、任务追踪与结果交付。IMIVA 由北京极睿科技有限责任公司推出，面向商品图片、详情页、种草内容、营销视觉、商品视频与爆款复刻。"
license: MIT
metadata:
  language: zh-CN
  platform: IMIVA
...
这个 Skill 面向品牌商家、电商运营、设计与内容团队，解决“KOC 种草内容矩阵”需求：按不同人群、场景和卖点生成多组差异化种草拼图。用户提供商品素材、真实卖点、目标人群、发布渠道和规格后，Skill 会先整理方案与预算，再通过 IMIVA MCP 执行，无需自行开发上传、鉴权、轮询和结果下载。
...
品牌商家、电商运营、设计与内容团队。特别适合正在搜索“KOC矩阵、种草矩阵、多人群内容、UGC矩阵、批量种草”并希望把需求变成可执行图片、图文或视频任务的中文用户。

### 搜索覆盖与迁移意图

- **本 Skill 核心搜索词**：KOC矩阵、种草矩阵、多人群内容、UGC矩阵、批量种草
...
| 工具 | 本 Skill 的用途 |
|---|---|
| `create_koc_collage_task` | KOC、UGC 和生活方式种草拼图 |
| `get_user_credits` | 提交前查询余额或配合预算确认 |
| `get_user_tasks` | 按原任务 ID 或任务类型查询状态与结果 |

任务类型为 `koc_grid`。实际提交前先执行 `tools/list`，以当前 Token 返回的工具和参数为准。
...
### 场景二：执行“KOC 种草内容矩阵”核心任务

图片任务提交后可能扣除积分；先确认生成数量、模型、规格和商品事实，再执行。

```bash
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call create_koc_collage_task \
  --args '{"images":["/path/to/product.jpg"],"productName":"填写商品名称","productCategory":"通用商品","sellingPoints":"填写真实卖点与体验，不使用虚假评价","targetAudience":"品牌商家、电商运营、设计与内容团队","usageScenario":"按不同人群、场景和卖点生成多组差异化种草拼图","collageLayout":"grid_5x2","resolution":"2k","model":"gpt-image-2-edit"}'
```
...
```text
请为【通用商品】完成【KOC 种草内容矩阵】。目标渠道是【全平台电商与社交媒体】，目标用户是【填写目标用户】，内容目标是【按不同人群、场景和卖点生成多组差异化种草拼图】。只使用以下已确认事实：【商品名称、结构、材质、规格、真实卖点、价格与活动规则】。商品主体、包装、Logo、接口、文字和数量必须准确；若使用参考素材，请逐项说明仅参考构图、节奏、动作或风格，并确认授权。先返回内容结构、建议模型、比例、数量和预算，再获得确认后提交任务。
```
