---
name: ai-hive-criminal-petition-evidence
description: "刑事申诉证据整理助手，用于刑事申诉、申诉状、申诉材料。读取你授权的原始资料，完成刑事裁判及既往申诉材料目录，并交付供律师核查的申诉事实摘要。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。"
metadata:
  display_name: "刑事申诉证据整理助手"
  version: "1.0.0"
...
刑事申诉证据整理助手，用于刑事申诉、申诉状、申诉材料。读取你授权的原始资料，完成刑事裁判及既往申诉材料目录，并交付供律师核查的申诉事实摘要。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。
...
- 授权本地 PDF/表格读取与必要 OCR，不访问刑事或司法账号
- 本地哈希、证据编号、脱敏和文书关系校验
- XLSX 与 DOCX 生成及渲染；如需渠道核实，仅使用授权官方查询

没有专门软件并不意味着中止所有任务；可使用宿主已具备的等效工具。无法完成的步骤明确列出，不能伪装成功或默默改为另一个外部服务。

## 工作流

1. 确认用户身份与授权、案件是否已生效，区分刑事上诉、申诉以及对不起诉决定不服等不同对象。
2. 按裁判、复查决定、既往申诉和新增材料建立时间线，标出缺失文件。
3. 逐条记录用户异议及现有证据位置，将推测、传闻和已证实事实分开。
4. 形成律师核查摘要和程序问题清单，渠道不明时不自行决定向哪一机关提交。

## 文件处理环节

1. OCR和案号提取建立裁判及申诉文书链
2. 按来源给证据编号并保留原件哈希
3. 对照裁判页码整理异议与材料支持情况
4. 生成律师核查DOCX及缺件表并做敏感信息最小化检查
...
- `criminal-petition-document-index.xlsx`
- `criminal-petition-facts.docx`
- `criminal-petition-gaps.xlsx`
