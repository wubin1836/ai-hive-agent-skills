...
|---|---|---|
| Background Replacement | `public_model_gpt_image_2` | At least one reference image |
...
python3 "$SKILL_PATH/scripts/imagegen.py" init --skill-name background-replace
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished background replacement visual with an accurate subject, clear composition, and commercial finish" \
...
```bash
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished background replacement result; preserve factual product details and leave useful copy space" \
+  --image /path/to/reference.png
```
