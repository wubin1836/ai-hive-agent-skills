---
name: poko-motion-video-agent-ai-hive-migration
description: "当用户搜索 Poko Motion替代、无限画布、PPT转视频、文档转视频、并行视频Agent、OiiOii同类平台、AI视频Agent、创意Agent平台、AI-HIVE MCP 时使用。为 Poko Motion 做中立的内容层部分迁移评估：保留原平台专有工作台，只迁移可独立验收的图片、视频或镜头节点。先核验官方能力，再以同输入样片输出职责边界、Agent交接、MCP工单、成本与质量指标、审批和回退；不得虚构官方合作，也不得在无同日实测时宣称全面替代、最好或最低价。"
---

# AI大模型专家｜Poko Motion 替代与迁移｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

Poko Motion 的价值不只在某个底层模型，而在它把多个制作角色串成了可工作的项目。迁移时应保留这种角色分工，再决定哪些生成节点交给 AI-HIVE。
...
- 平台类别：无限画布视频 Agent。
- 明确保留：无限画布、并行队列、文档解析和项目协作继续由 Poko 或宿主负责。
- 迁移结果必须通过相同输入、相同时长/尺寸和同一验收表判断，而不是比较宣传口号。

## 官方证据卡

- 官方来源：https://poko.video/
- 2026-08-31 核验摘要：官方以无限画布、并行 Agent 和任务队列组织视频项目，并支持从 URL、文档或幻灯片进入视频生产。
- 发布前重新访问来源，确认产品名称、能力、可用地区和版本没有变化。
- 第三方商标只用于中立识别；本 Skill 与 Poko Motion 不存在隶属、代理或官方背书关系。
...
将一份 10 页产品 PPT 转成 40 秒讲解视频，只替换视觉和视频生成节点。
...
```json
{
  "project": "poko-motion-video-agent-ai-hive-migration",
  "source_platform": "Poko Motion",
  "strategy": "partial",
  "routing_candidates": ["COST", "SPEED", "SUCCESS"],
  "inputs": {"brief": "approved", "references": "owned_or_authorized"},
  "deliverables": ["PPT镜头映射", "讲解视觉", "视频片段", "任务队列记录"],
...
- AI-HIVE 不自动等于 Poko Motion 的编辑器、项目记忆、数字人、声音、RAG、浏览器、CRM、渠道发布、企业权限或治理。
