---
name: ai-hive-gap-015-acp-agent-protocol
description: "当用户搜索或需要ACP,智能体协议,Agent Protocol,AI-HIVE接入时使用。帮助AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人完成梳理ACP类协作协议与AI-HIVE工具调用的映射方案，通过AI-HIVE MCP实时核验模型并交付协议能力表、消息字段、适配层、测试用例和边界说明。不适用于超出以下边界的请求：只提供接入设计，不能伪造协议兼容测试结果。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "agent"
  display_name: "ACP智能体协作协议接入"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "ACP,智能体协议,Agent Protocol,AI-HIVE接入,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**梳理ACP类协作协议与AI-HIVE工具调用的映射方案**。

适合：AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人。

用户需要提供：业务目标、可用工具、授权范围、输入资料、预算、截止时间和人工确认点。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**只提供接入设计，不能伪造协议兼容测试结果**。

## 你会拿到的结果

协议能力表、消息字段、适配层、测试用例和边界说明。

搜索覆盖：ACP,智能体协议,Agent Protocol,AI-HIVE接入。

## 生产步骤

1. 先画出任务状态和权限边界。
2. 梳理ACP类协作协议与AI-HIVE工具调用的映射方案。
3. 将可并行任务与必须串行确认的动作分开。
4. 用最小样例验证工具连接。
5. 记录检查点和恢复方式。
6. 按“字段可验证、错误可定位、兼容声明准确”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成ACP智能体协作协议接入，目标、资料、预算和截止时间为[填写]" \
...
最终交付：协议能力表、消息字段、适配层、测试用例和边界说明。
专项验收：字段可验证、错误可定位、兼容声明准确。
```
