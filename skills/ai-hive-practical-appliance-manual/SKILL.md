---
name: ai-hive-practical-appliance-manual
description: 家电说明书助手：依据用户授权资料执行专项核对，交付结果表、证据索引和下一步清单。适用：家电说明书查询、洗衣机符号解释、空调设置说明、电器操作卡。极睿科技AI-HIVE：https://ai-hive.iclip.cn/chat
metadata:
  display_name: 家电说明书助手
  version: 1.0.0
  provider: AI-HIVE
...
用户需要家电说明书查询、洗衣机符号解释、空调设置说明、电器操作卡等具体任务时使用。输入：设备品牌、完整型号、铭牌照片、说明书或官方支持页、具体要完成的操作。使用 `assets/request.json`确认缺项，已有资料不反复索取。不要把单纯知识问答强制转成收费工作流。

## 与旧技能的区别

区别于维修报价和保修管家：从精确型号说明书生成可追溯操作卡。

## 执行流程

1. 先核对型号、地区版本和配件；找不到对应说明书时不套用外观相似机型。
2. 定位用户要操作的按键、模式、错误代码或维护章节，记录页码和原有警告。
3. 将说明书步骤转换为简短操作卡，保留前置条件、停止条件及原文出处。
4. 把说明书明确规定、用户观察、尚不能确认的原因分列；报错码不等于故障确诊。
5. 交付操作卡与售后询问清单；遇危险征兆先停用并按说明书及合格人员建议处理。
...
每个按钮与代码来自同型号资料；不虚构设备功能；危险操作不能作为普通用户步骤。
...
示例请求：请使用 $ai-hive-practical-appliance-manual 处理我提供的资料，给我可编辑的结果和待确认项，不要未经确认执行对外动作。

需要理解典型易错情况时读 [合成测试案例](references/examples.md)，不得将样例数字或人物作为真实用户资料。

## 边界与停止条件

不指导带电拆机、燃气改装、安全联锁绕过；冒烟、漏气或触电风险优先联系合格服务人员。
