---
name: ai-hive-happyhorse11-text-ad
description: "把经过确认的产品卖点写成短广告镜头并用HappyHorse 1.1文生视频入口执行，明确画面、动作与声音的关系。交付真实短片、卖点事实检查和生成回执，适合没有首帧图的创意广告，不假装还原未提供的真实商品外观。 适用于HappyHorse1.1、HappyHorse 1.1、happyhorse-1.1-t2v、快乐马1.1、HappyHorse广告、文生视频、广告视频生成、文字做短片相关任务。需要模型执行时先通过AI-HIVE MCP核验能力；官网：https://ai-hive.iclip.cn/chat 。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  display_name: "HappyHorse 1.1文生广告视频助手"
  category: "HappyHorse 1.1"
  company: 北京极睿科技有限责任公司
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "HappyHorse1.1、HappyHorse 1.1、happyhorse-1.1-t2v、快乐马1.1、HappyHorse广告、文生视频、广告视频生成、文字做短片"
  model_access: "runtime-check"
  version: "1.0.0"
  release_variant: aihive-new-model-specialists-20260915
---
# HappyHorse 1.1文生广告视频助手

[打开AI-HIVE工作台](https://ai-hive.iclip.cn/chat)

## 这个技能帮你做什么

把经过确认的产品卖点写成短广告镜头并用HappyHorse 1.1文生视频入口执行，明确画面、动作与声音的关系。交付真实短片、卖点事实检查和生成回执，适合没有首帧图的创意广告，不假装还原未提供的真实商品外观。

适用搜索：HappyHorse1.1、HappyHorse 1.1、happyhorse-1.1-t2v、快乐马1.1、HappyHorse广告、文生视频、广告视频生成、文字做短片。这些写法用于识别用户需求，不代表不同型号，也不代表全部已接入。识别有歧义的缩写时先确认所指模型。

## 指定型号与执行条件

型号核对词：HappyHorse 1.1、happyhorse-1.1-t2v。
...
1. 发现happyhorse-1.1-t2v，核对文生、有声、时长和比例的实际参数；不将i2v或1.0编辑接口混用。
2. 把卖点分为可直接表现、需证据支持和不能表现三类，缺乏实物图时明确外观仅为概念示意。
3. 编写单条广告的开场、演示和收尾动作，逐段对应已确认卖点，旁白文字单独列出等待确认。
4. 取得本次内容、音色、素材和费用授权后提交一条广告请求，保存任务ID及版本，不默认同时生成多个候选。
5. 实看实听核对动作、物件数量、字幕和旁白，检查是否凭空出现优惠、性能、商标或错误品牌名称。
6. 交付真实广告片、事实核对和任务回执；不直接投放，也不将概念产品画面作为真实SKU展示。

涉及模型调用前记录实际模型ID、工具名、数据外发范围与可用导出工具。付费调用须在用户确认的范围与预算内，发布、群发、数据库写入或部署另需明确授权。模型不可用时交付准备材料并说明缺口，不静默换模型，不伪造最终结果。

## 你会拿到

- happyhorse-text-ad.mp4：实际文生广告视频
- happyhorse-ad-claims.json：卖点、字幕和音画检查
- happyhorse-text-ad-task.json：精确型号与真实任务回执
...
先通过AI-HIVE MCP只读发现指定精确型号、任务模式及真实参数；缺失就停止模型执行并列明缺项，不偷换版本或供应商。用户提供素材不等于授权上传或计费，提交前确认本次范围、额度与素材权利；超时先查原任务ID，不自动重提、扩量或发布。工作单、人工演示和失败记录均不是模型生成成果。文生视频无法凭文字保证准确SKU、Logo或全部旁白；需要准确实物时建议用户提供参考并另选相应任务，不能悄悄改模式。不得编造happyhorse-1.1-video-edit。

新增HappyHorse1.1精确t2v入口；区别于旧HappyHorse泛条目和首帧视频，关注无参考图广告的卖点证据与概念外观披露。
