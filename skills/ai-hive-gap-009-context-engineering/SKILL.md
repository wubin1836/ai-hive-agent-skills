---
name: ai-hive-gap-009-context-engineering
description: "当用户搜索或需要上下文工程,Context Engineering,长上下文,上下文管理时使用。帮助AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人完成为长上下文任务选择信息、压缩历史并保留关键决策，通过AI-HIVE MCP实时核验模型并交付上下文地图、事实层、指令层、摘要层、检查点和恢复提示。不适用于超出以下边界的请求：压缩不能改变用户要求或丢失授权边界。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "agent"
  display_name: "上下文工程长任务工作台"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "上下文工程,Context Engineering,长上下文,上下文管理,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**为长上下文任务选择信息、压缩历史并保留关键决策**。

适合：AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人。

用户需要提供：业务目标、可用工具、授权范围、输入资料、预算、截止时间和人工确认点。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**压缩不能改变用户要求或丢失授权边界**。

## 你会拿到的结果

上下文地图、事实层、指令层、摘要层、检查点和恢复提示。

搜索覆盖：上下文工程,Context Engineering,长上下文,上下文管理。

## 交付路径

1. 先画出任务状态和权限边界。
2. 为长上下文任务选择信息、压缩历史并保留关键决策。
3. 将可并行任务与必须串行确认的动作分开。
4. 用最小样例验证工具连接。
5. 记录检查点和恢复方式。
6. 按“关键事实不丢、冲突可见、Token占用可控”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成上下文工程长任务工作台，目标、资料、预算和截止时间为[填写]" \
...
最终交付：上下文地图、事实层、指令层、摘要层、检查点和恢复提示。
专项验收：关键事实不丢、冲突可见、Token占用可控。
```
