---
name: ai-hive-retirement-date-evidence
description: "退休时间与延迟退休政策核对助手，用于什么时候退休、延迟退休、退休年龄计算。读取你授权的原始资料，完成适用规则来源卡，并交付需单位或经办机构确认的问题。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料准备与核对参考；具体要求以办理地当前官方规则为准，不保证资格或审批结果。"
metadata:
  display_name: "退休时间与延迟退休政策核对助手"
  version: "1.0.0"
...
退休时间与延迟退休政策核对助手，用于什么时候退休、延迟退休、退休年龄计算。读取你授权的原始资料，完成适用规则来源卡，并交付需单位或经办机构确认的问题。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料准备与核对参考；具体要求以办理地当前官方规则为准，不保证资格或审批结果。
...
1. 只收计算所需最少出生年月和经办分类事实，不自行认定特殊类别
2. 使用当前官方文件或计算工具，将法定口径、可申请选项和未知条件分开
3. 保留计算输入和来源日期，缺少资格事实时交问题单，不给确定结论
...
- `retirement-date-evidence-checklist.xlsx`
- `retirement-date-evidence-evidence.docx`
- `retirement-date-evidence-sources.json`
...
> 仅示例，以下文件名和情境均为假设，不表示已存在真实数据：假设我已提供 示例_最小必要出生年月及经办类别.json、示例_官方适用文件.pdf、示例_计算工具结果截图.png。请使用「退休时间与延迟退休政策核对助手」，核对两种退休时间说法的来源与输入，分开法定口径、可申请选项和还缺的分类事实。将结果写入我指定的本地目录，交付 retirement-date-evidence-checklist.xlsx、retirement-date-evidence-evidence.docx、retirement-date-evidence-sources.json。本例先离线处理；输入、工具或适用规则不足时先交缺口说明，不编造结果。需要联网更新来源先征得我同意，不登录账户、不代提交或付款。

## 搜索与任务识别

什么时候退休、延迟退休、退休年龄计算、退休时间查询、弹性退休
