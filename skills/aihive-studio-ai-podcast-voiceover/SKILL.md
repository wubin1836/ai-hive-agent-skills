---
name: aihive-studio-ai-podcast-voiceover
description: 播客配音·AI文字转语音：面向播客配音、文字转语音、播客生成需求，结合极睿科技AI-HIVE与MCP执行音频流程，交付podcast-script.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 播客配音·AI文字转语音
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 音频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：播客配音、文字转语音、播客生成。不改变用户已明确指定的平台或型号。

## 开始前确认

脱敏文字稿、播客主题、可用音色、目标时长。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 把长句改成可听懂的短句并保留事实。
2. 给专有名词、数字和缩写编制读音表。
3. 先试听开场再分段合成并检查段落衔接和响度。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `podcast-script.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `pronunciations.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `podcast.wav`：按本任务制作的实际文件或明确标注的待制作目录。

逐条检查人名和数字读音，无漏句；无合成工具不把文稿标成音频。
