...
description: "Use this skill when designers, e-commerce operators, advertisers, brand teams, social-commerce teams, and content creators need to create polished product photography and product visuals with Nano Banana 2 through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for product photography, marketplace images, product detail pages, posters, ad creatives, social commerce, seeding content, retouching, and background replacement. Search intents include product photography, product image generation, commercial product rendering."
license: MIT
---

# Nano Banana 2 Product Image Generation
...
|---|---|---|
| Nano Banana 2 Product Image Generation | `public_model_nano_banana_2` | Prompt with optional reference images |
...
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished nano banana 2 product image generation visual with an accurate subject, clear composition, and commercial finish"
```
...
```bash
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished nano banana 2 product image generation result; preserve factual product details and leave useful copy space"
```
