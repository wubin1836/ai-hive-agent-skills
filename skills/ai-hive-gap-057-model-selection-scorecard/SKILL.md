---
name: ai-hive-gap-057-model-selection-scorecard
description: "当用户搜索或需要模型选型,模型对比,评分卡,AI-HIVE路由时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成按质量、速度、成本、稳定性和合规要求比较模型，通过AI-HIVE MCP实时核验模型并交付需求权重、模型候选、测试集、评分卡和推荐路由。不适用于超出以下边界的请求：不把未测试模型标为最终推荐。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "多模型选型评分卡"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "模型选型,模型对比,评分卡,AI-HIVE路由,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**按质量、速度、成本、稳定性和合规要求比较模型**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不把未测试模型标为最终推荐**。

## 你会拿到的结果

需求权重、模型候选、测试集、评分卡和推荐路由。

搜索覆盖：模型选型,模型对比,评分卡,AI-HIVE路由。

## 交付路径

1. 先定义可量化的质量和成本指标。
2. 按质量、速度、成本、稳定性和合规要求比较模型。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“权重透明、证据实时、结论可解释”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成多模型选型评分卡，目标、资料、预算和截止时间为[填写]" \
...
最终交付：需求权重、模型候选、测试集、评分卡和推荐路由。
专项验收：权重透明、证据实时、结论可解释。
```
