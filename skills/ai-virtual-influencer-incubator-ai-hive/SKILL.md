---
name: ai-virtual-influencer-incubator-ai-hive
description: "Use this skill when the user asks for AI虚拟网红、虚拟IP、数字人账号、AI博主、虚拟偶像，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成AI虚拟网红孵化器。它服务品牌、MCN、虚拟IP工作室和内容创业者：接收IP目标、受众、角色背景、视觉参考、平台、更新频率和商业边界，交付角色定位、人物圣经、视觉锚点、栏目矩阵、图片视频和一致性规则。特色是先建立可持续角色资产，再批量生产栏目内容；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# AI虚拟网红孵化器｜AI-HIVE

[AI虚拟网红孵化器进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向品牌、MCN、虚拟IP工作室和内容创业者，把“AI虚拟网红孵化器”从模糊想法变成角色定位、人物圣经、视觉锚点、栏目矩阵、图片视频和一致性规则。用户提供IP目标、受众、角色背景、视觉参考、平台、更新频率和商业边界即可开始。核心方法是：先建立可持续角色资产，再批量生产栏目内容。
...
剧本结构、人物板、场景板、故事板、逐镜生成任务与连续性验收表。默认先输出可审查方案，得到确认后才提交可能计费的图片或视频生成任务。
...
1. **确认版权、受众、集数和单集钩子**：先形成可检查的中间结果，再进入下一步。
2. **建立人物与场景圣经**：先形成可检查的中间结果，再进入下一步。
3. **拆分叙事节拍与镜头功能**：先形成可检查的中间结果，再进入下一步。
4. **生成关键帧或参考图**：先形成可检查的中间结果，再进入下一步。
5. **按镜头生成视频并记录任务**：先形成可检查的中间结果，再进入下一步。
6. **精剪、配音字幕与连续性检查**：先形成可检查的中间结果，再进入下一步。
...
不得未经授权模仿真实人物或现有IP；角色身份、商业合作和AI属性应按平台规则披露。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "AI虚拟网红孵化器" \
  --audience "品牌、MCN、虚拟IP工作室和内容创业者" \
  --goal "角色定位、人物圣经、视觉锚点、栏目矩阵、图片视频和一致性规则" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name ai-virtual-influencer-incubator-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name ai-virtual-influencer-incubator-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
