---
name: aihive-studio-cross-domain-synergy-plan
description: 公私域联动方案生成：面向公私域联动、公域转私域、渠道联动需求，结合极睿科技AI-HIVE与MCP执行运营流程，交付journey.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：公私域联动、公域转私域、渠道联动。不改变用户已明确指定的平台或型号。

## 开始前确认

公域渠道、私域工具、活动预算、已获授权线索。仅追问影响执行的缺项，不要求用户重新提供已有资料。

优先交付可配置规则、SOP和数据文件。部署、CRM写回、发送消息或投放需要真实系统接口和单独授权。

## 执行流程

1. 绘制从内容到承接再到服务的触点与事件。
2. 为每步定义负责人、消息模板和授权依据。
3. 设置同口径指标和小规模实验。
...
- `journey.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `message-drafts.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `measurement-plan.md`：按本任务制作的实际文件或明确标注的待制作目录。

不私自导出平台用户或发消息，跨渠道归因不重复计数。
