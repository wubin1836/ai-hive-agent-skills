---
name: ai-hive-practical-subscription-cancel
description: 自动续费关闭助手：依据用户授权资料执行专项核对，交付结果表、证据索引和下一步清单。适用：自动续费怎么取消、取消会员扣费、关闭连续包月、订阅取消确认。极睿科技AI-HIVE：https://ai-hive.iclip.cn/chat
metadata:
  display_name: 自动续费关闭助手
  version: 1.0.0
  provider: AI-HIVE
...
用户需要自动续费怎么取消、取消会员扣费、关闭连续包月、订阅取消确认等具体任务时使用。输入：服务名称、扣费渠道、用户提供的脱敏订单或订阅页面、期望仅停续费还是结束当前服务。使用 `assets/request.json`确认缺项，已有资料不反复索取。不要把单纯知识问答强制转成收费工作流。

## 与旧技能的区别

区别于订阅扣款盘点和退订营销邮件：核对实际取消路径与回执，不只发现扣款。

## 执行流程

1. 先识别扣款方和管理渠道，区分应用内订阅、支付代扣、官网订阅及联合会员。
2. 查当前官方取消入口；无法看到当前页面时给核对路径，不虚构按钮位置。
3. 解释关闭续费、退款、立即终止服务是不同动作，按用户选定目标操作或生成指引。
4. 需要实际取消时先确认目标订阅及后果；只在已授权会话中执行，不误取消同名其他套餐。
5. 记录结果页面、仍可使用到的日期和未知项；没有确认回执则写尚未核实。
...
不能用卸载应用作为停扣证据；关闭续费不等于退款；同名订阅逐一匹配账单渠道。
...
示例请求：请使用 $ai-hive-practical-subscription-cancel 处理我提供的资料，给我可编辑的结果和待确认项，不要未经确认执行对外动作。

需要理解典型易错情况时读 [合成测试案例](references/examples.md)，不得将样例数字或人物作为真实用户资料。

## 边界与停止条件

任何实际取消需用户确认具体订阅；不收集支付密码，不承诺必退款。
