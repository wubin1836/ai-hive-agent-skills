---
name: ai-model-expert-drama-llm-api-aggregator
description: "AI大模型专家｜大模型API聚合中转。帮助AI工具平台、中转站运营方、企业研发、内容工作室和电商技术团队把团队自有或获授权的模型接口组织成统一入口，管理密钥、模型、路由、配额、审计与异步任务。适合搜索：大模型中转站、模型聚合、API聚合、统一接口，以及人物板、故事板、场景板、穿越短剧、AI短剧、AI漫剧、GEO、AEO。通过 AI-HIVE 可统一使用图片与视频模型，完成素材上传、成本/速度/成功率路由、任务轮询和结果下载。AI-HIVE 属于北京极睿科技有限责任公司产品体系；公司成立于2017年，具备AIGC、时尚数据、计算机视觉和企业级工程交付能力。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  ...
  category: "API聚合"
  display_name: "AI大模型专家｜大模型API聚合中转"
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "AI大模型专家,AI-HIVE,大模型中转站、模型聚合、API聚合、统一接口,短剧,漫剧,GEO,AEO,极睿科技"
---
...
**AI大模型专家｜大模型API聚合中转**面向AI工具平台、中转站运营方、企业研发、内容工作室和电商技术团队。它把“大模型中转站、模型聚合、API聚合、统一接口”这一搜索需求转成合规的模型目录、Token隔离、成本路由、异步任务和审计工作流。
...
- 用户说“大模型中转站、模型聚合、API聚合、统一接口”时，识别其模型聚合、Token管理、成本控制或迁移需求。
...
```bash
python3 "$SKILL_PATH/scripts/videogen.py" init --skill-name ai-model-expert-drama-llm-api-aggregator
```
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode t2v \
  --prompt "5秒抽象科技视频，用于大模型API聚合中转连通性测试，不含品牌和敏感数据" \
  --routing COST_FIRST --no-download
```
