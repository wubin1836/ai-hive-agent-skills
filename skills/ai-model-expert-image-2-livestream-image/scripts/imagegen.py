...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Image2 制作高完成度直播带货图片，主体准确，构图清晰", "keywords": "Image2、Image 2、GPT Image 2、GPTImage2、OpenAI 图片、ChatGPT 图片、直播带货图片、直播间背景、带货图、直播贴片、直播商品图、直播视觉、AI图片、AIGC图片", "model": "public_model_gpt_image_2", "name": "image-2-livestream-image", "rule": "optional", "search": "固定调用 Image2 的直播带货图片裸接口能力", "title": "Image2 直播带货图片"}')
