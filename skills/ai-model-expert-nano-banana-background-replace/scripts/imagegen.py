...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana 制作高完成度图片换背景，主体准确，构图清晰", "keywords": "Nano Banana、NanoBanana、Nano Banana 2、香蕉模型、Gemini 图片、图片换背景、换背景、抠图换场景、背景替换、场景合成、白底转场景、AI图片、AIGC图片", "model": "public_model_nano_banana_2", "name": "nano-banana-background-replace", "rule": "image", "search": "固定调用 Nano Banana 的图片换背景裸接口能力", "title": "Nano Banana 图片换背景"}')
