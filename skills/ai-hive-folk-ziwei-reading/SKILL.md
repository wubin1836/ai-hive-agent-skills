---
name: ai-hive-folk-ziwei-reading
description: "紫微斗数命盘整理助手，用于紫微斗数怎么看、紫薇斗数、紫薇命盘。读取你授权的原始资料，完成带截图坐标的命盘资料JSON与SVG图谱，并交付字段来源及文化娱乐参考PDF。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供传统文化与娱乐参考，尊重科学；相关解读不是科学预测，不作为投资、医疗、法律或婚姻等重大决策依据。"
metadata:
  display_name: "紫微斗数命盘整理助手"
  version: "1.0.0"
...
紫微斗数命盘整理助手，用于紫微斗数怎么看、紫薇斗数、紫薇命盘。读取你授权的原始资料，完成带截图坐标的命盘资料JSON与SVG图谱，并交付字段来源及文化娱乐参考PDF。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供传统文化与娱乐参考，尊重科学；相关解读不是科学预测，不作为投资、医疗、法律或婚姻等重大决策依据。
...
1. 读取已核验盘图并去身份化，紫薇误写归并紫微斗数
2. OCR提取字段与坐标，检查宫位标签及重复遗漏
3. 疑点退回用户；没有验证过的历法算法就不自动排盘
4. 生成资料图谱和文化报告并逐项回读

## 文件处理环节

1. OCR识别盘面并保留字段坐标
2. 脚本检查宫位标签、重复遗漏；疑点退回确认
3. 生成JSON资料图谱与PDF报告并逐字段回读
...
- `ziwei-chart-data.json`
- `ziwei-source-map.svg`
- `ziwei-reference-report.pdf`
...
紫微斗数怎么看、紫薇斗数、紫薇命盘、紫微排盘解读、命宫是什么、紫微斗数术语
