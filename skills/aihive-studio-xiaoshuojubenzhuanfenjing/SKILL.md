---
name: aihive-studio-xiaoshuojubenzhuanfenjing
description: 小说·剧本转分镜：面向小说转分镜、剧本转分镜、分镜脚本需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付shots.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 小说·剧本转分镜
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：小说转分镜、剧本转分镜、分镜脚本。不改变用户已明确指定的平台或型号。

## 开始前确认

已授权小说或剧本、角色表、镜头时长和制作限制。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 把叙述转成可拍动作并保留必要台词。
2. 梳理人物进出场、轴线、道具与情绪推进。
3. 给每镜输出画面、声音和转场并核算总时长。
...
- `continuity.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `adaptation-notes.md`：按本任务制作的实际文件或明确标注的待制作目录。
