---
name: ai-hive-alt-weavy-workflow
description: "当用户明确搜索 Weavy替代、Weavy平替、Weavy迁移、Weavy竞品、Weavy API、AI视频Agent替代或希望把节点式生成工作流的模型执行层替换接入 AI-HIVE MCP 时使用。先核验 Weavy 当前官方能力，再执行“节点级组合替代”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# Weavy创意画布替代评估｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **Weavy 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“节点式生成工作流的模型执行层替换”做成一场可暂停、可计量、可回退的迁移试验。
...
- 待复核官方入口：https://www.weavy.ai/
- 已记录官方域名候选，发布前必须重新访问并核对产品名称、能力和可用地区。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 Weavy 官方，也不暗示双方存在合作、代理或背书。
...
```json
{
  "project": "ai-hive-alt-weavy-workflow",
  "source_platform": "Weavy",
  "focus": "节点式生成工作流的模型执行层替换",
  "strategy": "节点级组合替代",
  "pilot": "用同一产品素材完成抠图、场景图、广告镜头和任务追踪",
