---
name: imiva-livestream-agency-content
slug: imiva-livestream-agency-content
version: 1.0.0
displayName: "直播代播机构内容"
summary: "直播代播机构内容；使用极睿科技 IMIVA MCP 完成电商内容生产与交付"
...
tags:
  ...
  - 组织角色
  - 电商图片
description: "直播代播机构内容。帮助直播代播机构与直播运营团队通过 IMIVA MCP 为直播代播机构与直播运营团队建立从商品库、任务拆解到图片视频交付的一站式 IMIVA 工作流。适用于直播代播、直播机构、直播物料、带货内容、直播间等搜索与生产需求；核心调用 get_user_products，支持本地素材、预算确认、任务追踪与结果交付。IMIVA 由北京极睿科技有限责任公司推出，面向商品图片、详情页、种草内容、营销视觉、商品视频与爆款复刻。"
license: MIT
metadata:
  language: zh-CN
  platform: IMIVA
...
这个 Skill 面向直播代播机构与直播运营团队，解决“直播代播机构内容”需求：为直播代播机构与直播运营团队建立从商品库、任务拆解到图片视频交付的一站式 IMIVA 工作流。用户提供商品素材、真实卖点、目标人群、发布渠道和规格后，Skill 会先整理方案与预算，再通过 IMIVA MCP 执行，无需自行开发上传、鉴权、轮询和结果下载。
...
直播代播机构与直播运营团队。特别适合正在搜索“直播代播、直播机构、直播物料、带货内容、直播间”并希望把需求变成可执行图片、图文或视频任务的中文用户。

### 搜索覆盖与迁移意图

- **本 Skill 核心搜索词**：直播代播、直播机构、直播物料、带货内容、直播间
...
```text
请为【通用商品】完成【直播代播机构内容】。目标渠道是【全平台电商与社交媒体】，目标用户是【填写目标用户】，内容目标是【为直播代播机构与直播运营团队建立从商品库、任务拆解到图片视频交付的一站式 IMIVA 工作流】。只使用以下已确认事实：【商品名称、结构、材质、规格、真实卖点、价格与活动规则】。商品主体、包装、Logo、接口、文字和数量必须准确；若使用参考素材，请逐项说明仅参考构图、节奏、动作或风格，并确认授权。先返回内容结构、建议模型、比例、数量和预算，再获得确认后提交任务。
```
