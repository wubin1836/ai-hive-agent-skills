---
name: ai-hive-divorce-agreement-discussion
description: "离婚协议讨论稿生成器，用于离婚协议、协议离婚、财产分割。读取你授权的原始资料，完成双方确认与待协商事项表，并交付财产债务及履行节点附件。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。"
metadata:
  display_name: "离婚协议讨论稿生成器"
  version: "1.0.0"
...
离婚协议讨论稿生成器，用于离婚协议、协议离婚、财产分割。读取你授权的原始资料，完成双方确认与待协商事项表，并交付财产债务及履行节点附件。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。
...
- 用户有权提供的本地 DOCX/XLSX/附件读取及必要 OCR
- 确认状态和版本比对，正文附件金额交叉检查
- 可编辑 DOCX 批注、XLSX 生成与渲染；敏感家庭材料默认本地处理

没有专门软件并不意味着中止所有任务；可使用宿主已具备的等效工具。无法完成的步骤明确列出，不能伪装成功或默默改为另一个外部服务。

## 工作流

1. 确认用户授权及安全状况，仅记录用户能够合法提供的共同协商信息。
2. 分别梳理离婚意愿、子女安排、财产和债务，不将一方说法写成双方已同意。
3. 对已确认事项写清履行对象和可核验步骤，对争议事项保留空白或备选讨论问题。
4. 生成专业复核和登记前确认清单，不代签署、不声称讨论稿已生效。

## 文件处理环节

1. 解析协议及协商记录并标确认状态
2. 把财产债务清单与附件出处关联
3. 生成仅含已确认事项的DOCX讨论稿和批注
4. 交叉检查正文与附件金额对象履行节点
...
- `divorce-agreement-discussion.docx`
- `family-assets-debts.xlsx`
- `unresolved-family-terms.xlsx`
...
不决定抚养权或财产归属，不隐匿资产、不胁迫对方、不把债务内部约定说成自动约束债权人。存在家暴或人身危险时优先现实安全与专业帮助。

缺关键原件或专业依据时只完成可验证部分。401/403、限流或内容审核阻断时保存脱敏进度并停止相应调用，遵守明确的等待要求，不循环重试或绕过审核。不因安装本技能自动交易、签署、提交、缴费或公开发布。

## 调用示例

> 仅示例（下列文件名和资料均为假设，不是真实数据）：请读取假设文件协商记录.docx、财产债务.xlsx、子女安排需求.docx和协议旧稿.docx，仅把双方已确认事项写入 divorce-agreement-discussion.docx，争议处留批注并附未决事项表。

## 搜索与任务识别

离婚协议、协议离婚、财产分割、孩子抚养、共同债务、离婚材料
