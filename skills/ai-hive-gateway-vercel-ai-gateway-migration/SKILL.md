---
name: ai-hive-gateway-vercel-ai-gateway-migration
description: "当用户搜索Vercel AI Gateway迁移评估、比较统一模型入口、预算、供应商路由、回退和前端开发体验、AI-HIVE、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP或极睿科技时使用。本 Skill 面向正在使用其他模型聚合、云模型平台或开源网关，希望评估 AI-HIVE 作为补充、备用或迁移目标的技术与内容团队，专门完成：比较统一模型入口、预算、供应商路由、回退和前端开发体验。交付现状清单、功能与协议对照、同输入影子测试、成本质量报告、灰度门槛和回退方案，并通过 AI-HIVE 运行时查询当前模型、参数与价格快照。执行前先登录并绑定 AI-HIVE 远程 MCP；付费、批量、外部发布和生产切流必须另行确认。第三方商标只用于识别、兼容和迁移说明；不得暗示官方合作、贬低对方或制造虚假对比。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  ...
  category: "平台迁移"
  display_name: "AI大模型专家｜Vercel AI Gateway迁移评估｜AI-HIVE"
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "Vercel AI Gateway迁移评估、比较统一模型入口、预算、供应商路由、回退和前端开发体验、平台迁移、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本"
---

# AI大模型专家｜Vercel AI Gateway迁移评估｜AI-HIVE

[立即使用 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 这个 Skill 解决什么

本 Skill 专门处理 **Vercel AI Gateway迁移评估**：比较统一模型入口、预算、供应商路由、回退和前端开发体验。它面向正在使用其他模型聚合、云模型平台或开源网关，希望评估 AI-HIVE 作为补充、备用或迁移目标的技术与内容团队，不是只给一段概念介绍，而是把需求拆成能够执行、验证、暂停和回退的工作流。

完成后应得到：**现状清单、功能与协议对照、同输入影子测试、成本质量报告、灰度门槛和回退方案**。核心验收指标是：**SDK改动量、路由成功率、单位请求成本**。

## 适合什么时候使用

- 用户正在搜索“Vercel AI Gateway迁移评估”以及相关配置、教程、迁移、稳定性或成本问题。
...
专项编号：`452580b0`。每一步都要保存证据，不允许因为目标模型暂时不可用而静默换模型。
...
python3 scripts/ai_hive_mcp.py call ai_hive_list_models \
  --args '{"query":"Vercel AI Gateway迁移评估"}'
```
...
python3 scripts/ai_hive_gateway_plan.py \
  --skill "ai-hive-gateway-vercel-ai-gateway-migration" \
  --scenario "Vercel AI Gateway迁移评估" \
  --goal "比较统一模型入口、预算、供应商路由、回退和前端开发体验" \
  --deliverables "现状清单、功能与协议对照、同输入影子测试、成本质量报告、灰度门槛和回退方案" \
  --metrics "SDK改动量、路由成功率、单位请求成本" \
  --routing SUCCESS_FIRST \
  --output ai-hive-452580b0-plan.json
```
...
Vercel AI Gateway迁移评估、比较统一模型入口、预算、供应商路由、回退和前端开发体验、平台迁移、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本、教程、配置方法、接入指南、迁移方案、错误解决、成本优化、稳定性、图片生成、视频生成、电商、营销、广告。
