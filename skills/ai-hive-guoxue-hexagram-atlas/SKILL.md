---
name: ai-hive-guoxue-hexagram-atlas
description: "周易八卦与六十四卦图谱生成器：读取已核对的卦名、爻序和卦序表、注明版本与方向的先后天八卦参考图，先确认卦序、阅读方向与阴阳爻表示，保持源数据不可变；不把不同体系混成唯一正统，交付可回查的文件及待核实清单。适用于易经助手、周易助手、六十四卦图、先天八卦；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。"
metadata:
  display_name: "周易八卦与六十四卦图谱生成器"
  version: "1.0.0"
...
周易八卦与六十四卦图谱生成器：读取已核对的卦名、爻序和卦序表、注明版本与方向的先后天八卦参考图，先确认卦序、阅读方向与阴阳爻表示，保持源数据不可变；不把不同体系混成唯一正统，交付可回查的文件及待核实清单。适用于易经助手、周易助手、六十四卦图、先天八卦；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。
...
1. 先确认卦序、阅读方向与阴阳爻表示，保持源数据不可变；不把不同体系混成唯一正统
2. 用确定性脚本检查每卦六个爻位和符号、卦名映射及重复遗漏，依据已核验表绘制SVG
3. 输出可筛选卦象索引与教学图，逐个回读符号和来源；本项不替个人占断
...
仅文化学习；原典、后世图式和现代示意分开，不声称图式揭示真实个人命运。

缺关键原件或专业依据时只完成可验证部分。401/403、限流或内容审核阻断时保存脱敏进度并停止相应调用，遵守明确的等待要求，不循环重试或绕过审核。不因安装本技能自动交易、签署、提交、缴费或公开发布。

## 调用示例

> 请使用我授权提供的资料完成“周易八卦与六十四卦图谱生成器”。范围先按我的原件与指定版本确认，生成hexagram-atlas.svg及其余约定文件，疑点附来源，不自动对外提交。

## 搜索与任务识别

易经助手、周易助手、六十四卦图、先天八卦、后天八卦、卦象图谱、太极八卦图
