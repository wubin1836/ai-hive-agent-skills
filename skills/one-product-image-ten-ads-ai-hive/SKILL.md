---
name: one-product-image-ten-ads-ai-hive
description: "Use this skill when the user asks for 一图生成视频、10条广告、批量广告、商品图转视频、A/B测试，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成一张商品图生成10条广告。它服务素材稀缺的中小商家、代运营和广告测试团队：接收一张商品图、真实卖点、受众、平台、预算和品牌限制，交付10种不重复创意角度、脚本、首帧、视频任务、去重报告和测试表。特色是固定商品锚点，只改变钩子、场景、证据、人物或CTA中的一个变量；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 一张商品图生成10条广告｜AI-HIVE

[一张商品图生成10条广告进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向素材稀缺的中小商家、代运营和广告测试团队，把“一张商品图生成10条广告”从模糊想法变成10种不重复创意角度、脚本、首帧、视频任务、去重报告和测试表。用户提供一张商品图、真实卖点、受众、平台、预算和品牌限制即可开始。核心方法是：固定商品锚点，只改变钩子、场景、证据、人物或CTA中的一个变量。
...
十条广告不等于十次盲目生成；先小样去重，禁止为增加差异而编造卖点。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "一张商品图生成10条广告" \
  --audience "素材稀缺的中小商家、代运营和广告测试团队" \
  --goal "10种不重复创意角度、脚本、首帧、视频任务、去重报告和测试表" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name one-product-image-ten-ads-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name one-product-image-ten-ads-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
