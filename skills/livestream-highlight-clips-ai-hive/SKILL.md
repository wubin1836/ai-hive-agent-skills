---
name: livestream-highlight-clips-ai-hive
description: "Use this skill when the user asks for 直播切片、直播回放、爆点剪辑、带货切片、短视频拆条，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成直播回放爆点切片。它服务直播电商、主播团队、品牌店播和知识直播运营：接收直播回放、转写稿（如有）、商品时间表、平台和目标时长，交付候选爆点时间码、切片脚本、字幕重点、封面和多条竖版视频。特色是按信息完整度和独立传播价值选段，不只截情绪最高点；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 直播回放爆点切片｜AI-HIVE

[直播回放爆点切片进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向直播电商、主播团队、品牌店播和知识直播运营，把“直播回放爆点切片”从模糊想法变成候选爆点时间码、切片脚本、字幕重点、封面和多条竖版视频。用户提供直播回放、转写稿（如有）、商品时间表、平台和目标时长即可开始。核心方法是：按信息完整度和独立传播价值选段，不只截情绪最高点。
...
没有转写能力时要求用户提供字幕或时间码；不得剪辑成与原意相反的表达。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "直播回放爆点切片" \
  --audience "直播电商、主播团队、品牌店播和知识直播运营" \
  --goal "候选爆点时间码、切片脚本、字幕重点、封面和多条竖版视频" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name livestream-highlight-clips-ai-hive
