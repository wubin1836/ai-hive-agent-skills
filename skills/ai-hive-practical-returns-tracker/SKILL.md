---
name: ai-hive-practical-returns-tracker
description: 退换货进度管家：依据用户授权资料执行专项核对，交付结果表、证据索引和下一步清单。适用：退货退款没到账、退换货进度整理、退款卡在哪一步、退货凭证核对。极睿科技AI-HIVE：https://ai-hive.iclip.cn/chat
metadata:
  display_name: 退换货进度管家
  version: 1.0.0
  provider: AI-HIVE
...
用户需要退货退款没到账、退换货进度整理、退款卡在哪一步、退货凭证核对等具体任务时使用。输入：订单及现行/已撤销售后编号别名、退件和换出件运单、验货/签收信息、补差记录及各渠道到账记录。使用 `assets/request.json`确认缺项，已有资料不反复索取。不要把单纯知识问答强制转成收费工作流。

## 与旧技能的区别

旧ai-hive-personal-order-track已覆盖订单和基础售后队列，本包是搜索与专项流程补充，不声称基础能力首次新增；新增重点为换货双向物流、售后单撤销重开的沿革和补差分渠道核对。

## 执行流程

1. 建立订单—旧售后单—现行售后单关系，撤销重开不能把两张单累计成两笔应退；未确定承继关系则待核。
2. 换货单分别追踪用户寄回和商家换出两条物流，核对替换商品规格、数量、签收与验货；退件签收不等于换货完成。
3. 把批准、寄回、验货、换出、用户实收或退款分阶段排列，记录各阶段状态时间和证据。
4. 按当前官方规则核实每阶段时限、补件或暂停条件；单据重开是否重算期限需明确依据，不能自行刷新倒计时。
5. 按支付渠道拆分退款、补差、运费退补，逐笔匹配到账；不可把购物券、余额和银行卡回款混成同一种现金。
6. 交付双向物流与资金核对表、单据沿革及待补凭证；只有对应新货实收或全部资金匹配才标相应分支完成。
...
旧单与新单不重复计应退款；换货两条运单分开；各支付渠道和补差有独立证据；签收、验货和资金到账不混为一谈。
...
示例请求：请使用 $ai-hive-practical-returns-tracker 处理我提供的资料，给我可编辑的结果和待确认项，不要未经确认执行对外动作。

需要理解典型易错情况时读 [合成测试案例](references/examples.md)，不得将样例数字或人物作为真实用户资料。

## 边界与停止条件

不自动提交退货、寄件或投诉，不把时间线当作法律责任认定。
