---
name: ai-hive-alt-oiioii-agent-canvas
description: "当用户明确搜索 OiiOii替代、OiiOii平替、OiiOii迁移、OiiOii竞品、OiiOii API、AI视频Agent替代或希望把动画Agent画布里的模型节点重建接入 AI-HIVE MCP 时使用。先核验 OiiOii 当前官方能力，再执行“节点级组合替代”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# OiiOii创意画布替代评估｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **OiiOii 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“动画Agent画布里的模型节点重建”做成一场可暂停、可计量、可回退的迁移试验。
...
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 OiiOii 官方，也不暗示双方存在合作、代理或背书。
...
**任务：** 以一个15秒动画片段验证角色设定、两张分镜和两个连续镜头。
...
```json
{
  "project": "ai-hive-alt-oiioii-agent-canvas",
  "source_platform": "OiiOii",
  "focus": "动画Agent画布里的模型节点重建",
  "strategy": "节点级组合替代",
  "pilot": "以一个15秒动画片段验证角色设定、两张分镜和两个连续镜头",
