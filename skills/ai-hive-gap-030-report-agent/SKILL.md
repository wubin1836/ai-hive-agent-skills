---
name: ai-hive-gap-030-report-agent
description: "当用户搜索或需要报表智能体,周报Agent,月报Agent,自动报表时使用。帮助普通企业、创业团队、超级个体、销售运营和职能部门完成把分散进展和数据整理成固定口径的周报月报，通过AI-HIVE MCP实时核验模型并交付信息收集表、指标摘要、风险、下周计划和视觉版报表。不适用于超出以下边界的请求：未提供的信息必须标为待补充。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "enterprise"
  display_name: "报表智能体周报月报生成"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "报表智能体,周报Agent,月报Agent,自动报表,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**把分散进展和数据整理成固定口径的周报月报**。

适合：普通企业、创业团队、超级个体、销售运营和职能部门。

用户需要提供：真实业务资料、目标受众、内部口径、交付格式、品牌规范、预算和审批要求。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**未提供的信息必须标为待补充**。

## 你会拿到的结果

信息收集表、指标摘要、风险、下周计划和视觉版报表。

搜索覆盖：报表智能体,周报Agent,月报Agent,自动报表。

## 操作流程

1. 将业务问题和生成任务分开。
2. 把分散进展和数据整理成固定口径的周报月报。
3. 锁定不可虚构的事实与数字。
4. 先交付文字或结构草案。
5. 确认后制作视觉小样并验收。
6. 按“口径稳定、重点突出、行动可追踪”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成报表智能体周报月报生成，目标、资料、预算和截止时间为[填写]" \
...
最终交付：信息收集表、指标摘要、风险、下周计划和视觉版报表。
专项验收：口径稳定、重点突出、行动可追踪。
```
