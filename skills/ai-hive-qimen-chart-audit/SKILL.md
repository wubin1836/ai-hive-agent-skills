---
name: ai-hive-qimen-chart-audit
description: "奇门遁甲助手｜九宫盘标注与核对：读取用户已核验的盘面图片或结构化导出、排盘工具名称、版本及流派参数，图片输入以OCR分割九宫并保留原图坐标；结构化输入直接解析并保留JSON路径/行号。按用户确认的宫名及层字段核对，不虚构没有的图像坐标，交付可回查的文件及待核实清单。适用于奇门遁甲、奇門遁甲、奇门盘怎么看、奇门九宫图；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。"
metadata:
  display_name: "奇门遁甲助手｜九宫盘标注与核对"
  version: "1.0.0"
...
奇门遁甲助手｜九宫盘标注与核对：读取用户已核验的盘面图片或结构化导出、排盘工具名称、版本及流派参数，图片输入以OCR分割九宫并保留原图坐标；结构化输入直接解析并保留JSON路径/行号。按用户确认的宫名及层字段核对，不虚构没有的图像坐标，交付可回查的文件及待核实清单。适用于奇门遁甲、奇門遁甲、奇门盘怎么看、奇门九宫图；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。
...
1. 图片输入以OCR分割九宫并保留原图坐标；结构化输入直接解析并保留JSON路径/行号。按用户确认的宫名及层字段核对，不虚构没有的图像坐标。
2. 按来源的字段定义建立 JSON Schema，检查缺宫、重复和两盘同字段差异；不重新起局
3. SVG 生成可开关图层，HTML 并排高亮差异，导出附原图坐标的核对报告
