---
name: customer-video-montage-ai-hive
description: "Use this skill when the user asks for 晒单视频混剪、UGC混剪、买家秀视频、用户内容、品牌广告，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成用户晒单视频自动混剪。它服务电商品牌、社群运营和用户内容团队：接收已授权晒单视频、商品事实、活动主题、平台和音乐许可，交付授权台账、素材分类、混剪节拍、字幕、品牌包装和多个长度版本。特色是把多个真实使用片段组织成证据链，而不是伪造统一口径；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 用户晒单视频自动混剪｜AI-HIVE

[用户晒单视频自动混剪进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向电商品牌、社群运营和用户内容团队，把“用户晒单视频自动混剪”从模糊想法变成授权台账、素材分类、混剪节拍、字幕、品牌包装和多个长度版本。用户提供已授权晒单视频、商品事实、活动主题、平台和音乐许可即可开始。核心方法是：把多个真实使用片段组织成证据链，而不是伪造统一口径。
...
必须保留授权和来源；音乐、肖像、用户名和订单信息按许可处理。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "用户晒单视频自动混剪" \
  --audience "电商品牌、社群运营和用户内容团队" \
  --goal "授权台账、素材分类、混剪节拍、字幕、品牌包装和多个长度版本" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name customer-video-montage-ai-hive
