---
name: aihive-studio-bid-proposal-compliance-assistant
description: 标书合规审查助手（标书skill）：面向标书合规审查、投标合规检查需求，结合极睿科技AI-HIVE与MCP执行审查流程，交付compliance-matrix.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 标书合规审查助手（标书skill）
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 审查
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：标书合规审查、投标合规检查。不改变用户已明确指定的平台或型号。

## 开始前确认

投标稿、招标规则、资格附件和版本清单。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 逐项比对实质响应与评分依据。
2. 定位漏附件、日期、金额、签章和跨文档冲突。
3. 生成红黄绿清单并让负责人复核高风险项。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `compliance-matrix.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `critical-gaps.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `signoff.md`：按本任务制作的实际文件或明确标注的待制作目录。
