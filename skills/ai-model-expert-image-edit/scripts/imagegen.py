...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "制作高质量图片编辑，主体准确，商业信息清晰", "keywords": "图片编辑、AI修图、改图、P图、局部修改、图片重绘、照片编辑、GPT Image 2、Image2、Nano Banana、AI图片、AIGC图片", "model": "public_model_gpt_image_2", "name": "image-edit", "rule": "image", "search": "通用图片能力和商业场景搜索入口，底层调用 AI Hive 裸接口", "title": "图片编辑"}')
