---
name: aihive-studio-ecommerce-detail-module-studio
description: AI电商详情页模块：面向电商详情页、详情页模块、商品详情设计需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付module-map.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: AI电商详情页模块
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：电商详情页、详情页模块、商品详情设计。不改变用户已明确指定的平台或型号。

## 开始前确认

商品参数、卖点证据、详情页模块清单和宽度。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验图像生成或编辑、参考图输入、尺寸与下载工具。准确文字、透明通道、二维码要用真实文件工具另行检查。

## 执行流程

1. 给每个模块分配唯一问题并关联事实证据。
2. 分别制作首屏、细节、参数、使用说明的版式。
3. 图像生成后叠加准确文字并输出长图切片。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `module-map.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `detail-slices/`：按本任务制作的实际文件或明确标注的待制作目录。
- `text-layer.json`：按本任务制作的实际文件或明确标注的待制作目录。

每张切片衔接不重复，参数与售后条款不乱编。
