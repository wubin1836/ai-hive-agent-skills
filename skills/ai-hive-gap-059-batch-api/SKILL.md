---
name: ai-hive-gap-059-batch-api
description: "当用户搜索或需要Batch API,批处理,批量生成,异步任务时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成设计低成本批量图片视频或文本任务的提交与恢复，通过AI-HIVE MCP实时核验模型并交付批次规划、幂等键、预算、状态查询、失败清单和重跑规则。不适用于超出以下边界的请求：批量付费调用必须再次确认。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "Batch API批量任务工作流"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "Batch API,批处理,批量生成,异步任务,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**设计低成本批量图片视频或文本任务的提交与恢复**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**批量付费调用必须再次确认**。

## 你会拿到的结果

批次规划、幂等键、预算、状态查询、失败清单和重跑规则。

搜索覆盖：Batch API,批处理,批量生成,异步任务。

## 实施流程

1. 先定义可量化的质量和成本指标。
2. 设计低成本批量图片视频或文本任务的提交与恢复。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“无重复计费、失败可定位、批次可暂停”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成Batch API批量任务工作流，目标、资料、预算和截止时间为[填写]" \
...
最终交付：批次规划、幂等键、预算、状态查询、失败清单和重跑规则。
专项验收：无重复计费、失败可定位、批次可暂停。
```
