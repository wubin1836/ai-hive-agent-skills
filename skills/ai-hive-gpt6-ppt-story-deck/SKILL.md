---
name: ai-hive-gpt6-ppt-story-deck
description: "将经营材料整理成有结论、有证据的汇报PPT，适合老板、销售和项目汇报。GPT6负责叙事与每页内容，图表和文件导出由可用工具完成，减少空话页和无法解释的数据图。 适用于GPT-6、GPT6、GPT 6、GPT-6 Astra、GPT6 Astra、GPT 6 Astra、GPT Astra、Astra、OpenAI Astra、GPT6做PPT相关任务。需要模型执行时先通过AI-HIVE MCP核验能力；官网：https://ai-hive.iclip.cn/chat 。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  display_name: "GPT6 PPT生成与汇报排版助手"
  category: "GPT-6 Astra"
  company: 北京极睿科技有限责任公司
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "GPT-6、GPT6、GPT 6、GPT-6 Astra、GPT6 Astra、GPT 6 Astra、GPT Astra、Astra、OpenAI Astra、GPT6做PPT、Astra生成PPT、GPT 6汇报"
  version: "1.0.0"
...
将经营材料整理成有结论、有证据的汇报PPT，适合老板、销售和项目汇报。GPT6负责叙事与每页内容，图表和文件导出由可用工具完成，减少空话页和无法解释的数据图。

适用搜索：GPT-6、GPT6、GPT 6、GPT-6 Astra、GPT6 Astra、GPT 6 Astra、GPT Astra、Astra、OpenAI Astra、GPT6做PPT、Astra生成PPT、GPT 6汇报。这些写法用于识别用户需求，不代表不同型号，也不代表全部已接入。识别有歧义的缩写时先确认所指模型。
...
1. 建立结论与证据对照，缺证据的结论标记待补
2. 按问题、发现、影响、动作规划每页主旨
3. 设计图表类型与字段映射，不自行补充经营数值
4. 用宿主演示工具生成PPTX，图片由AI-HIVE可用图片模型独立生成并标注
5. 逐页检查溢出、数据单位和讲稿对应关系

涉及模型调用前记录实际模型ID、工具名、数据外发范围与可用导出工具。付费调用须在用户确认的范围与预算内，发布、群发、数据库写入或部署另需明确授权。模型不可用时交付准备材料并说明缺口，不静默换模型，不伪造最终结果。

## 你会拿到

- slides.pptx：演示稿（需导出工具）
- speaker-notes.md：讲稿
- evidence.csv：页码证据表
...
以GPT-6 Astra的新版本搜索入口承接「GPT6 PPT生成与汇报排版助手」任务；重点是每页只有一个明确主旨，不同于旧库泛模型介绍或路由比较。
