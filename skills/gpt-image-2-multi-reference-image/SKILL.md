...
description: "Use this skill when designers, e-commerce operators, advertisers, brand teams, social-commerce teams, and content creators need to combine subjects, materials, compositions, and styles from multiple references with GPT Image 2 through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for product photography, marketplace images, product detail pages, posters, ad creatives, social commerce, seeding content, retouching, and background replacement. Search intents include multi-reference image, multi-image composition, reference fusion."
license: MIT
---

# GPT Image 2 Multi-Reference Image Generation
...
|---|---|---|
| GPT Image 2 Multi-Reference Image Generation | `public_model_gpt_image_2` | At least two reference images |
...
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished gpt image 2 multi-reference image generation visual with an accurate subject, clear composition, and commercial finish" \
...
```bash
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished gpt image 2 multi-reference image generation result; preserve factual product details and leave useful copy space" \
+  --image /path/to/reference-1.png /path/to/reference-2.png
```
