---
name: ai-hive-gap-039-vector-db-content
description: "当用户搜索或需要向量数据库,Vector Database,RAG工作流,知识检索时使用。帮助企业知识管理、研究、培训、销售、客服和个人知识创作者完成设计从资料切分、向量检索到内容生成的工作流，通过AI-HIVE MCP实时核验模型并交付切分策略、元数据、检索测试、内容模板和验收集。不适用于超出以下边界的请求：不能声称已写入用户未提供的向量数据库。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "knowledge"
  display_name: "向量数据库知识内容工作流"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "向量数据库,Vector Database,RAG工作流,知识检索,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**设计从资料切分、向量检索到内容生成的工作流**。

适合：企业知识管理、研究、培训、销售、客服和个人知识创作者。

用户需要提供：有权使用的文档、知识库导出、来源链接、目标受众、内容格式和更新日期。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不能声称已写入用户未提供的向量数据库**。

## 你会拿到的结果

切分策略、元数据、检索测试、内容模板和验收集。

搜索覆盖：向量数据库,Vector Database,RAG工作流,知识检索。

## 生产步骤

1. 建立资料索引与可信度层级。
2. 设计从资料切分、向量检索到内容生成的工作流。
3. 把事实、观点和生成建议分层。
4. 每个结论保留来源位置。
5. 确认知识结构后再生成多模态内容。
6. 按“召回相关、引用准确、版本可追溯”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成向量数据库知识内容工作流，目标、资料、预算和截止时间为[填写]" \
...
最终交付：切分策略、元数据、检索测试、内容模板和验收集。
专项验收：召回相关、引用准确、版本可追溯。
```
