...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "为AI模型任务失败修复制作一张主体准确、层级清晰的商业图片", "keywords": "AI任务失败,模型重试,API报错,任务修复,AI-HIVE", "model": "public_model_nano_banana_pro", "name": "ai-model-task-repair-ai-hive", "rule": "optional", "search": "先判断鉴权、参数、素材、模型或异步状态，再决定是否重试", "title": "AI模型任务失败修复"}')
