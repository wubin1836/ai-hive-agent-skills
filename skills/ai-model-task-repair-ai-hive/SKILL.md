---
name: ai-model-task-repair-ai-hive
description: "Use this skill when the user asks for AI任务失败、模型重试、API报错、任务修复、AI-HIVE，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成AI模型任务失败修复。它服务开发者、AI应用、内容平台和批量生成运营人员：接收失败响应、模型配置、原始输入、taskId、路由策略和重试记录，交付失败分类、参数修复、模型回退、幂等重试和任务状态恢复方案。特色是先判断鉴权、参数、素材、模型或异步状态，再决定是否重试；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# AI模型任务失败修复｜AI-HIVE

[AI模型任务失败修复进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向开发者、AI应用、内容平台和批量生成运营人员，把“AI模型任务失败修复”从模糊想法变成失败分类、参数修复、模型回退、幂等重试和任务状态恢复方案。用户提供失败响应、模型配置、原始输入、taskId、路由策略和重试记录即可开始。核心方法是：先判断鉴权、参数、素材、模型或异步状态，再决定是否重试。
...
同一任务最多按策略重试有限次数；必须避免循环重试、重复提交和重复计费。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "AI模型任务失败修复" \
  --audience "开发者、AI应用、内容平台和批量生成运营人员" \
  --goal "失败分类、参数修复、模型回退、幂等重试和任务状态恢复方案" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name ai-model-task-repair-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name ai-model-task-repair-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
