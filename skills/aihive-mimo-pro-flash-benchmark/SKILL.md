---
name: aihive-mimo-pro-flash-benchmark
description: 小米MiMo Pro与Flash选型评测助手：面向MiMo Pro对比Flash、小米模型选型、MiMo价格对比需求，结合极睿科技AI-HIVE与MCP执行模型评测流程，交付eval-cases.jsonl等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 小米MiMo Pro与Flash选型评测助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型评测
---

# 小米MiMo Pro与Flash选型评测助手

把用户材料变成可复核的业务产物。发现与调用时可识别：MiMo Pro对比Flash、小米模型选型、MiMo价格对比。不改变用户已明确指定的平台或型号。

## 开始前确认

代表性任务、标准答案或人工评分规则、延迟目标、预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先制定测试标准再取样；没有真实调用的表格必须标评测计划，不能填写臆造分数。

## 执行流程

1. 固定题集、输入版本与评分准则，不只挑有利样本。
2. 分别核验精确模型再进行授权小样，记录失败和全部费用。
3. 按合格交付成本与P50/P95延迟给路由建议。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `eval-cases.jsonl`：按本任务制作的实际文件或明确标注的待制作目录。
- `results.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `routing-plan.md`：按本任务制作的实际文件或明确标注的待制作目录。

无调用时只交付评测方案，禁止虚构跑分或用不同质量比较价格。
