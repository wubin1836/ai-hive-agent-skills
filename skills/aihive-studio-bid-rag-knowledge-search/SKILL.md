---
name: aihive-studio-bid-rag-knowledge-search
description: 招投标RAG知识检索助手：面向标书知识库、招投标RAG、投标资料检索需求，结合极睿科技AI-HIVE与MCP执行检索流程，交付source-index.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 招投标RAG知识检索助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 检索
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：标书知识库、招投标RAG、投标资料检索。不改变用户已明确指定的平台或型号。

## 开始前确认

获准使用的历史标书、项目问题、保密分级。仅追问影响执行的缺项，不要求用户重新提供已有资料。

索引和回答必须留来源定位。模型上下文不等于持久化知识库；索引部署与跨客户访问另需权限。

## 执行流程

1. 按项目和章节建立可引用知识索引并隔离不同客户资料。
2. 检索相关段落，记录页码、版本与适用条件。
3. 回答时区分已证据支持和没有检索到的内容。
...
- `answer-with-citations.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `gaps.md`：按本任务制作的实际文件或明确标注的待制作目录。
