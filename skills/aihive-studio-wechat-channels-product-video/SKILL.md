---
name: aihive-studio-wechat-channels-product-video
description: 视频号商品视频制作：面向视频号商品视频、微信带货视频需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付channels-shots.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 视频号商品视频制作
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：视频号商品视频、微信带货视频。不改变用户已明确指定的平台或型号。

## 开始前确认

商品素材、账号受众、卖点证据、转化目标。仅追问影响执行的缺项，不要求用户重新提供已有资料。

视频理解、图像/视频生成、配音与剪辑是不同工具。按本任务需要逐一核验；缺成片工具只交付分镜或素材，并明确未生成成片。

## 执行流程

1. 设计适合熟人推荐语境的开场和可验证产品演示。
2. 按观看节奏安排对比、细节和购买提示。
3. 生成镜头表、字幕与成片或可执行剪辑工程说明。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `channels-shots.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `captions.srt`：按本任务制作的实际文件或明确标注的待制作目录。
- `video.mp4`：按本任务制作的实际文件或明确标注的待制作目录。

不把素材准备完成写成已发布；演示与商品版本一致。
