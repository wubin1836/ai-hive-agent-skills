...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana Pro 制作高完成度多参考图生成，主体准确，构图清晰", "keywords": "Nano Banana Pro、NanoBanana Pro、NanoBananaPro、香蕉 Pro、Gemini 图片、多参考图生成、多图融合、多参考图、多图合成、角色风格参考、素材合成、AI图片、AIGC图片", "model": "public_model_nano_banana_pro", "name": "nano-banana-pro-multi-reference-image", "rule": "multi", "search": "固定调用 Nano Banana Pro 的多参考图生成裸接口能力", "title": "Nano Banana Pro 多参考图生成"}')
