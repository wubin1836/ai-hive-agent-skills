---
name: fan-vote-story-co-creation-ai-hive
description: "Use this skill when the user asks for 粉丝投票剧情、共创短剧、互动IP、剧情投票、社群内容，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成粉丝投票共创剧情。它服务品牌IP、虚拟偶像、短剧账号和粉丝社区运营：接收角色设定、主线边界、候选选项、平台、投票周期和后续产能，交付可投票分支、候选海报、预告视频、结果承接脚本和更新计划。特色是给粉丝真实可执行的选择，并预先评估每条路径成本；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 粉丝投票共创剧情｜AI-HIVE

[粉丝投票共创剧情进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向品牌IP、虚拟偶像、短剧账号和粉丝社区运营，把“粉丝投票共创剧情”从模糊想法变成可投票分支、候选海报、预告视频、结果承接脚本和更新计划。用户提供角色设定、主线边界、候选选项、平台、投票周期和后续产能即可开始。核心方法是：给粉丝真实可执行的选择，并预先评估每条路径成本。
...
不得伪造投票结果或设计无法兑现的选项；收集用户数据要遵守隐私规则。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "粉丝投票共创剧情" \
  --audience "品牌IP、虚拟偶像、短剧账号和粉丝社区运营" \
  --goal "可投票分支、候选海报、预告视频、结果承接脚本和更新计划" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name fan-vote-story-co-creation-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name fan-vote-story-co-creation-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
