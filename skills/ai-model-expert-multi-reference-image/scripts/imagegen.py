...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "制作高质量多参考图生成，主体准确，商业信息清晰", "keywords": "多参考图生成、多图融合、多参考图、多图合成、角色风格参考、素材合成、GPT Image 2、Image2、Nano Banana、AI图片、AIGC图片", "model": "public_model_nano_banana_2", "name": "multi-reference-image", "rule": "multi", "search": "通用图片能力和商业场景搜索入口，底层调用 AI Hive 裸接口", "title": "多参考图生成"}')
