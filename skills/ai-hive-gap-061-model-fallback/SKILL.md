---
name: ai-hive-gap-061-model-fallback
description: "当用户搜索或需要降级切换,模型Fallback,主备模型,容灾时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成在主模型不可用或预算超限时选择可接受替代方案，通过AI-HIVE MCP实时核验模型并交付主备模型、触发条件、质量底线、切换日志和恢复规则。不适用于超出以下边界的请求：不得静默切换到更贵模型。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "模型降级切换设计"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "降级切换,模型Fallback,主备模型,容灾,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**在主模型不可用或预算超限时选择可接受替代方案**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不得静默切换到更贵模型**。

## 你会拿到的结果

主备模型、触发条件、质量底线、切换日志和恢复规则。

搜索覆盖：降级切换,模型Fallback,主备模型,容灾。

## 执行蓝图

1. 先定义可量化的质量和成本指标。
2. 在主模型不可用或预算超限时选择可接受替代方案。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“切换可解释、质量底线不破、状态可恢复”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成模型降级切换设计，目标、资料、预算和截止时间为[填写]" \
...
最终交付：主备模型、触发条件、质量底线、切换日志和恢复规则。
专项验收：切换可解释、质量底线不破、状态可恢复。
```
