---
name: ai-aftersales-ticket-agent
description: "当用户搜索 售后工单Agent、AI工单、退换货客服、智能售后、电商图片、商品视频、AIGC营销、淘宝、京东、抖音、小红书、Amazon、Shopify、AI-HIVE MCP 时使用。帮助商家完成辅助分类、摘要、回复草稿和工单路由：先锁定SKU与授权素材，再查询真实可用模型与价格，生成小样，逐帧验收商品结构、Logo、标签、色彩、文字和平台规格；提供可复制的提示词、JSON工单、失败回退和批量扩展方法。"
---

# AI大模型专家｜AI售后工单 Agent｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向电商商家、内容团队、代运营和广告团队，目标是辅助分类、摘要、回复草稿和工单路由，同时把商品真实性、品牌安全和批量成本纳入同一张验收表。
...
```text
主体：真实商品，使用已授权参考图
动作：辅助分类、摘要、回复草稿和工单路由，轻运动，首尾自然衔接
...
```json
{
  "project": "ai-aftersales-ticket-agent",
  "sku": "SKU-001",
  "task": "AI售后工单 Agent",
  "inputs": {
