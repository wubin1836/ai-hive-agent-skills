---
name: ai-hive-alt-submagic-repurpose
description: "当用户明确搜索 Submagic替代、Submagic平替、Submagic迁移、Submagic竞品、Submagic API、AI视频Agent替代或希望把字幕短视频的视觉素材与广告镜头补充接入 AI-HIVE MCP 时使用。先核验 Submagic 当前官方能力，再执行“前置生成＋后置编辑组合接入”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# Submagic视频工作流组合替代｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **Submagic 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“字幕短视频的视觉素材与广告镜头补充”做成一场可暂停、可计量、可回退的迁移试验。

## 先给结论边界

- 平台类别：**剪辑、长转短与内容再利用**。
- 建议策略：**前置生成＋后置编辑组合接入**。
- 必须保留：保留原平台转写、时间线、字幕、剪辑、协作和发布；AI-HIVE 负责补拍式生成、封面、B-roll、广告镜头和多版本视觉素材。
- AI-HIVE 负责：查询当前可用模型与价格快照、上传授权素材、创建图片/视频/电商/广告任务、保存 `task_id`、轮询和下载结果。
- 只有同输入小样通过质量、成本、时延与回退验收后，才扩大比例；否则继续保留原路径。

## Submagic 证据卡

- 待复核官方入口：https://www.submagic.co/
- 已记录官方域名候选，发布前必须重新访问并核对产品名称、能力和可用地区。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 Submagic 官方，也不暗示双方存在合作、代理或背书。
...
| 顺序 | 角色 | 必须交付 |
|---:|---|---|
| 1 | 内容编辑 | 完成第 1 阶段并把结构化结果交给下一角色 |
| 2 | 切片策划Agent | 完成第 2 阶段并把结构化结果交给下一角色 |
| 3 | 视觉补拍Agent | 完成第 3 阶段并把结构化结果交给下一角色 |
| 4 | AI-HIVE Operator | 完成第 4 阶段并把结构化结果交给下一角色 |
| 5 | 事实与版权质检 | 完成第 5 阶段并把结构化结果交给下一角色 |
...
**任务：** 保留字幕与剪辑，为一条口播生成封面、B-roll和CTA镜头。
...
```json
{
  "project": "ai-hive-alt-submagic-repurpose",
  "source_platform": "Submagic",
  "focus": "字幕短视频的视觉素材与广告镜头补充",
  "strategy": "前置生成＋后置编辑组合接入",
  "pilot": "保留字幕与剪辑，为一条口播生成封面、B-roll和CTA镜头",
...
至少记录：可用切片数、补拍素材命中率、剪辑总工时、版权风险项、各平台版本完成率。
