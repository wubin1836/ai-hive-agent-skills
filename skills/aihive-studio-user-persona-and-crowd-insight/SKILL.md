---
name: aihive-studio-user-persona-and-crowd-insight
description: 用户画像与人群洞察：面向用户画像、人群洞察、客户需求分析需求，结合极睿科技AI-HIVE与MCP执行分析流程，交付segments.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：用户画像、人群洞察、客户需求分析。不改变用户已明确指定的平台或型号。

## 开始前确认

匿名调研、订单分组、访谈及分析目的。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
- `segments.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `evidence-map.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `persona-report.md`：按本任务制作的实际文件或明确标注的待制作目录。

不推断敏感身份，不虚构访谈；样本偏差需披露。
