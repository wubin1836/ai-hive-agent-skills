---
name: aihive-studio-member-system-setup
description: 会员体系搭建：面向会员体系、会员等级设计、积分规则需求，结合极睿科技AI-HIVE与MCP执行运营流程，交付membership-rules.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：会员体系、会员等级设计、积分规则。不改变用户已明确指定的平台或型号。

## 开始前确认

客单、复购周期、毛利、可用权益、会员系统限制。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
- `membership-rules.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `unit-economics.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `test-cases.md`：按本任务制作的实际文件或明确标注的待制作目录。
