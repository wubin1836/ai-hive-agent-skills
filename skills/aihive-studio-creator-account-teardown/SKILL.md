---
name: aihive-studio-creator-account-teardown
description: 账号拆解与起号模板：面向账号拆解、起号模板、账号定位分析需求，结合极睿科技AI-HIVE与MCP执行分析流程，交付account-content.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 账号拆解与起号模板
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 分析
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：账号拆解、起号模板、账号定位分析。不改变用户已明确指定的平台或型号。

## 开始前确认

公开账号链接或用户导出内容、时间范围、对标目的。仅追问影响执行的缺项，不要求用户重新提供已有资料。

对用户授权的数据实际执行清洗与计算，保留统计口径、分母和来源。无联网工具时只分析已提供资料。

## 执行流程

1. 按选题、内容形式、发布频率和互动公开指标编码。
2. 区分相关性与因果，排除单条异常带来的误判。
3. 形成可执行起号实验及衡量周期。
...
- `account-content.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `diagnosis.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `test-calendar.csv`：按本任务制作的实际文件或明确标注的待制作目录。

标明样本覆盖范围和抓取时间，不声称知道私有流量或收入。
