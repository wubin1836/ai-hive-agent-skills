...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "为文字商标设计制作原创、清晰、可审查的商业视觉小样", "keywords": "文字商标设计、字标商标,文字logo,名称标志、品牌设计、AI图片、AI-HIVE", "model": "public_model_nano_banana_pro", "name": "visual-wordmark-trademark-ai-hive", "rule": "optional", "search": "提交前查询AI-HIVE当前图片模型与价格快照，不硬编码价格", "title": "AI大模型专家｜文字商标设计"}')
