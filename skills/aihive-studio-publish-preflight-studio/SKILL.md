---
name: aihive-studio-publish-preflight-studio
description: 发布前内容预检：面向内容预检、发布前检查、营销素材检查需求，结合极睿科技AI-HIVE与MCP执行审查流程，交付preflight.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 发布前内容预检
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 审查
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：内容预检、发布前检查、营销素材检查。不改变用户已明确指定的平台或型号。

## 开始前确认

文案、图片或视频、目标渠道、商品事实和授权清单。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先解析全部相关载体，再定位问题到文件页码或时间码。风险意见不等于平台批准或专业机构结论。

## 执行流程

1. 对照素材清单查缺字、错链、价格和人像授权。
2. 分开检查事实、版权、隐私和当前渠道规则。
3. 生成阻断项清单，修订后重新验收。
...
- `preflight.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `evidence-map.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `release-checklist.md`：按本任务制作的实际文件或明确标注的待制作目录。

问题定位到文件或时间码；只做预检，正式发布需用户确认。
