---
name: aihive-gpt6-luna-batch
description: GPT-6 Luna批量表格处理助手：面向GPT-6 Luna、GPT6 Luna、GPT Luna需求，结合极睿科技AI-HIVE与MCP执行模型批处理流程，交付processed.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: GPT-6 Luna批量表格处理助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型批处理
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：GPT-6 Luna、GPT6 Luna、GPT Luna、Luna批处理。不改变用户已明确指定的平台或型号。

## 开始前确认

脱敏任务表、唯一行ID、分类或提取schema、验收样本和预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

逐行保留ID、状态与费用；未知结果先查原任务，遇429尊重Retry-After后停止，不无限重试。

## 执行流程

1. 校验输入行并将分类、提取等聚焦任务与复杂推理分开。
2. 核验Luna接口参数，先做小样结构校验和人工抽检。
3. 按已批准批次运行并输出行级错误与费用台账。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `processed.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `failures.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `quality-report.md`：按本任务制作的实际文件或明确标注的待制作目录。
