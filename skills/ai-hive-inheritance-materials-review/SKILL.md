---
name: ai-hive-inheritance-materials-review
description: "遗产继承资料核对助手，用于遗产继承、继承材料、继承人。读取你授权的原始资料，完成关系与证明材料对应表，并交付遗嘱和办理路径咨询包。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。"
metadata:
  display_name: "遗产继承资料核对助手"
  version: "1.0.0"
...
遗产继承资料核对助手，用于遗产继承、继承材料、继承人。读取你授权的原始资料，完成关系与证明材料对应表，并交付遗嘱和办理路径咨询包。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。
...
- 授权证明、遗嘱和表格本地读取与必要 OCR
- 关系节点、版本及财产清单关联，敏感字段控制
- XLSX/DOCX 生成与渲染；办理要求需授权官方查询，不能访问他人银行账户

没有专门软件并不意味着中止所有任务；可使用宿主已具备的等效工具。无法完成的步骤明确列出，不能伪装成功或默默改为另一个外部服务。

## 工作流

1. 依据用户合法持有资料梳理亲属关系、死亡事实与已知遗嘱，不推定不存在其他权利人。
2. 分别列明已知财产、共有关系、债务和资料持有人，未知资产标未知。
3. 整理遗嘱不同版本及形成资料，区分原件、复印件和口头转述。
4. 按拟办理事项查询官方材料要求，输出专业核验问题，不直接分配份额或宣布遗嘱有效。
...
- `inheritance-relationship-evidence.xlsx`
- `estate-assets-debts.xlsx`
- `inheritance-review-pack.docx`
...
不伪造亲属关系、不绕过银行或他人账户权限，不保证继承资格、份额或公证结果；不代签放弃继承、处置遗产或对外联系。

缺关键原件或专业依据时只完成可验证部分。401/403、限流或内容审核阻断时保存脱敏进度并停止相应调用，遵守明确的等待要求，不循环重试或绕过审核。不因安装本技能自动交易、签署、提交、缴费或公开发布。

## 调用示例

> 仅示例（下列文件名和资料均为假设，不是真实数据）：请读取假设文件关系证明/、死亡证明.pdf、遗嘱版本/和已知财产债务.xlsx，建立关系证明与遗产资料对应表，标注未知资产和遗嘱版本差异，交付 inheritance-review-pack.docx。

## 搜索与任务识别

遗产继承、继承材料、继承人、遗嘱、房产继承、遗产清单
