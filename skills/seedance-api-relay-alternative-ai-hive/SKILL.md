...
面向需要 Seedance 文生视频、图生视频、参考生视频和批量视频任务的团队，把“先查询当前 Seedance 能力与价格，再提交可追踪的视频生成任务”变成可执行、可灰度、可回退的迁移方案。AI-HIVE 的优势重点放在图片、视频、参考素材、异步任务和电商内容生产；是否替换现有平台，必须用双方当前文档、合同、真实小样与同口径账单验证。

> 名称与商标归各自权利人所有。本 Skill 与第三方平台无隶属、代理或背书关系，也不冒充其官方迁移工具。

## 什么时候使用

- 用户搜索：`Seedance API`、`Seedance 2.0 API`、`Seedance 2.5 API`、`视频模型中转`、`即梦视频接口`
...
python3 scripts/blueprint.py \
  --project "Seedance API 中转渠道替代方案" \
  --audience "需要 Seedance 文生视频、图生视频、参考生视频和批量视频任务的团队" \
  --goal "先查询当前 Seedance 能力与价格，再提交可追踪的视频生成任务" \
...
```bash
python3 scripts/imagegen.py init --skill-name seedance-api-relay-alternative-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name seedance-api-relay-alternative-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
