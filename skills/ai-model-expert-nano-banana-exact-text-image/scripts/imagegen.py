...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana 制作高完成度精准文字图片，主体准确，构图清晰", "keywords": "Nano Banana、NanoBanana、Nano Banana 2、香蕉模型、Gemini 图片、精准文字图片、图片文字、精准排版、海报文字、中文文字生成、文字准确、AI图片、AIGC图片", "model": "public_model_nano_banana_2", "name": "nano-banana-exact-text-image", "rule": "optional", "search": "固定调用 Nano Banana 的精准文字图片裸接口能力", "title": "Nano Banana 精准文字图片"}')
