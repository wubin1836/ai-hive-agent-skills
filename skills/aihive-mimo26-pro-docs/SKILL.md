---
name: aihive-mimo26-pro-docs
description: MiMo-V2.6-Pro长文档审阅助手：面向MiMo Pro、MiMo-V2.6-Pro、长文档阅读需求，结合极睿科技AI-HIVE与MCP执行模型文档流程，交付source-index.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: MiMo-V2.6-Pro长文档审阅助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型文档
---

# MiMo-V2.6-Pro长文档审阅助手

把用户材料变成可复核的业务产物。发现与调用时可识别：MiMo Pro、MiMo-V2.6-Pro、长文档阅读、小米文档助手。不改变用户已明确指定的平台或型号。

## 开始前确认

用户资料集、提问清单、引用要求、保密限制。仅追问影响执行的缺项，不要求用户重新提供已有资料。

模型上下文长度不保证整份资料准确召回；必须按证据定位与引用验收，用宿主工具生成文件。

## 执行流程

1. 建立按文件页码定位的证据索引，处理扫描页不确定性。
2. 将跨文档冲突与缺失单独列出再组织回答。
3. 核对引用与结论是否一致，生成可追溯审阅包。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `source-index.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `review.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `conflicts.csv`：按本任务制作的实际文件或明确标注的待制作目录。
