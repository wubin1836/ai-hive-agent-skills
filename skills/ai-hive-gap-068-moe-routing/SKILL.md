---
name: ai-hive-gap-068-moe-routing
description: "当用户搜索或需要MoE,稀疏模型,Mixture of Experts,模型路由时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成理解MoE模型的有效参数、速度和成本差异并设计路由，通过AI-HIVE MCP实时核验模型并交付概念说明、任务评测、模型候选、成本表和路由规则。不适用于超出以下边界的请求：不得仅按总参数量判断质量与费用。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "MoE稀疏模型成本路由"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "MoE,稀疏模型,Mixture of Experts,模型路由,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**理解MoE模型的有效参数、速度和成本差异并设计路由**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不得仅按总参数量判断质量与费用**。

## 你会拿到的结果

概念说明、任务评测、模型候选、成本表和路由规则。

搜索覆盖：MoE,稀疏模型,Mixture of Experts,模型路由。

## 运行路线

1. 先定义可量化的质量和成本指标。
2. 理解MoE模型的有效参数、速度和成本差异并设计路由。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“版本清楚、指标可比、路由有测试”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成MoE稀疏模型成本路由，目标、资料、预算和截止时间为[填写]" \
...
最终交付：概念说明、任务评测、模型候选、成本表和路由规则。
专项验收：版本清楚、指标可比、路由有测试。
```
