---
name: aihive-studio-pdf-tupianwenzitiqu
description: PDF和图片文字提取：面向PDF文字提取、图片转文字、OCR识别需求，结合极睿科技AI-HIVE与MCP执行文档流程，交付extracted.md等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：PDF文字提取、图片转文字、OCR识别。不改变用户已明确指定的平台或型号。

## 开始前确认

用户PDF或图片、目标语言、页范围、表格保留需求。仅追问影响执行的缺项，不要求用户重新提供已有资料。

需要真实文件读写、格式转换和预览工具；没有Word/PPT生成或渲染工具时明确降级为结构化草稿。

## 执行流程

1. 先检测PDF文本层与扫描页并按页编号。
2. OCR后保留疑似错字与低置信位置。
3. 按原页顺序输出文本和表格并逐页抽检。
...
- `extracted.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `tables.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `ocr-review.csv`：按本任务制作的实际文件或明确标注的待制作目录。
