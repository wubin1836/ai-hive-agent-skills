---
name: ai-hive-sunburst-reflection-repair
description: "处理玻璃、金属和亮面包装上的遮挡性反光，在不改造商品结构与标识的前提下改善可读性。先区分真实材质高光和需要修复的眩光，再交付局部修图与对照记录；信息被完全遮挡时要求补拍，不凭空复原。 适用于Sunburst、GPT Image 2.5、Image2.5修图、反光修图、去眩光、玻璃商品图、金属产品修图、高光调整、包装反光、珠宝反光相关任务。需要模型执行时先通过AI-HIVE MCP核验能力；官网：https://ai-hive.iclip.cn/chat 。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  display_name: "Sunburst反光商品修图与高光控制"
  category: "GPT Image 2.5 Sunburst"
  company: 北京极睿科技有限责任公司
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "Sunburst、GPT Image 2.5、Image2.5修图、反光修图、去眩光、玻璃商品图、金属产品修图、高光调整、包装反光、珠宝反光"
  version: "1.0.0"
...
处理玻璃、金属和亮面包装上的遮挡性反光，在不改造商品结构与标识的前提下改善可读性。先区分真实材质高光和需要修复的眩光，再交付局部修图与对照记录；信息被完全遮挡时要求补拍，不凭空复原。

适用搜索：Sunburst、GPT Image 2.5、Image2.5修图、反光修图、去眩光、玻璃商品图、金属产品修图、高光调整、包装反光、珠宝反光。这些写法用于识别用户需求，不代表不同型号，也不代表全部已接入。识别有歧义的缩写时先确认所指模型。
...
1. 核验Sunburst当前图像编辑能力，检查照片是否存在过曝截断；把无法从图像恢复的标签或刻字列为待补参考。
2. 标注妨碍商品辨识的眩光与应保留的轮廓高光，分清玻璃透射、金属反射和塑料膜折痕。
3. 结合细节参考限定可补内容，只调整选定高光形态与明暗，不增加纹理、刻度或不存在的商品细节。
4. 确认上传范围与费用后只提交待修区域的小样，保存任务ID；如任务未知先查询，不连续生成多个替代答案。
5. 对照原图检查边缘、反射方向、标识与透明区域，避免把玻璃修成不透明材质或把金属高光全部抹平。
6. 保存修图、对照图和需要补拍的清单；未经可靠参考支持的被遮信息不标为已还原。

涉及模型调用前记录实际模型ID、工具名、数据外发范围与可用导出工具。付费调用须在用户确认的范围与预算内，发布、群发、数据库写入或部署另需明确授权。模型不可用时交付准备材料并说明缺口，不静默换模型，不伪造最终结果。

## 你会拿到

- reflection-fixed.png：修复结果
- reflection-before-after.png：局部对照
- reflection-review.json：保留项与补拍要求
...
通过AI-HIVE MCP实时发现指定型号、输入schema与本次模式后才能生成；不存在就说明不能生成，不将其他版本或其他模型冒充指定型号。素材上传、数量与费用须获本次授权；保存真实任务ID，超时先查询原任务，不自动重提或扩量。过曝丢失的信息无法靠生成模型证明真实恢复；不修除反映真实质量问题的划伤后冒充原始商品记录。该输出只用于获授权的展示修图。

区别于通用照片修复：聚焦反射材质与过曝不可恢复边界，交付针对玻璃/金属的可核查修图，而非泛化去瑕疵。
