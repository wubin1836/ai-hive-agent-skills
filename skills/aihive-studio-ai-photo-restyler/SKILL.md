---
name: aihive-studio-ai-photo-restyler
description: 照片转绘：面向照片转绘、照片变插画、照片风格转换需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付style-brief.md等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：照片转绘、照片变插画、照片风格转换。不改变用户已明确指定的平台或型号。

## 开始前确认

自有照片、目标视觉风格、保留的人物或产品特征。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验图像生成或编辑、参考图输入、尺寸与下载工具。准确文字、透明通道、二维码要用真实文件工具另行检查。

## 执行流程

1. 提取身份和外观锁定项，不猜测敏感身份。
2. 用视觉特征描述风格并确定允许变动区域。
3. 生成预览后逐项检查人物和物体的一致性。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `style-brief.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `variants/`：按本任务制作的实际文件或明确标注的待制作目录。
- `identity-check.md`：按本任务制作的实际文件或明确标注的待制作目录。
