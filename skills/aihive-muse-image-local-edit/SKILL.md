---
name: aihive-muse-image-local-edit
description: Muse Image局部改图助手：面向Muse改图、Muse图像编辑、Meta图片修改需求，结合极睿科技AI-HIVE与MCP执行模型图片流程，交付edit-regions.json等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Muse Image局部改图助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：Muse改图、Muse图像编辑、Meta图片修改。不改变用户已明确指定的平台或型号。

## 开始前确认

自有图片、待修改区域、固定元素和尺寸。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 用坐标或遮罩明确改动范围，保留原图。
2. 按实际编辑schema提交小样而非整图盲目重绘。
3. 对比修改区与保护区差异并输出版本记录。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `edit-regions.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `edited.png`：按本任务制作的实际文件或明确标注的待制作目录。
- `change-review.md`：按本任务制作的实际文件或明确标注的待制作目录。

保护区产品标签和人物身份保持一致；必要来源标识不删除。
