---
name: ai-hive-gap-001-background-agent-content
description: "当用户搜索或需要后台智能体,background agent,异步Agent,长任务时使用。帮助AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人完成让后台任务持续完成内容拆解、生成、查询与恢复，通过AI-HIVE MCP实时核验模型并交付后台任务图、检查点、内容批次、任务ID和恢复记录。不适用于超出以下边界的请求：后台运行不等于无限授权，付费、发送和发布仍需逐项确认。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "agent"
  display_name: "后台智能体连续内容生产"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "后台智能体,background agent,异步Agent,长任务,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**让后台任务持续完成内容拆解、生成、查询与恢复**。

适合：AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人。

用户需要提供：业务目标、可用工具、授权范围、输入资料、预算、截止时间和人工确认点。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**后台运行不等于无限授权，付费、发送和发布仍需逐项确认**。

## 你会拿到的结果

后台任务图、检查点、内容批次、任务ID和恢复记录。

搜索覆盖：后台智能体,background agent,异步Agent,长任务。

## 交付路径

1. 先画出任务状态和权限边界。
2. 让后台任务持续完成内容拆解、生成、查询与恢复。
3. 将可并行任务与必须串行确认的动作分开。
4. 用最小样例验证工具连接。
5. 记录检查点和恢复方式。
6. 按“长任务可恢复、重复调用可避免、每批结果可验收”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成后台智能体连续内容生产，目标、资料、预算和截止时间为[填写]" \
...
最终交付：后台任务图、检查点、内容批次、任务ID和恢复记录。
专项验收：长任务可恢复、重复调用可避免、每批结果可验收。
```
