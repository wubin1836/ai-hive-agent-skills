---
name: aihive-studio-ai-photo-cleanup-studio
description: AI去路人·照片消除工作室：面向照片消除、去路人、AI修图需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付edit-mask.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: AI去路人·照片消除工作室
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：照片消除、去路人、AI修图。不改变用户已明确指定的平台或型号。

## 开始前确认

用户有权编辑的照片、待移除对象和保留区域。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验图像生成或编辑、参考图输入、尺寸与下载工具。准确文字、透明通道、二维码要用真实文件工具另行检查。

## 执行流程

1. 先确认不是证据材料篡改并标注编辑区域。
2. 按背景纹理和透视制定填补约束。
3. 局部修复后放大检视边缘、影子和重复纹理。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `edit-mask.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `clean-photo.png`：按本任务制作的实际文件或明确标注的待制作目录。
- `edit-record.md`：按本任务制作的实际文件或明确标注的待制作目录。
