---
name: aihive-mimo25-tts
description: 小米MiMo-V2.5-TTS配音助手：面向MiMo TTS、小米语音合成、MiMo-V2.5-TTS需求，结合极睿科技AI-HIVE与MCP执行模型音频流程，交付pronunciations.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 小米MiMo-V2.5-TTS配音助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型音频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：MiMo TTS、小米语音合成、MiMo-V2.5-TTS。不改变用户已明确指定的平台或型号。

## 开始前确认

授权文字、语言、标准音色或已授权定制音色、读音表。仅追问影响执行的缺项，不要求用户重新提供已有资料。

语音识别、合成、音色定制和字幕时间轴各有独立能力要求，按实际schema和返回数据执行。

## 执行流程

1. 区分TTS与ASR，核验实际目录音色和支持语言。
2. 把数字与多音字标注后制作短句试听。
3. 按句生成并实际听检后拼接与交付。
...
- `pronunciations.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `tts-script.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `voiceover.wav`：按本任务制作的实际文件或明确标注的待制作目录。
