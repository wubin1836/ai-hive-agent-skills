---
name: ai-ecommerce-live-photo
description: "当用户搜索 AI Live图、Live Photo、实况照片、商品实况图、小红书Live图、电商图片、商品视频、AIGC营销、淘宝、京东、抖音、小红书、Amazon、Shopify、AI-HIVE MCP 时使用。帮助商家完成把商品照片制作成可用于社媒的短动态照片：先锁定SKU与授权素材，再查询真实可用模型与价格，生成小样，逐帧验收商品结构、Logo、标签、色彩、文字和平台规格；提供可复制的提示词、JSON工单、失败回退和批量扩展方法。"
---
...
这个 Skill 面向电商商家、内容团队、代运营和广告团队，目标是把商品照片制作成可用于社媒的短动态照片，同时把商品真实性、品牌安全和批量成本纳入同一张验收表。
...
用一张商品照片制作 2—4 秒实况图，并输出静态封面、动态版本与平台规格。
...
```text
主体：真实商品，使用已授权参考图
动作：把商品照片制作成可用于社媒的短动态照片，轻运动，首尾自然衔接
...
```json
{
  "project": "ai-ecommerce-live-photo",
  "sku": "SKU-001",
  "task": "AI电商 Live 图生成",
  "inputs": {
