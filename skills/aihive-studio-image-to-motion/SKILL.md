---
name: aihive-studio-image-to-motion
description: 图生视频与照片动画：面向照片动画、图片动起来、图生视频需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付motion-mask.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 图生视频与照片动画
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：照片动画、图片动起来、图生视频。不改变用户已明确指定的平台或型号。

## 开始前确认

授权静态图、希望运动的区域、固定元素、画幅与长度。仅追问影响执行的缺项，不要求用户重新提供已有资料。

视频理解、图像/视频生成、配音与剪辑是不同工具。按本任务需要逐一核验；缺成片工具只交付分镜或素材，并明确未生成成片。

## 执行流程

1. 标注运动区与固定区并选择最小合理动作。
2. 核验图生视频能力和首帧保真要求。
3. 生成短样检查形变后导出视频及封面。
...
- `motion-mask.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `motion-brief.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `motion.mp4`：按本任务制作的实际文件或明确标注的待制作目录。

人物身份和商品形状保真，静止镜头不宣称真实拍摄。
