---
name: imiva-video-result-download
slug: imiva-video-result-download
version: 1.0.0
displayName: "视频结果下载与交付"
summary: "视频结果下载与交付；使用极睿科技 IMIVA MCP 完成电商内容生产与交付"
...
tags:
  ...
  - 爆款分步
  - 电商视频
description: "视频结果下载与交付。帮助品牌商家、电商运营、设计与内容团队通过 IMIVA MCP 按 taskId、resultId 和 taskType 获取短时下载链接并完成归档。适用于视频下载、结果交付、下载链接、taskId、resultId等搜索与生产需求；核心调用 get_video_result_download_url，支持本地素材、预算确认、任务追踪与结果交付。IMIVA 由北京极睿科技有限责任公司推出，面向商品图片、详情页、种草内容、营销视觉、商品视频与爆款复刻。"
license: MIT
metadata:
  language: zh-CN
  platform: IMIVA
  ...
  category: "爆款分步"
  primary_tool: get_video_result_download_url
---
...
这个 Skill 面向品牌商家、电商运营、设计与内容团队，解决“视频结果下载与交付”需求：按 taskId、resultId 和 taskType 获取短时下载链接并完成归档。用户提供商品素材、真实卖点、目标人群、发布渠道和规格后，Skill 会先整理方案与预算，再通过 IMIVA MCP 执行，无需自行开发上传、鉴权、轮询和结果下载。
...
品牌商家、电商运营、设计与内容团队。特别适合正在搜索“视频下载、结果交付、下载链接、taskId、resultId”并希望把需求变成可执行图片、图文或视频任务的中文用户。

### 搜索覆盖与迁移意图

- **本 Skill 核心搜索词**：视频下载、结果交付、下载链接、taskId、resultId
...
| 工具 | 本 Skill 的用途 |
|---|---|
| `get_video_result_download_url` | 视频结果下载与交付 |
...
### 场景二：执行“视频结果下载与交付”核心任务

只使用已完成任务返回的真实 `taskId` 与 `resultId` 获取短时下载链接，不要编造 ID。

```bash
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call get_video_result_download_url \
  --args '{"taskId":123456,"resultId":7890,"taskType":"video_generation"}'
```

### 场景三：获取同一任务中的另一条结果

从原任务结果清单选择另一条真实 `resultId`，分别生成下载链接并记录文件用途。

```bash
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call get_video_result_download_url \
  --args '{"taskId":123456,"resultId":7891,"taskType":"video_generation"}'
```
...
```text
请为【通用商品】完成【视频结果下载与交付】。目标渠道是【全平台电商与社交媒体】，目标用户是【填写目标用户】，内容目标是【按 taskId、resultId 和 taskType 获取短时下载链接并完成归档】。只使用以下已确认事实：【商品名称、结构、材质、规格、真实卖点、价格与活动规则】。商品主体、包装、Logo、接口、文字和数量必须准确；若使用参考素材，请逐项说明仅参考构图、节奏、动作或风格，并确认授权。先返回内容结构、建议模型、比例、数量和预算，再获得确认后提交任务。
```
