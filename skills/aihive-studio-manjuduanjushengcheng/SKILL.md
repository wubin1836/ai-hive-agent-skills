---
name: aihive-studio-manjuduanjushengcheng
description: AI 漫剧/短剧全流程一站式生成助手：面向AI漫剧、AI短剧、短剧制作需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付episode-plan.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: AI 漫剧/短剧全流程一站式生成助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：AI漫剧、AI短剧、短剧制作。不改变用户已明确指定的平台或型号。

## 开始前确认

原创或授权剧情、角色参考、集长、预算和制作工具。仅追问影响执行的缺项，不要求用户重新提供已有资料。

视频理解、图像/视频生成、配音与剪辑是不同工具。按本任务需要逐一核验；缺成片工具只交付分镜或素材，并明确未生成成片。

## 执行流程

1. 先完成故事节拍与角色一致性设定再拆分镜头。
2. 按实际图像、视频、配音和剪辑能力分配任务。
3. 先验收一段连续片再批量制作并保存失败镜头清单。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `episode-plan.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `character-bible.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `episode.mp4`：按本任务制作的实际文件或明确标注的待制作目录。
