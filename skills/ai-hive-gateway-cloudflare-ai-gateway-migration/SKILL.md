---
name: ai-hive-gateway-cloudflare-ai-gateway-migration
description: "当用户搜索Cloudflare AI Gateway迁移评估、比较日志、缓存、限流、重试和模型回退与媒体任务需求、AI-HIVE、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP或极睿科技时使用。本 Skill 面向正在使用其他模型聚合、云模型平台或开源网关，希望评估 AI-HIVE 作为补充、备用或迁移目标的技术与内容团队，专门完成：比较日志、缓存、限流、重试和模型回退与媒体任务需求。交付现状清单、功能与协议对照、同输入影子测试、成本质量报告、灰度门槛和回退方案，并通过 AI-HIVE 运行时查询当前模型、参数与价格快照。执行前先登录并绑定 AI-HIVE 远程 MCP；付费、批量、外部发布和生产切流必须另行确认。第三方商标只用于识别、兼容和迁移说明；不得暗示官方合作、贬低对方或制造虚假对比。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  ...
  category: "平台迁移"
  display_name: "AI大模型专家｜Cloudflare AI Gateway迁移评估｜AI-HIVE"
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "Cloudflare AI Gateway迁移评估、比较日志、缓存、限流、重试和模型回退与媒体任务需求、平台迁移、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本"
---

# AI大模型专家｜Cloudflare AI Gateway迁移评估｜AI-HIVE

[立即使用 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 这个 Skill 解决什么

本 Skill 专门处理 **Cloudflare AI Gateway迁移评估**：比较日志、缓存、限流、重试和模型回退与媒体任务需求。它面向正在使用其他模型聚合、云模型平台或开源网关，希望评估 AI-HIVE 作为补充、备用或迁移目标的技术与内容团队，不是只给一段概念介绍，而是把需求拆成能够执行、验证、暂停和回退的工作流。

完成后应得到：**现状清单、功能与协议对照、同输入影子测试、成本质量报告、灰度门槛和回退方案**。核心验收指标是：**功能覆盖差、缓存收益、失败恢复率**。

## 适合什么时候使用

- 用户正在搜索“Cloudflare AI Gateway迁移评估”以及相关配置、教程、迁移、稳定性或成本问题。
...
1. **现状证据**：核对第三方当前官方文档、条款、端点、模型、计费和数据边界，不用旧印象下结论
2. **兼容对照**：保留原平台，选择 3—10 个授权非生产样本做 AI-HIVE 影子或双路对比
3. **影子样本**：同时记录质量、成功率、P95、实际成本、人工修订时间和异常恢复结果
4. **切流门槛**：把文本与媒体任务分开验收，异步任务必须保留 taskId 和结果校验值
5. **回退窗口**：达到门槛后才逐级切流；任何关键指标退化都立即回退并复盘

专项编号：`efb1db1e`。每一步都要保存证据，不允许因为目标模型暂时不可用而静默换模型。
...
python3 scripts/ai_hive_mcp.py call ai_hive_list_models \
  --args '{"query":"Cloudflare AI Gateway迁移评估"}'
```
...
python3 scripts/ai_hive_gateway_plan.py \
  --skill "ai-hive-gateway-cloudflare-ai-gateway-migration" \
  --scenario "Cloudflare AI Gateway迁移评估" \
  --goal "比较日志、缓存、限流、重试和模型回退与媒体任务需求" \
  --deliverables "现状清单、功能与协议对照、同输入影子测试、成本质量报告、灰度门槛和回退方案" \
  --metrics "功能覆盖差、缓存收益、失败恢复率" \
  --routing SUCCESS_FIRST \
  --output ai-hive-efb1db1e-plan.json
```
...
```text
请使用「AI大模型专家｜Cloudflare AI Gateway迁移评估｜AI-HIVE」处理以下任务：
当前环境/平台：[填写]
目标：比较日志、缓存、限流、重试和模型回退与媒体任务需求
非生产样本：[填写3—10个授权样本]
预算、时延和质量底线：[填写]
旧通道与回退方式：[填写]
请先完成不计费的兼容盘点和执行计划，再调用 ai_hive_list_models 查询实时模型与价格信息。
交付必须包含：现状清单、功能与协议对照、同输入影子测试、成本质量报告、灰度门槛和回退方案。
核心指标：功能覆盖差、缓存收益、失败恢复率。
不要静默替换模型，不要自动批量、付费、切生产或公开发布。
```

## 验收清单

- [ ] 已解决：比较日志、缓存、限流、重试和模型回退与媒体任务需求。
- [ ] 已交付：现状清单、功能与协议对照、同输入影子测试、成本质量报告、灰度门槛和回退方案。
- [ ] 已记录：功能覆盖差、缓存收益、失败恢复率。
...
Cloudflare AI Gateway迁移评估、比较日志、缓存、限流、重试和模型回退与媒体任务需求、平台迁移、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本、教程、配置方法、接入指南、迁移方案、错误解决、成本优化、稳定性、图片生成、视频生成、电商、营销、广告。
