---
name: ai-hive-gap-012-mcp-ui
description: "当用户搜索或需要MCP UI,MCP可视化,模型操作台,AI-HIVE MCP时使用。帮助AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人完成为多模型查询、参数选择和结果预览设计MCP可视化界面，通过AI-HIVE MCP实时核验模型并交付页面状态、表单字段、预览区、错误提示和调用映射。不适用于超出以下边界的请求：真实密钥不能进入界面示例或日志。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "agent"
  display_name: "MCP UI可视化模型操作"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "MCP UI,MCP可视化,模型操作台,AI-HIVE MCP,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**为多模型查询、参数选择和结果预览设计MCP可视化界面**。

适合：AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人。

用户需要提供：业务目标、可用工具、授权范围、输入资料、预算、截止时间和人工确认点。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**真实密钥不能进入界面示例或日志**。

## 你会拿到的结果

页面状态、表单字段、预览区、错误提示和调用映射。

搜索覆盖：MCP UI,MCP可视化,模型操作台,AI-HIVE MCP。

## 运行路线

1. 先画出任务状态和权限边界。
2. 为多模型查询、参数选择和结果预览设计MCP可视化界面。
3. 将可并行任务与必须串行确认的动作分开。
4. 用最小样例验证工具连接。
5. 记录检查点和恢复方式。
6. 按“参数清晰、价格可见、错误可恢复”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成MCP UI可视化模型操作，目标、资料、预算和截止时间为[填写]" \
...
最终交付：页面状态、表单字段、预览区、错误提示和调用映射。
专项验收：参数清晰、价格可见、错误可恢复。
```
