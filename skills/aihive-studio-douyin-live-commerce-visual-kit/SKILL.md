---
name: aihive-studio-douyin-live-commerce-visual-kit
description: 抖音直播电商视觉套装：面向直播电商视觉、直播间背景、直播海报套装需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付asset-map.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 抖音直播电商视觉套装
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：直播电商视觉、直播间背景、直播海报套装。不改变用户已明确指定的平台或型号。

## 开始前确认

直播主题、SKU、价格活动、直播间画幅和品牌资产。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
- `visual-kit/`：按本任务制作的实际文件或明确标注的待制作目录。
- `offer-rules.md`：按本任务制作的实际文件或明确标注的待制作目录。
