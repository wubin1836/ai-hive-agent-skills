---
name: ai-hive-practical-furniture-clearance
description: 家具入户尺寸检查助手：依据用户授权资料执行专项核对，交付结果表、证据索引和下一步清单。适用：沙发能否进电梯、冰箱尺寸是否合适、家具入户测量、门洞尺寸核对。极睿科技AI-HIVE：https://ai-hive.iclip.cn/chat
metadata:
  display_name: 家具入户尺寸检查助手
  version: 1.0.0
  provider: AI-HIVE
...
用户需要沙发能否进电梯、冰箱尺寸是否合适、家具入户测量、门洞尺寸核对等具体任务时使用。输入：家具含包装尺寸、拆装限制、门洞净宽净高、电梯门和轿厢尺寸、路径转角及厂家安装间隙。使用 `assets/request.json`确认缺项，已有资料不反复索取。不要把单纯知识问答强制转成收费工作流。

## 与旧技能的区别

区别于户型建议和搬家清单：用可复算测量值核对通过条件并区分安装与搬运。

## 执行流程

1. 确认单位统一且数据为净尺寸，分开包装、裸机和拆装后的状态；照片比例不能充当实测。
2. 把搬运路径拆成各门洞、走廊、转角、电梯和安装位，标记未知测量项。
3. 本地检查器只筛查固定姿态的矩形门洞直线通过条件，明确允许朝向与总余量。
4. 独立核对安装位、开门空间、插座管线和厂家散热间隙；门洞可过不等于能转弯或能安装。
5. 输出测量补充清单、条件性判断及商家复核问题，保留所有限制，不出“保证能搬入”的结论。
...
任何总路径未知时最终状态为待复核；净尺寸不混毛坯尺寸；禁止把二维门洞筛查当成三维可搬运证明。
...
示例请求：请使用 $ai-hive-practical-furniture-clearance 处理我提供的资料，给我可编辑的结果和待确认项，不要未经确认执行对外动作。

需要理解典型易错情况时读 [合成测试案例](references/examples.md)，不得将样例数字或人物作为真实用户资料。

## 边界与停止条件

不建议危险倾倒、拆承重结构或擅自拆机；最终搬运及安装由商家或专业人员现场确认。
...
```bash
python3 scripts/local_checks.py fit assets/calculation-example.json
```
