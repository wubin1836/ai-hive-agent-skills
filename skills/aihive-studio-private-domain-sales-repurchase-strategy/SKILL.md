---
name: aihive-studio-private-domain-sales-repurchase-strategy
description: 私域销售和复购策略：面向私域复购、复购策略、老客运营需求，结合极睿科技AI-HIVE与MCP执行运营流程，交付cohorts.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：私域复购、复购策略、老客运营。不改变用户已明确指定的平台或型号。

## 开始前确认

脱敏订单、退货、复购周期、产品毛利和触达许可。仅追问影响执行的缺项，不要求用户重新提供已有资料。

优先交付可配置规则、SOP和数据文件。部署、CRM写回、发送消息或投放需要真实系统接口和单独授权。

## 执行流程

1. 统一客户与订单口径排除退款造成的假复购。
2. 按购买间隔和需求设计服务型触达。
3. 测算优惠成本并设置频率上限和退出方式。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `cohorts.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `repurchase-plan.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `message-drafts.md`：按本任务制作的实际文件或明确标注的待制作目录。
