---
name: aihive-studio-comment-voc-miner
description: 评论分析工作室：面向评论分析、VOC分析、用户声音洞察需求，结合极睿科技AI-HIVE与MCP执行分析流程，交付comment-tags.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 评论分析工作室
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 分析
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：评论分析、VOC分析、用户声音洞察。不改变用户已明确指定的平台或型号。

## 开始前确认

授权导出的评论、时间范围、商品类别、需匿名的字段。仅追问影响执行的缺项，不要求用户重新提供已有资料。

对用户授权的数据实际执行清洗与计算，保留统计口径、分母和来源。无联网工具时只分析已提供资料。

## 执行流程

1. 去重并保留原评论ID，把观点与事实分开。
2. 编码需求、抱怨、障碍和购买动机并抽样复核。
3. 输出频次、代表片段与产品内容行动表。
...
- `comment-tags.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `voc-summary.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `action-backlog.csv`：按本任务制作的实际文件或明确标注的待制作目录。
