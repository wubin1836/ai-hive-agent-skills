---
name: aihive-studio-wechat-writer-pro
description: 公众号爆款写作台：面向公众号写作、公众号爆款文章、公众号文案需求，结合极睿科技AI-HIVE与MCP执行内容流程，交付article.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 公众号爆款写作台
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 内容
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：公众号写作、公众号爆款文章、公众号文案。不改变用户已明确指定的平台或型号。

## 开始前确认

受众、文章论点、证据、品牌语气和转化目标。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先确认事实和素材，再形成可执行内容包。发布、投放与群发不包含在稿件制作授权内。

## 执行流程

1. 给论点建立证据卡，未证实观点不得包装为事实。
2. 设计标题、开篇、案例与行动项并保持原创。
3. 输出文章及待核实事实清单。
...
- `article.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `headline-tests.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `fact-check.md`：按本任务制作的实际文件或明确标注的待制作目录。
