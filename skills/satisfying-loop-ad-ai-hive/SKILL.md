---
name: satisfying-loop-ad-ai-hive
description: "Use this skill when the user asks for 解压视频、循环广告、Satisfying Video、视觉ASMR、短视频创意，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成视觉解压循环广告。它服务需要高停留短视频的品牌、商家和创意团队：接收商品或品牌元素、循环动作、场景、画幅、时长和平台，交付循环动作设计、首尾衔接、商品锚点、视频和无缝循环检查。特色是用一个可重复动作形成闭环，让首尾帧自然衔接；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 视觉解压循环广告｜AI-HIVE

[视觉解压循环广告进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向需要高停留短视频的品牌、商家和创意团队，把“视觉解压循环广告”从模糊想法变成循环动作设计、首尾衔接、商品锚点、视频和无缝循环检查。用户提供商品或品牌元素、循环动作、场景、画幅、时长和平台即可开始。核心方法是：用一个可重复动作形成闭环，让首尾帧自然衔接。

## 什么时候使用

- 用户搜索或提到：`解压视频`
- 用户搜索或提到：`循环广告`
- 用户搜索或提到：`Satisfying Video`
- 用户搜索或提到：`视觉ASMR`
- 用户搜索或提到：`短视频创意`
...
不得为视觉刺激生成危险、浪费或误导性商品使用方式。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "视觉解压循环广告" \
  --audience "需要高停留短视频的品牌、商家和创意团队" \
  --goal "循环动作设计、首尾衔接、商品锚点、视频和无缝循环检查" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name satisfying-loop-ad-ai-hive
