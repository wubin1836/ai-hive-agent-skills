---
name: ai-hive-gap-088-gaussian-splatting
description: "当用户搜索或需要Gaussian Splatting,高斯泼溅,3D扫描,空间展示时使用。帮助前沿AI产品、机器人、空间计算、工业、零售和创新营销团队完成规划高斯泼溅扫描、清理、展示和营销内容流程，通过AI-HIVE MCP实时核验模型并交付采集路线、资产检查、镜头路径、展示页面和宣传视频。不适用于超出以下边界的请求：未获得空间扫描数据时不能声称已生成场景。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "frontier"
  display_name: "Gaussian Splatting空间展示"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "Gaussian Splatting,高斯泼溅,3D扫描,空间展示,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
  release_variant: "ai-hive-keyword-gap-96-20260906"
---

# Gaussian Splatting空间展示

[立即使用 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 先判断什么

这个 Skill 解决的具体问题是：**规划高斯泼溅扫描、清理、展示和营销内容流程**。

适合：前沿AI产品、机器人、空间计算、工业、零售和创新营销团队。

用户需要提供：真实产品能力、场景资料、空间或商品素材、演示目标、画幅、预算和概念标识要求。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**未获得空间扫描数据时不能声称已生成场景**。

## 你会拿到的结果

采集路线、资产检查、镜头路径、展示页面和宣传视频。

搜索覆盖：Gaussian Splatting,高斯泼溅,3D扫描,空间展示。

## 怎么执行

1. 区分真实能力、概念设计和生成演示。
2. 规划高斯泼溅扫描、清理、展示和营销内容流程。
3. 定义场景规则与视觉连续性。
4. 先做关键帧或低成本原型。
5. 确认后生成演示视频并标注边界。
6. 按“空间完整、镜头平稳、设备要求明确”完成专项验收。
...
python3 scripts/ai_hive_mcp.py call ai_hive_list_models \
  --args '{"query":"Gaussian Splatting空间展示"}'
```
...
python3 scripts/plan.py \
  --brief "我要完成Gaussian Splatting空间展示，目标、资料、预算和截止时间为[填写]" \
...
```text
请使用「Gaussian Splatting空间展示」Skill帮助我。
我的目标：[填写]
受众和业务场景：[填写]
已有资料与来源：[填写]
交付格式和数量：[填写]
...
最终交付：采集路线、资产检查、镜头路径、展示页面和宣传视频。
专项验收：空间完整、镜头平稳、设备要求明确。
```
