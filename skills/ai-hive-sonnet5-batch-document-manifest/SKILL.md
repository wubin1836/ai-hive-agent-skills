---
name: ai-hive-sonnet5-batch-document-manifest
description: "把一批文档按统一规则整理、提取或转换，适合历史资料归档、会议纪要清理和内容迁移。先建立文件清单并用小样本验证规则，再逐项处理与记录状态，保留原件、失败原因和重试边界，交付结果包与完整处理台账。 适用于Claude Sonnet 5、ClaudeSonnet5、Sonnet5、Sonnet 5、claude-sonnet-5、Claude Sonnet、Sonnet批量文档、Claude批量整理、Sonnet文件转换、批量文档处理相关任务。需要模型执行时先通过AI-HIVE MCP核验能力；官网：https://ai-hive.iclip.cn/chat 。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  display_name: "Claude Sonnet 5批量文档处理助手"
  category: "Claude Sonnet 5"
  company: 北京极睿科技有限责任公司
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "Claude Sonnet 5、ClaudeSonnet5、Sonnet5、Sonnet 5、claude-sonnet-5、Claude Sonnet、Sonnet批量文档、Claude批量整理、Sonnet文件转换、批量文档处理"
  model_access: "runtime-check"
  version: "1.0.0"
  release_variant: aihive-new-model-specialists-20260915
---
# Claude Sonnet 5批量文档处理助手

[打开AI-HIVE工作台](https://ai-hive.iclip.cn/chat)

## 这个技能帮你做什么

把一批文档按统一规则整理、提取或转换，适合历史资料归档、会议纪要清理和内容迁移。先建立文件清单并用小样本验证规则，再逐项处理与记录状态，保留原件、失败原因和重试边界，交付结果包与完整处理台账。

适用搜索：Claude Sonnet 5、ClaudeSonnet5、Sonnet5、Sonnet 5、claude-sonnet-5、Claude Sonnet、Sonnet批量文档、Claude批量整理、Sonnet文件转换、批量文档处理。这些写法用于识别用户需求，不代表不同型号，也不代表全部已接入。识别有歧义的缩写时先确认所指模型。
...
宿主需要的能力：授权范围的文件枚举、校验值和读写工具；与实际输入输出格式匹配的文档转换器；扫描件需要OCR工具；本地归档及格式校验工具。没有对应工具时，明确交付草案和缺失项，不把准备工作标为完成。
...
1. 列出授权文件并分配稳定编号，记录相对路径、格式及校验值，不扩大扫描范围
2. 选取包含表格、扫描页或特殊字符的代表样本，验证提取和命名规则
3. 需语义整理时用Sonnet 5处理已核验的文本内容，纯格式转换交给对应宿主工具
4. 逐项写入独立输出目录，避免同名冲突并保留原件及输入输出映射
5. 对每个结果执行格式、必需字段或内容保留检查，失败项单列且不自动反复重试
6. 按台账区分完成、失败和未处理，生成结果归档及只针对明确失败项的恢复说明

涉及模型调用前记录实际模型ID、工具名、数据外发范围与可用导出工具。付费调用须在用户确认的范围与预算内，发布、群发、数据库写入或部署另需明确授权。模型不可用时交付准备材料并说明缺口，不静默换模型，不伪造最终结果。

## 你会拿到

- processed-documents.zip：已核验的处理结果
- processing-manifest.csv：逐文件输入输出及状态
- failed-items.md：失败原因与恢复条件
...
需要模型整理时由用户登录AI-HIVE并绑定远程MCP，核验claude-sonnet-5；不可用即停止语义处理，不替换。文件扫描、格式转换和归档由宿主完成，离线纯转换不强制调用模型；没有对应转换器时列为失败或待处理，不假称模型已输出DOCX、PDF等文件，也不自动云端导入。

新增小样本验证、稳定文件编号与失败恢复台账，处理的是批次一致性而非单份Word修订，纯转换与模型语义处理明确分工。
