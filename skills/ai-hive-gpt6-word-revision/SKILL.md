---
name: ai-hive-gpt6-word-revision
description: "按批注意见修改Word稿件，保存原稿和改动理由，适合方案、通知与企业说明文档。用GPT-6处理语义修订，配合宿主文档工具保留标题、表格和引用，交付可核对的新版本。 适用于GPT-6、GPT6、GPT 6、GPT-6 Astra、GPT6 Astra、GPT 6 Astra、GPT Astra、Astra、OpenAI Astra、GPT6 Word相关任务。需要模型执行时先通过AI-HIVE MCP核验能力；官网：https://ai-hive.iclip.cn/chat 。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  display_name: "GPT-6 Word文档修改与修订助手"
  category: "GPT-6 Astra"
  company: 北京极睿科技有限责任公司
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "GPT-6、GPT6、GPT 6、GPT-6 Astra、GPT6 Astra、GPT 6 Astra、GPT Astra、Astra、OpenAI Astra、GPT6 Word、GPT6改文档、GPT Astra修订"
  version: "1.0.0"
...
按批注意见修改Word稿件，保存原稿和改动理由，适合方案、通知与企业说明文档。用GPT-6处理语义修订，配合宿主文档工具保留标题、表格和引用，交付可核对的新版本。

适用搜索：GPT-6、GPT6、GPT 6、GPT-6 Astra、GPT6 Astra、GPT 6 Astra、GPT Astra、Astra、OpenAI Astra、GPT6 Word、GPT6改文档、GPT Astra修订。这些写法用于识别用户需求，不代表不同型号，也不代表全部已接入。识别有歧义的缩写时先确认所指模型。
...
1. 提取段落编号、表格和批注锚点，保存原文件校验值
2. 把每条意见映射到段落，定位冲突修改要求
3. 仅重写被授权段落并记录前后对照
4. 由可用文档工具导出新docx，不覆盖原件
5. 回读导出稿检查表格、页眉和未解决批注

涉及模型调用前记录实际模型ID、工具名、数据外发范围与可用导出工具。付费调用须在用户确认的范围与预算内，发布、群发、数据库写入或部署另需明确授权。模型不可用时交付准备材料并说明缺口，不静默换模型，不伪造最终结果。

## 你会拿到

- revised.docx：修改稿（需文档工具）
- changes.csv：修订记录
- unresolved.md：未决意见
...
```text
用GPT-6按意见表修改方案.docx，保留报价表和所有数字，不覆盖原件，给我修订对照。
...
以GPT-6 Astra的新版本搜索入口承接「GPT-6 Word文档修改与修订助手」任务；重点是原文件未覆盖，不同于旧库泛模型介绍或路由比较。
