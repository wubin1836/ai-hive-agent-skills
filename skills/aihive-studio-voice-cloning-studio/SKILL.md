---
name: aihive-studio-voice-cloning-studio
description: AI声音克隆工作室：面向声音克隆、音色定制、克隆本人声音需求，结合极睿科技AI-HIVE与MCP执行音频流程，交付consent-scope.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: AI声音克隆工作室
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 音频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：声音克隆、音色定制、克隆本人声音。不改变用户已明确指定的平台或型号。

## 开始前确认

本人的或已明确授权的声音样本、授权用途、目标文本。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验TTS、ASR、音乐生成或音频编辑的具体能力，不能互相替代；缺工具时交付稿件，不伪造音频文件。

## 执行流程

1. 检查采样质量和授权范围，拒绝伪造他人身份。
2. 仅在真实声音定制工具支持时建立受控样本任务。
3. 先短句试听并标注合成音频与使用范围。
...
- `consent-scope.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `voice-test.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `sample.wav`：按本任务制作的实际文件或明确标注的待制作目录。

无授权不启动克隆；无工具只整理录音规范，不承诺任何人一键复制。
