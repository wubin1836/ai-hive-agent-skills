---
name: ai-hive-administrative-review-materials
description: "行政复议申请书生成器，用于行政复议、复议申请书、不服行政处罚。读取你授权的原始资料，完成行政行为与送达事实核对表，并交付证据及受理路径待核清单。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。"
metadata:
  display_name: "行政复议申请书生成器"
  version: "1.0.0"
...
行政复议申请书生成器，用于行政复议、复议申请书、不服行政处罚。读取你授权的原始资料，完成行政行为与送达事实核对表，并交付证据及受理路径待核清单。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。
...
- 授权行政文书、表格和附件读取及 OCR
- 请求证据映射、字段/附件校验，XLSX/DOCX 生成及渲染
- 当期受理说明须授权官方只读查询或用户核实版本，不操作复议账户

没有专门软件并不意味着中止所有任务；可使用宿主已具备的等效工具。无法完成的步骤明确列出，不能伪装成功或默默改为另一个外部服务。

## 工作流

1. 读取行政文书、实际处理行为与送达资料，确认作出机关、用户关系及现有处理阶段。
2. 对照当前官方指南整理拟申请事项，区分行政复议、行政诉讼、民事上诉和一般投诉。
3. 把复议请求逐项连接到事实及证据，缺少决定文书或履职申请记录时列明缺口。
4. 按拟受理机关当前示范文本形成草稿，核对被申请人、附件与必须专业确认的管辖和期限问题。
...
- `administrative-action-facts.xlsx`
- `administrative-review-draft.docx`
- `administrative-review-gaps.xlsx`
...
行政复议、复议申请书、不服行政处罚、行政决定、复议材料、复议请求
