---
name: happy-horse
description: "Use this skill when creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers need to create videos from prompts, first frames, and reference media with Happy Horse through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for ads, TVCs, product videos, e-commerce listings, social commerce, UGC-style seeding content, short dramas, motion comics, and storyboards. Search intents include AI video generation, text-to-video, image-to-video, reference-to-video, video editing, AIGC video."
license: MIT
---
...
Happy Horse Video Generation helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers create videos from prompts, first frames, and reference media with Happy Horse. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
Ads, tvcs, product videos, e-commerce listings, social commerce, ugc-style seeding content, short dramas, motion comics, and storyboards. It is also useful when users search for AI video generation, text-to-video, image-to-video, reference-to-video, video editing, AIGC video.

## Capabilities

| Mode | Capability | publicModelId |
|---|---|---|
| `edit` | Video Editing | `public_model_happyhorse_video_edit` |
| `i2v` | Image to Video | `public_model_happyhorse_i2v` |
| `r2v` | Reference to Video | `public_model_happyhorse_r2v` |
| `t2v` | Text to Video | `public_model_happyhorse_t2v` |
...
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode edit \
  --prompt "Happy Horse Video Generation: polished motion, clear subject, coherent action, stable camera, commercial finish"
```
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode edit \
  --prompt "Create a polished happy horse video generation result with coherent motion, stable identity, and a clear ending"
```
...
```bash
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode edit --prompt "Complex production brief" --no-download
