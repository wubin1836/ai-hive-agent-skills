---
name: aihive-qwen-image21-alpha
description: Qwen-Image-2.1透明商品图助手：面向Qwen Image 2.1、Qwen2.1图片、千问图片需求，结合极睿科技AI-HIVE与MCP执行模型图片流程，交付product.png等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Qwen-Image-2.1透明商品图助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型图片
---

# Qwen-Image-2.1透明商品图助手

把用户材料变成可复核的业务产物。发现与调用时可识别：Qwen Image 2.1、Qwen2.1图片、千问图片、透明PNG。不改变用户已明确指定的平台或型号。

## 开始前确认

商品参考、背景需求、目标尺寸、透明通道要求。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验指定型号的图像输入输出与格式能力。不存在时先报告，不以其他模型生成结果冒充指定模型。

## 执行流程

1. 核验实时工具是否暴露RGBA输出而非仅支持普通图片。
2. 锁定商品形状后生成或提取主体，不能生成不存在配件。
3. 用真实图像工具检查alpha并分别放在黑白背景上验收。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `product.png`：按本任务制作的实际文件或明确标注的待制作目录。
- `alpha-check.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `background-previews/`：按本任务制作的实际文件或明确标注的待制作目录。
