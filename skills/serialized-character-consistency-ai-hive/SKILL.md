---
name: serialized-character-consistency-ai-hive
description: "Use this skill when the user asks for 角色一致性、连载短剧、人物连续性、AI漫剧质检、虚拟IP，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成连载角色一致性守护。它服务AI短剧、漫剧、虚拟IP和长线内容制作团队：接收角色圣经、历史剧集、当前分镜或视频、允许变化项和剧情时间线，交付人物外观、服装、道具、语气、关系和时间线的连续性差异报告。特色是把稳定锚点与剧情允许变化分开检查；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 连载角色一致性守护｜AI-HIVE

[连载角色一致性守护进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向AI短剧、漫剧、虚拟IP和长线内容制作团队，把“连载角色一致性守护”从模糊想法变成人物外观、服装、道具、语气、关系和时间线的连续性差异报告。用户提供角色圣经、历史剧集、当前分镜或视频、允许变化项和剧情时间线即可开始。核心方法是：把稳定锚点与剧情允许变化分开检查。
...
模型判断不能替代主创；角色故意变化需在时间线中明确，避免误报。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "连载角色一致性守护" \
  --audience "AI短剧、漫剧、虚拟IP和长线内容制作团队" \
  --goal "人物外观、服装、道具、语气、关系和时间线的连续性差异报告" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name serialized-character-consistency-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name serialized-character-consistency-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
