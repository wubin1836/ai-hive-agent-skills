...
MiniMax H3 Video Reference to Video helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers reuse motion, pacing, camera movement, or structure from a reference video with MiniMax H3. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
|---|---|---|
| `r2v` | Reference to Video | `public_model_minimax_h3_r2v` |
...
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode r2v \
  --prompt "MiniMax H3 Video Reference to Video: polished motion, clear subject, coherent action, stable camera, commercial finish" \
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode r2v \
  --prompt "Create a polished minimax h3 video reference to video result with coherent motion, stable identity, and a clear ending" \
+  --video /path/to/reference.mp4
```
