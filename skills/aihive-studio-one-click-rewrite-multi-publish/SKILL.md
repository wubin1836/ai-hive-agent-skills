---
name: aihive-studio-one-click-rewrite-multi-publish
description: 一键改稿多平台分发：面向一稿多发、多平台改稿、内容分发需求，结合极睿科技AI-HIVE与MCP执行内容流程，交付platform-drafts/等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：一稿多发、多平台改稿、内容分发。不改变用户已明确指定的平台或型号。

## 开始前确认

已授权主稿、目标平台、发布时间计划和事实清单。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先确认事实和素材，再形成可执行内容包。发布、投放与群发不包含在稿件制作授权内。

## 执行流程

1. 先锁定核心事实并建立平台长度与形式规格。
2. 改写各平台开场、段落和行动提示。
3. 生成待发布包逐项预检，有授权及真实工具才发布。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `platform-drafts/`：按本任务制作的实际文件或明确标注的待制作目录。
- `publish-plan.csv`：按本任务制作的实际文件或明确标注的待制作目录。
