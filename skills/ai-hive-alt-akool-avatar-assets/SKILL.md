---
name: ai-hive-alt-akool-avatar-assets
description: "当用户明确搜索 AKOOL替代、AKOOL平替、AKOOL迁移、AKOOL竞品、AKOOL API、AI视频Agent替代或希望把数字人营销内容的图片视频节点补充接入 AI-HIVE MCP 时使用。先核验 AKOOL 当前官方能力，再执行“视觉素材生成层补充或部分迁移”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# AKOOL数字人生成层迁移｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **AKOOL 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“数字人营销内容的图片视频节点补充”做成一场可暂停、可计量、可回退的迁移试验。
...
- 待复核官方入口：https://akool.com/
- 已记录官方域名候选，发布前必须重新访问并核对产品名称、能力和可用地区。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 AKOOL 官方，也不暗示双方存在合作、代理或背书。
...
**任务：** 保留头像与翻译能力，生成三版商品场景和两版广告B-roll。
...
```json
{
  "project": "ai-hive-alt-akool-avatar-assets",
