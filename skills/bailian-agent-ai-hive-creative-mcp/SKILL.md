---
name: bailian-agent-ai-hive-creative-mcp
description: "当用户搜索 阿里云百炼、Agent Studio、多Agent、广告视频、AI-HIVE MCP、OiiOii同类平台、AI视频Agent、创意Agent平台、AI-HIVE MCP 时使用。为 阿里云百炼 Agent Studio 做中立的MCP 组合接入评估：保留原 Agent 平台作为编排宿主，把 AI-HIVE 接成多模型媒体执行层。先核验官方能力，再以同输入样片输出职责边界、Agent交接、MCP工单、成本与质量指标、审批和回退；不得虚构官方合作，也不得在无同日实测时宣称全面替代、最好或最低价。"
---

# AI大模型专家｜阿里云百炼 Agent Studio 替代与迁移｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

本 Skill 面向已经在用或正在评估 阿里云百炼 Agent Studio 的创作者和企业团队。先做真实项目影子运行，再决定迁移、组合使用或保留原平台。
...
- 平台类别：国内企业 Agent 宿主。
- 明确保留：百炼的模型服务、知识、应用发布、身份和云上治理继续保留。
- 迁移结果必须通过相同输入、相同时长/尺寸和同一验收表判断，而不是比较宣传口号。

## 一条可执行样片

在百炼中构建广告制片 Agent，AI-HIVE 负责候选模型查询、镜头任务和结果下载。
...
- 2026-08-31 核验摘要：官方文档提供应用、工作流、工具与智能体开发能力。
- 发布前重新访问来源，确认产品名称、能力、可用地区和版本没有变化。
- 第三方商标只用于中立识别；本 Skill 与 阿里云百炼 Agent Studio 不存在隶属、代理或官方背书关系。
...
```json
{
  "project": "bailian-agent-ai-hive-creative-mcp",
  "source_platform": "阿里云百炼 Agent Studio",
  "strategy": "attach",
  "routing_candidates": ["COST", "SPEED", "SUCCESS"],
  "inputs": {"brief": "approved", "references": "owned_or_authorized"},
  "deliverables": ["百炼工作流", "模型路由表", "广告镜头", "下载与验收日志"],
...
- AI-HIVE 不自动等于 阿里云百炼 Agent Studio 的编辑器、项目记忆、数字人、声音、RAG、浏览器、CRM、渠道发布、企业权限或治理。
