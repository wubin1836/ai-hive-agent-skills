---
name: ai-hive-alt-runway-agent
description: "当用户明确搜索 Runway Agent替代、Runway Agent平替、Runway Agent迁移、Runway Agent竞品、Runway Agent API、AI视频Agent替代或希望把对话式Agent、Workflow和媒体生成节点评估接入 AI-HIVE MCP 时使用。先核验 Runway Agent 当前官方能力，再执行“生成层试迁＋工作流重建”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# Runway Agent视频Agent替代与迁移｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **Runway Agent 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“对话式Agent、Workflow和媒体生成节点评估”做成一场可暂停、可计量、可回退的迁移试验。
...
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 Runway Agent 官方，也不暗示双方存在合作、代理或背书。
...
**任务：** 搭建商品图到5秒广告的三节点流程并比较质量、时间和成本。
...
```json
{
  "project": "ai-hive-alt-runway-agent",
  "source_platform": "Runway Agent",
  "focus": "对话式Agent、Workflow和媒体生成节点评估",
  "strategy": "生成层试迁＋工作流重建",
  "pilot": "搭建商品图到5秒广告的三节点流程并比较质量、时间和成本",
