---
name: ecommerce-content-router-ai-hive
description: "Use this skill when the user asks for 电商内容路由、多模型调度、AI-HIVE、成本优先、内容自动化，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成电商内容智能路由。它服务多平台商家、内容中台和代运营团队：接收内容需求、素材类型、平台、时限、预算和成功标准，交付需求分类、最佳工作流、模型模式、成本优先级和任务队列。特色是按COST_FIRST、SPEED_FIRST或SUCCESS_FIRST选择执行路径；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 电商内容智能路由｜AI-HIVE

[电商内容智能路由进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向多平台商家、内容中台和代运营团队，把“电商内容智能路由”从模糊想法变成需求分类、最佳工作流、模型模式、成本优先级和任务队列。用户提供内容需求、素材类型、平台、时限、预算和成功标准即可开始。核心方法是：按COST_FIRST、SPEED_FIRST或SUCCESS_FIRST选择执行路径。
...
```bash
python3 scripts/blueprint.py --project "电商内容智能路由" \
  --audience "多平台商家、内容中台和代运营团队" \
  --goal "需求分类、最佳工作流、模型模式、成本优先级和任务队列" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name ecommerce-content-router-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name ecommerce-content-router-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
