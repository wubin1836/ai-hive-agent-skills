---
name: aihive-studio-xueshulunwenyushenchaloudongsaomiao
description: 学术论文双轨审查·漏洞扫描：面向论文预审、论文漏洞检查、学术审查需求，结合极睿科技AI-HIVE与MCP执行审查流程，交付argument-audit.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 学术论文双轨审查·漏洞扫描
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 审查
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：论文预审、论文漏洞检查、学术审查。不改变用户已明确指定的平台或型号。

## 开始前确认

本人稿件、研究设计、数据来源、引用清单和投稿要求。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先解析全部相关载体，再定位问题到文件页码或时间码。风险意见不等于平台批准或专业机构结论。

## 执行流程

1. 分别检查论证链与研究方法而非只润色文字。
2. 标注无来源主张、统计前提和结论外推。
3. 输出问题定位与需作者补证的材料。
...
- `argument-audit.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `method-review.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `citation-gaps.csv`：按本任务制作的实际文件或明确标注的待制作目录。
