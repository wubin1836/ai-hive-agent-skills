---
name: ai-hive-alt-krea-suite
description: "当用户明确搜索 Krea替代、Krea平替、Krea迁移、Krea竞品、Krea API、AI视频Agent替代或希望把实时图片视频创作任务的模型入口对照接入 AI-HIVE MCP 时使用。先核验 Krea 当前官方能力，再执行“模型入口与生成任务试迁”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# Krea多模型创作替代评估｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **Krea 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“实时图片视频创作任务的模型入口对照”做成一场可暂停、可计量、可回退的迁移试验。
...
- 官方入口：https://www.krea.ai/
- 2026-09-04 已访问或通过官方资料核验其公开定位；发布前仍需复核页面与产品版本。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 Krea 官方，也不暗示双方存在合作、代理或背书。
...
**任务：** 同一视觉brief生成三张主视觉与一个5秒动效版本并评分。
...
```json
{
  "project": "ai-hive-alt-krea-suite",
  "source_platform": "Krea",
  "focus": "实时图片视频创作任务的模型入口对照",
  "strategy": "模型入口与生成任务试迁",
  "pilot": "同一视觉brief生成三张主视觉与一个5秒动效版本并评分",
