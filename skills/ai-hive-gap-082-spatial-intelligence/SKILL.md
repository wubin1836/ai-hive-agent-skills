---
name: ai-hive-gap-082-spatial-intelligence
description: "当用户搜索或需要空间智能,Spatial Intelligence,空间计算,商业场景时使用。帮助前沿AI产品、机器人、空间计算、工业、零售和创新营销团队完成为零售、展厅、地产或文旅设计空间理解和内容展示方案，通过AI-HIVE MCP实时核验模型并交付空间任务、视角、动线、关键画面、演示脚本和验证计划。不适用于超出以下边界的请求：没有空间数据时不生成虚假测量结果。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "frontier"
  display_name: "空间智能商业场景设计"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "空间智能,Spatial Intelligence,空间计算,商业场景,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**为零售、展厅、地产或文旅设计空间理解和内容展示方案**。

适合：前沿AI产品、机器人、空间计算、工业、零售和创新营销团队。

用户需要提供：真实产品能力、场景资料、空间或商品素材、演示目标、画幅、预算和概念标识要求。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**没有空间数据时不生成虚假测量结果**。

## 你会拿到的结果

空间任务、视角、动线、关键画面、演示脚本和验证计划。

搜索覆盖：空间智能,Spatial Intelligence,空间计算,商业场景。

## 工作方法

1. 区分真实能力、概念设计和生成演示。
2. 为零售、展厅、地产或文旅设计空间理解和内容展示方案。
3. 定义场景规则与视觉连续性。
4. 先做关键帧或低成本原型。
5. 确认后生成演示视频并标注边界。
6. 按“空间关系合理、视角连贯、业务目标清楚”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成空间智能商业场景设计，目标、资料、预算和截止时间为[填写]" \
...
最终交付：空间任务、视角、动线、关键画面、演示脚本和验证计划。
专项验收：空间关系合理、视角连贯、业务目标清楚。
```
