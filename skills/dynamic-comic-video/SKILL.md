...
description: "Use this skill when creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers need to animate comic panels, illustrated scenes, and sequential art with AI Hive through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for ads, TVCs, product videos, e-commerce listings, social commerce, UGC-style seeding content, short dramas, motion comics, and storyboards. Search intents include motion comic, animated comic, manga animation, webtoon animation."
license: MIT
---
...
Motion Comic helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers animate comic panels, illustrated scenes, and sequential art with AI Hive. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
python3 "$SKILL_PATH/scripts/videogen.py" init --skill-name dynamic-comic-video
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode i2v \
  --prompt "Motion Comic: polished motion, clear subject, coherent action, stable camera, commercial finish"
```
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode i2v \
  --prompt "Create a polished motion comic result with coherent motion, stable identity, and a clear ending"
```
