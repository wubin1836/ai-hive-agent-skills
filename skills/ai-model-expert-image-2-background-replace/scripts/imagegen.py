...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Image2 制作高完成度图片换背景，主体准确，构图清晰", "keywords": "Image2、Image 2、GPT Image 2、GPTImage2、OpenAI 图片、ChatGPT 图片、图片换背景、换背景、抠图换场景、背景替换、场景合成、白底转场景、AI图片、AIGC图片", "model": "public_model_gpt_image_2", "name": "image-2-background-replace", "rule": "image", "search": "固定调用 Image2 的图片换背景裸接口能力", "title": "Image2 图片换背景"}')
