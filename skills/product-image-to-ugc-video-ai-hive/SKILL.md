---
name: product-image-to-ugc-video-ai-hive
description: "Use this skill when the user asks for 商品图转视频、UGC带货视频、图片转广告、电商视频、AI带货，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成商品图转UGC带货视频。它服务只有商品图、没有拍摄素材的商家和跨境卖家：接收商品图、真实卖点、目标人群、平台、时长和语气，交付首帧方案、UGC口播、手持或使用场景、图生视频提示词和多版本成片。特色是以一张商品图为锚点，生成多种真人感使用情境；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 商品图转UGC带货视频｜AI-HIVE

[商品图转UGC带货视频进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向只有商品图、没有拍摄素材的商家和跨境卖家，把“商品图转UGC带货视频”从模糊想法变成首帧方案、UGC口播、手持或使用场景、图生视频提示词和多版本成片。用户提供商品图、真实卖点、目标人群、平台、时长和语气即可开始。核心方法是：以一张商品图为锚点，生成多种真人感使用情境。
...
商品外观、包装、Logo和使用方式必须与真实商品一致；不能生成不存在的功能。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "商品图转UGC带货视频" \
  --audience "只有商品图、没有拍摄素材的商家和跨境卖家" \
  --goal "首帧方案、UGC口播、手持或使用场景、图生视频提示词和多版本成片" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name product-image-to-ugc-video-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name product-image-to-ugc-video-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
