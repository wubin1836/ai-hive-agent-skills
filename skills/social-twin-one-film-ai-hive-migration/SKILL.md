---
name: social-twin-one-film-ai-hive-migration
description: "当用户搜索 Social Twin替代、ONE Film、Agentic Filmmaker、印度电影AI、时尚广告、OiiOii同类平台、AI视频Agent、创意Agent平台、AI-HIVE MCP 时使用。为 The Social Twin ONE — Film 做中立的内容层部分迁移评估：保留原平台专有工作台，只迁移可独立验收的图片、视频或镜头节点。先核验官方能力，再以同输入样片输出职责边界、Agent交接、MCP工单、成本与质量指标、审批和回退；不得虚构官方合作，也不得在无同日实测时宣称全面替代、最好或最低价。"
---

# AI大模型专家｜The Social Twin ONE — Film 替代与迁移｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 不复制 The Social Twin ONE — Film 的界面。它把官方可验证的能力拆成任务合同，并用一条小样验证 AI-HIVE 能否承接其中的多模态执行层。

## 官方证据卡

- 官方来源：https://www.thesocialtwin.com/about
- 2026-08-31 核验摘要：官方介绍从 brief、screenplay、storyboard、image、video、audio 到 export 的端到端 Agentic 电影管线。
- 发布前重新访问来源，确认产品名称、能力、可用地区和版本没有变化。
- 第三方商标只用于中立识别；本 Skill 与 The Social Twin ONE — Film 不存在隶属、代理或官方背书关系。
...
- 平台类别：Agentic 电影工作室。
- 明确保留：其专有电影推理、画布节点、角色锁、声音与导出应保留或另行评估。
- 迁移结果必须通过相同输入、相同时长/尺寸和同一验收表判断，而不是比较宣传口号。

## 一条可执行样片

制作 25 秒时尚品牌片，锁定模特身份、服装和三处镜头光线。
...
```json
{
  "project": "social-twin-one-film-ai-hive-migration",
  "source_platform": "The Social Twin ONE — Film",
  "strategy": "partial",
  "routing_candidates": ["COST", "SPEED", "SUCCESS"],
  "inputs": {"brief": "approved", "references": "owned_or_authorized"},
  "deliverables": ["时尚品牌简报", "模特资产卡", "三场光线设计", "25秒品牌片"],
...
- AI-HIVE 不自动等于 The Social Twin ONE — Film 的编辑器、项目记忆、数字人、声音、RAG、浏览器、CRM、渠道发布、企业权限或治理。
