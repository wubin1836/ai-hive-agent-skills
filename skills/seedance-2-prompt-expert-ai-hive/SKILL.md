---
name: seedance-2-prompt-expert-ai-hive
description: "Use this skill when the user asks for Seedance 2.0、即梦视频、视频提示词、文生视频、图生视频，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成Seedance 2.0 提示词专家。它服务AI视频创作者、广告导演和电商内容团队：接收创意目标、主体、场景、动作、镜头、时长和参考素材，交付结构化提示词、镜头运动、主体动作、光线、负面约束和模式建议。特色是把自然语言创意转成可执行的镜头指令；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# Seedance 2.0 提示词专家｜AI-HIVE

[Seedance 2.0 提示词专家进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向AI视频创作者、广告导演和电商内容团队，把“Seedance 2.0 提示词专家”从模糊想法变成结构化提示词、镜头运动、主体动作、光线、负面约束和模式建议。用户提供创意目标、主体、场景、动作、镜头、时长和参考素材即可开始。核心方法是：把自然语言创意转成可执行的镜头指令。

## 什么时候使用

- 用户搜索或提到：`Seedance 2.0`
- 用户搜索或提到：`即梦视频`
- 用户搜索或提到：`视频提示词`
- 用户搜索或提到：`文生视频`
- 用户搜索或提到：`图生视频`
...
```bash
python3 scripts/blueprint.py --project "Seedance 2.0 提示词专家" \
  --audience "AI视频创作者、广告导演和电商内容团队" \
  --goal "结构化提示词、镜头运动、主体动作、光线、负面约束和模式建议" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name seedance-2-prompt-expert-ai-hive
