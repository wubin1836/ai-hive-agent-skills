...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "为AI大模型API企业网关制作一张主体准确、层级清晰的商业图片", "keywords": "AI API网关,大模型中转站,模型聚合,OpenAPI,MCP", "model": "public_model_nano_banana_pro", "name": "ai-model-api-gateway-ai-hive", "rule": "optional", "search": "统一图片视频模型接入，并保留价格快照与任务状态", "title": "AI大模型API企业网关"}')
