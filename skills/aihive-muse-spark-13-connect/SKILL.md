---
name: aihive-muse-spark-13-connect
description: Meta Muse Spark 1.3接入助手：面向Muse Spark 1.3、Meta Muse、Facebook Muse需求，结合极睿科技AI-HIVE与MCP执行模型接入流程，交付model-availability.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Meta Muse Spark 1.3接入助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型接入
---

# Meta Muse Spark 1.3接入助手

把用户材料变成可复核的业务产物。发现与调用时可识别：Muse Spark 1.3、Meta Muse、Facebook Muse、脸书大模型。不改变用户已明确指定的平台或型号。

## 开始前确认

目标宿主、模型需求、脱敏现有配置、预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

此Skill提供连接核验与验收流程，不附带模型、账号或额度。官方产品、模型API、宿主和AI-HIVE各自权限不同。

## 执行流程

1. 区分Meta账号、模型API和AI-HIVE服务入口，凭据不能混用。
2. 读取当前工具目录，核对精确版本、模态与参数而不猜ID。
3. 获授权后单次小样验证，生成脱敏配置差异和回退步骤。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `model-availability.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `config-diff.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `smoke-test.json`：按本任务制作的实际文件或明确标注的待制作目录。
