---
name: image-reference-to-video
description: "Use this skill when creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers need to use reference images to control subjects, products, composition, or style with AI Hive through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for ads, TVCs, product videos, e-commerce listings, social commerce, UGC-style seeding content, short dramas, motion comics, and storyboards. Search intents include reference image video, style reference video, character reference video."
license: MIT
---
...
Image Reference to Video helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers use reference images to control subjects, products, composition, or style with AI Hive. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
python3 "$SKILL_PATH/scripts/videogen.py" init --skill-name image-reference-to-video
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode r2v \
  --prompt "Image Reference to Video: polished motion, clear subject, coherent action, stable camera, commercial finish" \
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode r2v \
  --prompt "Create a polished image reference to video result with coherent motion, stable identity, and a clear ending" \
+  --image /path/to/reference.png
```
