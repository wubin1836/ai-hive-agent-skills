---
name: ai-hive-alt-recraft-brand-assets
description: "当用户明确搜索 Recraft替代、Recraft平替、Recraft迁移、Recraft竞品、Recraft API、AI视频Agent替代或希望把品牌视觉资产与矢量设计的组合替代评估接入 AI-HIVE MCP 时使用。先核验 Recraft 当前官方能力，再执行“模型入口与生成任务试迁”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# Recraft多模型创作替代评估｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **Recraft 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“品牌视觉资产与矢量设计的组合替代评估”做成一场可暂停、可计量、可回退的迁移试验。
...
- 待复核官方入口：https://www.recraft.ai/
- 已记录官方域名候选，发布前必须重新访问并核对产品名称、能力和可用地区。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 Recraft 官方，也不暗示双方存在合作、代理或背书。
...
**任务：** 先保留矢量编辑，再用AI-HIVE生成三张场景图与广告镜头。
...
```json
{
  "project": "ai-hive-alt-recraft-brand-assets",
  "source_platform": "Recraft",
  "focus": "品牌视觉资产与矢量设计的组合替代评估",
  "strategy": "模型入口与生成任务试迁",
  "pilot": "先保留矢量编辑，再用AI-HIVE生成三张场景图与广告镜头",
