---
name: ai-fashion-tryon-motion
description: "当用户搜索 AI试穿动图、服装动图、模特换装、穿搭Live图、电商图片、商品视频、AIGC营销、淘宝、京东、抖音、小红书、Amazon、Shopify、AI-HIVE MCP 时使用。帮助商家完成制作服装穿着与姿态变化动效：先锁定SKU与授权素材，再查询真实可用模型与价格，生成小样，逐帧验收商品结构、Logo、标签、色彩、文字和平台规格；提供可复制的提示词、JSON工单、失败回退和批量扩展方法。"
---

# AI大模型专家｜AI服装试穿动图｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向电商商家、内容团队、代运营和广告团队，目标是制作服装穿着与姿态变化动效，同时把商品真实性、品牌安全和批量成本纳入同一张验收表。
...
使用已授权模特和服装图制作 3 个轻姿态动作，检查版型、印花与人体结构。
...
```json
{
  "project": "ai-fashion-tryon-motion",
  "sku": "SKU-001",
  "task": "AI服装试穿动图",
  "inputs": {
