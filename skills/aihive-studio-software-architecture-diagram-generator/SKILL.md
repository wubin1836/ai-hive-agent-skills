---
name: aihive-studio-software-architecture-diagram-generator
description: 软件架构图生成器：面向软件架构图、系统架构图、技术架构图需求，结合极睿科技AI-HIVE与MCP执行图解流程，交付architecture.mmd等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 软件架构图生成器
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图解
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：软件架构图、系统架构图、技术架构图。不改变用户已明确指定的平台或型号。

## 开始前确认

系统组件、调用关系、数据存储、部署边界和读者。仅追问影响执行的缺项，不要求用户重新提供已有资料。

需要可编辑图源与真实渲染器。无渲染能力可交付源代码并标待预览，不能把纯文字冒充图片。

## 执行流程

1. 区分逻辑架构和部署架构，标同步异步与信任边界。
2. 检查箭头方向和依赖循环。
3. 输出可编辑源和渲染图，敏感地址脱敏。
...
- `architecture.mmd`：按本任务制作的实际文件或明确标注的待制作目录。
- `architecture.svg`：按本任务制作的实际文件或明确标注的待制作目录。
- `decisions.md`：按本任务制作的实际文件或明确标注的待制作目录。
