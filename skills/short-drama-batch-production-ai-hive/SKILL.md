---
name: short-drama-batch-production-ai-hive
description: "Use this skill when the user asks for 短剧批量生产、AI短剧流水线、漫剧量产、多集视频、内容工厂，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成短剧批量生产流水线。它服务多集短剧、漫剧工作室和企业内容生产线：接收全剧本、人物场景资产、集数、单集时长、产能和预算，交付集数看板、资产复用表、逐镜任务、失败重试、连续性抽检和交付清单。特色是先做样集锁定标准，再按镜头批量生成；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 短剧批量生产流水线｜AI-HIVE

[短剧批量生产流水线进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向多集短剧、漫剧工作室和企业内容生产线，把“短剧批量生产流水线”从模糊想法变成集数看板、资产复用表、逐镜任务、失败重试、连续性抽检和交付清单。用户提供全剧本、人物场景资产、集数、单集时长、产能和预算即可开始。核心方法是：先做样集锁定标准，再按镜头批量生成。
...
```bash
python3 scripts/blueprint.py --project "短剧批量生产流水线" \
  --audience "多集短剧、漫剧工作室和企业内容生产线" \
  --goal "集数看板、资产复用表、逐镜任务、失败重试、连续性抽检和交付清单" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name short-drama-batch-production-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name short-drama-batch-production-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
