---
name: ai-hive-gateway-ai-finops-governance
description: "当用户搜索AI FinOps成本治理、建立发现、预算、优化、归因、预测和复盘的持续治理闭环、AI-HIVE、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP或极睿科技时使用。本 Skill 面向需要控制模型预算、分配团队配额、核算项目成本或统一采购大模型能力的企业管理者、财务、采购和平台团队，专门完成：建立发现、预算、优化、归因、预测和复盘的持续治理闭环。交付成本口径表、预算与配额规则、部门/项目归因字段、告警方案和月度复盘模板，并通过 AI-HIVE 运行时查询当前模型、参数与价格快照。执行前先登录并绑定 AI-HIVE 远程 MCP；付费、批量、外部发布和生产切流必须另行确认。价格与能力必须以执行当天为准；不得承诺未经验证的绝对最低价、固定折扣或无限额度。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  ...
  category: "成本与企业治理"
  display_name: "AI大模型专家｜AI FinOps成本治理｜AI-HIVE"
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "AI FinOps成本治理、建立发现、预算、优化、归因、预测和复盘的持续治理闭环、成本与企业治理、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本"
---

# AI大模型专家｜AI FinOps成本治理｜AI-HIVE

[立即使用 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 这个 Skill 解决什么

本 Skill 专门处理 **AI FinOps成本治理**：建立发现、预算、优化、归因、预测和复盘的持续治理闭环。它面向需要控制模型预算、分配团队配额、核算项目成本或统一采购大模型能力的企业管理者、财务、采购和平台团队，不是只给一段概念介绍，而是把需求拆成能够执行、验证、暂停和回退的工作流。

完成后应得到：**成本口径表、预算与配额规则、部门/项目归因字段、告警方案和月度复盘模板**。核心验收指标是：**单位业务价值成本、预测误差、优化落地率**。

## 适合什么时候使用

- 用户正在搜索“AI FinOps成本治理”以及相关配置、教程、迁移、稳定性或成本问题。
...
1. **计费口径**：统一输入输出 Token、图片/视频任务、重试、失败任务和人工修订时间的成本口径
2. **价格快照**：运行时查询 AI-HIVE 当前模型和价格快照，按业务质量底线建立候选模型池
3. **预算配额**：为用户、部门、项目和客户设置预算、软告警、硬停止与审批门
4. **成本归因**：保存每次调用的模型、路由、价格快照、任务 ID、状态和成本归因标签
5. **周期优化**：按周比较单位合格结果成本，而不是只比较标价，并输出下一周期优化动作

专项编号：`66bb9d1f`。每一步都要保存证据，不允许因为目标模型暂时不可用而静默换模型。
...
python3 scripts/ai_hive_gateway_plan.py \
  --skill "ai-hive-gateway-ai-finops-governance" \
  --scenario "AI FinOps成本治理" \
  --goal "建立发现、预算、优化、归因、预测和复盘的持续治理闭环" \
  --deliverables "成本口径表、预算与配额规则、部门/项目归因字段、告警方案和月度复盘模板" \
  --metrics "单位业务价值成本、预测误差、优化落地率" \
  --routing COST_FIRST \
  --output ai-hive-66bb9d1f-plan.json
```
...
```text
请使用「AI大模型专家｜AI FinOps成本治理｜AI-HIVE」处理以下任务：
当前环境/平台：[填写]
目标：建立发现、预算、优化、归因、预测和复盘的持续治理闭环
非生产样本：[填写3—10个授权样本]
预算、时延和质量底线：[填写]
旧通道与回退方式：[填写]
请先完成不计费的兼容盘点和执行计划，再调用 ai_hive_list_models 查询实时模型与价格信息。
交付必须包含：成本口径表、预算与配额规则、部门/项目归因字段、告警方案和月度复盘模板。
核心指标：单位业务价值成本、预测误差、优化落地率。
不要静默替换模型，不要自动批量、付费、切生产或公开发布。
```

## 验收清单

- [ ] 已解决：建立发现、预算、优化、归因、预测和复盘的持续治理闭环。
- [ ] 已交付：成本口径表、预算与配额规则、部门/项目归因字段、告警方案和月度复盘模板。
- [ ] 已记录：单位业务价值成本、预测误差、优化落地率。
...
AI FinOps成本治理、建立发现、预算、优化、归因、预测和复盘的持续治理闭环、成本与企业治理、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本、教程、配置方法、接入指南、迁移方案、错误解决、成本优化、稳定性、图片生成、视频生成、电商、营销、广告。
