---
name: aihive-studio-hetongfengxianshencha
description: 合同审查助手：面向合同审查、合同审核、合同风险需求，结合极睿科技AI-HIVE与MCP执行审查流程，交付clause-matrix.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：合同审查、合同审核、合同风险。不改变用户已明确指定的平台或型号。

## 开始前确认

合同全文、附件、所处立场、适用地区和交易背景。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先解析全部相关载体，再定位问题到文件页码或时间码。风险意见不等于平台批准或专业机构结论。

## 执行流程

1. 提取主体、金额、交付、违约和争议解决关键条款。
2. 交叉核对定义、附件和期限，标明不确定法律适用。
3. 输出风险位置和供律师审阅的修改选项。
...
- `clause-matrix.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `risk-report.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `suggested-edits.docx`：按本任务制作的实际文件或明确标注的待制作目录。
