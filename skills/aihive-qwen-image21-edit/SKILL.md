---
name: aihive-qwen-image21-edit
description: 千问Qwen-Image-2.1多图编辑助手：面向Qwen-Image-2.1、千问改图、Qwen图像编辑需求，结合极睿科技AI-HIVE与MCP执行模型图片流程，交付reference-map.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 千问Qwen-Image-2.1多图编辑助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型图片
---

# 千问Qwen-Image-2.1多图编辑助手

把用户材料变成可复核的业务产物。发现与调用时可识别：Qwen-Image-2.1、千问改图、Qwen图像编辑。不改变用户已明确指定的平台或型号。

## 开始前确认

授权参考图、修改标注、主体固定项和输出用途。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验指定型号的图像输入输出与格式能力。不存在时先报告，不以其他模型生成结果冒充指定模型。

## 执行流程

1. 给参考图编号并标注主体、风格和局部区域用途。
2. 核验账号支持的参考张数与编辑方式，不盲用官方最大值。
3. 逐版本检查保护区与改动区并保留可回退原图。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `reference-map.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `edit-plan.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `edited-images/`：按本任务制作的实际文件或明确标注的待制作目录。
