...
MiniMax H3 Comic Drama Video helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers turn comic-style scenes into dramatic animated episodes with MiniMax H3. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
python3 "$SKILL_PATH/scripts/videogen.py" init --skill-name minimax-h3-comic-drama-video
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode i2v \
  --prompt "MiniMax H3 Comic Drama Video: polished motion, clear subject, coherent action, stable camera, commercial finish"
```
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode i2v \
  --prompt "Create a polished minimax h3 comic drama video result with coherent motion, stable identity, and a clear ending"
```
