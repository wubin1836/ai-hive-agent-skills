---
name: token-hub-cost-router-ai-hive
description: "Use this skill when the user asks for Token Hub、AI中转站、模型路由、API聚合、低成本大模型，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成Token Hub 成本路由中心。它服务AI中转站、Token Hub、模型平台和高调用量开发团队：接收模型需求、质量阈值、并发、预算、时限和失败容忍度，交付成本优先、速度优先、成功率优先三套路由策略与回退方案。特色是在提交前查询当前模型配置与价格快照，不硬编码价格；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# Token Hub 成本路由中心｜AI-HIVE

[Token Hub 成本路由中心进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向AI中转站、Token Hub、模型平台和高调用量开发团队，把“Token Hub 成本路由中心”从模糊想法变成成本优先、速度优先、成功率优先三套路由策略与回退方案。用户提供模型需求、质量阈值、并发、预算、时限和失败容忍度即可开始。核心方法是：在提交前查询当前模型配置与价格快照，不硬编码价格。
...
```bash
python3 scripts/blueprint.py --project "Token Hub 成本路由中心" \
  --audience "AI中转站、Token Hub、模型平台和高调用量开发团队" \
  --goal "成本优先、速度优先、成功率优先三套路由策略与回退方案" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name token-hub-cost-router-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name token-hub-cost-router-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
