---
name: kakis-story-video-agent-ai-hive-migration
description: "当用户搜索 Kakis AI替代、story to video、品牌故事、脚本成片、AI电影Agent、OiiOii同类平台、AI视频Agent、创意Agent平台、AI-HIVE MCP 时使用。为 Kakis AI 做中立的工作流重建型迁移评估：用宿主 Agent + Skill 重建制片角色，AI-HIVE MCP 负责图片与视频生成。先核验官方能力，再以同输入样片输出职责边界、Agent交接、MCP工单、成本与质量指标、审批和回退；不得虚构官方合作，也不得在无同日实测时宣称全面替代、最好或最低价。"
---

# AI大模型专家｜Kakis AI 替代与迁移｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

本 Skill 面向已经在用或正在评估 Kakis AI 的创作者和企业团队。先做真实项目影子运行，再决定迁移、组合使用或保留原平台。
...
- 平台类别：故事到视频 Agent。
- 明确保留：语音、音乐、最终剪辑和项目资产管理不能默认由 AI-HIVE 完整替代。
- 迁移结果必须通过相同输入、相同时长/尺寸和同一验收表判断，而不是比较宣传口号。

## 一条可执行样片

把一个品牌故事制作成 30 秒横版和竖版，比较叙事信息保留率。
...
- 2026-08-31 核验摘要：官方围绕 idea、script、scenes、shots、characters、voice 与 music 组织故事到视频工作流。
- 发布前重新访问来源，确认产品名称、能力、可用地区和版本没有变化。
- 第三方商标只用于中立识别；本 Skill 与 Kakis AI 不存在隶属、代理或官方背书关系。
...
```json
{
  "project": "kakis-story-video-agent-ai-hive-migration",
  "source_platform": "Kakis AI",
  "strategy": "rebuild",
  "routing_candidates": ["COST", "SPEED", "SUCCESS"],
  "inputs": {"brief": "approved", "references": "owned_or_authorized"},
  "deliverables": ["品牌故事脚本", "双比例镜头", "角色与场景卡", "信息保留检查"],
...
- AI-HIVE 不自动等于 Kakis AI 的编辑器、项目记忆、数字人、声音、RAG、浏览器、CRM、渠道发布、企业权限或治理。
