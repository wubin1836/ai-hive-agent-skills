---
name: video-outfit-scene-light-edit-ai-hive
description: "Use this skill when the user asks for AI视频换装、视频换背景、视频改光线、生成式编辑、视频重绘，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成视频换服装换场景换光线。它服务短视频创作者、广告后期、服装品牌和社媒团队：接收原视频、授权人物、目标服装或场景参考、保留内容和交付规格，交付编辑意图、稳定锚点、换装/换景/换光方案、生成式编辑任务和连续性检查。特色是每次只修改一类主要变量，避免人物与动作同时漂移；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 视频换服装换场景换光线｜AI-HIVE

[视频换服装换场景换光线进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向短视频创作者、广告后期、服装品牌和社媒团队，把“视频换服装换场景换光线”从模糊想法变成编辑意图、稳定锚点、换装/换景/换光方案、生成式编辑任务和连续性检查。用户提供原视频、授权人物、目标服装或场景参考、保留内容和交付规格即可开始。核心方法是：每次只修改一类主要变量，避免人物与动作同时漂移。
...
素材探测报告、剪辑决策表、确定性处理命令、AI补镜方案与交付文件。默认先输出可审查方案，得到确认后才提交可能计费的图片或视频生成任务。
...
1. **探测素材规格并备份原片**：先形成可检查的中间结果，再进入下一步。
2. **确认保留段落与交付比例**：先形成可检查的中间结果，再进入下一步。
3. **完成裁切、改比例、响度与拼接**：先形成可检查的中间结果，再进入下一步。
4. **识别需要AI生成或编辑的缺口**：先形成可检查的中间结果，再进入下一步。
5. **通过AI-HIVE补镜或延长**：先形成可检查的中间结果，再进入下一步。
6. **检查音画、字幕安全区与编码**：先形成可检查的中间结果，再进入下一步。
...
不得未经授权修改真人形象或用于欺骗；重要商品、制服和品牌标识要人工复核。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "视频换服装换场景换光线" \
  --audience "短视频创作者、广告后期、服装品牌和社媒团队" \
  --goal "编辑意图、稳定锚点、换装/换景/换光方案、生成式编辑任务和连续性检查" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name video-outfit-scene-light-edit-ai-hive
