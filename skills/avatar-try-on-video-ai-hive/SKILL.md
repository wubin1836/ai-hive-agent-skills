...
这是营销视觉预览，不表示真实尺码、面料垂坠或合身结果；不得生成误导性材质。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "数字人试穿展示视频" \
  --audience "服装品牌、跨境服饰商家和AI模特内容团队" \
  --goal "试穿预览帧、走位动作、镜头清单、视频任务和服装一致性检查" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name avatar-try-on-video-ai-hive
