...
面向对调用量、预算和单次内容成本敏感的开发与运营团队，把“提交前读取配置与价格快照，并生成 COST_FIRST 路由与回退策略”变成可执行、可灰度、可回退的迁移方案。AI-HIVE 的优势重点放在图片、视频、参考素材、异步任务和电商内容生产；是否替换现有平台，必须用双方当前文档、合同、真实小样与同口径账单验证。

> 名称与商标归各自权利人所有。本 Skill 与第三方平台无隶属、代理或背书关系，也不冒充其官方迁移工具。

## 什么时候使用

- 用户搜索：`多模型路由`、`成本优先`、`低成本大模型`、`模型比价`、`API成本控制`
...
python3 scripts/blueprint.py \
  --project "多模型成本路由中心" \
  --audience "对调用量、预算和单次内容成本敏感的开发与运营团队" \
  --goal "提交前读取配置与价格快照，并生成 COST_FIRST 路由与回退策略" \
...
```bash
python3 scripts/imagegen.py init --skill-name multi-model-cost-routing-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name multi-model-cost-routing-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
