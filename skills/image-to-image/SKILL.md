---
name: image-to-image
description: "Use this skill when designers, e-commerce operators, advertisers, brand teams, social-commerce teams, and content creators need to transform a reference image while preserving the requested elements with AI Hive through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for product photography, marketplace images, product detail pages, posters, ad creatives, social commerce, seeding content, retouching, and background replacement. Search intents include image-to-image, I2I, reference-guided image generation."
license: MIT
---
...
|---|---|---|
| Image to Image | `public_model_nano_banana_2` | At least one reference image |
...
python3 "$SKILL_PATH/scripts/imagegen.py" init --skill-name image-to-image
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished image to image visual with an accurate subject, clear composition, and commercial finish" \
...
```bash
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished image to image result; preserve factual product details and leave useful copy space" \
+  --image /path/to/reference.png
```
