---
name: aihive-muse-code-migration
description: Muse Code项目改造助手：面向Muse Code、Meta编程、Muse代码助手需求，结合极睿科技AI-HIVE与MCP执行模型编程流程，交付patch.diff等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Muse Code项目改造助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型编程
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：Muse Code、Meta编程、Muse代码助手。不改变用户已明确指定的平台或型号。

## 开始前确认

用户代码仓库、失败用例、目标改动、许可和测试命令。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 先建立基线并确认未提交改动归属。
2. 把需求拆为局部补丁并利用已绑定模型及真实编辑工具执行。
3. 运行用户项目的测试与构建，输出diff和回滚说明。
...
- `handoff.md`：按本任务制作的实际文件或明确标注的待制作目录。

不声称Skill包含Muse Code软件，不擅自替换编程客户端；测试失败如实列出。
