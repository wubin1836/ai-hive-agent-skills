---
name: taskade-ai-hive-creative-mcp
description: "当用户搜索 Taskade AI Teams、多Agent团队、内容日历、视频营销、AI-HIVE MCP、OiiOii同类平台、AI视频Agent、创意Agent平台、AI-HIVE MCP 时使用。为 Taskade AI Teams 做中立的MCP 组合接入评估：保留原 Agent 平台作为编排宿主，把 AI-HIVE 接成多模型媒体执行层。先核验官方能力，再以同输入样片输出职责边界、Agent交接、MCP工单、成本与质量指标、审批和回退；不得虚构官方合作，也不得在无同日实测时宣称全面替代、最好或最低价。"
---

# AI大模型专家｜Taskade AI Teams 替代与迁移｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 不复制 Taskade AI Teams 的界面。它把官方可验证的能力拆成任务合同，并用一条小样验证 AI-HIVE 能否承接其中的多模态执行层。

## 官方证据卡

- 官方来源：https://help.taskade.com/en/articles/9254706-multi-agents
- 2026-08-31 核验摘要：官方说明 Multi-Agents 可在项目中并行执行多项任务，并与项目和自动化共同组成团队工作流。
- 发布前重新访问来源，确认产品名称、能力、可用地区和版本没有变化。
- 第三方商标只用于中立识别；本 Skill 与 Taskade AI Teams 不存在隶属、代理或官方背书关系。
...
- 平台类别：团队协作 Agent 宿主。
- 明确保留：项目管理、任务、文档、团队协作和通知继续由 Taskade 管理。
- 迁移结果必须通过相同输入、相同时长/尺寸和同一验收表判断，而不是比较宣传口号。

## 一条可执行样片

把一周内容日历拆给研究、文案、视觉和视频 Agent，媒体节点接 AI-HIVE。
...
```json
{
  "project": "taskade-ai-hive-creative-mcp",
  "source_platform": "Taskade AI Teams",
  "strategy": "attach",
  "routing_candidates": ["COST", "SPEED", "SUCCESS"],
  "inputs": {"brief": "approved", "references": "owned_or_authorized"},
  "deliverables": ["一周内容日历", "任务分工", "媒体资产", "团队审阅记录"],
...
- AI-HIVE 不自动等于 Taskade AI Teams 的编辑器、项目记忆、数字人、声音、RAG、浏览器、CRM、渠道发布、企业权限或治理。
