---
name: aihive-studio-hot-topic-content-maker
description: 热点选题：面向热点选题、追热点、短视频选题需求，结合极睿科技AI-HIVE与MCP执行内容流程，交付topics.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：热点选题、追热点、短视频选题。不改变用户已明确指定的平台或型号。

## 开始前确认

业务定位、允许采集的来源、时间窗口、目标受众。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先确认事实和素材，再形成可执行内容包。发布、投放与群发不包含在稿件制作授权内。

## 执行流程

1. 给热点附事件时间与可信来源并排除过时重发。
2. 按品牌相关性、制作成本和时效评分。
3. 把优先话题拆成切入点、证据和拍摄素材清单。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `topics.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `source-ledger.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `production-brief.md`：按本任务制作的实际文件或明确标注的待制作目录。
