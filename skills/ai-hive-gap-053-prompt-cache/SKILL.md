---
name: ai-hive-gap-053-prompt-cache
description: "当用户搜索或需要Prompt Cache,提示词缓存,KV Cache,大模型成本时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成识别稳定前缀与动态输入，设计可复用提示词缓存，通过AI-HIVE MCP实时核验模型并交付缓存边界、键设计、失效规则、成本估算和测试请求。不适用于超出以下边界的请求：未验证的平台缓存能力必须标为方案假设。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "Prompt Cache提示词缓存设计"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "Prompt Cache,提示词缓存,KV Cache,大模型成本,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**识别稳定前缀与动态输入，设计可复用提示词缓存**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**未验证的平台缓存能力必须标为方案假设**。

## 你会拿到的结果

缓存边界、键设计、失效规则、成本估算和测试请求。

搜索覆盖：Prompt Cache,提示词缓存,KV Cache,大模型成本。

## 执行蓝图

1. 先定义可量化的质量和成本指标。
2. 识别稳定前缀与动态输入，设计可复用提示词缓存。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“命中逻辑正确、过期可控、隐私不串用”完成专项验收。
...
python3 scripts/ai_hive_mcp.py call ai_hive_list_models \
  --args '{"query":"Prompt Cache提示词缓存设计"}'
```
...
python3 scripts/plan.py \
  --brief "我要完成Prompt Cache提示词缓存设计，目标、资料、预算和截止时间为[填写]" \
...
```text
请使用「Prompt Cache提示词缓存设计」Skill帮助我。
我的目标：[填写]
受众和业务场景：[填写]
已有资料与来源：[填写]
交付格式和数量：[填写]
...
最终交付：缓存边界、键设计、失效规则、成本估算和测试请求。
专项验收：命中逻辑正确、过期可控、隐私不串用。
```
