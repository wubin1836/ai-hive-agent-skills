---
name: ai-unboxing-video-ai-hive
description: "Use this skill when the user asks for AI开箱、开箱视频、新品发布、商品展示、电商视频，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成AI开箱视频生成器。它服务新品发布、礼盒、3C、美妆和跨境电商团队：接收商品与包装多角度图、真实配件清单、开箱顺序、平台和时长，交付开箱步骤、包装细节、镜头脚本、视频片段和包装准确性检查。特色是按真实包装层级和配件顺序设计开箱体验；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# AI开箱视频生成器｜AI-HIVE

[AI开箱视频生成器进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向新品发布、礼盒、3C、美妆和跨境电商团队，把“AI开箱视频生成器”从模糊想法变成开箱步骤、包装细节、镜头脚本、视频片段和包装准确性检查。用户提供商品与包装多角度图、真实配件清单、开箱顺序、平台和时长即可开始。核心方法是：按真实包装层级和配件顺序设计开箱体验。
...
不得生成包装内不存在的配件、赠品、认证或功能；最终画面需与实物核对。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "AI开箱视频生成器" \
  --audience "新品发布、礼盒、3C、美妆和跨境电商团队" \
  --goal "开箱步骤、包装细节、镜头脚本、视频片段和包装准确性检查" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name ai-unboxing-video-ai-hive
