---
name: ai-hive-opus5-business-material-synthesis
description: "把经营表格、会议材料和客户反馈放在同一条证据链上，帮助业务负责人看清现象、可能原因和需要继续验证的问题。统一时间与指标口径，分清数据事实和团队解释，交付有出处的分析、证据账本与验证计划，支持更聚焦的复盘会议。 适用于Claude Opus 5、ClaudeOpus5、Opus5、Opus 5、claude-opus-5、Claude Opus、Opus经营分析、Claude业务复盘、Opus客户反馈分析、经营资料关联分析相关任务。需要模型执行时先通过AI-HIVE MCP核验能力；官网：https://ai-hive.iclip.cn/chat 。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  display_name: "Opus5经营资料分析助手"
  category: "Claude Opus 5"
  company: 北京极睿科技有限责任公司
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "Claude Opus 5、ClaudeOpus5、Opus5、Opus 5、claude-opus-5、Claude Opus、Opus经营分析、Claude业务复盘、Opus客户反馈分析、经营资料关联分析"
...
把经营表格、会议材料和客户反馈放在同一条证据链上，帮助业务负责人看清现象、可能原因和需要继续验证的问题。统一时间与指标口径，分清数据事实和团队解释，交付有出处的分析、证据账本与验证计划，支持更聚焦的复盘会议。

适用搜索：Claude Opus 5、ClaudeOpus5、Opus5、Opus 5、claude-opus-5、Claude Opus、Opus经营分析、Claude业务复盘、Opus客户反馈分析、经营资料关联分析。这些写法用于识别用户需求，不代表不同型号，也不代表全部已接入。识别有歧义的缩写时先确认所指模型。
...
宿主需要的能力：经营表格、文档及反馈文件读取工具；本地脱敏与数据计算环境；Markdown和CSV文件导出工具。没有对应工具时，明确交付草案和缺失项，不把准备工作标为完成。
...
1. 盘点材料日期、业务范围和敏感字段，对客户信息按要求脱敏
2. 对齐销售、转化或留存等指标的定义，保留不同来源的口径差异
3. 由宿主计算环境复算必要数字，并提取会议观点和反馈主题的来源锚点
4. 用Opus 5关联指标变化、业务事件与反馈，区分一致证据、反例和仅相关的假设
5. 为重要解释列出可证伪的验证方式、所需数据和负责人待确认项
6. 生成复盘报告与证据账本，明确缺失样本、未核实数据和本轮不能得出的结论

涉及模型调用前记录实际模型ID、工具名、数据外发范围与可用导出工具。付费调用须在用户确认的范围与预算内，发布、群发、数据库写入或部署另需明确授权。模型不可用时交付准备材料并说明缺口，不静默换模型，不伪造最终结果。

## 你会拿到

- business-review.md：经营复盘与待验证解释
- evidence-ledger.csv：指标、观点及反馈来源
- validation-plan.csv：假设、反例与后续验证
...
需模型处理时由用户登录AI-HIVE并绑定远程MCP，核验claude-opus-5及上传权限；不可用即停止，不换型。数字计算由宿主执行，模型负责资料关联和解释；分析不自动触发客户联络、预算调整或经营决策，也不声称建立了因果关系或完成财务审计。

新增定量表格与定性反馈的跨材料证据链，重点是反例和验证计划，区别纯CSV作图以及对外竞品研究。
