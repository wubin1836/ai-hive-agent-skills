---
name: ai-hive-gateway-active-standby-dual-channel
description: "当用户搜索主备模型双通道容灾、建立主通道、备用通道、健康探测、切换门槛和恢复观察窗口、AI-HIVE、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP或极睿科技时使用。本 Skill 面向负责线上 AI 应用、内容生产流水线、模型网关可靠性或媒体任务交付的架构师、SRE 和工程团队，专门完成：建立主通道、备用通道、健康探测、切换门槛和恢复观察窗口。交付路由策略、SLO 指标、故障注入用例、观测字段、告警阈值和恢复手册，并通过 AI-HIVE 运行时查询当前模型、参数与价格快照。执行前先登录并绑定 AI-HIVE 远程 MCP；付费、批量、外部发布和生产切流必须另行确认。不得用无限重试掩盖故障；异步任务超时后应续查原 taskId，避免重复提交和重复计费。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  ...
  category: "稳定性工程"
  display_name: "AI大模型专家｜主备模型双通道容灾｜AI-HIVE"
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "主备模型双通道容灾、建立主通道、备用通道、健康探测、切换门槛和恢复观察窗口、稳定性工程、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本"
---

# AI大模型专家｜主备模型双通道容灾｜AI-HIVE

[立即使用 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 这个 Skill 解决什么

本 Skill 专门处理 **主备模型双通道容灾**：建立主通道、备用通道、健康探测、切换门槛和恢复观察窗口。它面向负责线上 AI 应用、内容生产流水线、模型网关可靠性或媒体任务交付的架构师、SRE 和工程团队，不是只给一段概念介绍，而是把需求拆成能够执行、验证、暂停和回退的工作流。

完成后应得到：**路由策略、SLO 指标、故障注入用例、观测字段、告警阈值和恢复手册**。核心验收指标是：**RTO、RPO、切换成功率**。

## 适合什么时候使用

- 用户正在搜索“主备模型双通道容灾”以及相关配置、教程、迁移、稳定性或成本问题。
...
专项编号：`db3c455a`。每一步都要保存证据，不允许因为目标模型暂时不可用而静默换模型。
...
python3 scripts/ai_hive_gateway_plan.py \
  --skill "ai-hive-gateway-active-standby-dual-channel" \
  --scenario "主备模型双通道容灾" \
  --goal "建立主通道、备用通道、健康探测、切换门槛和恢复观察窗口" \
  --deliverables "路由策略、SLO 指标、故障注入用例、观测字段、告警阈值和恢复手册" \
  --metrics "RTO、RPO、切换成功率" \
  --routing SUCCESS_FIRST \
  --output ai-hive-db3c455a-plan.json
```
...
```text
请使用「AI大模型专家｜主备模型双通道容灾｜AI-HIVE」处理以下任务：
当前环境/平台：[填写]
目标：建立主通道、备用通道、健康探测、切换门槛和恢复观察窗口
非生产样本：[填写3—10个授权样本]
预算、时延和质量底线：[填写]
旧通道与回退方式：[填写]
...
- [ ] 已解决：建立主通道、备用通道、健康探测、切换门槛和恢复观察窗口。
...
主备模型双通道容灾、建立主通道、备用通道、健康探测、切换门槛和恢复观察窗口、稳定性工程、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本、教程、配置方法、接入指南、迁移方案、错误解决、成本优化、稳定性、图片生成、视频生成、电商、营销、广告。
