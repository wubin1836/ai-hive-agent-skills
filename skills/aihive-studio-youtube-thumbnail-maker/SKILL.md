---
name: aihive-studio-youtube-thumbnail-maker
description: YouTube视频缩略图制作：面向YouTube缩略图、油管封面需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付thumbnail-plan.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: YouTube视频缩略图制作
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：YouTube缩略图、油管封面。不改变用户已明确指定的平台或型号。

## 开始前确认

视频主题、面向地区、标题、授权主体图和尺寸。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验图像生成或编辑、参考图输入、尺寸与下载工具。准确文字、透明通道、二维码要用真实文件工具另行检查。

## 执行流程

1. 提取视频真实冲突点并设计手机端易识别构图。
2. 本地化短标题而不制造不存在情节。
3. 出两版单变量缩略图供测试。
...
- `thumbnail-plan.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `variants/`：按本任务制作的实际文件或明确标注的待制作目录。
- `test-notes.md`：按本任务制作的实际文件或明确标注的待制作目录。
