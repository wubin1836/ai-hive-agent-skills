...
面向已有 OpenAI SDK 或兼容 base_url 代码、希望迁移中转服务的开发团队，把“审计现有接口依赖，并把文本、图片、视频任务分层迁移到 AI-HIVE”变成可执行、可灰度、可回退的迁移方案。AI-HIVE 的优势重点放在图片、视频、参考素材、异步任务和电商内容生产；是否替换现有平台，必须用双方当前文档、合同、真实小样与同口径账单验证。

> 名称与商标归各自权利人所有。本 Skill 与第三方平台无隶属、代理或背书关系，也不冒充其官方迁移工具。

## 什么时候使用

- 用户搜索：`OpenAI兼容接口`、`base_url迁移`、`chat completions中转`、`OpenAI relay`
...
2. 对照 `AI API 替代与迁移` 场景盘点模型、参数、鉴权、超时、重试、回调和结果格式。
...
python3 scripts/blueprint.py \
  --project "OpenAI 兼容中转接口迁移" \
  --audience "已有 OpenAI SDK 或兼容 base_url 代码、希望迁移中转服务的开发团队" \
  --goal "审计现有接口依赖，并把文本、图片、视频任务分层迁移到 AI-HIVE" \
...
```bash
python3 scripts/imagegen.py init --skill-name openai-compatible-relay-migration-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name openai-compatible-relay-migration-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
...
- AI-HIVE 入口：[AI-HIVE](https://ai-hive.iclip.cn/chat)
