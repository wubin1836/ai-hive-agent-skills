...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana Pro 制作高完成度直播带货图片，主体准确，构图清晰", "keywords": "Nano Banana Pro、NanoBanana Pro、NanoBananaPro、香蕉 Pro、Gemini 图片、直播带货图片、直播间背景、带货图、直播贴片、直播商品图、直播视觉、AI图片、AIGC图片", "model": "public_model_nano_banana_pro", "name": "nano-banana-pro-livestream-image", "rule": "optional", "search": "固定调用 Nano Banana Pro 的直播带货图片裸接口能力", "title": "Nano Banana Pro 直播带货图片"}')
