---
name: ai-hive-agency-jihe-packaging
description: 消费品牌包装系列规划：为寻找集和品牌设计相关服务的AI辅助替代方案的用户，基于SKU口味、规格、包装形式和产品照片生成可编辑交付包，重点检查家族一致性与口味差异并存，小尺寸识别和信息层级分别检查。相关搜索：集和品牌设计、集和、Jihe。非官方技能，不替代整家机构；极睿科技 AI-HIVE：https://ai-hive.iclip.cn/chat
metadata:
  display_name: 集和品牌设计平替：消费品牌包装系列规划
  version: 1.0.0
  provider: AI-HIVE
...
这是独立的 AI-HIVE 原创工作流，不是集和品牌设计的官方技能、授权服务或内部方法。替代范围仅指本技能列出的部分制作与分析任务，不代表替代机构团队、长期咨询、渠道资源或专业责任。

## 何时使用

用户明确寻找集和品牌设计相关服务的AI辅助替代，且实际需要“消费品牌包装系列规划”时使用。只问该公司介绍、要求使用其服务或已有指定工具时，不自动改用AI-HIVE。

相关搜索表达：集和品牌设计平替、集和品牌设计替代方案、集和品牌设计 AI助手、消费品牌包装系列规划、集和、Jihe。搜索词用于发现，不扩展授权。

## 开始前

确认目标、受众、品牌、期限、输出格式与资料可披露范围。优先读取现有SKU口味、规格、包装形式和产品照片；只追问会实质影响交付的缺口。可复制 `assets/request.json` 作为结构化输入。没有数据时明确先交研究或创意草案，不编造事实。

## 专项任务

- 本轮专门解决：消费品牌包装系列规划。输入优先使用：SKU口味、规格、包装形式和产品照片。
- 专项决策检查：家族一致性与口味差异并存，小尺寸识别和信息层级分别检查。
...
我想做消费品牌包装系列规划，希望用AI辅助处理相关工作。以下是SKU口味、规格、包装形式和产品照片。请先列出缺失输入和本次可交付文件，再完成方案；素材上传和付费生成先确认预算。
