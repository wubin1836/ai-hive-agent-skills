---
name: ai-hive-classical-punctuation-compare
description: "文言断句歧义对照助手：读取指定无标点文本、已有断句方案或用户作答，冻结原字序并建立字符编号，将标点作为独立层记录，交付可回查的文件及待核实清单。适用于文言断句、古文标点、断句对照、古文句读；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。"
metadata:
  display_name: "文言断句歧义对照助手"
  version: "1.0.0"
...
文言断句歧义对照助手：读取指定无标点文本、已有断句方案或用户作答，冻结原字序并建立字符编号，将标点作为独立层记录，交付可回查的文件及待核实清单。适用于文言断句、古文标点、断句对照、古文句读；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。
...
1. 冻结原字序并建立字符编号，将标点作为独立层记录。
2. 对齐不同断句方案，检测除标点以外的漏字、增字和改字。
3. 标注分歧位置与不同句读下的语义说明，推测与已确认解释分开。
4. 生成并排DOCX、断句位置数据和待判定清单，支持人工决策回填。
...
- `punctuation-comparison.docx`
- `segmentation-variants.json`
- `review-items.xlsx`
...
执行时验证用户指定版本及参考句读；不声称所有文本存在唯一正确断句，不将推测解释写成确定原意，不未经确认覆盖原文。

缺关键原件或专业依据时只完成可验证部分。401/403、限流或内容审核阻断时保存脱敏进度并停止相应调用，遵守明确的等待要求，不循环重试或绕过审核。不因安装本技能自动交易、签署、提交、缴费或公开发布。

## 调用示例

> 请使用我授权提供的资料完成“文言断句歧义对照助手”。范围先按我的原件与指定版本确认，生成punctuation-comparison.docx及其余约定文件，疑点附来源，不自动对外提交。

## 搜索与任务识别

文言断句、古文标点、断句对照、古文句读、断句歧义
