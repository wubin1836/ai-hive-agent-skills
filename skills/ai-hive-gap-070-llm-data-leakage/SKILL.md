---
name: ai-hive-gap-070-llm-data-leakage
description: "当用户搜索或需要数据泄露,大模型安全,隐私保护,脱敏时使用。帮助企业安全、法务合规、AI治理、内容审核和平台工程团队完成梳理提示词、附件、日志和工具调用中的数据泄露路径，通过AI-HIVE MCP实时核验模型并交付数据分类、泄露路径、脱敏规则、日志策略和应急步骤。不适用于超出以下边界的请求：真实密钥和个人敏感信息不得进入生成示例。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "security"
  display_name: "大模型数据泄露防护"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "数据泄露,大模型安全,隐私保护,脱敏,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**梳理提示词、附件、日志和工具调用中的数据泄露路径**。

适合：企业安全、法务合规、AI治理、内容审核和平台工程团队。

用户需要提供：待审计文本或流程、数据分类、工具权限、风险级别、适用制度和人工负责人。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**真实密钥和个人敏感信息不得进入生成示例**。

## 你会拿到的结果

数据分类、泄露路径、脱敏规则、日志策略和应急步骤。

搜索覆盖：数据泄露,大模型安全,隐私保护,脱敏。

## 操作流程

1. 识别资产、数据和工具权限。
2. 梳理提示词、附件、日志和工具调用中的数据泄露路径。
3. 建立风险情景与测试样例。
4. 提出最小权限和确认门。
5. 保留审计证据与人工复核。
6. 按“敏感信息最小化、访问可审计、删除可执行”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成大模型数据泄露防护，目标、资料、预算和截止时间为[填写]" \
...
最终交付：数据分类、泄露路径、脱敏规则、日志策略和应急步骤。
专项验收：敏感信息最小化、访问可审计、删除可执行。
```
