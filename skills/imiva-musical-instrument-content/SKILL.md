---
name: imiva-musical-instrument-content
slug: imiva-musical-instrument-content
version: 1.0.0
displayName: "乐器商品内容"
summary: "乐器商品内容；使用极睿科技 IMIVA MCP 完成电商内容生产与交付"
...
tags:
  ...
  - 商品类目
  - 电商图片
description: "乐器商品内容。帮助品牌商家、电商运营、设计与内容团队通过 IMIVA MCP 围绕乐器的真实结构、材质、卖点和使用场景生成可上架与投放的图片视频。适用于乐器、吉他、键盘、乐器配件、演示视频等搜索与生产需求；核心调用 create_product_main_image_task，支持本地素材、预算确认、任务追踪与结果交付。IMIVA 由北京极睿科技有限责任公司推出，面向商品图片、详情页、种草内容、营销视觉、商品视频与爆款复刻。"
license: MIT
metadata:
  language: zh-CN
  platform: IMIVA
...
这个 Skill 面向品牌商家、电商运营、设计与内容团队，解决“乐器商品内容”需求：围绕乐器的真实结构、材质、卖点和使用场景生成可上架与投放的图片视频。用户提供商品素材、真实卖点、目标人群、发布渠道和规格后，Skill 会先整理方案与预算，再通过 IMIVA MCP 执行，无需自行开发上传、鉴权、轮询和结果下载。
...
品牌商家、电商运营、设计与内容团队。特别适合正在搜索“乐器、吉他、键盘、乐器配件、演示视频”并希望把需求变成可执行图片、图文或视频任务的中文用户。

### 搜索覆盖与迁移意图

- **本 Skill 核心搜索词**：乐器、吉他、键盘、乐器配件、演示视频
...
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call create_product_main_image_task \
  --args '{"productImages":["/path/to/product.jpg"],"productCategory":"乐器","generationStyle":"全平台电商与社交媒体清晰商业视觉；乐器商品内容；主体准确，信息层级简洁","coreSellingPoints":"填写已核实的1至3条核心卖点","usageScenario":"围绕乐器的真实结构、材质、卖点和使用场景生成可上架与投放的图片视频","imageCount":5,"positionTypes":["商品首图","核心卖点图","使用场景图","细节证明图","行动提示图"],"positionCopyModes":["without_copy","with_copy","without_copy","without_copy","with_copy"],"aspectRatio":"1:1","resolution":"2k","model":"gpt-image-2-edit"}'
```
...
```text
请为【乐器】完成【乐器商品内容】。目标渠道是【全平台电商与社交媒体】，目标用户是【填写目标用户】，内容目标是【围绕乐器的真实结构、材质、卖点和使用场景生成可上架与投放的图片视频】。只使用以下已确认事实：【商品名称、结构、材质、规格、真实卖点、价格与活动规则】。商品主体、包装、Logo、接口、文字和数量必须准确；若使用参考素材，请逐项说明仅参考构图、节奏、动作或风格，并确认授权。先返回内容结构、建议模型、比例、数量和预算，再获得确认后提交任务。
```
