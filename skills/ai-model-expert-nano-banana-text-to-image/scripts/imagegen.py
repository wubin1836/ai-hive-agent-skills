...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana 制作高完成度文生图，主体准确，构图清晰", "keywords": "Nano Banana、NanoBanana、Nano Banana 2、香蕉模型、Gemini 图片、文生图、文字生成图片、文本转图片、text to image、T2I、AI绘画、AI图片、AIGC图片", "model": "public_model_nano_banana_2", "name": "nano-banana-text-to-image", "rule": "none", "search": "固定调用 Nano Banana 的文生图裸接口能力", "title": "Nano Banana 文生图"}')
