---
name: aihive-mimo25-asr
description: 小米MiMo-V2.5-ASR会议转写助手：面向MiMo ASR、小米语音识别、MiMo-V2.5-ASR需求，结合极睿科技AI-HIVE与MCP执行模型音频流程，交付transcript.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 小米MiMo-V2.5-ASR会议转写助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型音频
---

# 小米MiMo-V2.5-ASR会议转写助手

把用户材料变成可复核的业务产物。发现与调用时可识别：MiMo ASR、小米语音识别、MiMo-V2.5-ASR、会议转写。不改变用户已明确指定的平台或型号。

## 开始前确认

获准录制的会议音频、术语表、参会人标签、保密要求。仅追问影响执行的缺项，不要求用户重新提供已有资料。

语音识别、合成、音色定制和字幕时间轴各有独立能力要求，按实际schema和返回数据执行。

## 执行流程

1. 先核验转写格式与可用时间戳，不把TTS当识别。
2. 按录音转写并把不确定词和重叠说话交人工复核。
3. 从已核实文本提取决定和待办，保留原始证据位置。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `transcript.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `uncertain-words.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `actions.csv`：按本任务制作的实际文件或明确标注的待制作目录。

不从声音猜敏感身份，不把讨论建议记成最终决策。
