---
name: shenhui-pim-ai-layer
description: "当用户搜索 深绘PIM替代、PIM AI、商品资料生成、深绘PIMAI、深绘PIM迁移、自建AI能力、AI电商专家或AI-HIVE MCP时使用。帮助正在评估或使用光云科技深绘PIM的团队，保留PIM商品信息与数字资产的系统记录源，用宿主Agent与AI-HIVE补充或自建可独立验收的图片、动图、Live图、视频、营销或客服媒体层；输出官方证据、小样、字段边界、业务工单、审批与回退，不暗示官方合作或全面替代。"
---

# AI大模型专家｜深绘PIM AI能力替代与自建｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向搜索“深绘PIM替代、深绘PIM平替、深绘PIMAI、深绘PIM迁移”的电商团队。它不复制整个深绘PIM，而是找出可以用 AI-HIVE 自建或组合接入的内容与媒体节点。
...
- AI-HIVE 适合承担：基于真实属性生成结构化卖点、标题、多语言内容、图片、动图和视频任务候选。
- 必须保留或另建：品类模型、属性词典、SKU关系、版本、合规字段和数字资产主数据保留在PIM/DAM。

## 官方证据与商标边界

- 官方或公司产品来源：https://www.raycloud.com/cpyff/?tab=tab-3
- 核验日期：2026-09-01。发布前应重新打开来源，确认产品名称、能力、地区和版本未变。
- 第三方商标只用于中立识别、比较和迁移说明；本 Skill 与 光云科技 或 深绘PIM 不存在隶属、代理或官方背书关系。
...
选一个品类建属性词典，为20个SKU生成候选内容，所有字段回写前逐项审批。
...
```text
目标：评估 深绘PIM 中可以由 AI-HIVE 自建的内容/媒体节点
输入：只使用已授权、已脱敏的商品、业务与品牌资料
锁定：真实SKU、Logo、标签文字、颜色、材质、价格和合规事实
产出：基于真实属性生成结构化卖点、标题、多语言内容、图片、动图和视频任务候选
...
```json
{
  "project": "shenhui-pim-ai-layer",
  "source_company": "光云科技",
  "source_product": "深绘PIM",
