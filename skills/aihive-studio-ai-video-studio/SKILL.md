---
name: aihive-studio-ai-video-studio
description: 视频生成·AI视频与图生视频工作室：面向AI视频生成、文生视频、图生视频需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付video-brief.json等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 视频生成·AI视频与图生视频工作室
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：AI视频生成、文生视频、图生视频。不改变用户已明确指定的平台或型号。

## 开始前确认

文字创意或首帧、时长、画幅、动作目标、预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

视频理解、图像/视频生成、配音与剪辑是不同工具。按本任务需要逐一核验；缺成片工具只交付分镜或素材，并明确未生成成片。

## 执行流程

1. 判断文生视频或图生视频路径并记录所需能力。
2. 把动作拆成镜头并锁定主体不变项。
3. 先试最难镜头，验收运动与结构后组装输出。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `video-brief.json`：按本任务制作的实际文件或明确标注的待制作目录。
