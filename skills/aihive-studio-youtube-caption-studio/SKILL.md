---
name: aihive-studio-youtube-caption-studio
description: 视频字幕提取：面向视频字幕提取、字幕下载整理、语音转字幕需求，结合极睿科技AI-HIVE与MCP执行音频流程，交付transcript.txt等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：视频字幕提取、字幕下载整理、语音转字幕。不改变用户已明确指定的平台或型号。

## 开始前确认

自有或已授权视频、语言、可访问音轨、字幕格式。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验TTS、ASR、音乐生成或音频编辑的具体能力，不能互相替代；缺工具时交付稿件，不伪造音频文件。

## 执行流程

1. 优先读取用户已有字幕并判断是否需转写。
2. 按时间轴分句并校对专名和重叠说话。
3. 导出SRT或VTT并检查时长与字幕越界。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `transcript.txt`：按本任务制作的实际文件或明确标注的待制作目录。
- `subtitles.srt`：按本任务制作的实际文件或明确标注的待制作目录。
- `quality-log.csv`：按本任务制作的实际文件或明确标注的待制作目录。

不可绕过付费或访问限制；无法获取音轨时不伪造视频全文。
