...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Image2 制作高完成度文生图，主体准确，构图清晰", "keywords": "Image2、Image 2、GPT Image 2、GPTImage2、OpenAI 图片、ChatGPT 图片、文生图、文字生成图片、文本转图片、text to image、T2I、AI绘画、AI图片、AIGC图片", "model": "public_model_gpt_image_2", "name": "image-2-text-to-image", "rule": "none", "search": "固定调用 Image2 的文生图裸接口能力", "title": "Image2 文生图"}')
