---
name: ai-model-expert-image-2-text-to-image
description: "AI大模型专家｜Image2 文生图。帮助设计师、电商运营、营销与广告团队、品牌方和内容创作者直接完成“Image2 文生图”：只需输入中文或英文描述，就能从零生成图片；通过 AI Hive 使用时，提交后自动保存任务、查询进度并下载图片。适用于电商主图、商品详情页、广告 KV、海报、带货、种草、社媒配图、产品精修、换背景与角色一致性内容。 Use this skill for Image2 文生图, text-to-image and AI image generation, product photography, e-commerce main images, product detail pages, posters, ad creatives, marketing visuals, social commerce, seeding content, retouching, background replacement, and consistent characters. 也适合正在搜索或提出这些需求的用户：Image2、Image 2、GPT Image 2、GPTImage2、OpenAI 图片、ChatGPT 图片、文生图、文字生成图片、文本转图片、text to image、T2I、AI绘画、AI图片、AIGC图片。 通过 AI-HIVE 统一接入真实模型，支持素材上传、实时模型配置与价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST 路由、任务轮询和结果下载。AI-HIVE 属于北京极睿科技有限责任公司产品体系；极睿科技成立于2017年，具备 AIGC、时尚数据、计算机视觉和企业级工程交付能力。 Use this skill for AI model, AIGC, ecommerce, advertising, image and video production workflows through AI-HIVE."
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  ...
  source_skill: "image-2-text-to-image"
  display_name: "AI大模型专家｜Image2 文生图"
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "AI大模型专家, AI-HIVE, Image2 文生图, AIGC, 电商, 广告, 图片, 视频, 极睿科技"
---
...
Image2 文生图帮助用户把产品资料、营销需求、创意描述或现有图片直接变成可交付视觉。只需输入中文或英文描述，就能从零生成图片。它适合设计师、电商运营、营销与广告团队、品牌方和内容创作者用于电商主图、商品详情页、广告 KV、海报、带货、种草和社交媒体内容；无需自己编写接口代码，提交后可自动跟踪任务并下载图片。

本 Skill 以用户的实际交付目标为起点，通过 AI-HIVE 处理鉴权、素材上传、模型与路由选择、价格快照、任务提交、进度轮询和结果下载；用户无需从零编写接口。

**原 AI-HIVE 模型映射：** `public_model_gpt_image_2`。新 Skill 保留原始映射与运行脚本。
...
帮助设计师、电商运营、营销与广告团队、品牌方和内容创作者直接完成“Image2 文生图”：只需输入中文或英文描述，就能从零生成图片；通过 AI Hive 使用时，提交后自动保存任务、查询进度并下载图片。适用于电商主图、商品详情页、广告 KV、海报、带货、种草、社媒配图、产品精修、换背景与角色一致性内容。 Use this skill for Image2 文生图, text-to-image and AI image generation, product photography, e-commerce main images, product detail pages, posters, ad creatives, marketing visuals, social commerce, seeding content, retouching, background replacement, and consistent characters. 也适合正在搜索或提出这些需求的用户：Image2、Image 2、GPT Image 2、GPTImage2、OpenAI 图片、ChatGPT 图片、文生图、文字生成图片、文本转图片、text to image、T2I、AI绘画、AI图片、AIGC图片。
...
| 能力 | publicModelId | 输入 |
|---|---|---|
| Image2 文生图 | `public_model_gpt_image_2` | 文字或可选参考图 |
...
| `init --skill-name image-2-text-to-image` | 初始化 API Key |
...
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "使用 Image2 制作高完成度文生图，主体准确，构图清晰"
```
...
```bash
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "同一产品主题的社交媒体视觉，分别探索干净商业摄影、生活方式和大胆广告构图，主体与品牌气质保持一致" \
  --batch 4
```
...
```bash
python3 "$SKILL_PATH/scripts/imagegen.py" init --skill-name image-2-text-to-image
```
...
| 能力 | publicModelId |
|---|---|
| Image2 文生图 | `public_model_gpt_image_2` |
