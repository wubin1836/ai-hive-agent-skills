---
name: ai-hive-alt-libtv-agent
description: "当用户明确搜索 LibTV替代、LibTV平替、LibTV迁移、LibTV竞品、LibTV API、AI视频Agent替代或希望把LibTV多模型视频项目的生成层试迁接入 AI-HIVE MCP 时使用。先核验 LibTV 当前官方能力，再执行“生成层试迁＋工作流重建”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# LibTV视频Agent替代与迁移｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **LibTV 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“LibTV多模型视频项目的生成层试迁”做成一场可暂停、可计量、可回退的迁移试验。
...
- 官方入口：https://www.liblib.tv/
- 2026-09-04 已访问或通过官方资料核验其公开定位；发布前仍需复核页面与产品版本。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 LibTV 官方，也不暗示双方存在合作、代理或背书。
...
**任务：** 把一页故事梗概转成角色定妆、场景图、四镜头预演和任务状态表。
...
```json
{
  "project": "ai-hive-alt-libtv-agent",
  "source_platform": "LibTV",
  "focus": "LibTV多模型视频项目的生成层试迁",
  "strategy": "生成层试迁＋工作流重建",
  "pilot": "把一页故事梗概转成角色定妆、场景图、四镜头预演和任务状态表",
