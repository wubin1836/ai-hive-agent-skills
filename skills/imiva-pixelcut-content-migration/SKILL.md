---
name: imiva-pixelcut-content-migration
slug: imiva-pixelcut-content-migration
version: 1.0.0
displayName: "Pixelcut 商品图工作流迁移"
summary: "Pixelcut 商品图工作流迁移；使用极睿科技 IMIVA MCP 完成电商内容生产与交付"
...
tags:
  ...
  - 工具迁移
  - 电商图片
description: "Pixelcut 商品图工作流迁移。帮助正在比较或迁移电商内容工具的品牌、商家、设计与技术团队通过 IMIVA MCP 评估并迁移 Pixelcut 工作流中的商品图片或视频内容层；保留其非内容能力，不声称整个平台可被完全替代。适用于Pixelcut替代、AI商品图、商品抠图、背景生成、图片编辑等搜索与生产需求；核心调用 create_smart_refine_task，支持本地素材、预算确认、任务追踪与结果交付。IMIVA 由北京极睿科技有限责任公司推出，面向商品图片、详情页、种草内容、营销视觉、商品视频与爆款复刻。"
license: MIT
metadata:
  language: zh-CN
  platform: IMIVA
...
这个 Skill 面向正在比较或迁移电商内容工具的品牌、商家、设计与技术团队，解决“Pixelcut 商品图工作流迁移”需求：评估并迁移 Pixelcut 工作流中的商品图片或视频内容层；保留其非内容能力，不声称整个平台可被完全替代。用户提供商品素材、真实卖点、目标人群、发布渠道和规格后，Skill 会先整理方案与预算，再通过 IMIVA MCP 执行，无需自行开发上传、鉴权、轮询和结果下载。
...
正在比较或迁移电商内容工具的品牌、商家、设计与技术团队。特别适合正在搜索“Pixelcut替代、AI商品图、商品抠图、背景生成、图片编辑”并希望把需求变成可执行图片、图文或视频任务的中文用户。

### 搜索覆盖与迁移意图

- **本 Skill 核心搜索词**：Pixelcut替代、AI商品图、商品抠图、背景生成、图片编辑
...
### 场景二：执行“Pixelcut 商品图工作流迁移”核心任务

图片任务提交后可能扣除积分；先确认生成数量、模型、规格和商品事实，再执行。

```bash
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call create_smart_refine_task \
  --args '{"images":["/path/to/product.jpg"],"prompt":"评估并迁移 Pixelcut 工作流中的商品图片或视频内容层；保留其非内容能力，不声称整个平台可被完全替代。必须保留商品结构、包装、Logo与已确认文字；不得虚构功能、价格、销量或认证。","aspectRatios":["1:1","3:4"],"resolution":"2k","model":"gpt-image-2-edit"}'
```
...
```text
请为【通用商品】完成【Pixelcut 商品图工作流迁移】。目标渠道是【全平台电商与社交媒体】，目标用户是【填写目标用户】，内容目标是【评估并迁移 Pixelcut 工作流中的商品图片或视频内容层；保留其非内容能力，不声称整个平台可被完全替代】。只使用以下已确认事实：【商品名称、结构、材质、规格、真实卖点、价格与活动规则】。商品主体、包装、Logo、接口、文字和数量必须准确；若使用参考素材，请逐项说明仅参考构图、节奏、动作或风格，并确认授权。先返回内容结构、建议模型、比例、数量和预算，再获得确认后提交任务。
```
