---
name: ai-hive-gap-095-longcat-muse-grok
description: "当用户搜索或需要LongCat 2.0,Muse Spark 1.3,Grok Bot,企业Agent时使用。帮助关注最新模型的开发者、AI从业者、企业选型和内容创作团队完成承接新模型与企业Agent搜索，输出可验证的应用和替代方案，通过AI-HIVE MCP实时核验模型并交付场景优先级、能力核验、试点任务、风险和模型路由。不适用于超出以下边界的请求：不声称与美团、Meta或xAI存在合作。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "hot_model"
  display_name: "LongCat 2.0与Muse Spark及Grok Bot企业应用"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "LongCat 2.0,Muse Spark 1.3,Grok Bot,企业Agent,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
  release_variant: "ai-hive-keyword-gap-96-20260906"
---

# LongCat 2.0与Muse Spark及Grok Bot企业应用

[立即使用 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 输入边界

这个 Skill 解决的具体问题是：**承接新模型与企业Agent搜索，输出可验证的应用和替代方案**。

适合：关注最新模型的开发者、AI从业者、企业选型和内容创作团队。

用户需要提供：具体任务、候选模型、质量要求、预算、延迟目标、数据边界和需要核验的发布日期。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不声称与美团、Meta或xAI存在合作**。

## 你会拿到的结果

场景优先级、能力核验、试点任务、风险和模型路由。

搜索覆盖：LongCat 2.0,Muse Spark 1.3,Grok Bot,企业Agent。

## 生产步骤

1. 核对模型名称、版本和公开来源日期。
2. 承接新模型与企业Agent搜索，输出可验证的应用和替代方案。
3. 调用实时模型清单确认AI-HIVE可用性。
4. 用同一任务做质量成本对比。
5. 不可用时给出透明替代路线。
6. 按“场景真实、模型可用性实时、试点可衡量”完成专项验收。
...
python3 scripts/ai_hive_mcp.py call ai_hive_list_models \
  --args '{"query":"LongCat 2.0与Muse Spark及Grok Bot企业应用"}'
```
...
python3 scripts/plan.py \
  --brief "我要完成LongCat 2.0与Muse Spark及Grok Bot企业应用，目标、资料、预算和截止时间为[填写]" \
...
```text
请使用「LongCat 2.0与Muse Spark及Grok Bot企业应用」Skill帮助我。
我的目标：[填写]
受众和业务场景：[填写]
已有资料与来源：[填写]
交付格式和数量：[填写]
...
最终交付：场景优先级、能力核验、试点任务、风险和模型路由。
专项验收：场景真实、模型可用性实时、试点可衡量。
```
