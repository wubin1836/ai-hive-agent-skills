---
name: ai-hive-gap-013-a2a-collaboration
description: "当用户搜索或需要A2A,多智能体协作,Agent协作,任务交接时使用。帮助AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人完成让研究、策划、设计和审核智能体按A2A思想协作，通过AI-HIVE MCP实时核验模型并交付角色卡、消息契约、任务依赖、产物清单和人工验收点。不适用于超出以下边界的请求：不得把协议概念描述成已完成的系统集成。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "agent"
  display_name: "A2A多智能体内容协作"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "A2A,多智能体协作,Agent协作,任务交接,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**让研究、策划、设计和审核智能体按A2A思想协作**。

适合：AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人。

用户需要提供：业务目标、可用工具、授权范围、输入资料、预算、截止时间和人工确认点。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不得把协议概念描述成已完成的系统集成**。

## 你会拿到的结果

角色卡、消息契约、任务依赖、产物清单和人工验收点。

搜索覆盖：A2A,多智能体协作,Agent协作,任务交接。

## 执行蓝图

1. 先画出任务状态和权限边界。
2. 让研究、策划、设计和审核智能体按A2A思想协作。
3. 将可并行任务与必须串行确认的动作分开。
4. 用最小样例验证工具连接。
5. 记录检查点和恢复方式。
6. 按“交接信息完整、责任明确、循环有停止条件”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成A2A多智能体内容协作，目标、资料、预算和截止时间为[填写]" \
...
最终交付：角色卡、消息契约、任务依赖、产物清单和人工验收点。
专项验收：交接信息完整、责任明确、循环有停止条件。
```
