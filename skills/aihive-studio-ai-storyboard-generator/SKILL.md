---
name: aihive-studio-ai-storyboard-generator
description: 分镜生成器：面向分镜生成器、故事板、AI分镜需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付storyboard.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：分镜生成器、故事板、AI分镜。不改变用户已明确指定的平台或型号。

## 开始前确认

剧本、角色设定、地点、时长与制作预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

视频理解、图像/视频生成、配音与剪辑是不同工具。按本任务需要逐一核验；缺成片工具只交付分镜或素材，并明确未生成成片。

## 执行流程

1. 把剧情拆为动作可拍的场景并给镜头编号。
2. 规划景别、运动、轴线和台词时间。
3. 按实际图像能力生成分镜或先交付镜头表。
...
- `boards/`：按本任务制作的实际文件或明确标注的待制作目录。

镜头时长合计吻合，角色服装和空间关系不跳变。
