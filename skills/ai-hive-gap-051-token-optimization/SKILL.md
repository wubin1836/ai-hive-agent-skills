---
name: ai-hive-gap-051-token-optimization
description: "当用户搜索或需要Token优化,Token成本,大模型降本,上下文优化时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成分析上下文和请求结构以减少无效Token消耗，通过AI-HIVE MCP实时核验模型并交付Token预算、冗余清单、压缩方案、缓存建议和测试表。不适用于超出以下边界的请求：不得用删除关键约束换取低成本。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "Token优化与成本控制"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "Token优化,Token成本,大模型降本,上下文优化,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**分析上下文和请求结构以减少无效Token消耗**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不得用删除关键约束换取低成本**。

## 你会拿到的结果

Token预算、冗余清单、压缩方案、缓存建议和测试表。

搜索覆盖：Token优化,Token成本,大模型降本,上下文优化。

## 实施流程

1. 先定义可量化的质量和成本指标。
2. 分析上下文和请求结构以减少无效Token消耗。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“成本下降且任务质量不退化”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成Token优化与成本控制，目标、资料、预算和截止时间为[填写]" \
...
最终交付：Token预算、冗余清单、压缩方案、缓存建议和测试表。
专项验收：成本下降且任务质量不退化。
```
