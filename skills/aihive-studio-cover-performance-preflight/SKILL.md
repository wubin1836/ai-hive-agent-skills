---
name: aihive-studio-cover-performance-preflight
description: 封面优化预检：面向封面优化、封面检测、封面点击率分析需求，结合极睿科技AI-HIVE与MCP执行审查流程，交付cover-audit.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：封面优化、封面检测、封面点击率分析。不改变用户已明确指定的平台或型号。

## 开始前确认

封面文件、标题、展示位、历史点击曝光数据。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 检查缩略图信息层级和裁切安全区。
2. 把画面清晰度、标题承诺和内容一致性分别打分。
3. 设计单变量AB测试并计算可比样本指标。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `cover-audit.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `ab-plan.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `metrics.csv`：按本任务制作的实际文件或明确标注的待制作目录。

不能用设计评分冒充真实CTR；没有曝光分母不作点击率比较。
