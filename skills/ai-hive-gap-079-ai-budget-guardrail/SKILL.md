---
name: ai-hive-gap-079-ai-budget-guardrail
description: "当用户搜索或需要预算护栏,AI成本治理,企业AI预算,模型费用时使用。帮助企业安全、法务合规、AI治理、内容审核和平台工程团队完成为团队和任务设置预算、并发、模型等级和异常停止规则，通过AI-HIVE MCP实时核验模型并交付预算表、额度策略、预警阈值、停止条件和复盘记录。不适用于超出以下边界的请求：不得静默超预算或擅自降级质量。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "security"
  display_name: "企业AI预算护栏"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "预算护栏,AI成本治理,企业AI预算,模型费用,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**为团队和任务设置预算、并发、模型等级和异常停止规则**。

适合：企业安全、法务合规、AI治理、内容审核和平台工程团队。

用户需要提供：待审计文本或流程、数据分类、工具权限、风险级别、适用制度和人工负责人。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不得静默超预算或擅自降级质量**。

## 你会拿到的结果

预算表、额度策略、预警阈值、停止条件和复盘记录。

搜索覆盖：预算护栏,AI成本治理,企业AI预算,模型费用。

## 生产步骤

1. 识别资产、数据和工具权限。
2. 为团队和任务设置预算、并发、模型等级和异常停止规则。
3. 建立风险情景与测试样例。
4. 提出最小权限和确认门。
5. 保留审计证据与人工复核。
6. 按“超额可阻断、费用可归属、异常可解释”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成企业AI预算护栏，目标、资料、预算和截止时间为[填写]" \
...
最终交付：预算表、额度策略、预警阈值、停止条件和复盘记录。
专项验收：超额可阻断、费用可归属、异常可解释。
```
