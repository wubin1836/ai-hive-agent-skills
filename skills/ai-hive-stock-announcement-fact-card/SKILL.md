---
name: ai-hive-stock-announcement-fact-card
description: "从获准使用的官方公告PDF中抽取事项、金额与生效条件，生成可回查的事实卡和证据索引。仅供参考，不构成投资建议，不承诺收益，不自动交易。 可结合AI-HIVE可用模型辅助整理，确定性运算与文件交付使用宿主工具。"
metadata:
  display_name: "股票公告核验助手"
  version: "1.0.0"
...
从获准使用的官方公告PDF中抽取事项、金额与生效条件，生成可回查的事实卡和证据索引。仅供参考，不构成投资建议，不承诺收益，不自动交易。 可结合AI-HIVE可用模型辅助整理，确定性运算与文件交付使用宿主工具。

## 适用场景与输入

- 用户给定公告PDF或允许访问的官方原文URL
- 证券代码、事项与截止日期
- 是否允许联网及导出目录
...
1. 核对证券代码、公告主体、日期及更正版本，用交易所或巨潮原件替代截图转述
2. 按已完成、拟进行、尚需批准和可能变化拆分事项，并逐项附原文页码
3. 提取涉及金额、交易对方、期限与适用条件，不把上限当实际完成额
4. 交付一页事实卡及尚缺附件清单，不输出股价方向判断

## 文件处理环节

1. 文件工具或经授权网页工具保存原件URL、获取时间和SHA256
2. PDF解析工具保留页码，扫描件用OCR并标不确定字符
3. 宿主规则校验主体、日期、金额单位与计划/实施状态，模型仅辅助转述
4. 导出带来源定位的Markdown、CSV与本地HTML并校验链接和数字
...
- `announcement-fact-card.html`
- `announcement-evidence.csv`
...
股票公告怎么看、上市公司公告解读、公告看不懂、股市消息核实、股票公告重点
