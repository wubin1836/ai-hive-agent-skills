---
name: ad-first-frame-animation-ai-hive
description: "Use this skill when the user asks for 首帧动态化、3秒钩子、图片转视频、信息流广告、广告开场，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成广告首帧动态化。它服务信息流投手、短视频编导和拥有静态海报的商家：接收广告首图或海报、商品事实、受众、平台和需要保留的文字，交付首帧停留诊断、动态动作方案、图生视频提示词和多个3秒开场。特色是只动画化最能解释卖点的一个视觉动作；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 广告首帧动态化｜AI-HIVE

[广告首帧动态化进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向信息流投手、短视频编导和拥有静态海报的商家，把“广告首帧动态化”从模糊想法变成首帧停留诊断、动态动作方案、图生视频提示词和多个3秒开场。用户提供广告首图或海报、商品事实、受众、平台和需要保留的文字即可开始。核心方法是：只动画化最能解释卖点的一个视觉动作。
...
动态效果不能扭曲商品、Logo或文字；平台首屏安全区和AI标识需按当前规则检查。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "广告首帧动态化" \
  --audience "信息流投手、短视频编导和拥有静态海报的商家" \
  --goal "首帧停留诊断、动态动作方案、图生视频提示词和多个3秒开场" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name ad-first-frame-animation-ai-hive
