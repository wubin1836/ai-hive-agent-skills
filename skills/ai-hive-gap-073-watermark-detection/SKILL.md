---
name: ai-hive-gap-073-watermark-detection
description: "当用户搜索或需要水印检测,AI水印,图片检测,视频检测时使用。帮助企业安全、法务合规、AI治理、内容审核和平台工程团队完成在发布前检查明显水印、平台标记和来源风险，通过AI-HIVE MCP实时核验模型并交付检测清单、问题位置、处理建议、原始文件和复核记录。不适用于超出以下边界的请求：不得移除用于保护第三方版权的水印。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "security"
  display_name: "AI图片视频水印检测"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "水印检测,AI水印,图片检测,视频检测,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**在发布前检查明显水印、平台标记和来源风险**。

适合：企业安全、法务合规、AI治理、内容审核和平台工程团队。

用户需要提供：待审计文本或流程、数据分类、工具权限、风险级别、适用制度和人工负责人。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不得移除用于保护第三方版权的水印**。

## 你会拿到的结果

检测清单、问题位置、处理建议、原始文件和复核记录。

搜索覆盖：水印检测,AI水印,图片检测,视频检测。

## 交付路径

1. 识别资产、数据和工具权限。
2. 在发布前检查明显水印、平台标记和来源风险。
3. 建立风险情景与测试样例。
4. 提出最小权限和确认门。
5. 保留审计证据与人工复核。
6. 按“水印位置明确、处理不损害权利声明”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成AI图片视频水印检测，目标、资料、预算和截止时间为[填写]" \
...
最终交付：检测清单、问题位置、处理建议、原始文件和复核记录。
专项验收：水印位置明确、处理不损害权利声明。
```
