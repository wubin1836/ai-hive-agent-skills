---
name: ai-hive-meihua-rule-replay
description: "梅花易数助手｜取数记录复算：读取用户已记录的取数过程、原数与中间结果、用户合法持有并确认的规则页、版本和示例答案，OCR 取数笔记并抽成原数、操作、结果、来源四列表，人工确认识别疑点，交付可回查的文件及待核实清单。适用于梅花易数、梅花易數、梅易、梅花起卦记录；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。"
metadata:
  display_name: "梅花易数助手｜取数记录复算"
  version: "1.0.0"
...
梅花易数助手｜取数记录复算：读取用户已记录的取数过程、原数与中间结果、用户合法持有并确认的规则页、版本和示例答案，OCR 取数笔记并抽成原数、操作、结果、来源四列表，人工确认识别疑点，交付可回查的文件及待核实清单。适用于梅花易数、梅花易數、梅易、梅花起卦记录；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。

## 适用场景与输入

- 用户已记录的取数过程、原数与中间结果
- 用户合法持有并确认的规则页、版本和示例答案
- 已有核验卦象；无可靠规则时只做记录对照
...
1. OCR 取数笔记并抽成原数、操作、结果、来源四列表，人工确认识别疑点
2. 表格公式按给定规则复算算术过程并跑用户确认示例；不一致或未通过则停算并标错
3. 生成可复算工作簿与规则版本差异报告，保留原数不以结果倒推或改写输入
