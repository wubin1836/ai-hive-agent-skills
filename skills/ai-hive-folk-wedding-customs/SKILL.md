---
name: ai-hive-folk-wedding-customs
description: "两地婚俗协调助手，用于南北婚俗差异、两地结婚习俗、接亲流程怎么定。读取你授权的原始资料，完成双方习俗—自愿程度—成本—冲突对照表，并交付协商版流程单与礼貌沟通文稿。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供传统文化与娱乐参考，尊重科学；相关解读不是科学预测，不作为投资、医疗、法律或婚姻等重大决策依据。"
metadata:
  display_name: "两地婚俗协调助手"
  version: "1.0.0"
...
两地婚俗协调助手，用于南北婚俗差异、两地结婚习俗、接亲流程怎么定。读取你授权的原始资料，完成双方习俗—自愿程度—成本—冲突对照表，并交付协商版流程单与礼貌沟通文稿。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供传统文化与娱乐参考，尊重科学；相关解读不是科学预测，不作为投资、医疗、法律或婚姻等重大决策依据。
...
1. 分别记录双方地区、个人意愿和实际预算
2. 以地方非遗资料核对特定习俗，不外推全国
3. 将称谓、环节和物品转成可协商选项
4. 确认双方同意后输出流程草稿，不代发通知

## 文件处理环节

1. 检索地方非遗项目并标适用地域
2. 工作表合并选项，检查人员时间物品冲突
3. 生成版本化流程与协商稿并核对待确认项
...
- `wedding-customs-agreement.xlsx`
- `ceremony-runbook.docx`
...
尊重婚姻自主，不设计羞辱性婚闹或性别歧视；不因属相八字否定关系，不作财产法律结论。

缺关键原件或专业依据时只完成可验证部分。401/403、限流或内容审核阻断时保存脱敏进度并停止相应调用，遵守明确的等待要求，不循环重试或绕过审核。不因安装本技能自动交易、签署、提交、缴费或公开发布。

## 调用示例

> 请使用我授权提供的资料完成“两地婚俗协调助手”。范围先按我的原件与指定版本确认，生成wedding-customs-agreement.xlsx及其余约定文件，疑点附来源，不自动对外提交。

## 搜索与任务识别

南北婚俗差异、两地结婚习俗、接亲流程怎么定、敬茶说什么、婚礼礼节清单、婚俗协商
