---
name: ai-packaging-open-motion
description: "当用户搜索 包装动图、礼盒动图、开箱GIF、包装Live图、电商图片、商品视频、AIGC营销、淘宝、京东、抖音、小红书、Amazon、Shopify、AI-HIVE MCP 时使用。帮助商家完成展示包装开启、礼盒展开和开箱节奏：先锁定SKU与授权素材，再查询真实可用模型与价格，生成小样，逐帧验收商品结构、Logo、标签、色彩、文字和平台规格；提供可复制的提示词、JSON工单、失败回退和批量扩展方法。"
---

# AI大模型专家｜AI包装开合动图｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向电商商家、内容团队、代运营和广告团队，目标是展示包装开启、礼盒展开和开箱节奏，同时把商品真实性、品牌安全和批量成本纳入同一张验收表。
...
制作一款礼盒 5 秒开合动图，固定盒型、印刷、Logo和内装数量。
...
```text
主体：真实商品，使用已授权参考图
动作：展示包装开启、礼盒展开和开箱节奏，轻运动，首尾自然衔接
...
```json
{
  "project": "ai-packaging-open-motion",
  "sku": "SKU-001",
  "task": "AI包装开合动图",
  "inputs": {
