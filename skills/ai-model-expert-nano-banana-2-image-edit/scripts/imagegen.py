...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana 2 制作高完成度图片编辑，主体准确，构图清晰", "keywords": "Nano Banana 2、NanoBanana2、Nano Banana、香蕉2、Gemini 图片、图片编辑、AI修图、改图、P图、局部修改、图片重绘、照片编辑、AI图片、AIGC图片", "model": "public_model_nano_banana_2", "name": "nano-banana-2-image-edit", "rule": "image", "search": "固定调用 Nano Banana 2 的图片编辑裸接口能力", "title": "Nano Banana 2 图片编辑"}')
