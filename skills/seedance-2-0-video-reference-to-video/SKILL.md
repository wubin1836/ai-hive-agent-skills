---
name: seedance-2-0-video-reference-to-video
description: "Use this skill when creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers need to reuse motion, pacing, camera movement, or structure from a reference video with Seedance 2.0 through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for ads, TVCs, product videos, e-commerce listings, social commerce, UGC-style seeding content, short dramas, motion comics, and storyboards. Search intents include video-to-video, motion reference, camera reference, reference video generation."
license: MIT
---
...
Seedance 2.0 Video Reference to Video helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers reuse motion, pacing, camera movement, or structure from a reference video with Seedance 2.0. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
Ads, tvcs, product videos, e-commerce listings, social commerce, ugc-style seeding content, short dramas, motion comics, and storyboards. It is also useful when users search for video-to-video, motion reference, camera reference, reference video generation.

## Capabilities

| Mode | Capability | publicModelId |
|---|---|---|
| `r2v` | Reference to Video | `public_model_seedance_r2v` |
...
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode r2v \
  --prompt "Seedance 2.0 Video Reference to Video: polished motion, clear subject, coherent action, stable camera, commercial finish" \
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode r2v \
  --prompt "Create a polished seedance 2.0 video reference to video result with coherent motion, stable identity, and a clear ending" \
+  --video /path/to/reference.mp4
```
...
```bash
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode r2v --prompt "Complex production brief" --no-download \
+  --video /path/to/reference.mp4
