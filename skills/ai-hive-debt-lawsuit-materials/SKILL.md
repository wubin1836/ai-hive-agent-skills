---
name: ai-hive-debt-lawsuit-materials
description: "欠款起诉材料助手，用于欠钱不还、借钱不还、欠款起诉。读取你授权的原始资料，完成借款交付与还款勾稽表，并交付证据与待补信息目录。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。"
metadata:
  display_name: "欠款起诉材料助手"
  version: "1.0.0"
...
欠款起诉材料助手，用于欠钱不还、借钱不还、欠款起诉。读取你授权的原始资料，完成借款交付与还款勾稽表，并交付证据与待补信息目录。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。

## 适用场景与输入

- 借条或借款协议扫描件
- 银行或支付平台脱敏转账及还款CSV/PDF
- 有关借款和还款的原始沟通记录
...
- 宿主获授权读取 PDF/CSV；扫描件需要可用 OCR 并支持原页定位
- 可复算 XLSX 生成及读取校验；DOCX 生成和渲染检查
- 若需当期诉状模板，须有经授权的官方只读查询或用户提供已核实模板

没有专门软件并不意味着中止所有任务；可使用宿主已具备的等效工具。无法完成的步骤明确列出，不能伪装成功或默默改为另一个外部服务。

## 工作流

1. 确认是借款还是货款等其他交易，核对当事人、借条原文及现有处理阶段。
2. 关联每笔资金交付与归还，分别记录本金、约定费用和存在争议的款项。
3. 按当前适用的官方示范文本组织诉求、事实和证据出处，不将用户猜测写成事实。
4. 检查被告信息、请求金额、管辖和日期资料是否齐全，形成待专业核验清单。
...
- `loan-transactions.xlsx`
- `loan-complaint-draft.docx`
- `loan-evidence-gaps.xlsx`
...
不保证借贷关系成立、利息可获支持或胜诉；不得补造借条、代查私人住址或暴力催收。管辖与期限应及时向法院或律师核实，不自动立案。

缺关键原件或专业依据时只完成可验证部分。401/403、限流或内容审核阻断时保存脱敏进度并停止相应调用，遵守明确的等待要求，不循环重试或绕过审核。不因安装本技能自动交易、签署、提交、缴费或公开发布。

## 调用示例

> 仅示例（下列文件名和资料均为假设，不是真实数据）：请读取假设文件借条.pdf、借款流水.csv和还款聊天.pdf，去重并对应每批借款与还款，生成 loan-transactions.xlsx、loan-complaint-draft.docx 和待补证据表，只做本地草稿。

## 搜索与任务识别

欠钱不还、借钱不还、欠款起诉、借条、起诉状、还款记录
