---
name: ad-hook-variant-generator-ai-hive
description: "Use this skill when the user asks for 广告钩子、黄金3秒、开场变体、短视频广告、素材测试，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成广告钩子批量变体。它服务短视频投手、编导和需要持续测试开场的团队：接收商品事实、受众痛点、平台、历史钩子和禁用表达，交付问题、反差、结果、场景、视觉冲突等钩子矩阵与视频小样。特色是核心卖点不变，只替换首屏进入方式；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 广告钩子批量变体｜AI-HIVE

[广告钩子批量变体进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向短视频投手、编导和需要持续测试开场的团队，把“广告钩子批量变体”从模糊想法变成问题、反差、结果、场景、视觉冲突等钩子矩阵与视频小样。用户提供商品事实、受众痛点、平台、历史钩子和禁用表达即可开始。核心方法是：核心卖点不变，只替换首屏进入方式。
...
不得使用虚假倒计时、伪造新闻、恐吓、夸大功效或无法兑现的结果型钩子。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "广告钩子批量变体" \
  --audience "短视频投手、编导和需要持续测试开场的团队" \
  --goal "问题、反差、结果、场景、视觉冲突等钩子矩阵与视频小样" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name ad-hook-variant-generator-ai-hive
