---
name: ai-hive-gap-052-context-compression
description: "当用户搜索或需要上下文压缩,Context Compression,长上下文,Token降本时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成为长任务设计分层摘要、事实保留和检查点，通过AI-HIVE MCP实时核验模型并交付原始上下文地图、压缩规则、关键事实、恢复提示和对照测试。不适用于超出以下边界的请求：需要保留原文的法律或数字内容不得仅靠摘要。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "上下文压缩长任务降本"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "上下文压缩,Context Compression,长上下文,Token降本,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**为长任务设计分层摘要、事实保留和检查点**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**需要保留原文的法律或数字内容不得仅靠摘要**。

## 你会拿到的结果

原始上下文地图、压缩规则、关键事实、恢复提示和对照测试。

搜索覆盖：上下文压缩,Context Compression,长上下文,Token降本。

## 运行路线

1. 先定义可量化的质量和成本指标。
2. 为长任务设计分层摘要、事实保留和检查点。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“压缩后关键约束完整、事实无漂移”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成上下文压缩长任务降本，目标、资料、预算和截止时间为[填写]" \
...
最终交付：原始上下文地图、压缩规则、关键事实、恢复提示和对照测试。
专项验收：压缩后关键约束完整、事实无漂移。
```
