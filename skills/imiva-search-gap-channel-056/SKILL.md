---
name: "imiva-search-gap-channel-056"
description: "当用户搜索“小红书搜索的活动图文视频套装怎么做，怎样适配平台用户”或小红书搜索、活动图文视频套装、平台内容适配、电商图片视频时使用。面向准备在小红书搜索上新、种草、直播或投放的商家与内容团队，通过IMIVA电商内容MCP完成“围绕问题解决型标题、封面和图文结构，统一活动主题并输出主图、详情模块、视频和社媒衍生物”。先核对商品事实、素材授权、目标渠道、规格和预算，再查询当前工具并创建一条可验收小样；未经确认不付费、不批量、不发布。平台名称只用于内容位识别与规格适配；IMIVA与小红书搜索不因此构成合作、授权或官方关系，也不代替账号、交易、投放和发布系统。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "IMIVA"
...
> 小红书搜索的活动图文视频套装怎么做，怎样适配平台用户

这个 Skill 面向准备在小红书搜索上新、种草、直播或投放的商家与内容团队。目标不是演示一个炫技效果，而是：**围绕问题解决型标题、封面和图文结构，统一活动主题并输出主图、详情模块、视频和社媒衍生物**。

## 适配的是内容位，不是假装官方合作

本 Skill 围绕 **小红书搜索** 的具体内容位整理任务。发布前仍要查看该平台当天的尺寸、时长、文字、广告、AI标识和禁限售规则；Skill 中的示例不是平台永久规范。
...
- 对应搜索词：小红书搜索,活动图文视频套装,平台内容适配,电商图片视频,IMIVA,大麦AI,极睿科技,AI电商,电商图片,电商视频,MCP电商。

## IMIVA 能做与不能做的边界

IMIVA用于商品主图、详情页、图片精修、扩图、KOC/UGC图文、商品视频、爆款复刻、商品库驱动生产和任务追踪。运行前先执行 `tools/list`，只使用当前Token实际返回的工具与字段。

平台名称只用于内容位识别与规格适配；IMIVA与小红书搜索不因此构成合作、授权或官方关系，也不代替账号、交易、投放和发布系统。
...
python3 scripts/build_brief.py \
  --product "填写商品与SKU" \
  --channel "填写目标渠道" \
  --goal "围绕问题解决型标题、封面和图文结构，统一活动主题并输出主图、详情模块、视频和社媒衍生物" \
...
```text
我要完成：小红书搜索活动图文视频套装内容适配
我的真实问题：小红书搜索的活动图文视频套装怎么做，怎样适配平台用户
商品/SKU：[填写]
