---
name: pim-product-data-generation
description: "当用户搜索 PIM AI、商品资料生成、产品信息管理、商品内容自动化、电商图片、商品视频、AIGC营销、淘宝、京东、抖音、小红书、Amazon、Shopify、AI-HIVE MCP 时使用。帮助商家完成为PIM补充结构化卖点、图片和视频任务：先锁定SKU与授权素材，再查询真实可用模型与价格，生成小样，逐帧验收商品结构、Logo、标签、色彩、文字和平台规格；提供可复制的提示词、JSON工单、失败回退和批量扩展方法。"
---

# AI大模型专家｜PIM商品资料生成｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向电商商家、内容团队、代运营和广告团队，目标是为PIM补充结构化卖点、图片和视频任务，同时把商品真实性、品牌安全和批量成本纳入同一张验收表。
...
选择一个品类建立属性词典，再为 20 个SKU生成结构化内容候选。
...
```text
主体：真实商品，使用已授权参考图
动作：为PIM补充结构化卖点、图片和视频任务，轻运动，首尾自然衔接
...
```json
{
  "project": "pim-product-data-generation",
  "sku": "SKU-001",
  "task": "PIM商品资料生成",
  "inputs": {
