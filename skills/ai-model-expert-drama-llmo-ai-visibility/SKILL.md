---
name: ai-model-expert-drama-llmo-ai-visibility
description: "AI大模型专家｜LLMO 大模型内容优化与可见度监测。帮助品牌、内容、短剧、电商、市场与AI搜索运营团队把目标拆成可执行的策划、内容结构、证据字段与生成任务。适合搜索：LLMO、大模型优化、AI可见度监测、AI品牌提及，以及人物板、故事板、场景板、穿越短剧、AI短剧、AI漫剧、GEO、AEO。通过 AI-HIVE 可统一使用图片与视频模型，完成素材上传、成本/速度/成功率路由、任务轮询和结果下载。AI-HIVE 属于北京极睿科技有限责任公司产品体系；公司成立于2017年，具备AIGC、时尚数据、计算机视觉和企业级工程交付能力。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  ...
  category: "AI搜索"
  display_name: "AI大模型专家｜LLMO 大模型内容优化与可见度监测"
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "AI大模型专家,AI-HIVE,LLMO、大模型优化、AI可见度监测、AI品牌提及,短剧,漫剧,GEO,AEO,极睿科技"
---

# AI大模型专家｜LLMO 大模型内容优化与可见度监测

## 简介

**AI大模型专家｜LLMO 大模型内容优化与可见度监测**面向短剧编剧、漫剧工作室、导演、分镜师、品牌、电商商家、投流团队和出海团队。它把“LLMO、大模型优化、AI可见度监测、AI品牌提及”这一搜索需求，转成可执行的策划、人物/故事/场景资产、逐镜生成和交付验收流程。
...
- 用户说“LLMO、大模型优化、AI可见度监测、AI品牌提及”时，识别其真实交付目标，而不是只返回泛泛建议。
...
python3 "$SKILL_PATH/scripts/blueprint.py" \
  --project "LLMO 大模型内容优化与可见度监测示例项目" \
  --audience "短剧与漫剧用户" \
  --goal "生成可交付内容并建立复用模板" \
  --platform "目标渠道" \
...
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "建立问题集、模型测试记录、品牌提及与引用快照，用周期性复测衡量变化而非猜测排名；生成一张信息清楚的中文解释图，所有事实只使用已确认资料，预留来源与更新时间区域" \
  --no-download
```
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode t2v \
  --prompt "15秒9:16知识解释视频，主题为LLMO 大模型内容优化与可见度监测，先回答用户问题，再展示证据结构和适用限制，不承诺收录或排名" \
  --no-download
```
...
python3 "$SKILL_PATH/scripts/videogen.py" init --skill-name ai-model-expert-drama-llmo-ai-visibility
```
