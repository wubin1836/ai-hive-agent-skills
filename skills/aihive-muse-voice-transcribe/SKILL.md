---
name: aihive-muse-voice-transcribe
description: Muse Voice录音转写与字幕助手：面向Muse Voice、Muse Transcribe、Meta语音转文字需求，结合极睿科技AI-HIVE与MCP执行模型音频流程，交付transcript.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Muse Voice录音转写与字幕助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型音频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：Muse Voice、Muse Transcribe、Meta语音转文字。不改变用户已明确指定的平台或型号。

## 开始前确认

授权录音、说话人标记、术语表、输出字幕格式。仅追问影响执行的缺项，不要求用户重新提供已有资料。

语音识别、合成、音色定制和字幕时间轴各有独立能力要求，按实际schema和返回数据执行。

## 执行流程

1. 先核验转写接口的文件格式、时间戳粒度与说话人能力。
2. 按实际返回标注转写及不确定词，不凭空制造逐词时间戳。
3. 校对专名后导出文字和可验证粒度字幕。
...
- `turns.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `subtitles.srt`：按本任务制作的实际文件或明确标注的待制作目录。
