# 知识付费助手：具体任务示例

这些是可交给宿主 Agent 的任务请求，不是伪造的 AI-HIVE API。所有示例均需按当前用户范围执行；媒体生成需要另行明确数量和预算。

## 示例 1：从经验到小课

```text
我做招聘5年，想做面试官训练产品，每周只能投入6小时，先不要卖。
```

期望交付：比较录播课和两次线上工作坊的交付成本，形成最小课程、面试评分练习和5人试学反馈计划，不预测保证收入。

结构化任务参考（给 Agent 读取，不直接 POST 到 MCP）：

```json
{
  "skill": "ai-hive-assistant-knowledge-product",
  "request": "我做招聘5年，想做面试官训练产品，每周只能投入6小时，先不要卖。",
  "external_actions": "未授权，不执行发布/投递/群发",
  "media_budget": "未授权，先给方案或询问预算"
}
```

## 示例 2：资料包验证

```text
我有40张自制门店排班表，想做付费模板包，帮我判断还缺什么。
```

期望交付：梳理使用人群、表格说明、示例数据、版本兼容和使用边界，交付样章与用户测试任务，不编造购买需求。

结构化任务参考（给 Agent 读取，不直接 POST 到 MCP）：

```json
{
  "skill": "ai-hive-assistant-knowledge-product",
  "request": "我有40张自制门店排班表，想做付费模板包，帮我判断还缺什么。",
  "external_actions": "未授权，不执行发布/投递/群发",
  "media_budget": "未授权，先给方案或询问预算"
}
```

## 有媒体需求时怎么接入

先读 [MCP 绑定说明](mcp-binding.md)。根据任务找到当前真实工具，查询模型与 schema 后创建本地参数文件；不要将上面的任务描述对象误当作工具参数。已授权生成时再执行该说明的单次调用命令，并查询任务状态。纯文字和文件任务不执行这一步。
