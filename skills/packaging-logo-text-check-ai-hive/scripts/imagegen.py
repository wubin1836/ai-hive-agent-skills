...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "为包装Logo与文字准确性检查制作一张主体准确、层级清晰的商业图片", "keywords": "包装检查,Logo检测,文字准确性,AI商品图,包装质检", "model": "public_model_gpt_image_2", "name": "packaging-logo-text-check-ai-hive", "rule": "optional", "search": "对照标准包装逐块检查，优先发现易导致误导的变化", "title": "包装Logo与文字准确性检查"}')
