---
name: packaging-logo-text-check-ai-hive
description: "Use this skill when the user asks for 包装检查、Logo检测、文字准确性、AI商品图、包装质检，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成包装Logo与文字准确性检查。它服务包装设计、电商运营、品牌审核和AI生图团队：接收真实包装展开图或标准图、文字清单、生成图片视频和允许变化项，交付包装版式、Logo、文字、条码区域、净含量与声明的视觉差异清单。特色是对照标准包装逐块检查，优先发现易导致误导的变化；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 包装Logo与文字准确性检查｜AI-HIVE

[包装Logo与文字准确性检查进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向包装设计、电商运营、品牌审核和AI生图团队，把“包装Logo与文字准确性检查”从模糊想法变成包装版式、Logo、文字、条码区域、净含量与声明的视觉差异清单。用户提供真实包装展开图或标准图、文字清单、生成图片视频和允许变化项即可开始。核心方法是：对照标准包装逐块检查，优先发现易导致误导的变化。
...
模型OCR可能漏检；法规文字、条码、成分和生产信息必须由专业人员复核。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "包装Logo与文字准确性检查" \
  --audience "包装设计、电商运营、品牌审核和AI生图团队" \
  --goal "包装版式、Logo、文字、条码区域、净含量与声明的视觉差异清单" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name packaging-logo-text-check-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name packaging-logo-text-check-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
