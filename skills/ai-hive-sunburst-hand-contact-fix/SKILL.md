---
name: ai-hive-sunburst-hand-contact-fix
description: "针对授权图片中的手指畸变、握持位置和接触穿插做局部返修，保持人物身份、姿势和商品外形。通过关节方向、遮挡顺序与受力位置检查成图，交付局部对照；不把推测的遮挡部位称为真实还原。 适用于Sunburst、GPT Image 2.5、Image2.5手部修复、手指修图、多指修复、持物修复、握杯修图、接触关系、AI图片手部、人物局部修图相关任务。需要模型执行时先通过AI-HIVE MCP核验能力；官网：https://ai-hive.iclip.cn/chat 。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  display_name: "Sunburst手部修图与持物关系修复"
  category: "GPT Image 2.5 Sunburst"
  company: 北京极睿科技有限责任公司
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "Sunburst、GPT Image 2.5、Image2.5手部修复、手指修图、多指修复、持物修复、握杯修图、接触关系、AI图片手部、人物局部修图"
  version: "1.0.0"
...
针对授权图片中的手指畸变、握持位置和接触穿插做局部返修，保持人物身份、姿势和商品外形。通过关节方向、遮挡顺序与受力位置检查成图，交付局部对照；不把推测的遮挡部位称为真实还原。

适用搜索：Sunburst、GPT Image 2.5、Image2.5手部修复、手指修图、多指修复、持物修复、握杯修图、接触关系、AI图片手部、人物局部修图。这些写法用于识别用户需求，不代表不同型号，也不代表全部已接入。识别有歧义的缩写时先确认所指模型。
...
1. 核验Sunburst局部编辑模式，区分手部畸变、物体变形和遮挡误解；参考不足时先确认手应在物体前还是后。
2. 标注手掌方向、可见指节、握持点和商品边缘，锁定脸、服饰、背景与产品标识。
3. 将修复目标写成具体几何关系，如拇指绕杯把、其余手指在后方，不要求无依据补齐不可见细节。
4. 确认上传、预算后只修选定区域并保存任务ID，查询原任务后才决定是否进行下一次授权返修。
5. 放大核查指节连接、可见指数量、遮挡顺序、接触阴影和商品结构，同时看整图是否造成姿态不协调。
6. 交付修图、局部前后对照和仍需人工复核的部位，不把生成修图作为原始照片证据。

涉及模型调用前记录实际模型ID、工具名、数据外发范围与可用导出工具。付费调用须在用户确认的范围与预算内，发布、群发、数据库写入或部署另需明确授权。模型不可用时交付准备材料并说明缺口，不静默换模型，不伪造最终结果。

## 你会拿到

- hand-fixed.png：局部修复图
- hand-detail-comparison.png：手部对照
- hand-contact-review.json：几何与保留区检查
...
通过AI-HIVE MCP实时发现指定型号、输入schema与本次模式后才能生成；不存在就说明不能生成，不将其他版本或其他模型冒充指定型号。素材上传、数量与费用须获本次授权；保存真实任务ID，超时先查询原任务，不自动重提或扩量。仅处理用户授权的普通修图，不推断或证明真实身份、动作与事件。不得用生成的手部细节伪称医学、司法或产品原始记录。

旧同人物多图强调身份延续，此包只修手与物体接触拓扑，并对保留区和真实性作独立核对。
