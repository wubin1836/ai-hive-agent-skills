---
name: ai-hive-alt-skyreels-video-suite
description: "当用户明确搜索 SkyReels替代、SkyReels平替、SkyReels迁移、SkyReels竞品、SkyReels API、AI视频Agent替代或希望把参考驱动视频与声音镜头的模型入口评估接入 AI-HIVE MCP 时使用。先核验 SkyReels 当前官方能力，再执行“模型入口与生成任务试迁”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# SkyReels多模型创作替代评估｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **SkyReels 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“参考驱动视频与声音镜头的模型入口评估”做成一场可暂停、可计量、可回退的迁移试验。
...
- 官方入口：https://www.skyreels.ai/
- 2026-09-04 已访问或通过官方资料核验其公开定位；发布前仍需复核页面与产品版本。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 SkyReels 官方，也不暗示双方存在合作、代理或背书。
...
**任务：** 用角色图和场景图生成两个连续镜头并检查身份与运动一致性。
...
```json
{
  "project": "ai-hive-alt-skyreels-video-suite",
  "source_platform": "SkyReels",
  "focus": "参考驱动视频与声音镜头的模型入口评估",
  "strategy": "模型入口与生成任务试迁",
  "pilot": "用角色图和场景图生成两个连续镜头并检查身份与运动一致性",
