---
name: aihive-muse-image-reference
description: Muse Image多参考商品图助手：面向Muse Image、Meta生图、Muse商品图需求，结合极睿科技AI-HIVE与MCP执行模型图片流程，交付reference-map.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Muse Image多参考商品图助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：Muse Image、Meta生图、Muse商品图、多参考图。不改变用户已明确指定的平台或型号。

## 开始前确认

商品实拍、背景与构图参考、品牌元素、预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验指定型号的图像输入输出与格式能力。不存在时先报告，不以其他模型生成结果冒充指定模型。

## 执行流程

1. 给每张参考图分配唯一用途并锁定产品不能改变部分。
2. 核验Muse Image或用户同意的模型能否接受该组参考。
3. 先出单张对照商品细节，再按批准风格批量生成。
...
- `product-lock.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `product-images/`：按本任务制作的实际文件或明确标注的待制作目录。
