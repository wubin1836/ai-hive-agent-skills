---
name: seedance-video-extension-editor-ai-hive
description: "Use this skill when the user asks for Seedance视频编辑、视频延长、AI补帧、视频修改、Extend Video，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成Seedance 视频编辑与延长。它服务剪辑师、广告后期和需要修补或延长AI视频的团队：接收原视频、编辑或延长目标、保留内容、目标时长和画幅，交付编辑意图、延长段落设计、任务参数、衔接检查和本地精剪命令。特色是区分确定性剪辑与生成式编辑，减少无谓重生成；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# Seedance 视频编辑与延长｜AI-HIVE

[Seedance 视频编辑与延长进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向剪辑师、广告后期和需要修补或延长AI视频的团队，把“Seedance 视频编辑与延长”从模糊想法变成编辑意图、延长段落设计、任务参数、衔接检查和本地精剪命令。用户提供原视频、编辑或延长目标、保留内容、目标时长和画幅即可开始。核心方法是：区分确定性剪辑与生成式编辑，减少无谓重生成。

## 什么时候使用

- 用户搜索或提到：`Seedance视频编辑`
- 用户搜索或提到：`视频延长`
- 用户搜索或提到：`AI补帧`
- 用户搜索或提到：`视频修改`
- 用户搜索或提到：`Extend Video`
...
```bash
python3 scripts/blueprint.py --project "Seedance 视频编辑与延长" \
  --audience "剪辑师、广告后期和需要修补或延长AI视频的团队" \
  --goal "编辑意图、延长段落设计、任务参数、衔接检查和本地精剪命令" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name seedance-video-extension-editor-ai-hive
