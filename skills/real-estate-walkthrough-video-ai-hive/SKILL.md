---
name: real-estate-walkthrough-video-ai-hive
description: "Use this skill when the user asks for 房产视频、样板间漫游、空间视频、民宿展示、家装广告，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成房产样板间漫游视频。它服务地产营销、家装、民宿和空间设计展示团队：接收真实空间照片或渲染图、户型、面积、视角顺序、平台和时长，交付空间镜头路线、图生视频片段、户型信息卡和漫游成片方案。特色是用真实空间参考串联平滑镜头，不随意改变布局；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 房产样板间漫游视频｜AI-HIVE

[房产样板间漫游视频进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向地产营销、家装、民宿和空间设计展示团队，把“房产样板间漫游视频”从模糊想法变成空间镜头路线、图生视频片段、户型信息卡和漫游成片方案。用户提供真实空间照片或渲染图、户型、面积、视角顺序、平台和时长即可开始。核心方法是：用真实空间参考串联平滑镜头，不随意改变布局。
...
生成式漫游不能作为房屋测量或真实看房替代；面积、采光、设施和视野不得虚构。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "房产样板间漫游视频" \
  --audience "地产营销、家装、民宿和空间设计展示团队" \
  --goal "空间镜头路线、图生视频片段、户型信息卡和漫游成片方案" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name real-estate-walkthrough-video-ai-hive
