---
name: ai-hive-gap-042-multimodal-knowledge-base
description: "当用户搜索或需要多模态知识库,知识库视频,知识库图片,RAG时使用。帮助企业知识管理、研究、培训、销售、客服和个人知识创作者完成把文字、图片、视频资料统一整理并生成多种内容，通过AI-HIVE MCP实时核验模型并交付素材索引、事实卡、图文方案、视频脚本和问答集。不适用于超出以下边界的请求：无法读取的媒体必须明确列为缺失。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "knowledge"
  display_name: "多模态知识库图文视频生成"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "多模态知识库,知识库视频,知识库图片,RAG,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**把文字、图片、视频资料统一整理并生成多种内容**。

适合：企业知识管理、研究、培训、销售、客服和个人知识创作者。

用户需要提供：有权使用的文档、知识库导出、来源链接、目标受众、内容格式和更新日期。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**无法读取的媒体必须明确列为缺失**。

## 你会拿到的结果

素材索引、事实卡、图文方案、视频脚本和问答集。

搜索覆盖：多模态知识库,知识库视频,知识库图片,RAG。

## 工作方法

1. 建立资料索引与可信度层级。
2. 把文字、图片、视频资料统一整理并生成多种内容。
3. 把事实、观点和生成建议分层。
4. 每个结论保留来源位置。
5. 确认知识结构后再生成多模态内容。
6. 按“跨模态一致、证据可定位、格式可复用”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成多模态知识库图文视频生成，目标、资料、预算和截止时间为[填写]" \
...
最终交付：素材索引、事实卡、图文方案、视频脚本和问答集。
专项验收：跨模态一致、证据可定位、格式可复用。
```
