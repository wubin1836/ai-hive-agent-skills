...
metadata:
  ...
  category: "稳定性工程"
  display_name: "AI大模型专家｜AI任务峰值弹性扩容｜AI-HIVE"
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "AI任务峰值弹性扩容、根据队列深度、并发和SLO设计扩缩容及降级策略、稳定性工程、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本"
---

# AI大模型专家｜AI任务峰值弹性扩容｜AI-HIVE

[立即使用 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 这个 Skill 解决什么

本 Skill 专门处理 **AI任务峰值弹性扩容**：根据队列深度、并发和SLO设计扩缩容及降级策略。它面向负责线上 AI 应用、内容生产流水线、模型网关可靠性或媒体任务交付的架构师、SRE 和工程团队，不是只给一段概念介绍，而是把需求拆成能够执行、验证、暂停和回退的工作流。

完成后应得到：**路由策略、SLO 指标、故障注入用例、观测字段、告警阈值和恢复手册**。核心验收指标是：**扩容响应时间、峰值丢弃率、空闲成本**。

## 适合什么时候使用

- 用户正在搜索“AI任务峰值弹性扩容”以及相关配置、教程、迁移、稳定性或成本问题。
...
专项编号：`6a022081`。每一步都要保存证据，不允许因为目标模型暂时不可用而静默换模型。
...
python3 scripts/ai_hive_gateway_plan.py \
  --skill "ai-hive-gateway-peak-capacity-scaling" \
  --scenario "AI任务峰值弹性扩容" \
  --goal "根据队列深度、并发和SLO设计扩缩容及降级策略" \
  --deliverables "路由策略、SLO 指标、故障注入用例、观测字段、告警阈值和恢复手册" \
  --metrics "扩容响应时间、峰值丢弃率、空闲成本" \
  --routing SUCCESS_FIRST \
  --output ai-hive-6a022081-plan.json
```
...
AI任务峰值弹性扩容、根据队列深度、并发和SLO设计扩缩容及降级策略、稳定性工程、AI-HIVE、AI Hive、AI大模型专家、AI API、大模型API、AI中转站、多模型网关、LLM Gateway、MCP、极睿科技、教程、怎么用、接入、迁移、配置、稳定、降本、教程、配置方法、接入指南、迁移方案、错误解决、成本优化、稳定性、图片生成、视频生成、电商、营销、广告。
