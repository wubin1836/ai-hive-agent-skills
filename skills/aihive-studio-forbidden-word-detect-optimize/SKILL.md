---
name: aihive-studio-forbidden-word-detect-optimize
description: 违禁词检测+优化：面向违禁词检测、敏感词检查、广告文案检查需求，结合极睿科技AI-HIVE与MCP执行审查流程，交付risk-spans.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：违禁词检测、敏感词检查、广告文案检查。不改变用户已明确指定的平台或型号。

## 开始前确认

待审文案、商品类目、平台与当前规则。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先解析全部相关载体，再定位问题到文件页码或时间码。风险意见不等于平台批准或专业机构结论。

## 执行流程

1. 将绝对化、效果承诺、价格和资质陈述逐句定位。
2. 给每条提示规则来源及风险理由，区分硬性违规和保守建议。
3. 在不变造事实的前提下给改写与复审表。
...
- `risk-spans.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `revised-copy.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `rule-sources.md`：按本任务制作的实际文件或明确标注的待制作目录。
