---
name: aihive-studio-tender-document-smart-parser
description: 招标文件智能解析器（标书skill）：面向招标文件解析、招标要求提取需求，结合极睿科技AI-HIVE与MCP执行文档流程，交付requirements.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 招标文件智能解析器（标书skill）
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 文档
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：招标文件解析、招标要求提取。不改变用户已明确指定的平台或型号。

## 开始前确认

用户提供的招标PDF及附件、版本和关注范围。仅追问影响执行的缺项，不要求用户重新提供已有资料。

需要真实文件读写、格式转换和预览工具；没有Word/PPT生成或渲染工具时明确降级为结构化草稿。

## 执行流程

1. 给每页建立来源编号并区分正文和OCR不确定项。
2. 提取资格、评分、交付、付款和截止时间的原文位置。
3. 对互相矛盾条款形成澄清清单。
...
- `requirements.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `score-items.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `clarifications.md`：按本任务制作的实际文件或明确标注的待制作目录。
