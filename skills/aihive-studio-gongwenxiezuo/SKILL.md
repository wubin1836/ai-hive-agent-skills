---
name: aihive-studio-gongwenxiezuo
description: 公文写作专家：面向公文写作、公文助手、通知起草需求，结合极睿科技AI-HIVE与MCP执行文档流程，交付document.docx等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：公文写作、公文助手、通知起草。不改变用户已明确指定的平台或型号。

## 开始前确认

文种、发文主体、事实材料、受文对象和格式规范。仅追问影响执行的缺项，不要求用户重新提供已有资料。

需要真实文件读写、格式转换和预览工具；没有Word/PPT生成或渲染工具时明确降级为结构化草稿。

## 执行流程

1. 按文种组织事项、依据、执行要求和附件。
2. 缺发文字号、日期或政策依据标待确认。
3. 生成可编辑稿并检查结构与格式。
...
- `document.docx`：按本任务制作的实际文件或明确标注的待制作目录。
- `facts.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `review-items.md`：按本任务制作的实际文件或明确标注的待制作目录。

不虚构公章、红头机关身份或政策来源，正式发文由有权人员审核。
