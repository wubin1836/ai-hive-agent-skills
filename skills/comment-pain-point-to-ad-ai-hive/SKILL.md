---
name: comment-pain-point-to-ad-ai-hive
description: "Use this skill when the user asks for 评论区洞察、评论转广告、用户痛点、短视频选题、电商增长，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成评论区痛点转广告。它服务内容运营、商品团队、投手和用户研究人员：接收评论导出或截图、商品事实、平台、受众和需要回避的问题，交付评论聚类、痛点优先级、问题型钩子、回应脚本和广告视频任务。特色是把真实问题变成内容选题，不把抱怨直接当作营销话术；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 评论区痛点转广告｜AI-HIVE

[评论区痛点转广告进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向内容运营、商品团队、投手和用户研究人员，把“评论区痛点转广告”从模糊想法变成评论聚类、痛点优先级、问题型钩子、回应脚本和广告视频任务。用户提供评论导出或截图、商品事实、平台、受众和需要回避的问题即可开始。核心方法是：把真实问题变成内容选题，不把抱怨直接当作营销话术。
...
评论样本不能代表全部用户；不得泄露账号、订单或个人信息，也不能伪造品牌回复。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "评论区痛点转广告" \
  --audience "内容运营、商品团队、投手和用户研究人员" \
  --goal "评论聚类、痛点优先级、问题型钩子、回应脚本和广告视频任务" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name comment-pain-point-to-ad-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name comment-pain-point-to-ad-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
