---
name: aihive-studio-viral-video-teardown-remake
description: 爆款视频拆解·短视频复刻：面向爆款视频拆解、视频复刻、短视频模仿结构需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付structure.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 爆款视频拆解·短视频复刻
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：爆款视频拆解、视频复刻、短视频模仿结构。不改变用户已明确指定的平台或型号。

## 开始前确认

有权使用的参考视频、自有素材、目标产品和平台。仅追问影响执行的缺项，不要求用户重新提供已有资料。

视频理解、图像/视频生成、配音与剪辑是不同工具。按本任务需要逐一核验；缺成片工具只交付分镜或素材，并明确未生成成片。

## 执行流程

1. 以时间码标注钩子、信息密度、转折和转化位置。
2. 抽象为节奏结构而不复制原台词、人物或水印。
3. 换成用户素材重写分镜并输出可剪辑镜头表。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `structure.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `original-script.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `edit-plan.csv`：按本任务制作的实际文件或明确标注的待制作目录。
