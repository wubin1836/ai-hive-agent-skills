---
name: ai-hive-gap-056-model-evals-regression
description: "当用户搜索或需要模型评测,Evals,回归测试,LLM评估时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成用真实任务样例建立模型质量与成本回归测试，通过AI-HIVE MCP实时核验模型并交付评测集、评分标准、基线、回归报告和选型建议。不适用于超出以下边界的请求：榜单成绩不能替代企业真实任务评测。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "模型评测与回归测试"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "模型评测,Evals,回归测试,LLM评估,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**用真实任务样例建立模型质量与成本回归测试**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**榜单成绩不能替代企业真实任务评测**。

## 你会拿到的结果

评测集、评分标准、基线、回归报告和选型建议。

搜索覆盖：模型评测,Evals,回归测试,LLM评估。

## 怎么执行

1. 先定义可量化的质量和成本指标。
2. 用真实任务样例建立模型质量与成本回归测试。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“样例代表业务、评分可复核、版本可比较”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成模型评测与回归测试，目标、资料、预算和截止时间为[填写]" \
...
最终交付：评测集、评分标准、基线、回归报告和选型建议。
专项验收：样例代表业务、评分可复核、版本可比较。
```
