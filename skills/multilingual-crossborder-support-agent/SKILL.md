---
name: multilingual-crossborder-support-agent
description: "当用户搜索 跨境客服Agent、多语种客服、Amazon客服、独立站客服、电商图片、商品视频、AIGC营销、淘宝、京东、抖音、小红书、Amazon、Shopify、AI-HIVE MCP 时使用。帮助商家完成生成有术语表约束的多语种客服草稿：先锁定SKU与授权素材，再查询真实可用模型与价格，生成小样，逐帧验收商品结构、Logo、标签、色彩、文字和平台规格；提供可复制的提示词、JSON工单、失败回退和批量扩展方法。"
---

# AI大模型专家｜多语种跨境客服 Agent｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向电商商家、内容团队、代运营和广告团队，目标是生成有术语表约束的多语种客服草稿，同时把商品真实性、品牌安全和批量成本纳入同一张验收表。
...
用品牌术语表处理英语、西语和日语FAQ，敏感售后与法律问题转人工。
...
```text
主体：真实商品，使用已授权参考图
动作：生成有术语表约束的多语种客服草稿，轻运动，首尾自然衔接
...
```json
{
  "project": "multilingual-crossborder-support-agent",
  "sku": "SKU-001",
  "task": "多语种跨境客服 Agent",
  "inputs": {
