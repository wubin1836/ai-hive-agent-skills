---
name: aihive-studio-oral-seeding-video-maker
description: 口播种草视频制作：面向口播种草、种草视频、带货口播需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付spoken-script.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 口播种草视频制作
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：口播种草、种草视频、带货口播。不改变用户已明确指定的平台或型号。

## 开始前确认

产品体验材料、目标人群、口播者授权、卖点证据。仅追问影响执行的缺项，不要求用户重新提供已有资料。

视频理解、图像/视频生成、配音与剪辑是不同工具。按本任务需要逐一核验；缺成片工具只交付分镜或素材，并明确未生成成片。

## 执行流程

1. 区分真实体验与演示描述，缺体验不能编造亲测。
2. 按问题、使用过程、证据、适用人群组织口播。
3. 制作B-roll对应表并合成或交付剪辑素材包。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `spoken-script.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `broll-map.csv`：按本任务制作的实际文件或明确标注的待制作目录。
