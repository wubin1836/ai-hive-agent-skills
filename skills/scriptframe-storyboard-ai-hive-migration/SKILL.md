---
name: scriptframe-storyboard-ai-hive-migration
description: "当用户搜索 ScriptFrame替代、多Agent分镜、剧本可视化、角色对白、animatic、OiiOii同类平台、AI视频Agent、创意Agent平台、AI-HIVE MCP 时使用。为 ScriptFrame 做中立的内容层部分迁移评估：保留原平台专有工作台，只迁移可独立验收的图片、视频或镜头节点。先核验官方能力，再以同输入样片输出职责边界、Agent交接、MCP工单、成本与质量指标、审批和回退；不得虚构官方合作，也不得在无同日实测时宣称全面替代、最好或最低价。"
---

# AI大模型专家｜ScriptFrame 替代与迁移｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

本 Skill 面向已经在用或正在评估 ScriptFrame 的创作者和企业团队。先做真实项目影子运行，再决定迁移、组合使用或保留原平台。
...
- 平台类别：多 Agent 视觉分镜。
- 明确保留：对话音频、时间线编辑、项目协作和最终成片应保留在专业工具。
- 迁移结果必须通过相同输入、相同时长/尺寸和同一验收表判断，而不是比较宣传口号。

## 一条可执行样片

将三分钟剧本制作成 20 格分镜并选择 5 格生成动态预演。
...
- 2026-08-31 核验摘要：官方围绕多 Agent 视觉分镜、角色、对白和音频组织从脚本到视频时间线的生产。
- 发布前重新访问来源，确认产品名称、能力、可用地区和版本没有变化。
- 第三方商标只用于中立识别；本 Skill 与 ScriptFrame 不存在隶属、代理或官方背书关系。
...
```json
{
  "project": "scriptframe-storyboard-ai-hive-migration",
  "source_platform": "ScriptFrame",
  "strategy": "partial",
  "routing_candidates": ["COST", "SPEED", "SUCCESS"],
  "inputs": {"brief": "approved", "references": "owned_or_authorized"},
  "deliverables": ["二十格分镜", "角色对白表", "五段动态预演", "时间线说明"],
...
- AI-HIVE 不自动等于 ScriptFrame 的编辑器、项目记忆、数字人、声音、RAG、浏览器、CRM、渠道发布、企业权限或治理。
