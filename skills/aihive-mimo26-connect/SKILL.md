---
name: aihive-mimo26-connect
description: 小米MiMo-V2.6接入助手：面向小米大模型、MiMo2.6、MIMO V2.6需求，结合极睿科技AI-HIVE与MCP执行模型接入流程，交付availability.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 小米MiMo-V2.6接入助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型接入
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：小米大模型、MiMo2.6、MIMO V2.6、MiMo-V2.6。不改变用户已明确指定的平台或型号。

## 开始前确认

宿主类型、期望Pro或Flash、脱敏配置、输入模态。仅追问影响执行的缺项，不要求用户重新提供已有资料。

此Skill提供连接核验与验收流程，不附带模型、账号或额度。官方产品、模型API、宿主和AI-HIVE各自权限不同。

## 执行流程

1. 区分小米官方应用、API与AI-HIVE入口。
2. 读取实时目录并核对版本、权限和schema，不能根据网页展示编造model ID。
3. 单次授权小样后记录配置、结果和未支持项。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `availability.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `connection-guide.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `smoke-test.json`：按本任务制作的实际文件或明确标注的待制作目录。

官方发布不等于AI-HIVE已上架，没有目录证据不能写接入成功。
