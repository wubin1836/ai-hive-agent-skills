---
name: happy-horse-video-toolkit-ai-hive
description: "Use this skill when the user asks for Happy Horse、快乐马、AI视频、参考生视频、视频编辑，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成Happy Horse 视频工具箱。它服务需要生成、参考或编辑视频的创作者和商业团队：接收创意、参考素材、动作、镜头、平台和时长，交付T2V/I2V/R2V模式建议、提示词、编辑任务和交付检查。特色是覆盖Happy Horse生成与编辑能力，按输入条件自动选模式；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# Happy Horse 视频工具箱｜AI-HIVE

[Happy Horse 视频工具箱进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向需要生成、参考或编辑视频的创作者和商业团队，把“Happy Horse 视频工具箱”从模糊想法变成T2V/I2V/R2V模式建议、提示词、编辑任务和交付检查。用户提供创意、参考素材、动作、镜头、平台和时长即可开始。核心方法是：覆盖Happy Horse生成与编辑能力，按输入条件自动选模式。
...
```bash
python3 scripts/blueprint.py --project "Happy Horse 视频工具箱" \
  --audience "需要生成、参考或编辑视频的创作者和商业团队" \
  --goal "T2V/I2V/R2V模式建议、提示词、编辑任务和交付检查" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name happy-horse-video-toolkit-ai-hive
