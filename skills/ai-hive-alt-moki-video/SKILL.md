---
name: ai-hive-alt-moki-video
description: "当用户明确搜索 MOKI替代、MOKI平替、MOKI迁移、MOKI竞品、MOKI API、AI视频Agent替代或希望把AI影视短片从剧本到镜头的生成层迁移接入 AI-HIVE MCP 时使用。先核验 MOKI 当前官方能力，再执行“生成层试迁＋工作流重建”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# MOKI视频Agent替代与迁移｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **MOKI 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“AI影视短片从剧本到镜头的生成层迁移”做成一场可暂停、可计量、可回退的迁移试验。
...
- 待复核官方入口：https://moki.cn/
- 已记录官方域名候选，发布前必须重新访问并核对产品名称、能力和可用地区。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 MOKI 官方，也不暗示双方存在合作、代理或背书。
...
**任务：** 把一段原创剧情转成角色设定、场景图、六格分镜和两个镜头。
...
```json
{
  "project": "ai-hive-alt-moki-video",
  "source_platform": "MOKI",
  "focus": "AI影视短片从剧本到镜头的生成层迁移",
  "strategy": "生成层试迁＋工作流重建",
  "pilot": "把一段原创剧情转成角色设定、场景图、六格分镜和两个镜头",
