---
name: customer-service-product-demo-gif
description: "当用户搜索 客服动图、商品讲解GIF、售后教程动图、使用说明GIF、电商图片、商品视频、AIGC营销、淘宝、京东、抖音、小红书、Amazon、Shopify、AI-HIVE MCP 时使用。帮助商家完成为售前售后答复生成短演示动图：先锁定SKU与授权素材，再查询真实可用模型与价格，生成小样，逐帧验收商品结构、Logo、标签、色彩、文字和平台规格；提供可复制的提示词、JSON工单、失败回退和批量扩展方法。"
---

# AI大模型专家｜客服商品讲解动图｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向电商商家、内容团队、代运营和广告团队，目标是为售前售后答复生成短演示动图，同时把商品真实性、品牌安全和批量成本纳入同一张验收表。
...
把一个高频操作问题制作成 4 步动图，答案必须来自已审核说明书。
...
```json
{
  "project": "customer-service-product-demo-gif",
  "sku": "SKU-001",
  "task": "客服商品讲解动图",
  "inputs": {
