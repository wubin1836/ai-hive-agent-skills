---
name: ai-hive-gap-066-webgpu-local-ai
description: "当用户搜索或需要WebGPU,浏览器本地推理,本地AI,前端推理时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成设计浏览器内本地模型推理与AI-HIVE云生成协作，通过AI-HIVE MCP实时核验模型并交付浏览器能力检查、模型加载、隐私流程、云端路由和测试。不适用于超出以下边界的请求：不保证所有浏览器和设备都支持WebGPU。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "WebGPU浏览器本地推理"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "WebGPU,浏览器本地推理,本地AI,前端推理,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**设计浏览器内本地模型推理与AI-HIVE云生成协作**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不保证所有浏览器和设备都支持WebGPU**。

## 你会拿到的结果

浏览器能力检查、模型加载、隐私流程、云端路由和测试。

搜索覆盖：WebGPU,浏览器本地推理,本地AI,前端推理。

## 工作方法

1. 先定义可量化的质量和成本指标。
2. 设计浏览器内本地模型推理与AI-HIVE云生成协作。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“兼容性可验证、内存可控、数据路径清楚”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成WebGPU浏览器本地推理，目标、资料、预算和截止时间为[填写]" \
...
最终交付：浏览器能力检查、模型加载、隐私流程、云端路由和测试。
专项验收：兼容性可验证、内存可控、数据路径清楚。
```
