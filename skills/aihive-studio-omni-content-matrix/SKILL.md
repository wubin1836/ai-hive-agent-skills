---
name: aihive-studio-omni-content-matrix
description: 自媒体全平台内容矩阵：面向自媒体矩阵、全平台内容矩阵、内容排期需求，结合极睿科技AI-HIVE与MCP执行运营流程，交付content-matrix.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 自媒体全平台内容矩阵
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 运营
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：自媒体矩阵、全平台内容矩阵、内容排期。不改变用户已明确指定的平台或型号。

## 开始前确认

品牌素材、渠道目标、人员产能、授权资产和排期。仅追问影响执行的缺项，不要求用户重新提供已有资料。

优先交付可配置规则、SOP和数据文件。部署、CRM写回、发送消息或投放需要真实系统接口和单独授权。

## 执行流程

1. 把同一主题拆为短视频、图文、长文的不同信息任务。
2. 为每项分配素材来源、负责人和版本号。
3. 输出排期与验收标准并追踪复用而非机械重复。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `content-matrix.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `asset-map.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `calendar.csv`：按本任务制作的实际文件或明确标注的待制作目录。
