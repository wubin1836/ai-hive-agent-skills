---
name: ai-hive-gazetteer-place-index
description: "地方志古今地名索引助手：读取已转写且允许使用的方志选段、卷页和版本信息，提取地名、类型和文本中记载的隶属关系，记录卷页和原文跨度，交付可回查的文件及待核实清单。适用于地方志整理、古今地名、方志地名、地方文化资料；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。"
metadata:
  display_name: "地方志古今地名索引助手"
  version: "1.0.0"
...
地方志古今地名索引助手：读取已转写且允许使用的方志选段、卷页和版本信息，提取地名、类型和文本中记载的隶属关系，记录卷页和原文跨度，交付可回查的文件及待核实清单。适用于地方志整理、古今地名、方志地名、地方文化资料；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。
...
1. 提取地名、类型和文本中记载的隶属关系，记录卷页和原文跨度。
2. 对同名异地、地名变迁和疑似现代对应建立候选映射，不直接合并。
3. 将对应证据、确认状态和坐标来源分别记录，仅有可靠坐标的地点进入地图数据。
4. 生成地名索引、已核验GeoJSON和待考清单，检测重复坐标及无出处映射。
...
- `gazetteer-place-index.xlsx`
- `verified-places.geojson`
- `unresolved-toponyms.md`
...
执行时核验指定方志版本、现代对应资料与坐标来源；没有可靠定位的地点不落图，不把历史地名对应当作现行行政边界或产权依据，不默认规划实地路线。

缺关键原件或专业依据时只完成可验证部分。401/403、限流或内容审核阻断时保存脱敏进度并停止相应调用，遵守明确的等待要求，不循环重试或绕过审核。不因安装本技能自动交易、签署、提交、缴费或公开发布。

## 调用示例

> 请使用我授权提供的资料完成“地方志古今地名索引助手”。范围先按我的原件与指定版本确认，生成gazetteer-place-index.xlsx及其余约定文件，疑点附来源，不自动对外提交。

## 搜索与任务识别

地方志整理、古今地名、方志地名、地方文化资料、地名索引
