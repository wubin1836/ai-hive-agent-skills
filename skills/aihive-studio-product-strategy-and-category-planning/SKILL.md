---
name: aihive-studio-product-strategy-and-category-planning
description: 选品策略与品类规划：面向选品策略、品类规划、商品组合需求，结合极睿科技AI-HIVE与MCP执行运营流程，交付sku-analysis.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 选品策略与品类规划
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 运营
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：选品策略、品类规划、商品组合。不改变用户已明确指定的平台或型号。

## 开始前确认

脱敏销售、成本、库存、退货和品类目标。仅追问影响执行的缺项，不要求用户重新提供已有资料。

优先交付可配置规则、SOP和数据文件。部署、CRM写回、发送消息或投放需要真实系统接口和单独授权。

## 执行流程

1. 统一商品与订单口径计算贡献毛利和周转。
2. 区分引流、利润、复购与试验品并注明依据。
3. 设计小批量验证和下架阈值。
...
- `sku-analysis.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `category-plan.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `test-batch.csv`：按本任务制作的实际文件或明确标注的待制作目录。
