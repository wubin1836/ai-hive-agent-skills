...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Image2 制作高完成度精准文字图片，主体准确，构图清晰", "keywords": "Image2、Image 2、GPT Image 2、GPTImage2、OpenAI 图片、ChatGPT 图片、精准文字图片、图片文字、精准排版、海报文字、中文文字生成、文字准确、AI图片、AIGC图片", "model": "public_model_gpt_image_2", "name": "image-2-exact-text-image", "rule": "optional", "search": "固定调用 Image2 的精准文字图片裸接口能力", "title": "Image2 精准文字图片"}')
