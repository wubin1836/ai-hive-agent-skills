---
name: product-asset-auto-montage-ai-hive
description: "Use this skill when the user asks for 商品素材混剪、素材库剪辑、多SKU视频、电商内容工厂、批量视频，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成商品素材库自动混剪。它服务多SKU商家、内容中台、代运营和广告后期团队：接收商品图片视频目录、SKU表、品牌规范、平台、时长和目标，交付素材索引、镜头标签、选片规则、多个混剪版本和缺口补镜。特色是先按SKU与镜头功能归档，再生成可追溯混剪；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 商品素材库自动混剪｜AI-HIVE

[商品素材库自动混剪进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向多SKU商家、内容中台、代运营和广告后期团队，把“商品素材库自动混剪”从模糊想法变成素材索引、镜头标签、选片规则、多个混剪版本和缺口补镜。用户提供商品图片视频目录、SKU表、品牌规范、平台、时长和目标即可开始。核心方法是：先按SKU与镜头功能归档，再生成可追溯混剪。
...
禁止跨SKU误用素材；旧包装、过期价格和非授权素材必须排除。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "商品素材库自动混剪" \
  --audience "多SKU商家、内容中台、代运营和广告后期团队" \
  --goal "素材索引、镜头标签、选片规则、多个混剪版本和缺口补镜" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name product-asset-auto-montage-ai-hive
