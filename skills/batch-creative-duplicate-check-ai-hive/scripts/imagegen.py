...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "为批量素材重复度检测制作一张主体准确、层级清晰的商业图片", "keywords": "素材去重,批量图片检查,视频重复度,创意重复,内容工厂质检", "model": "public_model_gpt_image_2", "name": "batch-creative-duplicate-check-ai-hive", "rule": "optional", "search": "区分技术重复、视觉近似和创意同质化", "title": "批量素材重复度检测"}')
