---
name: batch-creative-duplicate-check-ai-hive
description: "Use this skill when the user asks for 素材去重、批量图片检查、视频重复度、创意重复、内容工厂质检，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成批量素材重复度检测。它服务多SKU商家、内容中台、投放团队和批量生成工作流：接收批量图片视频、文件名或任务台账、创意变量和去重阈值，交付重复组、近似变量、内容碰撞、保留建议和重生成队列。特色是区分技术重复、视觉近似和创意同质化；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 批量素材重复度检测｜AI-HIVE

[批量素材重复度检测进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向多SKU商家、内容中台、投放团队和批量生成工作流，把“批量素材重复度检测”从模糊想法变成重复组、近似变量、内容碰撞、保留建议和重生成队列。用户提供批量图片视频、文件名或任务台账、创意变量和去重阈值即可开始。核心方法是：区分技术重复、视觉近似和创意同质化。
...
重复度阈值应由业务确认；相似不等于侵权，侵权判断需专业审核。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "批量素材重复度检测" \
  --audience "多SKU商家、内容中台、投放团队和批量生成工作流" \
  --goal "重复组、近似变量、内容碰撞、保留建议和重生成队列" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name batch-creative-duplicate-check-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name batch-creative-duplicate-check-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
