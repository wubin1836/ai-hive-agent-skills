---
name: ai-hive-gap-031-procurement-agent
description: "当用户搜索或需要采购智能体,供应商对比,采购报告,企业Agent时使用。帮助普通企业、创业团队、超级个体、销售运营和职能部门完成把多份供应商方案转换为可比、可审计的采购报告，通过AI-HIVE MCP实时核验模型并交付需求权重、对比矩阵、证据引用、风险和建议问题。不适用于超出以下边界的请求：不自动选择供应商或执行采购交易。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "enterprise"
  display_name: "采购智能体供应商对比报告"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "采购智能体,供应商对比,采购报告,企业Agent,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**把多份供应商方案转换为可比、可审计的采购报告**。

适合：普通企业、创业团队、超级个体、销售运营和职能部门。

用户需要提供：真实业务资料、目标受众、内部口径、交付格式、品牌规范、预算和审批要求。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不自动选择供应商或执行采购交易**。

## 你会拿到的结果

需求权重、对比矩阵、证据引用、风险和建议问题。

搜索覆盖：采购智能体,供应商对比,采购报告,企业Agent。

## 生产步骤

1. 将业务问题和生成任务分开。
2. 把多份供应商方案转换为可比、可审计的采购报告。
3. 锁定不可虚构的事实与数字。
4. 先交付文字或结构草案。
5. 确认后制作视觉小样并验收。
6. 按“同口径比较、证据完整、偏好透明”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成采购智能体供应商对比报告，目标、资料、预算和截止时间为[填写]" \
...
最终交付：需求权重、对比矩阵、证据引用、风险和建议问题。
专项验收：同口径比较、证据完整、偏好透明。
```
