---
name: aihive-studio-ai-image-generation-studio
description: 文生图与图片生成工作室：面向文生图、AI图片生成、图片工作室需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付image-brief.json等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 文生图与图片生成工作室
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：文生图、AI图片生成、图片工作室。不改变用户已明确指定的平台或型号。

## 开始前确认

生成用途、尺寸、品牌参考、保留元素和禁用项。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验图像生成或编辑、参考图输入、尺寸与下载工具。准确文字、透明通道、二维码要用真实文件工具另行检查。

## 执行流程

1. 明确构图、主体、材质和输出格式并核实参考图授权。
2. 按实际工具schema组织生成和局部编辑任务。
3. 先选构图再修细节并输出变更记录。
...
- `image-brief.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `images/`：按本任务制作的实际文件或明确标注的待制作目录。
- `revision-log.md`：按本任务制作的实际文件或明确标注的待制作目录。

不能把提示词当图片；检查文字拼写、畸形和品牌一致性。
