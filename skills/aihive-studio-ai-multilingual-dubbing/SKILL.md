---
name: aihive-studio-ai-multilingual-dubbing
description: AI配音：面向多语言配音、AI配音、视频译配需求，结合极睿科技AI-HIVE与MCP执行音频流程，交付bilingual-script.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：多语言配音、AI配音、视频译配。不改变用户已明确指定的平台或型号。

## 开始前确认

授权音视频、原文、目标语言、术语表、说话人设置。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 转写并人工核实专名、数字和语义歧义。
2. 按时长改写译文且保留原意。
3. 核验TTS能力后生成分句配音并对齐时间轴。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `bilingual-script.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `dubbing.wav`：按本任务制作的实际文件或明确标注的待制作目录。
