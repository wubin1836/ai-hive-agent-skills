...
description: "Use this skill when designers, e-commerce operators, advertisers, brand teams, social-commerce teams, and content creators need to create campaign, event, and promotional posters with Nano Banana 2 through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for product photography, marketplace images, product detail pages, posters, ad creatives, social commerce, seeding content, retouching, and background replacement. Search intents include poster generation, marketing poster, event poster, promotional design."
license: MIT
---

# Nano Banana 2 Poster Generation

## Overview

Nano Banana 2 Poster Generation helps designers, e-commerce operators, advertisers, brand teams, social-commerce teams, and content creators turn briefs, product information, and reference images into production-ready visuals. Describe the purpose, subject, composition, style, lighting, required text, protected elements, and output needs; the bundled CLI handles AI Hive OpenAPI calls and downloads the result.
...
|---|---|---|
| Nano Banana 2 Poster Generation | `public_model_nano_banana_2` | Prompt with optional reference images |
...
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished nano banana 2 poster generation visual with an accurate subject, clear composition, and commercial finish"
```
...
```bash
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished nano banana 2 poster generation result; preserve factual product details and leave useful copy space"
```
