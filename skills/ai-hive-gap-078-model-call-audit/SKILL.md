---
name: ai-hive-gap-078-model-call-audit
description: "当用户搜索或需要模型审计,调用审计,AI合规,模型日志时使用。帮助企业安全、法务合规、AI治理、内容审核和平台工程团队完成形成模型调用、数据使用、费用和结果去向的审计材料，通过AI-HIVE MCP实时核验模型并交付调用台账、数据类别、用途、费用、输出位置和审批记录。不适用于超出以下边界的请求：不得把审计记录当作法律合规认证。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "security"
  display_name: "模型调用审计与合规"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "模型审计,调用审计,AI合规,模型日志,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**形成模型调用、数据使用、费用和结果去向的审计材料**。

适合：企业安全、法务合规、AI治理、内容审核和平台工程团队。

用户需要提供：待审计文本或流程、数据分类、工具权限、风险级别、适用制度和人工负责人。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不得把审计记录当作法律合规认证**。

## 你会拿到的结果

调用台账、数据类别、用途、费用、输出位置和审批记录。

搜索覆盖：模型审计,调用审计,AI合规,模型日志。

## 操作流程

1. 识别资产、数据和工具权限。
2. 形成模型调用、数据使用、费用和结果去向的审计材料。
3. 建立风险情景与测试样例。
4. 提出最小权限和确认门。
5. 保留审计证据与人工复核。
6. 按“记录可追溯、用途一致、权限匹配”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成模型调用审计与合规，目标、资料、预算和截止时间为[填写]" \
...
最终交付：调用台账、数据类别、用途、费用、输出位置和审批记录。
专项验收：记录可追溯、用途一致、权限匹配。
```
