---
name: aihive-mimo26-distill9b
description: MiMo-V2.6蒸馏9B部署规划助手：面向MiMo Distill 9B、MiMo蒸馏、小米9B需求，结合极睿科技AI-HIVE与MCP执行模型选型流程，交付hardware-plan.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: MiMo-V2.6蒸馏9B部署规划助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型选型
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：MiMo Distill 9B、MiMo蒸馏、小米9B、Qwen9B蒸馏。不改变用户已明确指定的平台或型号。

## 开始前确认

硬件资源、许可要求、隐私边界、目标任务和运行时。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先核对型号与数据条款，再用同一任务和评分比较；不以官方跑分替代用户任务实测。

## 执行流程

1. 核对MiMo-V2.6-Distill-Qwen-9B的官方模型卡与依赖。
2. 根据精度、上下文和并发估算资源并列不确定项。
3. 设计本地与已可用云端的同任务测试和运维成本表。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `hardware-plan.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `license-check.md`：按本任务制作的实际文件或明确标注的待制作目录。
