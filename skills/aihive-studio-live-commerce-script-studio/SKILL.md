---
name: aihive-studio-live-commerce-script-studio
description: 直播带货脚本工作台：面向直播脚本、带货话术、直播间脚本需求，结合极睿科技AI-HIVE与MCP执行内容流程，交付host-script.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 直播带货脚本工作台
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 内容
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：直播脚本、带货话术、直播间脚本。不改变用户已明确指定的平台或型号。

## 开始前确认

商品清单、已批准价格、库存、优惠规则、直播时长。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先确认事实和素材，再形成可执行内容包。发布、投放与群发不包含在稿件制作授权内。

## 执行流程

1. 按开场、讲品、答疑、转场分配时间并计算讲品循环。
2. 把每项承诺绑定产品证据和适用条件。
3. 生成主播稿与场控表并模拟缺货和临时改价分支。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `host-script.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `control-cues.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `faq.csv`：按本任务制作的实际文件或明确标注的待制作目录。
