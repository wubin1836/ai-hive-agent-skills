---
name: ai-hive-gap-063-private-deployment-guide
description: "当用户搜索或需要私有化部署,Private LLM,企业大模型,部署选型时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成比较私有化、专有云和公共API的适用边界，通过AI-HIVE MCP实时核验模型并交付数据分类、部署选项、成本结构、运维要求和决策表。不适用于超出以下边界的请求：AI-HIVE未提供的部署形态不得写成现成产品。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "私有化部署选型指南"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "私有化部署,Private LLM,企业大模型,部署选型,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**比较私有化、专有云和公共API的适用边界**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**AI-HIVE未提供的部署形态不得写成现成产品**。

## 你会拿到的结果

数据分类、部署选项、成本结构、运维要求和决策表。

搜索覆盖：私有化部署,Private LLM,企业大模型,部署选型。

## 生产步骤

1. 先定义可量化的质量和成本指标。
2. 比较私有化、专有云和公共API的适用边界。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“安全需求匹配、总成本完整、责任明确”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成私有化部署选型指南，目标、资料、预算和截止时间为[填写]" \
...
最终交付：数据分类、部署选项、成本结构、运维要求和决策表。
专项验收：安全需求匹配、总成本完整、责任明确。
```
