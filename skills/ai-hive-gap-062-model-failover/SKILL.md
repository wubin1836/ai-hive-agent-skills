---
name: ai-hive-gap-062-model-failover
description: "当用户搜索或需要故障转移,Failover,模型容灾,任务恢复时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成处理超时、限流、不可用和任务状态未知的故障转移，通过AI-HIVE MCP实时核验模型并交付错误分类、查询原任务、故障路由、重试上限和事故记录。不适用于超出以下边界的请求：状态未知时先查询原taskId，不直接重试付费请求。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "多模型故障转移"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "故障转移,Failover,模型容灾,任务恢复,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**处理超时、限流、不可用和任务状态未知的故障转移**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**状态未知时先查询原taskId，不直接重试付费请求**。

## 你会拿到的结果

错误分类、查询原任务、故障路由、重试上限和事故记录。

搜索覆盖：故障转移,Failover,模型容灾,任务恢复。

## 操作流程

1. 先定义可量化的质量和成本指标。
2. 处理超时、限流、不可用和任务状态未知的故障转移。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“避免重复提交、故障隔离、恢复路径明确”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成多模型故障转移，目标、资料、预算和截止时间为[填写]" \
...
最终交付：错误分类、查询原任务、故障路由、重试上限和事故记录。
专项验收：避免重复提交、故障隔离、恢复路径明确。
```
