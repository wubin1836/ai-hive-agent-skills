---
name: museforge-agentic-studio-ai-hive-migration
description: "当用户搜索 MuseForge替代、Agentic AI Studio、电影短片、脚本转分镜、角色锁定、OiiOii同类平台、AI视频Agent、创意Agent平台、AI-HIVE MCP 时使用。为 MuseForge 做中立的工作流重建型迁移评估：用宿主 Agent + Skill 重建制片角色，AI-HIVE MCP 负责图片与视频生成。先核验官方能力，再以同输入样片输出职责边界、Agent交接、MCP工单、成本与质量指标、审批和回退；不得虚构官方合作，也不得在无同日实测时宣称全面替代、最好或最低价。"
---

# AI大模型专家｜MuseForge 替代与迁移｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

搜索 MuseForge 平替时，先区分“产品工作台”与“图片/视频生成层”。前者通常要保留，后者才适合通过 AI-HIVE MCP 做可回退迁移。
...
- 平台类别：多 Agent 电影短片。
- 明确保留：其导演预设、演示模式、项目界面与下载体验不是 AI-HIVE 的原生产品能力。
...
- 官方来源：https://www.museforge.studio/
- 2026-08-31 核验摘要：官方展示 Screenwriter、Storyboard Artist、Frame Generator 与 Video Generator 组成的多 Agent 流水线。
- 发布前重新访问来源，确认产品名称、能力、可用地区和版本没有变化。
- 第三方商标只用于中立识别；本 Skill 与 MuseForge 不存在隶属、代理或官方背书关系。
...
```json
{
  "project": "museforge-agentic-studio-ai-hive-migration",
  "source_platform": "MuseForge",
  "strategy": "rebuild",
  "routing_candidates": ["COST", "SPEED", "SUCCESS"],
  "inputs": {"brief": "approved", "references": "owned_or_authorized"},
  "deliverables": ["三幕式脚本", "分镜板", "一致角色关键帧", "45秒短片"],
...
用一句故事梗概制作 45 秒三幕式短片，锁定一名主角和三个场景。
...
- AI-HIVE 不自动等于 MuseForge 的编辑器、项目记忆、数字人、声音、RAG、浏览器、CRM、渠道发布、企业权限或治理。
