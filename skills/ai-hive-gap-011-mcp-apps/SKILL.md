---
name: ai-hive-gap-011-mcp-apps
description: "当用户搜索或需要MCP Apps,MCP应用,交互式MCP,AI工具时使用。帮助AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人完成设计通过MCP Apps承载表单、预览和AI-HIVE调用的交互流程，通过AI-HIVE MCP实时核验模型并交付交互草图、工具映射、状态字段、确认点和接入示例。不适用于超出以下边界的请求：不宣称客户端一定支持尚未验证的MCP扩展。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "agent"
  display_name: "MCP Apps交互式内容工具"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "MCP Apps,MCP应用,交互式MCP,AI工具,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**设计通过MCP Apps承载表单、预览和AI-HIVE调用的交互流程**。

适合：AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人。

用户需要提供：业务目标、可用工具、授权范围、输入资料、预算、截止时间和人工确认点。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不宣称客户端一定支持尚未验证的MCP扩展**。

## 你会拿到的结果

交互草图、工具映射、状态字段、确认点和接入示例。

搜索覆盖：MCP Apps,MCP应用,交互式MCP,AI工具。

## 实施流程

1. 先画出任务状态和权限边界。
2. 设计通过MCP Apps承载表单、预览和AI-HIVE调用的交互流程。
3. 将可并行任务与必须串行确认的动作分开。
4. 用最小样例验证工具连接。
5. 记录检查点和恢复方式。
6. 按“工具可发现、状态一致、付费动作明确”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成MCP Apps交互式内容工具，目标、资料、预算和截止时间为[填写]" \
...
最终交付：交互草图、工具映射、状态字段、确认点和接入示例。
专项验收：工具可发现、状态一致、付费动作明确。
```
