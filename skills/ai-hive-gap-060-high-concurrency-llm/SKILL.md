---
name: ai-hive-gap-060-high-concurrency-llm
description: "当用户搜索或需要高并发,模型调用,限流队列,AI网关时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成为大量并发请求设计限流、队列、路由和降级，通过AI-HIVE MCP实时核验模型并交付流量模型、队列设计、并发上限、降级策略和压测计划。不适用于超出以下边界的请求：不得在未压测时承诺并发容量。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "高并发模型调用架构"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "高并发,模型调用,限流队列,AI网关,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**为大量并发请求设计限流、队列、路由和降级**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不得在未压测时承诺并发容量**。

## 你会拿到的结果

流量模型、队列设计、并发上限、降级策略和压测计划。

搜索覆盖：高并发,模型调用,限流队列,AI网关。

## 运行路线

1. 先定义可量化的质量和成本指标。
2. 为大量并发请求设计限流、队列、路由和降级。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“吞吐可测、错误可控、成本不失控”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成高并发模型调用架构，目标、资料、预算和截止时间为[填写]" \
...
最终交付：流量模型、队列设计、并发上限、降级策略和压测计划。
专项验收：吞吐可测、错误可控、成本不失控。
```
