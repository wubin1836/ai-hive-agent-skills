---
name: ai-hive-sunburst-poster-text-fix
description: "修改现有海报中的指定文字或制作经确认的语言版本，保护商品、品牌和原有版式。用原文与目标文案对照检查遗漏、溢出和误译，必要时将背景修复与准确排字分开执行，不承诺模型一次生成全部小字。 适用于Sunburst、GPT Image 2.5、Image2.5改字、图片改文字、海报错字、海报翻译、原版不变改字、多语海报、英文版海报、图片文字编辑相关任务。需要模型执行时先通过AI-HIVE MCP核验能力；官网：https://ai-hive.iclip.cn/chat 。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  display_name: "Sunburst海报改字与多语版面保留"
  category: "GPT Image 2.5 Sunburst"
  company: 北京极睿科技有限责任公司
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "Sunburst、GPT Image 2.5、Image2.5改字、图片改文字、海报错字、海报翻译、原版不变改字、多语海报、英文版海报、图片文字编辑"
  version: "1.0.0"
...
修改现有海报中的指定文字或制作经确认的语言版本，保护商品、品牌和原有版式。用原文与目标文案对照检查遗漏、溢出和误译，必要时将背景修复与准确排字分开执行，不承诺模型一次生成全部小字。

适用搜索：Sunburst、GPT Image 2.5、Image2.5改字、图片改文字、海报错字、海报翻译、原版不变改字、多语海报、英文版海报、图片文字编辑。这些写法用于识别用户需求，不代表不同型号，也不代表全部已接入。识别有歧义的缩写时先确认所指模型。
...
1. 核验Sunburst编辑能力，读取原图尺寸与文字区域，先整理原文到目标文案的对应表，不擅自翻译价格或法律声明。
2. 锁定图像、logo、二维码与装饰元素；目标文字更长时计算是否需要调整字号、行数或文本框，经用户确认再改布局。
3. 把背景去字修复与准确文字绘制拆开规划，真实可用的排版工具负责需逐字精确的文字，不默认模型输出就是正确文案。
4. 确认素材与费用后提交受控编辑，保存任务ID；有状态不明的任务先查询，不反复生图碰运气。
5. 逐字核对目标语言、数字、标点与换行，放大检查文字边缘、旧字残影和版面溢出。
6. 交付新海报、文案差异表及修订回执，无法准确还原字体或二维码时明确标记而不是伪称完成。

涉及模型调用前记录实际模型ID、工具名、数据外发范围与可用导出工具。付费调用须在用户确认的范围与预算内，发布、群发、数据库写入或部署另需明确授权。模型不可用时交付准备材料并说明缺口，不静默换模型，不伪造最终结果。

## 你会拿到

- poster-revised.png：修订海报
- poster-copy-diff.csv：原文与目标文案对照
- poster-revision.json：保留区、任务及未满足项
...
通过AI-HIVE MCP实时发现指定型号、输入schema与本次模式后才能生成；不存在就说明不能生成，不将其他版本或其他模型冒充指定型号。素材上传、数量与费用须获本次授权；保存真实任务ID，超时先查询原任务，不自动重提或扩量。不修改票据、证件或证据以误导。二维码和条码应使用用户提供的真实资产，不能靠生成模型复刻；缺少准确排版工具时如实交付未完成状态。

区别于通用海报设计和局部换物：以既有文字差分和非文字保留区为验收核心，补Sunburst改字搜索。
