---
name: aihive-muse-image-poster
description: Muse Image活动海报助手：面向Muse海报、Facebook生图、活动海报制作需求，结合极睿科技AI-HIVE与MCP执行模型图片流程，交付poster.png等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Muse Image活动海报助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：Muse海报、Facebook生图、活动海报制作。不改变用户已明确指定的平台或型号。

## 开始前确认

活动信息、品牌色、素材权利、准确文案和二维码目标。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 先锁定活动日期、价格和报名链接。
2. 图像模型负责视觉底图，准确文字与可扫码二维码由可验证排版工具制作。
3. 导出后做OCR与扫码检查。
...
- `poster.png`：按本任务制作的实际文件或明确标注的待制作目录。
- `copy-lock.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `scan-check.md`：按本任务制作的实际文件或明确标注的待制作目录。

二维码未经扫描不能标可用；AI生成文字需核对，不能伪造优惠。
