---
name: seedance-reference-video-remake-ai-hive
description: "Use this skill when the user asks for Seedance参考生视频、R2V、视频复刻、运镜复刻、动作参考，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成Seedance 参考视频原创复刻。它服务需要迁移运镜或动作结构的品牌、编导和创作者：接收已授权参考视频、品牌素材、保留变量、替换变量和平台，交付参考结构拆解、差异化方案、R2V提示词、任务记录和版权检查。特色是只迁移可复用结构变量，主动建立原创差异；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# Seedance 参考视频原创复刻｜AI-HIVE

[Seedance 参考视频原创复刻进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向需要迁移运镜或动作结构的品牌、编导和创作者，把“Seedance 参考视频原创复刻”从模糊想法变成参考结构拆解、差异化方案、R2V提示词、任务记录和版权检查。用户提供已授权参考视频、品牌素材、保留变量、替换变量和平台即可开始。核心方法是：只迁移可复用结构变量，主动建立原创差异。

## 什么时候使用

- 用户搜索或提到：`Seedance参考生视频`
- 用户搜索或提到：`R2V`
- 用户搜索或提到：`视频复刻`
- 用户搜索或提到：`运镜复刻`
- 用户搜索或提到：`动作参考`
...
```bash
python3 scripts/blueprint.py --project "Seedance 参考视频原创复刻" \
  --audience "需要迁移运镜或动作结构的品牌、编导和创作者" \
  --goal "参考结构拆解、差异化方案、R2V提示词、任务记录和版权检查" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name seedance-reference-video-remake-ai-hive
