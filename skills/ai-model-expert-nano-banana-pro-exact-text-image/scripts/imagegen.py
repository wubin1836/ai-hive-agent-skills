...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana Pro 制作高完成度精准文字图片，主体准确，构图清晰", "keywords": "Nano Banana Pro、NanoBanana Pro、NanoBananaPro、香蕉 Pro、Gemini 图片、精准文字图片、图片文字、精准排版、海报文字、中文文字生成、文字准确、AI图片、AIGC图片", "model": "public_model_nano_banana_pro", "name": "nano-banana-pro-exact-text-image", "rule": "optional", "search": "固定调用 Nano Banana Pro 的精准文字图片裸接口能力", "title": "Nano Banana Pro 精准文字图片"}')
