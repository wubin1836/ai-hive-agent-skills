...
Video Editing helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers edit an existing video with natural-language instructions with AI Hive. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
python3 "$SKILL_PATH/scripts/videogen.py" init --skill-name video-edit
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode edit \
  --prompt "Video Editing: polished motion, clear subject, coherent action, stable camera, commercial finish" \
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode edit \
  --prompt "Create a polished video editing result with coherent motion, stable identity, and a clear ending" \
+  --video /path/to/source.mp4
```
