---
name: aihive-studio-private-domain-tag-system-build
description: 私域标签体系搭建：面向私域标签、客户标签体系、标签字典需求，结合极睿科技AI-HIVE与MCP执行运营流程，交付tag-dictionary.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：私域标签、客户标签体系、标签字典。不改变用户已明确指定的平台或型号。

## 开始前确认

业务目标、脱敏客户字段、标签用途和更新频率。仅追问影响执行的缺项，不要求用户重新提供已有资料。

优先交付可配置规则、SOP和数据文件。部署、CRM写回、发送消息或投放需要真实系统接口和单独授权。

## 执行流程

1. 区分事实标签与推断标签并去除不必要敏感属性。
2. 定义触发条件、有效期、冲突优先级和数据来源。
3. 用小样本标注并交付映射与维护规则。
...
- `tag-dictionary.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `mapping-rules.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `sample-tags.csv`：按本任务制作的实际文件或明确标注的待制作目录。
