...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Image2 制作高完成度多参考图生成，主体准确，构图清晰", "keywords": "Image2、Image 2、GPT Image 2、GPTImage2、OpenAI 图片、ChatGPT 图片、多参考图生成、多图融合、多参考图、多图合成、角色风格参考、素材合成、AI图片、AIGC图片", "model": "public_model_gpt_image_2", "name": "image-2-multi-reference-image", "rule": "multi", "search": "固定调用 Image2 的多参考图生成裸接口能力", "title": "Image2 多参考图生成"}')
