...
SKILL_CONFIG = json.loads("{\"model\": \"public_model_seedream_5_0_lite\", \"name\": \"seedream-5-lite-multi-reference-image\", \"rule\": \"multi\", \"title\": \"Seedream 5.0 Lite Multi-Reference Image Generation\"}")

def _validate_image_inputs(args):
    count = len(args.image or [])
    rule = SKILL_CONFIG.get("rule", "optional")
    if rule == "none" and count:
        raise SystemExit("This text-to-image skill does not accept reference images.")
    if rule == "image" and count < 1:
        raise SystemExit("This skill requires at least one --image.")
    if rule == "multi" and count < 2:
        raise SystemExit("This multi-reference skill requires at least two --image inputs.")
...
def skill_generate(client, args):
    ...
    print(f"Model: {forwarded.model}")
    cmd_image(client, forwarded)

def build_skill_parser():
    parser = argparse.ArgumentParser(
        prog="imagegen.py",
        description=SKILL_CONFIG["title"] + " — AI Hive OpenAPI image skill",
    )
    sub = parser.add_subparsers(dest="command", required=True)
    p = sub.add_parser("generate", help="Generate or edit image")
    p.add_argument("--prompt", required=True, help="Image description or edit instructions")
    ...
    add_common_args(p)
    p = sub.add_parser("upload", help="Upload image")
