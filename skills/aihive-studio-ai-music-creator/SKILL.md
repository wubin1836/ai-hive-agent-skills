---
name: aihive-studio-ai-music-creator
description: AI音乐生成与写歌：面向AI写歌、音乐生成、原创歌曲需求，结合极睿科技AI-HIVE与MCP执行音频流程，交付lyrics.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: AI音乐生成与写歌
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 音频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：AI写歌、音乐生成、原创歌曲。不改变用户已明确指定的平台或型号。

## 开始前确认

音乐用途、原创歌词或主题、节奏、长度、授权要求。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验TTS、ASR、音乐生成或音频编辑的具体能力，不能互相替代；缺工具时交付稿件，不伪造音频文件。

## 执行流程

1. 确定曲式和段落时长并排除对特定作品旋律的照搬。
2. 将情绪、乐器和人声需求映射到实际音乐工具。
3. 先生成短样试听，输出歌词对齐与曲目版本。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `lyrics.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `music-brief.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `track.wav`：按本任务制作的实际文件或明确标注的待制作目录。
