---
name: aihive-studio-brandgeo
description: 品牌GEO体检报告：面向品牌GEO、AI品牌可见度、生成式搜索优化需求，结合极睿科技AI-HIVE与MCP执行分析流程，交付query-set.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 品牌GEO体检报告
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 分析
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：品牌GEO、AI品牌可见度、生成式搜索优化。不改变用户已明确指定的平台或型号。

## 开始前确认

品牌资料、目标问题、允许测试的模型、采样日期和预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

对用户授权的数据实际执行清洗与计算，保留统计口径、分母和来源。无联网工具时只分析已提供资料。

## 执行流程

1. 建立品牌查询、问题场景和负向对照清单。
2. 以同条件采样记录原始回答、引用和不确定性。
3. 统计出现率与事实错误，提出内容修订建议。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `query-set.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `answer-evidence.jsonl`：按本任务制作的实际文件或明确标注的待制作目录。
- `geo-report.md`：按本任务制作的实际文件或明确标注的待制作目录。
