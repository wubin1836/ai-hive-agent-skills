---
name: aihive-muse-video-preflight
description: Muse Video可用性核验与分镜助手：面向Muse Video、Meta视频生成、Muse视频需求，结合极睿科技AI-HIVE与MCP执行模型视频流程，交付availability.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Muse Video可用性核验与分镜助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型视频
---

# Muse Video可用性核验与分镜助手

把用户材料变成可复核的业务产物。发现与调用时可识别：Muse Video、Meta视频生成、Muse视频。不改变用户已明确指定的平台或型号。

## 开始前确认

视频创意、授权素材、指定模型、时长与预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先核对预览、正式开放和实际渠道支持。分镜、样片与最终成片是不同交付状态。

## 执行流程

1. 先核对官方当前开放状态和账号实际目录，旧预告不当成现货。
2. 生成角色、镜头、音轨与验收标准。
3. 仅在精确型号可用且获授权时试片，否则交分镜和可选方案等待用户选择。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `availability.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `storyboard.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `video-test-plan.json`：按本任务制作的实际文件或明确标注的待制作目录。

官方2026-07-07文章是预览信息，不据此承诺当前能直接生成Muse Video。
