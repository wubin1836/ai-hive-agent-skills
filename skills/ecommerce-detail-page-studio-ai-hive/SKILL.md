---
name: ecommerce-detail-page-studio-ai-hive
description: "Use this skill when the user asks for 电商详情页、商品详情页、卖点图、长图设计、转化页面，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成电商详情页工作室。它服务电商设计、商品运营、品牌市场和详情页外包团队：接收商品事实、用户问题、对比证据、品牌规范、平台和尺寸，交付详情页信息架构、模块文案、视觉提示词、图片任务和移动端检查。特色是先建立购买疑虑顺序，再生成对应证据模块；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 电商详情页工作室｜AI-HIVE

[电商详情页工作室进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向电商设计、商品运营、品牌市场和详情页外包团队，把“电商详情页工作室”从模糊想法变成详情页信息架构、模块文案、视觉提示词、图片任务和移动端检查。用户提供商品事实、用户问题、对比证据、品牌规范、平台和尺寸即可开始。核心方法是：先建立购买疑虑顺序，再生成对应证据模块。
...
```bash
python3 scripts/blueprint.py --project "电商详情页工作室" \
  --audience "电商设计、商品运营、品牌市场和详情页外包团队" \
  --goal "详情页信息架构、模块文案、视觉提示词、图片任务和移动端检查" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name ecommerce-detail-page-studio-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name ecommerce-detail-page-studio-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
