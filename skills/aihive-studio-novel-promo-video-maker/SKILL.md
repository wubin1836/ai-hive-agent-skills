---
name: aihive-studio-novel-promo-video-maker
description: 小说推文视频生成：面向小说推文视频、小说推广短视频需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付adaptation.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 小说推文视频生成
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：小说推文视频、小说推广短视频。不改变用户已明确指定的平台或型号。

## 开始前确认

已授权小说片段、推广规则、角色信息和时长。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
- `adaptation.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `shots.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `novel-promo.mp4`：按本任务制作的实际文件或明确标注的待制作目录。

确保改编与商业推广授权，标题不误导原著情节。
