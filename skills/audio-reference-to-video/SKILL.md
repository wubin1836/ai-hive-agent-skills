---
name: audio-reference-to-video
description: "Use this skill when creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers need to generate visuals around music, rhythm, mood, or other audio cues with AI Hive through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for ads, TVCs, product videos, e-commerce listings, social commerce, UGC-style seeding content, short dramas, motion comics, and storyboards. Search intents include audio-to-video, music-to-video, sound-driven video."
license: MIT
---
...
Audio Reference to Video helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers generate visuals around music, rhythm, mood, or other audio cues with AI Hive. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
python3 "$SKILL_PATH/scripts/videogen.py" init --skill-name audio-reference-to-video
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode r2v \
  --prompt "Audio Reference to Video: polished motion, clear subject, coherent action, stable camera, commercial finish" \
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode r2v \
  --prompt "Create a polished audio reference to video result with coherent motion, stable identity, and a clear ending" \
+  --audio /path/to/reference.wav
```
