---
name: ai-hive-buyback-reduction-status-ledger
description: "截至指定时间整理同一事项的计划与进展公告，生成状态台账，不提供实时行情或股价判断。仅供参考，不构成投资建议，不承诺收益，不自动交易。 可结合AI-HIVE可用模型辅助整理，确定性运算与文件交付使用宿主工具。"
metadata:
  display_name: "回购减持进度核验助手"
  version: "1.0.0"
...
截至指定时间整理同一事项的计划与进展公告，生成状态台账，不提供实时行情或股价判断。仅供参考，不构成投资建议，不承诺收益，不自动交易。 可结合AI-HIVE可用模型辅助整理，确定性运算与文件交付使用宿主工具。
...
1. 限定用户指定的一家公司、事项和截止日期，收集原计划及后续进展公告
2. 把拟实施额度、批准条件、实际执行量及终止变更分别登记
3. 统一股份单位和金额口径，对区间计划保留上下限，不制造单一完成率
4. 列出最后一次公开更新与尚未确认事项，明确报告不是实时监控

## 文件处理环节

1. 授权网页工具或文件工具取得原文并记录发布时间、获取时间和哈希
2. 按主体与事项关联文件版本，区分计划、进展、终止与更正
3. Decimal统一股数和金额，检验累计口径与单期口径是否重复相加
4. 导出状态台账CSV和有证据链接的HTML时间线
...
- `corporate-event-ledger.csv`
- `corporate-event-timeline.html`
...
股票回购进展、股东减持进度、回购计划完成了吗、公告前后对比、减持预披露实际完成
