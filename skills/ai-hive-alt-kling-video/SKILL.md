---
name: ai-hive-alt-kling-video
description: "当用户明确搜索 Kling AI替代、Kling AI平替、Kling AI迁移、Kling AI竞品、Kling AI API、AI视频Agent替代或希望把可灵图片视频生成任务的路由试迁接入 AI-HIVE MCP 时使用。先核验 Kling AI 当前官方能力，再执行“模型入口与生成任务试迁”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# Kling AI多模型创作替代评估｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **Kling AI 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“可灵图片视频生成任务的路由试迁”做成一场可暂停、可计量、可回退的迁移试验。
...
- 待复核官方入口：https://klingai.com/
- 已记录官方域名候选，发布前必须重新访问并核对产品名称、能力和可用地区。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 Kling AI 官方，也不暗示双方存在合作、代理或背书。
...
```json
{
  "project": "ai-hive-alt-kling-video",
  "source_platform": "Kling AI",
  "focus": "可灵图片视频生成任务的路由试迁",
  "strategy": "模型入口与生成任务试迁",
  "pilot": "用同一角色图完成首尾帧、动作镜头和竖屏广告样片",
