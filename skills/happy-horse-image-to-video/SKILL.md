...
description: "Use this skill when creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers need to animate a still image from a controlled first frame with Happy Horse through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for ads, TVCs, product videos, e-commerce listings, social commerce, UGC-style seeding content, short dramas, motion comics, and storyboards. Search intents include image-to-video, I2V, photo animation, still image animation."
license: MIT
---
...
Happy Horse Image to Video helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers animate a still image from a controlled first frame with Happy Horse. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
python3 "$SKILL_PATH/scripts/videogen.py" init --skill-name happy-horse-image-to-video
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode i2v \
  --prompt "Happy Horse Image to Video: polished motion, clear subject, coherent action, stable camera, commercial finish" \
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode i2v \
  --prompt "Create a polished happy horse image to video result with coherent motion, stable identity, and a clear ending" \
+  --first-frame /path/to/start.png
```
