---
name: aihive-studio-internal-comms
description: 内部沟通文档撰写：面向内部沟通文档、公司通知、员工公告需求，结合极睿科技AI-HIVE与MCP执行文档流程，交付announcement.docx等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 内部沟通文档撰写
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 文档
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：内部沟通文档、公司通知、员工公告。不改变用户已明确指定的平台或型号。

## 开始前确认

公告事项、受众、事实来源、保密级别和发送范围。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 区分已决定、拟议与需确认事项。
2. 按受众制作摘要、行动项和FAQ。
3. 核对时间、责任人与不可披露信息后交付待审稿。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `announcement.docx`：按本任务制作的实际文件或明确标注的待制作目录。
- `faq.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `action-items.csv`：按本任务制作的实际文件或明确标注的待制作目录。
