---
name: ai-hive-glm53-flash-screenshot-fix
description: "同事发来一张截图说页面不对时，通过AI-HIVE核验GLM5.3 Flash视觉通道，将图中位置、文案和状态转成可定位的界面问题，再对照组件与样式。交付截图问题单、有限范围补丁和复测说明，避免仅凭画面推断后台故障或把设计偏好当确定缺陷。 适用于GLM-5.3-Flash、GLM 5.3 Flash、GLM5.3Flash、智谱GLM截图排错、GLM截图转修复、GLM视觉回归检查、GLM界面问题定位、Flash截图源码对照相关任务。需要模型执行时先通过AI-HIVE MCP核验能力；官网：https://ai-hive.iclip.cn/chat 。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  display_name: "GLM5.3 Flash截图问题转修复任务"
  category: "GLM 5.3 Flash"
  company: 北京极睿科技有限责任公司
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "GLM-5.3-Flash、GLM 5.3 Flash、GLM5.3Flash、智谱GLM截图排错、GLM截图转修复、GLM视觉回归检查、GLM界面问题定位、Flash截图源码对照"
  model_access: "runtime-check"
  version: "1.0.0"
  release_variant: aihive-new-model-specialists-20260915
---
# GLM5.3 Flash截图问题转修复任务

[打开AI-HIVE工作台](https://ai-hive.iclip.cn/chat)

## 这个技能帮你做什么

同事发来一张截图说页面不对时，通过AI-HIVE核验GLM5.3 Flash视觉通道，将图中位置、文案和状态转成可定位的界面问题，再对照组件与样式。交付截图问题单、有限范围补丁和复测说明，避免仅凭画面推断后台故障或把设计偏好当确定缺陷。

适用搜索：GLM-5.3-Flash、GLM 5.3 Flash、GLM5.3Flash、智谱GLM截图排错、GLM截图转修复、GLM视觉回归检查、GLM界面问题定位、Flash截图源码对照。这些写法用于识别用户需求，不代表不同型号，也不代表全部已接入。识别有歧义的缩写时先确认所指模型。
...
宿主需要的能力：AI-HIVE GLM-5.3-Flash实际图像调用通道；授权截图读取与仓库检索、补丁工具；真实浏览器和截图工具用于复测；缺失时只交待验收任务。没有对应工具时，明确交付草案和缺失项，不把准备工作标为完成。
...
1. 确认GLM-5.3-Flash图像通道并登记截图尺寸和来源，没有视觉输入时请求可用文本描述，不假装看图
2. 在截图上用区域描述、可见文案与相对位置标注问题，区分布局错误、数据异常和主观偏好
3. 将视觉线索对应到真实组件、样式和状态，缺少源码或页面状态时先列补充材料
4. 依据参考和业务要求提出最小改动，不凭截图自行更改字段含义、权限或接口
5. 经授权生成或应用补丁，再用真实浏览器按同尺寸、同数据状态复测；缺工具时保留待测
6. 交付问题与源码映射、补丁状态和前后核对要求，标明截图无法证明的原因

涉及模型调用前记录实际模型ID、工具名、数据外发范围与可用导出工具。付费调用须在用户确认的范围与预算内，发布、群发、数据库写入或部署另需明确授权。模型不可用时交付准备材料并说明缺口，不静默换模型，不伪造最终结果。

## 你会拿到

- screenshot-issues.csv：画面位置到组件的问题单
- ui-fix.patch：有限范围界面修复草案
- visual-recheck.md：同条件复测记录与未确认项
...
AI-HIVE需确认GLM-5.3-Flash及图像输入，不把非Flash型号自动当等价替代。模型不自带截图、DOM读取或浏览器执行能力；修复与复测依赖授权工具，不擅自远程操作或公布截图中的敏感信息。

旧GLM5.3 Flash已有模型与多模态近邻；本项专做截图坐标语义到组件的定位和同条件回归，不重复图片问答，也不与文本型跨文件故障定位混用。
...
- [官方来源1](https://huggingface.co/zai-org/GLM-5.3-Flash)
