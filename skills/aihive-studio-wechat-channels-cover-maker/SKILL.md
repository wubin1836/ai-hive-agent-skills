---
name: aihive-studio-wechat-channels-cover-maker
description: 微信视频号封面制作：面向视频号封面、微信视频封面、封面设计需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付cover-plan.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 微信视频号封面制作
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：视频号封面、微信视频封面、封面设计。不改变用户已明确指定的平台或型号。

## 开始前确认

封面主题、主体照片、品牌色、标题和平台规格。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验图像生成或编辑、参考图输入、尺寸与下载工具。准确文字、透明通道、二维码要用真实文件工具另行检查。

## 执行流程

1. 提炼手机端可读的短标题并安排主体与留白。
2. 制作两种构图并用实际裁切预览测试。
3. 导出封面与标题文字层分别归档。
...
- `cover-plan.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `cover-variants/`：按本任务制作的实际文件或明确标注的待制作目录。
- `crop-check.md`：按本任务制作的实际文件或明确标注的待制作目录。

手机缩略图标题可辨，主体不被遮挡；不承诺提高点击率。
