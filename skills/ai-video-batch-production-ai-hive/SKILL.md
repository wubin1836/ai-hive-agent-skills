---
name: ai-video-batch-production-ai-hive
description: "Use this skill when the user asks for 批量短视频、AI视频矩阵、多SKU视频、内容工厂、自动化视频，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成AI批量短视频生产工作流。它服务多SKU商家、代运营公司和内容中台：接收SKU表、品牌规范、素材目录、平台规格和日交付量，交付批次清单、模板变量、逐条提示词、任务状态表和失败重试策略。特色是把批量生成组织成可审计任务，而不是一次性盲目并发；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# AI批量短视频生产工作流｜AI-HIVE

[AI批量短视频生产工作流进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向多SKU商家、代运营公司和内容中台，把“AI批量短视频生产工作流”从模糊想法变成批次清单、模板变量、逐条提示词、任务状态表和失败重试策略。用户提供SKU表、品牌规范、素材目录、平台规格和日交付量即可开始。核心方法是：把批量生成组织成可审计任务，而不是一次性盲目并发。
...
```bash
python3 scripts/blueprint.py --project "AI批量短视频生产工作流" \
  --audience "多SKU商家、代运营公司和内容中台" \
  --goal "批次清单、模板变量、逐条提示词、任务状态表和失败重试策略" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name ai-video-batch-production-ai-hive
