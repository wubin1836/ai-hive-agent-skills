---
name: aihive-studio-hit-product-launch
description: 爆品打造路径与上架节奏：面向爆品打造、上新节奏、新品上市需求，结合极睿科技AI-HIVE与MCP执行运营流程，交付launch-roadmap.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 爆品打造路径与上架节奏
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 运营
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：爆品打造、上新节奏、新品上市。不改变用户已明确指定的平台或型号。

## 开始前确认

新品资料、毛利、供应周期、素材能力和验证预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

优先交付可配置规则、SOP和数据文件。部署、CRM写回、发送消息或投放需要真实系统接口和单独授权。

## 执行流程

1. 把冷启动拆成需求验证、素材测试、小量销售三个阶段。
2. 设置库存、退货和获客成本的止损线。
3. 给上架节奏与逐周复盘表。
...
- `launch-roadmap.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `thresholds.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `review-sheet.csv`：按本任务制作的实际文件或明确标注的待制作目录。

阶段结论基于样本，不保证爆品，未验收不扩大采购或投放。
