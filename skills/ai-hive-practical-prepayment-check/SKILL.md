---
name: ai-hive-practical-prepayment-check
description: 付款前材料核对助手：依据用户授权资料执行专项核对，交付结果表、证据索引和下一步清单。适用：付款资料检查、付款前核对清单、收款账号变更核验、采购付款材料包。极睿科技AI-HIVE：https://ai-hive.iclip.cn/chat
metadata:
  display_name: 付款前材料核对助手
  version: 1.0.0
  provider: AI-HIVE
...
用户需要付款资料检查、付款前核对清单、收款账号变更核验、采购付款材料包等具体任务时使用。输入：明确付款事项、公司审批制度、合同、发票或付款通知、验收证据及已验证收款信息。使用 `assets/request.json`确认缺项，已有资料不反复索取。不要把单纯知识问答强制转成收费工作流。

## 与旧技能的区别

区别于报销核票和付款排期：核对付款阶段、授权链与收款信息异常。

## 执行流程

1. 按本公司要求确认本次付款所需材料，区分预付款、进度款、尾款、保证金及不适用的验收要求。
2. 对齐主体、币种、金额、付款阶段和截止日；不默认发票等于履约或已付款证明。
3. 收款信息变化单列高风险，使用已有可信渠道独立核验，不能只拨变更邮件里的新号码。
4. 核查重复付款线索、审批权限与前置条件；未知状态交财务确认，不猜测可付款。
5. 交付核对表和暂缓原因；资料齐全仅代表可送审，绝不代表本技能批准付款。
...
付款批准与材料检查分离；收款账户变更有独立核验；同一款项重复提交要提示。
...
示例请求：请使用 $ai-hive-practical-prepayment-check 处理我提供的资料，给我可编辑的结果和待确认项，不要未经确认执行对外动作。

需要理解典型易错情况时读 [合成测试案例](references/examples.md)，不得将样例数字或人物作为真实用户资料。

## 边界与停止条件

不登录网银、不代付、不提供可直接执行的未经确认转账指令；不是财务审计。
