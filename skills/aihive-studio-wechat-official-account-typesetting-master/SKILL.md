---
name: aihive-studio-wechat-official-account-typesetting-master
description: 微信公众号排版大师：面向公众号排版、微信文章排版、图文排版需求，结合极睿科技AI-HIVE与MCP执行文档流程，交付article.html等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 微信公众号排版大师
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 文档
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：公众号排版、微信文章排版、图文排版。不改变用户已明确指定的平台或型号。

## 开始前确认

定稿文章、图片、账号风格、移动端宽度和编辑器要求。仅追问影响执行的缺项，不要求用户重新提供已有资料。

需要真实文件读写、格式转换和预览工具；没有Word/PPT生成或渲染工具时明确降级为结构化草稿。

## 执行流程

1. 清理标题层级、段落间距和图注，保留引用。
2. 生成兼容性优先的图文HTML并检查外链和图片替代文字。
3. 在可用预览中核对移动端显示。
...
- `article.html`：按本任务制作的实际文件或明确标注的待制作目录。
- `assets/`：按本任务制作的实际文件或明确标注的待制作目录。
- `layout-check.md`：按本任务制作的实际文件或明确标注的待制作目录。
