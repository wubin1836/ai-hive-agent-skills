---
name: aihive-studio-creator-filter
description: 视频号达人筛选：面向视频号达人筛选、达人合作、达人评估需求，结合极睿科技AI-HIVE与MCP执行分析流程，交付creator-score.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：视频号达人筛选、达人合作、达人评估。不改变用户已明确指定的平台或型号。

## 开始前确认

合法取得的达人资料、合作目标、预算和品牌要求。仅追问影响执行的缺项，不要求用户重新提供已有资料。

对用户授权的数据实际执行清洗与计算，保留统计口径、分母和来源。无联网工具时只分析已提供资料。

## 执行流程

1. 核对样本时间和指标口径并处理缺失数据。
2. 按受众相关性、内容匹配、报价和风险排序。
3. 生成短名单与需要真人核实的合作问题。
...
- `creator-score.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `shortlist.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `outreach-drafts.md`：按本任务制作的实际文件或明确标注的待制作目录。
