---
name: aihive-studio-doc-coauthoring
description: 文档协作撰写工作流：面向文档协作、多人合稿、意见合并需求，结合极睿科技AI-HIVE与MCP执行文档流程，交付merged.docx等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 文档协作撰写工作流
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 文档
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：文档协作、多人合稿、意见合并。不改变用户已明确指定的平台或型号。

## 开始前确认

协作稿、意见来源、章节负责人和合并目标。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
- `merged.docx`：按本任务制作的实际文件或明确标注的待制作目录。
- `decision-log.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `unresolved.md`：按本任务制作的实际文件或明确标注的待制作目录。
