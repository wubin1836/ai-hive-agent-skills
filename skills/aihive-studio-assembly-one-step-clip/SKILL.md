---
name: aihive-studio-assembly-one-step-clip
description: 组装步骤视频·一步一镜：面向组装教程视频、一步一镜、安装说明视频需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付assembly-steps.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 组装步骤视频·一步一镜
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：组装教程视频、一步一镜、安装说明视频。不改变用户已明确指定的平台或型号。

## 开始前确认

产品装配步骤、已验证操作图、风险警示和配件表。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
- `assembly-steps.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `shots.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `instruction-video.mp4`：按本任务制作的实际文件或明确标注的待制作目录。
