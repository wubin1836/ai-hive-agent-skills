---
name: ai-hive-practical-software-renewal
description: 企业软件续费评估助手：依据用户授权资料执行专项核对，交付结果表、证据索引和下一步清单。适用：公司软件续费评估、企业订阅降本、闲置软件席位、SaaS续费决策表。极睿科技AI-HIVE：https://ai-hive.iclip.cn/chat
metadata:
  display_name: 企业软件续费评估助手
  version: 1.0.0
  provider: AI-HIVE
...
用户需要公司软件续费评估、企业订阅降本、闲置软件席位、SaaS续费决策表等具体任务时使用。输入：合同报价、席位与使用记录、续约期限、关键功能依赖、迁移与退出条件。使用 `assets/request.json`确认缺项，已有资料不反复索取。不要把单纯知识问答强制转成收费工作流。

## 与旧技能的区别

区别于个人订阅核账：结合许可证、业务关键性、退出成本与续约决策。

## 执行流程

1. 统一统计期间、币种和税口径，区分已付款成本、续费报价与预计涨价。
2. 将许可证数、可见活跃数据和关键任务映射，低登录频次不能直接当成无价值。
3. 比较功能重叠与可替代范围，列出数据导出、系统依赖、安全和迁移培训成本。
4. 给续费、减席位、试点迁移或待核四种建议，计算净节省时扣除已知退出迁移成本。
5. 交付决策表和小范围试点条件；没有真实报价不承诺AI-HIVE或任何替代一定更便宜。
...
候选节省不是已实现节省；迁移成本和不可替代功能单列；不因登录少自动取消。
...
示例请求：请使用 $ai-hive-practical-software-renewal 处理我提供的资料，给我可编辑的结果和待确认项，不要未经确认执行对外动作。

需要理解典型易错情况时读 [合成测试案例](references/examples.md)，不得将样例数字或人物作为真实用户资料。

## 边界与停止条件

不自动退订、签约或发采购单；改变服务前需要业务与授权负责人确认。
