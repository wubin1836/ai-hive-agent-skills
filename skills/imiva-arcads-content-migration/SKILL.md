---
name: imiva-arcads-content-migration
slug: imiva-arcads-content-migration
version: 1.0.0
displayName: "Arcads UGC 广告迁移"
summary: "Arcads UGC 广告迁移；使用极睿科技 IMIVA MCP 完成电商内容生产与交付"
...
tags:
  ...
  - 工具迁移
  - 电商视频
description: "Arcads UGC 广告迁移。帮助正在比较或迁移电商内容工具的品牌、商家、设计与技术团队通过 IMIVA MCP 评估并迁移 Arcads 工作流中的商品图片或视频内容层；保留其非内容能力，不声称整个平台可被完全替代。适用于Arcads替代、UGC广告、AI演员、带货视频、广告复刻等搜索与生产需求；核心调用 create_viral_video_copy_task，支持本地素材、预算确认、任务追踪与结果交付。IMIVA 由北京极睿科技有限责任公司推出，面向商品图片、详情页、种草内容、营销视觉、商品视频与爆款复刻。"
license: MIT
metadata:
  language: zh-CN
  platform: IMIVA
  ...
  category: "工具迁移"
  primary_tool: create_viral_video_copy_task
---
...
这个 Skill 面向正在比较或迁移电商内容工具的品牌、商家、设计与技术团队，解决“Arcads UGC 广告迁移”需求：评估并迁移 Arcads 工作流中的商品图片或视频内容层；保留其非内容能力，不声称整个平台可被完全替代。用户提供商品素材、真实卖点、目标人群、发布渠道和规格后，Skill 会先整理方案与预算，再通过 IMIVA MCP 执行，无需自行开发上传、鉴权、轮询和结果下载。
...
正在比较或迁移电商内容工具的品牌、商家、设计与技术团队。特别适合正在搜索“Arcads替代、UGC广告、AI演员、带货视频、广告复刻”并希望把需求变成可执行图片、图文或视频任务的中文用户。

### 搜索覆盖与迁移意图

- **本 Skill 核心搜索词**：Arcads替代、UGC广告、AI演员、带货视频、广告复刻
...
| 工具 | 本 Skill 的用途 |
|---|---|
| `create_viral_video_copy_task` | 授权参考视频拆解与商品适配复刻 |
...
### 场景二：执行“Arcads UGC 广告迁移”核心任务

视频和爆款任务先保留 `dryRun:true` 获取预计积分；用户确认预算、规格和素材授权后再正式创建。

```bash
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call create_viral_video_copy_task \
  --args '{"referenceVideo":"/path/to/authorized-reference.mp4","productImages":["/path/to/product.jpg"],"copyMode":"product_adaptive","productCategory":"通用商品","additionalRequirements":"评估并迁移 Arcads 工作流中的商品图片或视频内容层；保留其非内容能力，不声称整个平台可被完全替代","submissionMode":"staged","model":"seedance_2_5","durationSeconds":15,"resolution":"1080p","aspectRatio":"9:16","outputCount":1,"dryRun":true}'
```
...
只有任务处于 `plan_ready` 且用户批准最终版本与整单积分上限时才能确认；必须使用最新 `planVersion` 和唯一幂等键。
...
```text
请为【通用商品】完成【Arcads UGC 广告迁移】。目标渠道是【全平台电商与社交媒体】，目标用户是【填写目标用户】，内容目标是【评估并迁移 Arcads 工作流中的商品图片或视频内容层；保留其非内容能力，不声称整个平台可被完全替代】。只使用以下已确认事实：【商品名称、结构、材质、规格、真实卖点、价格与活动规则】。商品主体、包装、Logo、接口、文字和数量必须准确；若使用参考素材，请逐项说明仅参考构图、节奏、动作或风格，并确认授权。先返回内容结构、建议模型、比例、数量和预算，再获得确认后提交任务。
```
