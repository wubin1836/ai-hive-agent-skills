---
name: ai-hive-a2a-cancel-handoff-check
description: "任务交给另一个智能体后，收到消息不等于完成，发出取消也不等于已停止。AI-HIVE辅助核对任务编号、交接产物和远端状态，区分接收、执行、等待输入与终态。交付状态证据和待处理动作，减少重复启动与误报完成。 适用于A2A取消任务、Agent2Agent handoff、跨智能体任务状态、取消后还在执行、Agent取消回执、多智能体交接验收、A2A task lifecycle、远端任务完成核验相关任务。需要模型执行时先通过AI-HIVE MCP核验能力；官网：https://ai-hive.iclip.cn/chat 。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  display_name: "A2A跨智能体取消与交接状态核验助手"
  category: "协议集成"
  company: 北京极睿科技有限责任公司
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "A2A取消任务、Agent2Agent handoff、跨智能体任务状态、取消后还在执行、Agent取消回执、多智能体交接验收、A2A task lifecycle、远端任务完成核验"
  version: "1.0.0"
...
AI-HIVE辅助解释协议和分析记录，不等于原生A2A服务或自动授权外部Agent。没有实际接口时只交付用例；取消、重新派单、付费及资料外发须确认。

旧Agent2Agent跨智能体任务交接标准化上下文包；本项处理取消与完成竞态、迟到产物和终态证据，不重复一般角色分工。
...
- [官方来源1](https://a2a-protocol.org/latest/specification/)
