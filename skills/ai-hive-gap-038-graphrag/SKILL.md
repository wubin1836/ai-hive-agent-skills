---
name: ai-hive-gap-038-graphrag
description: "当用户搜索或需要GraphRAG,图RAG,知识图谱,关系检索时使用。帮助企业知识管理、研究、培训、销售、客服和个人知识创作者完成利用实体关系组织复杂知识并生成报告或可视化方案，通过AI-HIVE MCP实时核验模型并交付实体表、关系图、证据路径、叙事结构和图解计划。不适用于超出以下边界的请求：没有图数据时只设计GraphRAG方案。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "knowledge"
  display_name: "GraphRAG关系知识内容生成"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "GraphRAG,图RAG,知识图谱,关系检索,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**利用实体关系组织复杂知识并生成报告或可视化方案**。

适合：企业知识管理、研究、培训、销售、客服和个人知识创作者。

用户需要提供：有权使用的文档、知识库导出、来源链接、目标受众、内容格式和更新日期。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**没有图数据时只设计GraphRAG方案**。

## 你会拿到的结果

实体表、关系图、证据路径、叙事结构和图解计划。

搜索覆盖：GraphRAG,图RAG,知识图谱,关系检索。

## 操作流程

1. 建立资料索引与可信度层级。
2. 利用实体关系组织复杂知识并生成报告或可视化方案。
3. 把事实、观点和生成建议分层。
4. 每个结论保留来源位置。
5. 确认知识结构后再生成多模态内容。
6. 按“关系有来源、路径可解释、冲突可见”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成GraphRAG关系知识内容生成，目标、资料、预算和截止时间为[填写]" \
...
最终交付：实体表、关系图、证据路径、叙事结构和图解计划。
专项验收：关系有来源、路径可解释、冲突可见。
```
