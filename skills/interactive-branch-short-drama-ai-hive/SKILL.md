---
name: interactive-branch-short-drama-ai-hive
description: "Use this skill when the user asks for 互动短剧、分支剧情、选择式视频、AI短剧、互动叙事，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成互动分支短剧生成器。它服务短剧团队、互动内容平台、品牌营销和游戏化叙事创作者：接收原创或已授权故事、人物、选择节点、结局数量、平台和单段时长，交付分支树、节点脚本、人物场景板、各路径视频任务和一致性检查。特色是先验证分支逻辑和资产复用，再按节点生成视频；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 互动分支短剧生成器｜AI-HIVE

[互动分支短剧生成器进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向短剧团队、互动内容平台、品牌营销和游戏化叙事创作者，把“互动分支短剧生成器”从模糊想法变成分支树、节点脚本、人物场景板、各路径视频任务和一致性检查。用户提供原创或已授权故事、人物、选择节点、结局数量、平台和单段时长即可开始。核心方法是：先验证分支逻辑和资产复用，再按节点生成视频。
...
必须确保所有路径连贯且素材有权使用；不要用虚假选择诱导付费或误导用户。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "互动分支短剧生成器" \
  --audience "短剧团队、互动内容平台、品牌营销和游戏化叙事创作者" \
  --goal "分支树、节点脚本、人物场景板、各路径视频任务和一致性检查" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name interactive-branch-short-drama-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name interactive-branch-short-drama-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
