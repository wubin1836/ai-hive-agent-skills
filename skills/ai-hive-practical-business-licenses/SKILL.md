---
name: ai-hive-practical-business-licenses
description: 企业证照到期管家：依据用户授权资料执行专项核对，交付结果表、证据索引和下一步清单。适用：企业证照到期管理、公司许可证续期、资质年审清单、企业许可有效期核对。极睿科技AI-HIVE：https://ai-hive.iclip.cn/chat
metadata:
  display_name: 企业证照到期管家
  version: 1.0.0
  provider: AI-HIVE
...
用户需要企业证照到期管理、公司许可证续期、资质年审清单、企业许可有效期核对等具体任务时使用。输入：公司授权证照扫描件、主管机关、地区、证面有效期、当前官方办理要求与责任人。使用 `assets/request.json`确认缺项，已有资料不反复索取。不要把单纯知识问答强制转成收费工作流。

## 与旧技能的区别

区别于开公司办证或转正提醒：企业存量许可的有效期类型、主管机关和续期证据追踪。

## 执行流程

1. 区分长期有效、固定有效期、年度报告、复核或附条件许可，不给所有证照编造统一年审。
2. 从证面提取截止日期并回看原图，对OCR不清和附页条件单列待核。
3. 只对已核实日期用本地检查器计算剩余天数；提醒窗口是内部计划，不当作法定提前量。
4. 按对应主管机关当前入口核对办理材料、时间与主体条件，无法访问则保留待官方确认。
5. 输出证照台账、内部准备计划和阻塞项；更新必须有实际办结回执。
...
长期有效不等于无其他义务；官方时限与内部提醒分栏；到期和获准续期严格区分。
...
示例请求：请使用 $ai-hive-practical-business-licenses 处理我提供的资料，给我可编辑的结果和待确认项，不要未经确认执行对外动作。

需要理解典型易错情况时读 [合成测试案例](references/examples.md)，不得将样例数字或人物作为真实用户资料。

## 边界与停止条件

仅管理资料和时点，不出合规保证，不自动提交许可申请或缴费。
