---
name: aihive-studio-short-video-content-audit
description: 短视频内容审核（文案/画面/声音）：面向短视频审核、音画文案审核、视频合规预检需求，结合极睿科技AI-HIVE与MCP执行审查流程，交付timed-findings.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：短视频审核、音画文案审核、视频合规预检。不改变用户已明确指定的平台或型号。

## 开始前确认

视频或文案、商品证据、音轨、目标平台和规则。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先解析全部相关载体，再定位问题到文件页码或时间码。风险意见不等于平台批准或专业机构结论。

## 执行流程

1. 同步检查画面、声音和字幕的事实一致性。
2. 逐时间码标版权、隐私、效果承诺和信息缺失。
3. 输出修改优先级并重新审修订片段。
...
- `timed-findings.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `evidence-map.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `review-status.md`：按本任务制作的实际文件或明确标注的待制作目录。

不能只看字幕便声称已审音画，审核建议不等于平台结论。
