...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "制作高质量图生图，主体准确，商业信息清晰", "keywords": "图生图、图片生成图片、以图生图、image to image、I2I、参考图重绘、GPT Image 2、Image2、Nano Banana、AI图片、AIGC图片", "model": "public_model_nano_banana_2", "name": "image-to-image", "rule": "image", "search": "通用图片能力和商业场景搜索入口，底层调用 AI Hive 裸接口", "title": "图生图"}')
