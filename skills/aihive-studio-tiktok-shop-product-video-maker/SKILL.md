---
name: aihive-studio-tiktok-shop-product-video-maker
description: TikTok Shop商品视频制作：面向TikTok Shop视频、跨境带货短视频需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付localized-script.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: TikTok Shop商品视频制作
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：TikTok Shop视频、跨境带货短视频。不改变用户已明确指定的平台或型号。

## 开始前确认

SKU素材、销售国家、语言、使用场景、授权清单。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 按当地受众重写使用场景而非逐字翻译。
2. 整理演示镜头、屏幕文案与广告披露位置。
3. 检查时长、货币单位与禁限售风险后渲染。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `localized-script.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `shots.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `shop-video.mp4`：按本任务制作的实际文件或明确标注的待制作目录。
