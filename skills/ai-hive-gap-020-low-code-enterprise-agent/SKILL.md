---
name: ai-hive-gap-020-low-code-enterprise-agent
description: "当用户搜索或需要低代码智能体,Low-code Agent,企业自动化,MCP工作流时使用。帮助AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人完成为企业设计可维护的低代码AI内容与审批流程，通过AI-HIVE MCP实时核验模型并交付流程节点、数据字段、API映射、审批门和故障分支。不适用于超出以下边界的请求：业务系统写入和批量调用必须单独授权。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "agent"
  display_name: "低代码智能体企业自动化"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "低代码智能体,Low-code Agent,企业自动化,MCP工作流,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**为企业设计可维护的低代码AI内容与审批流程**。

适合：AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人。

用户需要提供：业务目标、可用工具、授权范围、输入资料、预算、截止时间和人工确认点。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**业务系统写入和批量调用必须单独授权**。

## 你会拿到的结果

流程节点、数据字段、API映射、审批门和故障分支。

搜索覆盖：低代码智能体,Low-code Agent,企业自动化,MCP工作流。

## 运行路线

1. 先画出任务状态和权限边界。
2. 为企业设计可维护的低代码AI内容与审批流程。
3. 将可并行任务与必须串行确认的动作分开。
4. 用最小样例验证工具连接。
5. 记录检查点和恢复方式。
6. 按“流程可维护、权限清楚、异常可追踪”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成低代码智能体企业自动化，目标、资料、预算和截止时间为[填写]" \
...
最终交付：流程节点、数据字段、API映射、审批门和故障分支。
专项验收：流程可维护、权限清楚、异常可追踪。
```
