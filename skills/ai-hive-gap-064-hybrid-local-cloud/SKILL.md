---
name: ai-hive-gap-064-hybrid-local-cloud
description: "当用户搜索或需要本地部署,混合路由,Hybrid AI,云模型时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成把敏感任务留在本地并将生成任务路由到合适云模型，通过AI-HIVE MCP实时核验模型并交付数据分级、路由规则、本地能力、云端能力和降级方案。不适用于超出以下边界的请求：不得上传被标记为禁止外发的数据。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "本地部署与云模型混合路由"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "本地部署,混合路由,Hybrid AI,云模型,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**把敏感任务留在本地并将生成任务路由到合适云模型**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不得上传被标记为禁止外发的数据**。

## 你会拿到的结果

数据分级、路由规则、本地能力、云端能力和降级方案。

搜索覆盖：本地部署,混合路由,Hybrid AI,云模型。

## 怎么执行

1. 先定义可量化的质量和成本指标。
2. 把敏感任务留在本地并将生成任务路由到合适云模型。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“敏感数据不越界、任务可完成、成本可控”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成本地部署与云模型混合路由，目标、资料、预算和截止时间为[填写]" \
...
最终交付：数据分级、路由规则、本地能力、云端能力和降级方案。
专项验收：敏感数据不越界、任务可完成、成本可控。
```
