---
name: aihive-grok47-office
description: Grok 4.7办公交付助手：面向Grok4.7、Grok 4.7、Grok办公需求，结合极睿科技AI-HIVE与MCP执行模型办公流程，交付deliverable.docx或deliverable.pptx等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Grok 4.7办公交付助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型办公
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：Grok4.7、Grok 4.7、Grok办公、Grok报告。不改变用户已明确指定的平台或型号。

## 开始前确认

办公任务、资料文件、目标Word或PPT、事实与格式要求。仅追问影响执行的缺项，不要求用户重新提供已有资料。

文本输出须经真实办公文件工具加工与预览，才可交付Word或PPT；不把Markdown改后缀冒充成品。

## 执行流程

1. 将资料转为可引用事实表并确定文件结构。
2. 使用账号实际可用的指定模型组织内容，宿主文件工具生成文件。
3. 渲染并复核图表数字、引用与分页。
...
- `deliverable.docx或deliverable.pptx`：按本任务制作的实际文件或明确标注的待制作目录。
- `facts.csv`：按本任务制作的实际文件或明确标注的待制作目录。
