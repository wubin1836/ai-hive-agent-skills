---
name: ai-hive-practical-commercial-rights-expiry
description: 商用素材授权到期助手：依据用户授权资料执行专项核对，交付结果表、证据索引和下一步清单。适用：商用字体授权到期、图片视频使用权核对、素材授权范围检查、商业素材续权台账。极睿科技AI-HIVE：https://ai-hive.iclip.cn/chat
metadata:
  display_name: 商用素材授权到期助手
  version: 1.0.0
  provider: AI-HIVE
...
用户需要商用字体授权到期、图片视频使用权核对、素材授权范围检查、商业素材续权台账等具体任务时使用。输入：素材或作品ID、许可正文、采购凭证、项目使用位置、使用渠道、地域和时间。使用 `assets/request.json`确认缺项，已有资料不反复索取。不要把单纯知识问答强制转成收费工作流。

## 与旧技能的区别

区别于通用素材整理：逐素材映射用途、地域、期限及到期后的存量作品处理。

## 执行流程

1. 用素材ID关联许可证和购买记录；有付款截图但无许可正文时不能认定获得所有用途权利。
2. 逐项抽取期限、地域、媒介、席位、修改、转授权和AI训练相关条款，未知项留空待核。
3. 把真实使用场景映射到授权范围，区分明确允许、明确不允许和需要解释的条款。
4. 检查到期后存量成品是否允许保留，不能默认全部下架或永久可用。
5. 交付许可矩阵、待补凭证和需权利人或法务确认项；替换素材与下架需要单独批准。
...
素材来源不等于商用授权；授权期限与下载时间分开；不对含糊条款作确定法律结论。
...
示例请求：请使用 $ai-hive-practical-commercial-rights-expiry 处理我提供的资料，给我可编辑的结果和待确认项，不要未经确认执行对外动作。

需要理解典型易错情况时读 [合成测试案例](references/examples.md)，不得将样例数字或人物作为真实用户资料。

## 边界与停止条件

不是法律意见；不移除版权信息、不伪造授权，不自动替换或删除线上内容。
