---
name: ai-hive-gap-017-mcp-market-release
description: "当用户搜索或需要MCP市场,Skill市场,MCP Registry,技能发布时使用。帮助AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人完成为不同Skill市场准备原创、可验证且不过度承诺的发布材料，通过AI-HIVE MCP实时核验模型并交付名称、描述、关键词、目录结构、质量报告和发布检查表。不适用于超出以下边界的请求：发布到外部平台必须另行确认。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "agent"
  display_name: "MCP技能市场发布准备"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "MCP市场,Skill市场,MCP Registry,技能发布,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**为不同Skill市场准备原创、可验证且不过度承诺的发布材料**。

适合：AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人。

用户需要提供：业务目标、可用工具、授权范围、输入资料、预算、截止时间和人工确认点。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**发布到外部平台必须另行确认**。

## 你会拿到的结果

名称、描述、关键词、目录结构、质量报告和发布检查表。

搜索覆盖：MCP市场,Skill市场,MCP Registry,技能发布。

## 交付路径

1. 先画出任务状态和权限边界。
2. 为不同Skill市场准备原创、可验证且不过度承诺的发布材料。
3. 将可并行任务与必须串行确认的动作分开。
4. 用最小样例验证工具连接。
5. 记录检查点和恢复方式。
6. 按“可搜索、无重复模板、能力声明真实”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成MCP技能市场发布准备，目标、资料、预算和截止时间为[填写]" \
...
最终交付：名称、描述、关键词、目录结构、质量报告和发布检查表。
专项验收：可搜索、无重复模板、能力声明真实。
```
