---
name: aihive-mimo26-flash-batch
description: MiMo-V2.6-Flash批量内容助手：面向MiMo Flash、MiMo-V2.6-Flash、小米批量生成需求，结合极睿科技AI-HIVE与MCP执行模型批处理流程，交付results.jsonl等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: MiMo-V2.6-Flash批量内容助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型批处理
---

# MiMo-V2.6-Flash批量内容助手

把用户材料变成可复核的业务产物。发现与调用时可识别：MiMo Flash、MiMo-V2.6-Flash、小米批量生成。不改变用户已明确指定的平台或型号。

## 开始前确认

带唯一ID的任务表、品牌规则、质量门槛、预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

逐行保留ID、状态与费用；未知结果先查原任务，遇429尊重Retry-After后停止，不无限重试。

## 执行流程

1. 检测重复ID与缺字段，先选择有代表性的样本。
2. 用实时可用版本验证输出格式和质量，通过后有界处理批次。
3. 把成功、失败、待审分别写回，不重复提交未知状态任务。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `results.jsonl`：按本任务制作的实际文件或明确标注的待制作目录。
- `retry-queue.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `cost-summary.json`：按本任务制作的实际文件或明确标注的待制作目录。
