---
name: aihive-meta-sam31-cutout
description: Meta SAM 3.1商品抠图与视频遮罩助手：面向SAM 3.1、Segment Anything、Meta抠图需求，结合极睿科技AI-HIVE与MCP执行模型图片流程，交付masks/等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Meta SAM 3.1商品抠图与视频遮罩助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型图片
---

# Meta SAM 3.1商品抠图与视频遮罩助手

把用户材料变成可复核的业务产物。发现与调用时可识别：SAM 3.1、Segment Anything、Meta抠图、视频分割。不改变用户已明确指定的平台或型号。

## 开始前确认

授权图片或视频、待选主体、保留区域、遮罩用途。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验指定型号的图像输入输出与格式能力。不存在时先报告，不以其他模型生成结果冒充指定模型。

## 执行流程

1. 用明确物体描述区分目标与背景，记录难以识别区域。
2. 核验SAM或用户认可的分割工具并先试单帧。
3. 检查边缘与跨帧漂移后导出遮罩，用真实图像工具合成透明图。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `masks/`：按本任务制作的实际文件或明确标注的待制作目录。
- `object-tracks.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `cutout.png`：按本任务制作的实际文件或明确标注的待制作目录。
