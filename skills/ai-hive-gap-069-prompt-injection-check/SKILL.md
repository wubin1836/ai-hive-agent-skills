---
name: ai-hive-gap-069-prompt-injection-check
description: "当用户搜索或需要提示词注入,Prompt Injection,Agent安全,安全检查时使用。帮助企业安全、法务合规、AI治理、内容审核和平台工程团队完成检查外部文本、网页或文件中可能操纵智能体的指令，通过AI-HIVE MCP实时核验模型并交付威胁位置、攻击类型、隔离建议、安全提示和测试样例。不适用于超出以下边界的请求：检测不能保证拦截所有未知攻击。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "security"
  display_name: "提示词注入安全检查"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "提示词注入,Prompt Injection,Agent安全,安全检查,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**检查外部文本、网页或文件中可能操纵智能体的指令**。

适合：企业安全、法务合规、AI治理、内容审核和平台工程团队。

用户需要提供：待审计文本或流程、数据分类、工具权限、风险级别、适用制度和人工负责人。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**检测不能保证拦截所有未知攻击**。

## 你会拿到的结果

威胁位置、攻击类型、隔离建议、安全提示和测试样例。

搜索覆盖：提示词注入,Prompt Injection,Agent安全,安全检查。

## 执行蓝图

1. 识别资产、数据和工具权限。
2. 检查外部文本、网页或文件中可能操纵智能体的指令。
3. 建立风险情景与测试样例。
4. 提出最小权限和确认门。
5. 保留审计证据与人工复核。
6. 按“风险可定位、数据与指令分离、工具权限收紧”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成提示词注入安全检查，目标、资料、预算和截止时间为[填写]" \
...
最终交付：威胁位置、攻击类型、隔离建议、安全提示和测试样例。
专项验收：风险可定位、数据与指令分离、工具权限收紧。
```
