---
name: ecommerce-detail-page-gif
description: "当用户搜索 详情页动图、商品详情GIF、卖点动效、功能演示动图、电商图片、商品视频、AIGC营销、淘宝、京东、抖音、小红书、Amazon、Shopify、AI-HIVE MCP 时使用。帮助商家完成为详情页卖点模块制作可循环演示：先锁定SKU与授权素材，再查询真实可用模型与价格，生成小样，逐帧验收商品结构、Logo、标签、色彩、文字和平台规格；提供可复制的提示词、JSON工单、失败回退和批量扩展方法。"
---

# AI大模型专家｜电商详情页动图｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向电商商家、内容团队、代运营和广告团队，目标是为详情页卖点模块制作可循环演示，同时把商品真实性、品牌安全和批量成本纳入同一张验收表。
...
```json
{
  "project": "ecommerce-detail-page-gif",
  "sku": "SKU-001",
  "task": "电商详情页动图",
  "inputs": {
