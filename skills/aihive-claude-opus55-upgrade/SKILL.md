---
name: aihive-claude-opus55-upgrade
description: Claude Opus 5.5迁移验收助手：面向Claude Opus5.5、Claude 5.5、Opus 5.5需求，结合极睿科技AI-HIVE与MCP执行模型评测流程，交付compatibility.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: Claude Opus 5.5迁移验收助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型评测
---

# Claude Opus 5.5迁移验收助手

把用户材料变成可复核的业务产物。发现与调用时可识别：Claude Opus5.5、Claude 5.5、Opus 5.5、Claude新模型。不改变用户已明确指定的平台或型号。

## 开始前确认

旧版任务样本、工具协议、预算、正确性标准和数据边界。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先制定测试标准再取样；没有真实调用的表格必须标评测计划，不能填写臆造分数。

## 执行流程

1. 核验Opus 5.5的正式名称、渠道ID与工具参数。
2. 按相同输入测试旧新模型，特别记录长任务状态和工具失败。
3. 形成差异表与回退条件，经用户确认才改应用配置。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `compatibility.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `eval-results.jsonl`：按本任务制作的实际文件或明确标注的待制作目录。
- `rollback.md`：按本任务制作的实际文件或明确标注的待制作目录。
