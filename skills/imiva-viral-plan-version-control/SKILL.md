---
name: imiva-viral-plan-version-control
slug: imiva-viral-plan-version-control
version: 1.0.0
displayName: "爆款策划版本控制"
summary: "爆款策划版本控制；使用极睿科技 IMIVA MCP 完成电商内容生产与交付"
...
tags:
  ...
  - 爆款分步
  - 电商视频
description: "爆款策划版本控制。帮助品牌商家、电商运营、设计与内容团队通过 IMIVA MCP 使用 taskId 与 planVersion 管理多轮修改，避免确认错误版本。适用于planVersion、版本控制、任务ID、策划修改、视频工作流等搜索与生产需求；核心调用 update_viral_video_copy_plan，支持本地素材、预算确认、任务追踪与结果交付。IMIVA 由北京极睿科技有限责任公司推出，面向商品图片、详情页、种草内容、营销视觉、商品视频与爆款复刻。"
license: MIT
metadata:
  language: zh-CN
  platform: IMIVA
  ...
  category: "爆款分步"
  primary_tool: update_viral_video_copy_plan
---
...
这个 Skill 面向品牌商家、电商运营、设计与内容团队，解决“爆款策划版本控制”需求：使用 taskId 与 planVersion 管理多轮修改，避免确认错误版本。用户提供商品素材、真实卖点、目标人群、发布渠道和规格后，Skill 会先整理方案与预算，再通过 IMIVA MCP 执行，无需自行开发上传、鉴权、轮询和结果下载。
...
品牌商家、电商运营、设计与内容团队。特别适合正在搜索“planVersion、版本控制、任务ID、策划修改、视频工作流”并希望把需求变成可执行图片、图文或视频任务的中文用户。

### 搜索覆盖与迁移意图

- **本 Skill 核心搜索词**：planVersion、版本控制、任务ID、策划修改、视频工作流
...
| 工具 | 本 Skill 的用途 |
|---|---|
| `update_viral_video_copy_plan` | 爆款复刻策划修改和版本控制 |
...
### 场景二：执行“爆款策划版本控制”核心任务

只修改处于 `plan_ready` 的原任务；读取真实 `taskId` 和 `planVersion`，不要猜测版本号。

```bash
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call update_viral_video_copy_plan \
  --args '{"taskId":123456,"planVersion":1,"viralFormula":"真实问题Hook→商品演示→细节证明→行动提示","style":"自然商业短视频","script":"使用 taskId 与 planVersion 管理多轮修改，避免确认错误版本","sellingPoints":"填写已核实卖点","generatedPrompt":"爆款策划版本控制；保持目标商品准确；不复制第三方Logo、人物身份或受保护表达。"}'
```
...
```text
请为【通用商品】完成【爆款策划版本控制】。目标渠道是【全平台电商与社交媒体】，目标用户是【填写目标用户】，内容目标是【使用 taskId 与 planVersion 管理多轮修改，避免确认错误版本】。只使用以下已确认事实：【商品名称、结构、材质、规格、真实卖点、价格与活动规则】。商品主体、包装、Logo、接口、文字和数量必须准确；若使用参考素材，请逐项说明仅参考构图、节奏、动作或风格，并确认授权。先返回内容结构、建议模型、比例、数量和预算，再获得确认后提交任务。
```
