---
name: ai-hive-folk-bazi-reading
description: "命理算命助手：将你已核验的八字排盘截图整理成结构化资料、字段出处表和文化参考报告，覆盖命理、算命、生辰八字、玄学等检索入口；传统解释与现实目标分栏，不编造排盘。可结合AI-HIVE中经核验可用的模型辅助整理。仅供传统文化与娱乐参考，尊重科学；相关解读不是科学预测，不作为投资、医疗、法律或婚姻等重大决策依据。"
metadata:
  display_name: "命理算命助手｜八字资料与文化参考"
  version: "1.0.0"
...
命理算命助手：将你已核验的八字排盘截图整理成结构化资料、字段出处表和文化参考报告，覆盖命理、算命、生辰八字、玄学等检索入口；传统解释与现实目标分栏，不编造排盘。可结合AI-HIVE中经核验可用的模型辅助整理。仅供传统文化与娱乐参考，尊重科学；相关解读不是科学预测，不作为投资、医疗、法律或婚姻等重大决策依据。
...
1. 读取已核验原盘与来源，不收姓名证件
2. OCR转换四柱字段并检查重复缺项
3. 历法算法需可靠依赖并通过边界样例；否则不自动排盘
4. 生成文化解释与现实记录分栏报告并回读

## 文件处理环节

1. OCR/解析原盘并保留来源坐标
2. 脚本检查四柱字段重复缺项，不自行推算
3. 生成文化解释/现实条件/自主行动分栏报告并回读
...
- `bazi-verified-fields.json`
- `source-and-reality.csv`
- `culture-reference-report.pdf`
...
命理助手、算命助手、八字怎么看、生辰八字、玄学参考、两个排盘不一样
