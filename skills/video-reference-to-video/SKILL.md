...
Video Reference to Video helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers reuse motion, pacing, camera movement, or structure from a reference video with AI Hive. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
python3 "$SKILL_PATH/scripts/videogen.py" init --skill-name video-reference-to-video
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode r2v \
  --prompt "Video Reference to Video: polished motion, clear subject, coherent action, stable camera, commercial finish" \
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode r2v \
  --prompt "Create a polished video reference to video result with coherent motion, stable identity, and a clear ending" \
+  --video /path/to/reference.mp4
```
