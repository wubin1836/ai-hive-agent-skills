---
name: aihive-studio-bid-proposal-full-process-expert
description: 标书制作全流程专家：面向标书制作、投标文件、标书全流程需求，结合极睿科技AI-HIVE与MCP执行文档流程，交付response-matrix.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 标书制作全流程专家
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 文档
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：标书制作、投标文件、标书全流程。不改变用户已明确指定的平台或型号。

## 开始前确认

招标文件、公司真实资质、可公开案例、截止时间。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 抽取否决条款和评分点建立响应矩阵。
2. 只用真实材料组织商务与技术方案，缺证据标待补。
3. 编排目录并对页码、签章和附件逐项预检。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `response-matrix.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `proposal.docx`：按本任务制作的实际文件或明确标注的待制作目录。
- `missing-evidence.md`：按本任务制作的实际文件或明确标注的待制作目录。
