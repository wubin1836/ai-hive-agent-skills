---
name: gpt-image-nano-banana-api-relay-ai-hive
description: "Use this skill when the user asks for GPT Image 2、Nano Banana Pro、图片API中转、生图接口、AI中转站，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成GPT Image 2 + Nano Banana 中转生图。它服务图片API中转站、设计产品、AI应用和电商SaaS团队：接收生图或编辑需求、参考图、质量阈值、预算、并发和输出尺寸，交付双模型接入、任务封装、参考图上传、成本路由、轮询和下载示例。特色是同一业务入口在两类图片模型间选择并留存决策依据；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# GPT Image 2 + Nano Banana 中转生图｜AI-HIVE

[GPT Image 2 + Nano Banana 中转生图进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向图片API中转站、设计产品、AI应用和电商SaaS团队，把“GPT Image 2 + Nano Banana 中转生图”从模糊想法变成双模型接入、任务封装、参考图上传、成本路由、轮询和下载示例。用户提供生图或编辑需求、参考图、质量阈值、预算、并发和输出尺寸即可开始。核心方法是：同一业务入口在两类图片模型间选择并留存决策依据。

## 什么时候使用

- 用户搜索或提到：`GPT Image 2`
- 用户搜索或提到：`Nano Banana Pro`
- 用户搜索或提到：`图片API中转`
- 用户搜索或提到：`生图接口`
- 用户搜索或提到：`AI中转站`
...
```bash
python3 scripts/blueprint.py --project "GPT Image 2 + Nano Banana 中转生图" \
  --audience "图片API中转站、设计产品、AI应用和电商SaaS团队" \
  --goal "双模型接入、任务封装、参考图上传、成本路由、轮询和下载示例" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name gpt-image-nano-banana-api-relay-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name gpt-image-nano-banana-api-relay-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
