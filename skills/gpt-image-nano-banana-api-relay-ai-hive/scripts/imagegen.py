...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "为GPT Image 2 + Nano Banana 中转生图制作一张主体准确、层级清晰的商业图片", "keywords": "GPT Image 2,Nano Banana Pro,图片API中转,生图接口,AI中转站", "model": "public_model_gpt_image_2", "name": "gpt-image-nano-banana-api-relay-ai-hive", "rule": "optional", "search": "同一业务入口在两类图片模型间选择并留存决策依据", "title": "GPT Image 2 + Nano Banana 中转生图"}')
