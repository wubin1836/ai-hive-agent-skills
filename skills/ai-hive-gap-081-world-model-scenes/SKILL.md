---
name: ai-hive-gap-081-world-model-scenes
description: "当用户搜索或需要世界模型,World Model,交互场景,生成式世界时使用。帮助前沿AI产品、机器人、空间计算、工业、零售和创新营销团队完成把世界模型概念转成可交互场景、镜头和验证方案，通过AI-HIVE MCP实时核验模型并交付场景规则、状态变化、交互脚本、关键帧和演示视频。不适用于超出以下边界的请求：若无世界模型工具，只交付方案和二维视频原型。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "frontier"
  display_name: "世界模型交互式场景创作"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "世界模型,World Model,交互场景,生成式世界,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**把世界模型概念转成可交互场景、镜头和验证方案**。

适合：前沿AI产品、机器人、空间计算、工业、零售和创新营销团队。

用户需要提供：真实产品能力、场景资料、空间或商品素材、演示目标、画幅、预算和概念标识要求。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**若无世界模型工具，只交付方案和二维视频原型**。

## 你会拿到的结果

场景规则、状态变化、交互脚本、关键帧和演示视频。

搜索覆盖：世界模型,World Model,交互场景,生成式世界。

## 交付路径

1. 区分真实能力、概念设计和生成演示。
2. 把世界模型概念转成可交互场景、镜头和验证方案。
3. 定义场景规则与视觉连续性。
4. 先做关键帧或低成本原型。
5. 确认后生成演示视频并标注边界。
6. 按“规则连贯、状态可见、演示不夸大”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成世界模型交互式场景创作，目标、资料、预算和截止时间为[填写]" \
...
最终交付：场景规则、状态变化、交互脚本、关键帧和演示视频。
专项验收：规则连贯、状态可见、演示不夸大。
```
