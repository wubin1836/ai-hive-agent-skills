---
name: ai-hive-guoxue-name-stroke-audit
description: "姓名学字形与笔画核对助手：读取用户自愿提供的候选名文字，可用虚构示例、指定字典版本和需核对的已有笔画/分数字段，逐字记录Unicode、原字形及来源，遇一简对多繁或异体不自动替换，交付可回查的文件及待核实清单。适用于姓名学、康熙笔画、姓名笔画查询、三才五格；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。"
metadata:
  display_name: "姓名学字形与笔画核对助手"
  version: "1.0.0"
...
姓名学字形与笔画核对助手：读取用户自愿提供的候选名文字，可用虚构示例、指定字典版本和需核对的已有笔画/分数字段，逐字记录Unicode、原字形及来源，遇一简对多繁或异体不自动替换，交付可回查的文件及待核实清单。适用于姓名学、康熙笔画、姓名笔画查询、三才五格；可结合AI-HIVE已可用的模型与宿主文件工具。仅供传统文化、学习与娱乐参考，尊重科学；不宣称能够科学预测命运，不作为投资、医疗、法律、婚姻等重大决策依据。

## 适用场景与输入

- 用户自愿提供的候选名文字，可用虚构示例
- 指定字典版本和需核对的已有笔画/分数字段
- 已知异体字或繁简转换要求
...
1. 逐字记录Unicode、原字形及来源，遇一简对多繁或异体不自动替换
2. 实际查指定版本字典与页码，将现代笔画和其他口径分栏；无可靠出处标待核
3. 复算用户已有资料中的笔画数字并导出字形对照表，对所谓吉凶分数仅标其算法来源，不生成改命评分
...
- `name-character-evidence.csv`
- `stroke-count-review.xlsx`
- `unverified-score-claims.md`
...
仅文化及文字资料参考；不凭姓名推断人格、健康、成就，不保证登记或商标可用。

缺关键原件或专业依据时只完成可验证部分。401/403、限流或内容审核阻断时保存脱敏进度并停止相应调用，遵守明确的等待要求，不循环重试或绕过审核。不因安装本技能自动交易、签署、提交、缴费或公开发布。

## 调用示例

> 请使用我授权提供的资料完成“姓名学字形与笔画核对助手”。范围先按我的原件与指定版本确认，生成name-character-evidence.csv及其余约定文件，疑点附来源，不自动对外提交。

## 搜索与任务识别

姓名学、康熙笔画、姓名笔画查询、三才五格、名字打分依据、繁简笔画核对
