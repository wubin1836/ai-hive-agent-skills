---
name: ai-hive-gap-058-llm-benchmark
description: "当用户搜索或需要Benchmark,大模型排行榜,模型评测,业务测试时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成把公开Benchmark与企业任务测试结合形成判断，通过AI-HIVE MCP实时核验模型并交付基准解释、业务测试、差异表、风险和结论。不适用于超出以下边界的请求：公开分数需注明日期和来源。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "大模型Benchmark业务对比"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "Benchmark,大模型排行榜,模型评测,业务测试,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**把公开Benchmark与企业任务测试结合形成判断**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**公开分数需注明日期和来源**。

## 你会拿到的结果

基准解释、业务测试、差异表、风险和结论。

搜索覆盖：Benchmark,大模型排行榜,模型评测,业务测试。

## 工作方法

1. 先定义可量化的质量和成本指标。
2. 把公开Benchmark与企业任务测试结合形成判断。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“指标含义正确、避免单榜定胜负、版本明确”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成大模型Benchmark业务对比，目标、资料、预算和截止时间为[填写]" \
...
最终交付：基准解释、业务测试、差异表、风险和结论。
专项验收：指标含义正确、避免单榜定胜负、版本明确。
```
