---
name: aihive-studio-city-weather-poster-generator
description: 城市天气画报生成：面向城市天气海报、天气画报需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付weather-source.json等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 城市天气画报生成
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：城市天气海报、天气画报。不改变用户已明确指定的平台或型号。

## 开始前确认

城市、日期、已确认天气来源、品牌和输出尺寸。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验图像生成或编辑、参考图输入、尺寸与下载工具。准确文字、透明通道、二维码要用真实文件工具另行检查。

## 执行流程

1. 读取带时间的真实天气并区分预报与实测。
2. 先确定天气信息层级再生成背景。
3. 把温度、单位和日期用准确文字层叠加。
...
- `weather-source.json`：按本任务制作的实际文件或明确标注的待制作目录。
