---
name: aihive-studio-ecommerce-listing-image-set
description: AI电商上架套图：面向电商套图、上架图片、商品主图套装需求，结合极睿科技AI-HIVE与MCP执行图片流程，交付image-plan.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: AI电商上架套图
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 图片
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：电商套图、上架图片、商品主图套装。不改变用户已明确指定的平台或型号。

## 开始前确认

商品实拍、SKU规格、上架渠道、尺寸与卖点证据。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验图像生成或编辑、参考图输入、尺寸与下载工具。准确文字、透明通道、二维码要用真实文件工具另行检查。

## 执行流程

1. 建立同SKU外观锁定表并区分主图、细节图和场景图。
2. 按渠道规则安排套图顺序和文字安全区。
3. 先出一张校对产品形状再批量出图并保留版本编号。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `image-plan.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `product-lock.json`：按本任务制作的实际文件或明确标注的待制作目录。
- `listing-images/`：按本任务制作的实际文件或明确标注的待制作目录。

套图无串SKU或虚构配件，所有尺寸信息与输入一致，导出分辨率合格。
