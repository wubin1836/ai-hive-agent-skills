---
name: aihive-studio-wechat-article-visual-story
description: 公众号图文全案：面向公众号图文全案、公众号文章配图需求，结合极睿科技AI-HIVE与MCP执行内容流程，交付article.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 公众号图文全案
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 内容
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：公众号图文全案、公众号文章配图。不改变用户已明确指定的平台或型号。

## 开始前确认

文章主题、参考材料、品牌视觉和配图需求。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
1. 先形成带来源的文章骨架再标图文分工。
2. 给每张配图明确图解目的和不能臆造的数据。
3. 制作图文稿与图片并检查移动端阅读顺序。
...
- `image-map.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `illustrations/`：按本任务制作的实际文件或明确标注的待制作目录。

数据图由真实数据绘制，图片不重复正文堆字；不直接群发。
