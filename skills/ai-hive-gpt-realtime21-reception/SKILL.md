---
name: ai-hive-gpt-realtime21-reception
description: "为已有语音接待场景整理可实施的配置、话术和测试用例，重点验证型号确认、字母数字复述、打断、静音和转人工。只有实际实时连接可用并授权后才试运行，交付配置与真实测试结果，不自动接通电话或上线客服。 适用于GPTRealtime2.1、GPT Realtime 2.1、GPT-Realtime-2.1、gpt-realtime-2.1、GPT语音接待、语音客服配置、语音机器人、转人工测试相关任务。需要模型执行时先通过AI-HIVE MCP核验能力；官网：https://ai-hive.iclip.cn/chat 。"
license: MIT
metadata:
  language: zh-CN
  platform: AI-HIVE
  display_name: "GPT Realtime 2.1语音接待配置助手"
  category: "GPT-Realtime-2.1"
  company: 北京极睿科技有限责任公司
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "GPTRealtime2.1、GPT Realtime 2.1、GPT-Realtime-2.1、gpt-realtime-2.1、GPT语音接待、语音客服配置、语音机器人、转人工测试"
  model_access: "runtime-check"
  version: "1.0.0"
  release_variant: aihive-new-model-specialists-20260915
---
# GPT Realtime 2.1语音接待配置助手

[打开AI-HIVE工作台](https://ai-hive.iclip.cn/chat)

## 这个技能帮你做什么

为已有语音接待场景整理可实施的配置、话术和测试用例，重点验证型号确认、字母数字复述、打断、静音和转人工。只有实际实时连接可用并授权后才试运行，交付配置与真实测试结果，不自动接通电话或上线客服。

适用搜索：GPTRealtime2.1、GPT Realtime 2.1、GPT-Realtime-2.1、gpt-realtime-2.1、GPT语音接待、语音客服配置、语音机器人、转人工测试。这些写法用于识别用户需求，不代表不同型号，也不代表全部已接入。识别有歧义的缩写时先确认所指模型。

## 指定型号与执行条件

型号核对词：GPT-Realtime-2.1、gpt-realtime-2.1。

发布/接入状态：官方型号资料已核对；AI-HIVE账号与精确接口待运行时核验。

宿主需要的能力：gpt-realtime-2.1只读型号发现与真实实时会话；宿主配置schema、音频测试和可追溯会话日志；仅在用户授权时可用的业务查询与人工转接接口。没有对应工具时，明确交付草案和缺失项，不把准备工作标为完成。
...
1. 发现精确gpt-realtime-2.1及宿主实时连接能力，区分模型会话、电话平台和业务查询接口，记录未接通依赖。
2. 把FAQ划分为可答、需查询、必须转人工三类，明确不能承诺的退款、折扣及敏感信息请求。
3. 编写接待语音规则：短句、订单字母数字复述、确认后继续、用户打断即停；只配置实际支持的参数。
4. 按宿主schema生成配置草案和最小工具权限，写入的是凭据引用名而不是密钥；不得自动绑定号码或改生产路由。
5. 经测试授权和费用确认后运行静音、噪声、打断、错号与转人工用例，记录会话ID、工具回执与失败点。
6. 交付脱敏配置、测试用例和验收报告；未执行用例写未测试，转人工只是文本意图时不得判定真实转接成功。

涉及模型调用前记录实际模型ID、工具名、数据外发范围与可用导出工具。付费调用须在用户确认的范围与预算内，发布、群发、数据库写入或部署另需明确授权。模型不可用时交付准备材料并说明缺口，不静默换模型，不伪造最终结果。

## 你会拿到

- gpt-reception-config.json：按宿主schema核对的脱敏配置或明确草案
- gpt-reception-cases.csv：接待、打断与转人工用例
- gpt-reception-report.json：真实测试回执与未测依赖
...
- 实际模型ID为gpt-realtime-2.1而非同族默认型号
- 配置不含凭据且业务工具权限最小化
- 测试状态区分通过、失败和未执行
- 转人工等外部动作只凭真实平台回执判定
...
先通过AI-HIVE MCP只读发现指定精确型号、任务模式及真实参数；缺失就停止模型执行并列明缺项，不偷换版本或供应商。用户提供素材不等于授权上传或计费，提交前确认本次范围、额度与素材权利；超时先查原任务ID，不自动重提、扩量或发布。工作单、人工演示和失败记录均不是模型生成成果。模型支持语音不等于有电话号码、SIP线路或客户通信授权。默认只产配置与用例；无实际会话时停止试运行，不能声称已上线。生产绑定、外呼和客户数据写入需另行明确授权。

区别于GPT Live泛助手，新增Realtime2.1精确语音接待配置、号码复述、打断与真实转接回执验收。
...
- [官方来源1](https://developers.openai.com/api/docs/models/gpt-realtime-2.1)
