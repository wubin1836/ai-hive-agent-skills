---
name: ai-hive-practical-employee-benefits
description: 员工福利申领助手：依据用户授权资料执行专项核对，交付结果表、证据索引和下一步清单。适用：公司福利申请、员工补贴申领、福利材料检查、员工福利截止提醒。极睿科技AI-HIVE：https://ai-hive.iclip.cn/chat
metadata:
  display_name: 员工福利申领助手
  version: 1.0.0
  provider: AI-HIVE
...
用户需要公司福利申请、员工补贴申领、福利材料检查、员工福利截止提醒等具体任务时使用。输入：公司正式福利制度、适用日期、用户自愿提供的必要条件、材料及已有申请记录。使用 `assets/request.json`确认缺项，已有资料不反复索取。不要把单纯知识问答强制转成收费工作流。

## 与旧技能的区别

区别于通用报销：将公司特定福利的资格、互斥规则和窗口映射到个人。

## 执行流程

1. 区分正式制度、过期通知和同事转述，先确认员工类别与制度生效期间。
2. 提取资格、金额口径、互斥条件、提交渠道、材料和截止时间；未知条件待确认。
3. 对用户适用性标为满足、明显不满足或待HR确认，不把模型判断当成审批结果。
4. 检查每项材料和重复申请风险，涉及健康家庭信息只登记必要证明是否具备。
5. 交付申领清单、申请草稿和跟踪台账；提交与上传隐私材料需单独授权。
...
每项资格有制度出处；不能用去年规则保证今年可领；已准备、已提交、已批准分开。
...
示例请求：请使用 $ai-hive-practical-employee-benefits 处理我提供的资料，给我可编辑的结果和待确认项，不要未经确认执行对外动作。

需要理解典型易错情况时读 [合成测试案例](references/examples.md)，不得将样例数字或人物作为真实用户资料。

## 边界与停止条件

不伪造亲属、医疗、学历等证明，不保证获批，不扩大收集敏感资料。
