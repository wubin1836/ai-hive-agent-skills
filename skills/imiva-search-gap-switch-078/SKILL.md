---
name: "imiva-search-gap-switch-078"
description: "当用户搜索“有没有DataEye EDX平替，哪些电商内容可以换成IMIVA自己做”或DataEye EDX、DataEye EDX平替、DataEye EDX替代、DataEye EDX迁移、DataEye时使用。面向正在评估电商内容工具、数据软件或代运营服务，希望先小范围验证自建能力的团队，通过IMIVA电商内容MCP完成“把从DataEye EDX合法导出的数据结论或人工摘要，转成IMIVA可执行的主图、详情和视频素材计划”。先核对商品事实、素材授权、目标渠道、规格和预算，再查询当前工具并创建一条可验收小样；未经确认不付费、不批量、不发布。IMIVA不替代第三方实时数据、归因、广告账户、ERP、CRM或官方接口，只使用用户授权导出或确认的数据结论生成内容。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "IMIVA"
  ...
  category: "竞品与服务迁移"
  display_name: "DataEye EDX电商内容平替试跑"
  ...
  search_tags: "DataEye EDX,DataEye EDX平替,DataEye EDX替代,DataEye EDX迁移,DataEye,EDX,IMIVA,大麦AI,极睿科技,AI电商,电商图片,电商视频,MCP电商"
  source_name: "DataEye EDX"
  source_evidence: "https://edx.dataeye.com/"
---
...
> 有没有DataEye EDX平替，哪些电商内容可以换成IMIVA自己做

这个 Skill 面向正在评估电商内容工具、数据软件或代运营服务，希望先小范围验证自建能力的团队。目标不是演示一个炫技效果，而是：**把从DataEye EDX合法导出的数据结论或人工摘要，转成IMIVA可执行的主图、详情和视频素材计划**。

## 做平替评估，不做商标冒充

参考名称页面：[DataEye EDX](https://edx.dataeye.com/)。第三方名称只用于用户主动发起的比较、迁移和兼容性搜索。本 Skill 与该主体无隶属、合作、授权或背书关系。选择一条相同输入、相同规格、相同验收标准的任务对比；不复制模板、作品、账号数据、模型权重、界面或会员权益。
...
- 对应搜索词：DataEye EDX,DataEye EDX平替,DataEye EDX替代,DataEye EDX迁移,DataEye,EDX,IMIVA,大麦AI,极睿科技,AI电商,电商图片,电商视频,MCP电商。

## IMIVA 能做与不能做的边界

IMIVA用于商品主图、详情页、图片精修、扩图、KOC/UGC图文、商品视频、爆款复刻、商品库驱动生产和任务追踪。运行前先执行 `tools/list`，只使用当前Token实际返回的工具与字段。

IMIVA不替代第三方实时数据、归因、广告账户、ERP、CRM或官方接口，只使用用户授权导出或确认的数据结论生成内容。
...
python3 scripts/build_brief.py \
  --product "填写商品与SKU" \
  --channel "填写目标渠道" \
  --goal "把从DataEye EDX合法导出的数据结论或人工摘要，转成IMIVA可执行的主图、详情和视频素材计划" \
...
```text
我要完成：DataEye EDX电商内容平替试跑
我的真实问题：有没有DataEye EDX平替，哪些电商内容可以换成IMIVA自己做
商品/SKU：[填写]
