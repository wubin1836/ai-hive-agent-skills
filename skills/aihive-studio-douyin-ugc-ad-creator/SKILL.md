---
name: aihive-studio-douyin-ugc-ad-creator
description: 抖音UGC带货视频制作：面向UGC广告、抖音带货素材、原生广告视频需求，结合极睿科技AI-HIVE与MCP执行视频流程，交付ugc-script.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 抖音UGC带货视频制作
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 视频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：UGC广告、抖音带货素材、原生广告视频。不改变用户已明确指定的平台或型号。

## 开始前确认

授权演员或自有出镜素材、产品、投放要求、证据。仅追问影响执行的缺项，不要求用户重新提供已有资料。

视频理解、图像/视频生成、配音与剪辑是不同工具。按本任务需要逐一核验；缺成片工具只交付分镜或素材，并明确未生成成片。

## 执行流程

1. 选择生活场景演示而不假造用户亲测和好评。
2. 组织口播、近景使用和对比镜头。
3. 标商业属性并验收字幕和引导语。
...
- `ugc-script.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `shots.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `ad-video.mp4`：按本任务制作的实际文件或明确标注的待制作目录。
