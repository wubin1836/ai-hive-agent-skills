---
name: ai-hive-gap-029-data-agent
description: "当用户搜索或需要数据智能体,Data Agent,数据分析,图表生成时使用。帮助普通企业、创业团队、超级个体、销售运营和职能部门完成从结构化数据形成可复核分析、图表和内容解释，通过AI-HIVE MCP实时核验模型并交付数据检查、分析步骤、图表、结论证据和异常列表。不适用于超出以下边界的请求：不得编造缺失数据或把预测写成事实。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "enterprise"
  display_name: "数据智能体自动分析与图表"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "数据智能体,Data Agent,数据分析,图表生成,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**从结构化数据形成可复核分析、图表和内容解释**。

适合：普通企业、创业团队、超级个体、销售运营和职能部门。

用户需要提供：真实业务资料、目标受众、内部口径、交付格式、品牌规范、预算和审批要求。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不得编造缺失数据或把预测写成事实**。

## 你会拿到的结果

数据检查、分析步骤、图表、结论证据和异常列表。

搜索覆盖：数据智能体,Data Agent,数据分析,图表生成。

## 执行蓝图

1. 将业务问题和生成任务分开。
2. 从结构化数据形成可复核分析、图表和内容解释。
3. 锁定不可虚构的事实与数字。
4. 先交付文字或结构草案。
5. 确认后制作视觉小样并验收。
6. 按“计算正确、图表匹配、结论有依据”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成数据智能体自动分析与图表，目标、资料、预算和截止时间为[填写]" \
...
最终交付：数据检查、分析步骤、图表、结论证据和异常列表。
专项验收：计算正确、图表匹配、结论有依据。
```
