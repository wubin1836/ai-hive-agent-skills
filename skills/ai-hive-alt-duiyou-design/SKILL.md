---
name: ai-hive-alt-duiyou-design
description: "当用户明确搜索 堆友替代、堆友平替、堆友迁移、堆友竞品、堆友 API、AI视频Agent替代或希望把电商设计素材生成任务的组合替代接入 AI-HIVE MCP 时使用。先核验 堆友 当前官方能力，再执行“模型入口与生成任务试迁”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---
...
这个 Skill 面向已经在用、正在比较或正在搜索 **堆友 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“电商设计素材生成任务的组合替代”做成一场可暂停、可计量、可回退的迁移试验。
...
- 待复核官方入口：https://d.design/
- 已记录官方域名候选，发布前必须重新访问并核对产品名称、能力和可用地区。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 堆友 官方，也不暗示双方存在合作、代理或背书。
...
```json
{
  "project": "ai-hive-alt-duiyou-design",
  "source_platform": "堆友",
  "focus": "电商设计素材生成任务的组合替代",
  "strategy": "模型入口与生成任务试迁",
  "pilot": "生成商品主图背景、场景图、促销KV素材并保留人工排版",
