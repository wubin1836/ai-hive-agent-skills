...
MiniMax H3 Image Reference to Video helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers use reference images to control subjects, products, composition, or style with MiniMax H3. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode r2v \
  --prompt "MiniMax H3 Image Reference to Video: polished motion, clear subject, coherent action, stable camera, commercial finish" \
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode r2v \
  --prompt "Create a polished minimax h3 image reference to video result with coherent motion, stable identity, and a clear ending" \
+  --image /path/to/reference.png
```
