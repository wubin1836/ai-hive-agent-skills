---
name: aihive-studio-brand-jingle-studio
description: 广告歌生成·品牌主题曲：面向广告歌、品牌主题曲、品牌音乐需求，结合极睿科技AI-HIVE与MCP执行音频流程，交付jingle-lyrics.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 广告歌生成·品牌主题曲
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 音频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：广告歌、品牌主题曲、品牌音乐。不改变用户已明确指定的平台或型号。

## 开始前确认

品牌名称读音、标语、广告长度、风格与授权范围。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
- `jingle-lyrics.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `pronunciations.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `jingle.wav`：按本任务制作的实际文件或明确标注的待制作目录。
