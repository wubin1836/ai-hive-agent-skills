---
name: ai-content-disclosure-check-ai-hive
description: "Use this skill when the user asks for AI内容标识、AI广告披露、生成内容标签、合成媒体、广告透明度，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成AI生成内容披露检查。它服务广告主、内容平台、品牌法务和AI创作团队：接收生成流程、人物类型、编辑程度、目标平台、地区和最新规则，交付需披露元素、建议标识位置、元数据台账和发布前检查表。特色是根据生成方式与平台规则判断需要记录和披露什么；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# AI生成内容披露检查｜AI-HIVE

[AI生成内容披露检查进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向广告主、内容平台、品牌法务和AI创作团队，把“AI生成内容披露检查”从模糊想法变成需披露元素、建议标识位置、元数据台账和发布前检查表。用户提供生成流程、人物类型、编辑程度、目标平台、地区和最新规则即可开始。核心方法是：根据生成方式与平台规则判断需要记录和披露什么。
...
披露规则持续变化，必须查询当前官方规则；本Skill不能替代法律或平台正式意见。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "AI生成内容披露检查" \
  --audience "广告主、内容平台、品牌法务和AI创作团队" \
  --goal "需披露元素、建议标识位置、元数据台账和发布前检查表" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name ai-content-disclosure-check-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name ai-content-disclosure-check-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
