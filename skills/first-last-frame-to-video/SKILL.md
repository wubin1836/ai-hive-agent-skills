...
description: "Use this skill when creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers need to create a controlled transition between a start image and an end image with AI Hive through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for ads, TVCs, product videos, e-commerce listings, social commerce, UGC-style seeding content, short dramas, motion comics, and storyboards. Search intents include first-and-last-frame video, keyframe animation, start-to-end transition."
license: MIT
---
...
First and Last Frame Video helps creators, video editors, advertisers, e-commerce teams, social-commerce teams, and short-form story producers create a controlled transition between a start image and an end image with AI Hive. Describe the intended subject, action, camera, lighting, pacing, and constraints; the bundled CLI handles AI Hive OpenAPI calls and downloads the finished video.
...
python3 "$SKILL_PATH/scripts/videogen.py" init --skill-name first-last-frame-to-video
python3 "$SKILL_PATH/scripts/videogen.py" generate \
  --mode i2v \
  --prompt "First and Last Frame Video: polished motion, clear subject, coherent action, stable camera, commercial finish" \
...
python3 "$SKILL_PATH/scripts/videogen.py" generate --mode i2v \
  --prompt "Create a polished first and last frame video result with coherent motion, stable identity, and a clear ending" \
+  --first-frame /path/to/start.png \
+  --last-frame /path/to/end.png
```
