---
name: ai-hive-alt-yanyu-tech-verify
description: "当用户明确搜索 演雨科技替代、演雨科技平替、演雨科技迁移、演雨科技竞品、演雨科技 API、AI视频Agent替代或希望把确认准确产品、官网与能力后再判断替代边界接入 AI-HIVE MCP 时使用。先核验 演雨科技 当前官方能力，再执行“先核验、后评估，不直接宣称替代”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# 演雨科技产品核验与迁移评估｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **演雨科技 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“确认准确产品、官网与能力后再判断替代边界”做成一场可暂停、可计量、可回退的迁移试验。

## 先给结论边界

- 平台类别：**待核实平台**。
- 建议策略：**先核验、后评估，不直接宣称替代**。
- 必须保留：在取得准确官网、产品页和实际账号之前，不推断其功能、价格、主体或与 AI-HIVE 的替代关系。
- AI-HIVE 负责：查询当前可用模型与价格快照、上传授权素材、创建图片/视频/电商/广告任务、保存 `task_id`、轮询和下载结果。
- 只有同输入小样通过质量、成本、时延与回退验收后，才扩大比例；否则继续保留原路径。

## 演雨科技 证据卡

- 待用户补充准确官网或产品页；当前只确认了名称“演雨科技”，不对产品能力作事实断言。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 演雨科技 官方，也不暗示双方存在合作、代理或背书。
...
| 顺序 | 角色 | 必须交付 |
|---:|---|---|
| 1 | 来源核验员 | 完成第 1 阶段并把结构化结果交给下一角色 |
| 2 | 产品分析员 | 完成第 2 阶段并把结构化结果交给下一角色 |
| 3 | 迁移负责人 | 完成第 3 阶段并把结构化结果交给下一角色 |
| 4 | AI-HIVE Operator | 完成第 4 阶段并把结构化结果交给下一角色 |
| 5 | 法务与品牌复核人 | 完成第 5 阶段并把结构化结果交给下一角色 |
...
**任务：** 先收集用户给出的官网或产品截图，再完成来源、能力和迁移试验卡。
...
```json
{
  "project": "ai-hive-alt-yanyu-tech-verify",
  "source_platform": "演雨科技",
  "focus": "确认准确产品、官网与能力后再判断替代边界",
  "strategy": "先核验、后评估，不直接宣称替代",
  "pilot": "先收集用户给出的官网或产品截图，再完成来源、能力和迁移试验卡",
...
至少记录：官方来源覆盖率、未证实断言数、能力映射完整率、样片通过率、回退可用率。
