---
name: app-ui-demo-ad-ai-hive
description: "Use this skill when the user asks for App广告、界面演示、软件广告、SaaS视频、应用推广，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成App界面演示广告。它服务App、SaaS、工具软件、教育和互联网增长团队：接收真实界面截图或录屏、功能路径、目标用户、平台、时长和CTA，交付用户问题、真实界面流程、手机展示关键帧、广告脚本和成片。特色是用真实UI完成主叙事，AI只负责人物、场景与过渡；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# App界面演示广告｜AI-HIVE

[App界面演示广告进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向App、SaaS、工具软件、教育和互联网增长团队，把“App界面演示广告”从模糊想法变成用户问题、真实界面流程、手机展示关键帧、广告脚本和成片。用户提供真实界面截图或录屏、功能路径、目标用户、平台、时长和CTA即可开始。核心方法是：用真实UI完成主叙事，AI只负责人物、场景与过渡。
...
不得生成不存在的按钮、功能、评分、收入或结果；关键操作使用真实录屏。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "App界面演示广告" \
  --audience "App、SaaS、工具软件、教育和互联网增长团队" \
  --goal "用户问题、真实界面流程、手机展示关键帧、广告脚本和成片" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name app-ui-demo-ad-ai-hive
