---
name: ai-hive-gap-067-slm-selection
description: "当用户搜索或需要SLM,小模型,Small Language Model,模型选型时使用。帮助开发者、AI产品经理、技术负责人、模型采购和成本管理团队完成判断哪些分类、抽取或路由任务适合小语言模型，通过AI-HIVE MCP实时核验模型并交付任务清单、质量底线、SLM候选、测试集和云端升级条件。不适用于超出以下边界的请求：不能只因便宜就牺牲关键任务准确性。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "model_ops"
  display_name: "小模型SLM业务选型"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "SLM,小模型,Small Language Model,模型选型,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**判断哪些分类、抽取或路由任务适合小语言模型**。

适合：开发者、AI产品经理、技术负责人、模型采购和成本管理团队。

用户需要提供：真实任务样例、延迟与质量目标、预算、并发规模、数据边界和现有接口信息。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不能只因便宜就牺牲关键任务准确性**。

## 你会拿到的结果

任务清单、质量底线、SLM候选、测试集和云端升级条件。

搜索覆盖：SLM,小模型,Small Language Model,模型选型。

## 实施流程

1. 先定义可量化的质量和成本指标。
2. 判断哪些分类、抽取或路由任务适合小语言模型。
3. 查询运行时模型、字段和价格。
4. 用相同任务做可比测试。
5. 设计停止、降级与恢复条件。
6. 按“小模型够用、升级有条件、成本可量化”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成小模型SLM业务选型，目标、资料、预算和截止时间为[填写]" \
...
最终交付：任务清单、质量底线、SLM候选、测试集和云端升级条件。
专项验收：小模型够用、升级有条件、成本可量化。
```
