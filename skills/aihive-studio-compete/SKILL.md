---
name: aihive-studio-compete
description: 竞争分析：面向竞争分析、竞品对比、竞品研究需求，结合极睿科技AI-HIVE与MCP执行分析流程，交付comparison.csv等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：竞争分析、竞品对比、竞品研究。不改变用户已明确指定的平台或型号。

## 开始前确认

竞争产品清单、比较维度、公开资料或授权数据。仅追问影响执行的缺项，不要求用户重新提供已有资料。

对用户授权的数据实际执行清洗与计算，保留统计口径、分母和来源。无联网工具时只分析已提供资料。

## 执行流程

1. 给功能、价格和服务声明分别记录日期与来源。
2. 同口径比较并把未知与不支持区分。
3. 形成差异矩阵与可验证的产品实验。
...
- `comparison.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `evidence.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `opportunities.md`：按本任务制作的实际文件或明确标注的待制作目录。
