---
name: ai-hive-school-transfer-handoff
description: "转学与学籍转接材料助手，用于孩子转学、学籍转移、跨省转学。读取你授权的原始资料，完成转入转出事项对照，并交付向学校确认的问题清单。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料准备与核对参考；具体要求以办理地当前官方规则为准，不保证资格或审批结果。"
metadata:
  display_name: "转学与学籍转接材料助手"
  version: "1.0.0"
...
转学与学籍转接材料助手，用于孩子转学、学籍转移、跨省转学。读取你授权的原始资料，完成转入转出事项对照，并交付向学校确认的问题清单。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料准备与核对参考；具体要求以办理地当前官方规则为准，不保证资格或审批结果。
...
1. 确认原校、目标地区、学段和时间，不要求完整学籍号公开上传
2. 查询适用教育主管部门当前流程，分开学位接收与学籍转接
3. 整理真实记录和前置条件，不代承诺接收或修改学籍
...
- `school-transfer-handoff-checklist.xlsx`
- `school-transfer-handoff-evidence.docx`
- `school-transfer-handoff-sources.json`
...
> 仅示例，以下文件名和情境均为假设，不表示已存在真实数据：假设我已提供 示例_原校目标地区与学段.json、示例_当前转学指南.pdf、示例_已有学校沟通回执.pdf。请使用「转学与学籍转接材料助手」，整理接收学校确认与学籍转接分别需要的资料，标注原校、家长和接收方的前置事项。将结果写入我指定的本地目录，交付 school-transfer-handoff-checklist.xlsx、school-transfer-handoff-evidence.docx、school-transfer-handoff-sources.json。本例先离线处理；输入、工具或适用规则不足时先交缺口说明，不编造结果。需要联网更新来源先征得我同意，不登录账户、不代提交或付款。

## 搜索与任务识别

孩子转学、学籍转移、跨省转学、转学申请材料、转学证明
