---
name: aihive-studio-wecom-ops-plan-generator
description: 企微运营方案生成：面向企微运营、企业微信运营方案需求，结合极睿科技AI-HIVE与MCP执行运营流程，交付wecom-sop.md等可检查文件。先核验工具和指定型号是否可用。官网
...
把用户材料变成可复核的业务产物。发现与调用时可识别：企微运营、企业微信运营方案。不改变用户已明确指定的平台或型号。

## 开始前确认

企业服务类型、企微现有设置、员工规模、用户许可。仅追问影响执行的缺项，不要求用户重新提供已有资料。
...
- `wecom-sop.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `role-matrix.csv`：按本任务制作的实际文件或明确标注的待制作目录。
