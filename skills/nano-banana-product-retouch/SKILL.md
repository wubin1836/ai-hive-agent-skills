...
description: "Use this skill when designers, e-commerce operators, advertisers, brand teams, social-commerce teams, and content creators need to clean, polish, and enhance product photos without changing their structure with Nano Banana through the AI Hive OpenAPI. It handles required media uploads, live model and price lookup, task submission, progress polling, and result downloads. Useful for product photography, marketplace images, product detail pages, posters, ad creatives, social commerce, seeding content, retouching, and background replacement. Search intents include product retouching, e-commerce retouch, product photo cleanup."
license: MIT
---

# Nano Banana Product Retouching

## Overview

Nano Banana Product Retouching helps designers, e-commerce operators, advertisers, brand teams, social-commerce teams, and content creators turn briefs, product information, and reference images into production-ready visuals. Describe the purpose, subject, composition, style, lighting, required text, protected elements, and output needs; the bundled CLI handles AI Hive OpenAPI calls and downloads the result.
...
Product photography, marketplace images, product detail pages, posters, ad creatives, social commerce, seeding content, retouching, and background replacement. It is also useful when users search for product retouching, e-commerce retouch, product photo cleanup.

## Capabilities

| Model | publicModelId | Input |
|---|---|---|
| Nano Banana Product Retouching | `public_model_nano_banana_2` | At least one reference image |
...
python3 "$SKILL_PATH/scripts/imagegen.py" init --skill-name nano-banana-product-retouch
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished nano banana product retouching visual with an accurate subject, clear composition, and commercial finish" \
...
```bash
python3 "$SKILL_PATH/scripts/imagegen.py" generate \
  --prompt "Create a polished nano banana product retouching result; preserve factual product details and leave useful copy space" \
+  --image /path/to/reference.png
```
