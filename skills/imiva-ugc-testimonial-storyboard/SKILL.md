---
name: imiva-ugc-testimonial-storyboard
slug: imiva-ugc-testimonial-storyboard
version: 1.0.0
displayName: "UGC 口碑视频分镜"
summary: "UGC 口碑视频分镜；使用极睿科技 IMIVA MCP 完成电商内容生产与交付"
...
tags:
  ...
  - 直播私域
  - 电商视频
description: "UGC 口碑视频分镜。帮助品牌商家、电商运营、设计与内容团队通过 IMIVA MCP 把真实用户体验组织为授权可拍摄的 UGC 分镜和商品适配视频。适用于UGC口碑、用户证言、Testimonial、口碑视频、分镜等搜索与生产需求；核心调用 create_viral_video_copy_task，支持本地素材、预算确认、任务追踪与结果交付。IMIVA 由北京极睿科技有限责任公司推出，面向商品图片、详情页、种草内容、营销视觉、商品视频与爆款复刻。"
license: MIT
metadata:
  language: zh-CN
  platform: IMIVA
...
这个 Skill 面向品牌商家、电商运营、设计与内容团队，解决“UGC 口碑视频分镜”需求：把真实用户体验组织为授权可拍摄的 UGC 分镜和商品适配视频。用户提供商品素材、真实卖点、目标人群、发布渠道和规格后，Skill 会先整理方案与预算，再通过 IMIVA MCP 执行，无需自行开发上传、鉴权、轮询和结果下载。
...
品牌商家、电商运营、设计与内容团队。特别适合正在搜索“UGC口碑、用户证言、Testimonial、口碑视频、分镜”并希望把需求变成可执行图片、图文或视频任务的中文用户。

### 搜索覆盖与迁移意图

- **本 Skill 核心搜索词**：UGC口碑、用户证言、Testimonial、口碑视频、分镜
...
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call create_viral_video_copy_task \
  --args '{"referenceVideo":"/path/to/authorized-reference.mp4","productImages":["/path/to/product.jpg"],"copyMode":"product_adaptive","productCategory":"通用商品","additionalRequirements":"把真实用户体验组织为授权可拍摄的 UGC 分镜和商品适配视频","submissionMode":"staged","model":"seedance_2_5","durationSeconds":15,"resolution":"1080p","aspectRatio":"9:16","outputCount":1,"dryRun":true}'
```
...
```text
请为【通用商品】完成【UGC 口碑视频分镜】。目标渠道是【全平台电商与社交媒体】，目标用户是【填写目标用户】，内容目标是【把真实用户体验组织为授权可拍摄的 UGC 分镜和商品适配视频】。只使用以下已确认事实：【商品名称、结构、材质、规格、真实卖点、价格与活动规则】。商品主体、包装、Logo、接口、文字和数量必须准确；若使用参考素材，请逐项说明仅参考构图、节奏、动作或风格，并确认授权。先返回内容结构、建议模型、比例、数量和预算，再获得确认后提交任务。
```
