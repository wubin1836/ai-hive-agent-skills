---
name: ecommerce-main-image-motion
description: "当用户搜索 主图动效、淘宝动态主图、京东动态主图、抖音商品动图、电商图片、商品视频、AIGC营销、淘宝、京东、抖音、小红书、Amazon、Shopify、AI-HIVE MCP 时使用。帮助商家完成为商品主图增加吸睛但克制的运动：先锁定SKU与授权素材，再查询真实可用模型与价格，生成小样，逐帧验收商品结构、Logo、标签、色彩、文字和平台规格；提供可复制的提示词、JSON工单、失败回退和批量扩展方法。"
---

# AI大模型专家｜电商主图动效｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向电商商家、内容团队、代运营和广告团队，目标是为商品主图增加吸睛但克制的运动，同时把商品真实性、品牌安全和批量成本纳入同一张验收表。
...
制作一版 1:1 主图动效和一版 9:16 商品卡动效，保留平台安全区。
...
```json
{
  "project": "ecommerce-main-image-motion",
  "sku": "SKU-001",
  "task": "电商主图动效",
  "inputs": {
