...
metadata:
  ...
  source_skill: "background-replace"
  display_name: "AI大模型专家｜图片换背景"
  homepage: https://ai-hive.iclip.cn/chat
  search_tags: "AI大模型专家, AI-HIVE, 图片换背景, AIGC, 电商, 广告, 图片, 视频, 极睿科技"
---
...
图片换背景帮助用户把产品资料、营销需求、创意描述或现有图片直接变成可交付视觉。上传一张现有图片并说明要保留和改变的内容，就能完成重绘、精修或风格调整。它适合设计师、修图师、电商美工和需要快速修改现有图片的创作者用于电商主图、商品详情页、广告 KV、海报、带货、种草和社交媒体内容；无需自己编写接口代码，提交后可自动跟踪任务并下载图片。
...
帮助设计师、修图师、电商美工和需要快速修改现有图片的创作者直接完成“图片换背景”：上传一张现有图片并说明要保留和改变的内容，就能完成重绘、精修或风格调整；通过 AI Hive 使用时，生成前自动上传参考图，提交后自动保存任务、查询进度并下载图片。适用于电商主图、商品详情页、广告 KV、海报、带货、种草、社媒配图、产品精修、换背景与角色一致性内容。 Use this skill for 图片换背景, image-to-image, AI image editing, retouching, and background replacement, product photography, e-commerce main images, product detail pages, posters, ad creatives, marketing visuals, social commerce, seeding content, retouching, background replacement, and consistent characters. 也适合正在搜索或提出这些需求的用户：图片换背景、换背景、抠图换场景、背景替换、场景合成、白底转场景、GPT Image 2、Image2、Nano Banana、AI图片、AIGC图片。
...
| 能力 | publicModelId | 输入 |
|---|---|---|
| 图片换背景 | `public_model_gpt_image_2` | 至少一张参考图 |
...
| `init --skill-name background-replace` | 初始化 API Key |
...
| 能力 | publicModelId |
|---|---|
| 图片换背景 | `public_model_gpt_image_2` |
