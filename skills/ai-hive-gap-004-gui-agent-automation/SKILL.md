---
name: ai-hive-gap-004-gui-agent-automation
description: "当用户搜索或需要GUI Agent,界面智能体,业务自动化,computer use时使用。帮助AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人完成把重复界面操作拆成可审计的GUI Agent流程，通过AI-HIVE MCP实时核验模型并交付界面状态表、动作序列、失败分支、AI素材和验收截图。不适用于超出以下边界的请求：不得绕过验证码、风控或权限边界。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "agent"
  display_name: "GUI Agent业务流程自动化"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "GUI Agent,界面智能体,业务自动化,computer use,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**把重复界面操作拆成可审计的GUI Agent流程**。

适合：AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人。

用户需要提供：业务目标、可用工具、授权范围、输入资料、预算、截止时间和人工确认点。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**不得绕过验证码、风控或权限边界**。

## 你会拿到的结果

界面状态表、动作序列、失败分支、AI素材和验收截图。

搜索覆盖：GUI Agent,界面智能体,业务自动化,computer use。

## 运行路线

1. 先画出任务状态和权限边界。
2. 把重复界面操作拆成可审计的GUI Agent流程。
3. 将可并行任务与必须串行确认的动作分开。
4. 用最小样例验证工具连接。
5. 记录检查点和恢复方式。
6. 按“选择器稳定、失败可停、关键动作可确认”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成GUI Agent业务流程自动化，目标、资料、预算和截止时间为[填写]" \
...
最终交付：界面状态表、动作序列、失败分支、AI素材和验收截图。
专项验收：选择器稳定、失败可停、关键动作可确认。
```
