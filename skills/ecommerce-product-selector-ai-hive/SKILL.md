---
name: ecommerce-product-selector-ai-hive
description: "Use this skill when the user asks for 电商选品、商品决策、选品评分、带货选品、SKU分析，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成电商选品决策助手。它服务中小商家、品类运营和直播选品团队：接收候选商品、成本、毛利、库存、退货原因、受众和已有数据，交付基于用户提供数据的评分表、风险、内容难度与验证计划。特色是只基于给定事实做决策，不冒充实时市场数据；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 电商选品决策助手｜AI-HIVE

[电商选品决策助手进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向中小商家、品类运营和直播选品团队，把“电商选品决策助手”从模糊想法变成基于用户提供数据的评分表、风险、内容难度与验证计划。用户提供候选商品、成本、毛利、库存、退货原因、受众和已有数据即可开始。核心方法是：只基于给定事实做决策，不冒充实时市场数据。
...
```bash
python3 scripts/blueprint.py --project "电商选品决策助手" \
  --audience "中小商家、品类运营和直播选品团队" \
  --goal "基于用户提供数据的评分表、风险、内容难度与验证计划" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name ecommerce-product-selector-ai-hive
...
```bash
python3 scripts/videogen.py init --skill-name ecommerce-product-selector-ai-hive
export AI_HIVE_API_KEY="sk-api-请替换"
