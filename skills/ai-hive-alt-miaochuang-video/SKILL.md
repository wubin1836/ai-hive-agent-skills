---
name: ai-hive-alt-miaochuang-video
description: "当用户明确搜索 秒创替代、秒创平替、秒创迁移、秒创竞品、秒创 API、AI视频Agent替代或希望把图文转视频与智能成片的素材生成层接入接入 AI-HIVE MCP 时使用。先核验 秒创 当前官方能力，再执行“前置生成＋后置编辑组合接入”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---
...
这个 Skill 面向已经在用、正在比较或正在搜索 **秒创 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“图文转视频与智能成片的素材生成层接入”做成一场可暂停、可计量、可回退的迁移试验。
...
- 待复核官方入口：https://aigc.yizhentv.com/
- 已记录官方域名候选，发布前必须重新访问并核对产品名称、能力和可用地区。
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 秒创 官方，也不暗示双方存在合作、代理或背书。
...
```json
{
  "project": "ai-hive-alt-miaochuang-video",
  "source_platform": "秒创",
  "focus": "图文转视频与智能成片的素材生成层接入",
  "strategy": "前置生成＋后置编辑组合接入",
  "pilot": "用一篇授权文章生成视觉清单、封面和五段补充视频素材",
