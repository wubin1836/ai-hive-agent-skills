---
name: aihive-gpt6-sol-engineering
description: GPT-6 Sol工程交付助手：面向GPT-6 Sol、GPT6 Sol、GPT Sol需求，结合极睿科技AI-HIVE与MCP执行模型编程流程，交付patch.diff等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: GPT-6 Sol工程交付助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型编程
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：GPT-6 Sol、GPT6 Sol、GPT Sol、Sol编程。不改变用户已明确指定的平台或型号。

## 开始前确认

用户仓库、需求、测试命令、数据范围和预算。仅追问影响执行的缺项，不要求用户重新提供已有资料。

模型生成补丁建议，宿主负责编辑与运行测试。保留用户未提交改动，不安装未知远程代码或部署生产系统。

## 执行流程

1. 核验GPT-6 Sol在当前渠道的ID、推理参数与工具支持。
2. 先跑基线测试再实施局部补丁并记录文件改动。
3. 运行回归与构建，输出结果和剩余问题。
