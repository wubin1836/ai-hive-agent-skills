...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana 制作高完成度直播带货图片，主体准确，构图清晰", "keywords": "Nano Banana、NanoBanana、Nano Banana 2、香蕉模型、Gemini 图片、直播带货图片、直播间背景、带货图、直播贴片、直播商品图、直播视觉、AI图片、AIGC图片", "model": "public_model_nano_banana_2", "name": "nano-banana-livestream-image", "rule": "optional", "search": "固定调用 Nano Banana 的直播带货图片裸接口能力", "title": "Nano Banana 直播带货图片"}')
