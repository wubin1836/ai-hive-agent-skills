---
name: apparel-model-swap-ai-hive
description: "Use this skill when the user asks for 服装换模特、AI模特、电商服装图、虚拟拍摄、商品图，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成AI服装换模特效果图。它服务服装品牌、摄影团队、跨境卖家和商品设计师：接收服装图、授权模特参考或人物条件、背景、比例和平台，交付保留服装的换模特效果图、多姿势版本和商品一致性检查。特色是服装版型和纹理是锚点，只替换人物与场景；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# AI服装换模特效果图｜AI-HIVE

[AI服装换模特效果图进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向服装品牌、摄影团队、跨境卖家和商品设计师，把“AI服装换模特效果图”从模糊想法变成保留服装的换模特效果图、多姿势版本和商品一致性检查。用户提供服装图、授权模特参考或人物条件、背景、比例和平台即可开始。核心方法是：服装版型和纹理是锚点，只替换人物与场景。
...
不得未经授权复制真人；生成结果是营销视觉，需人工检查服装结构、颜色和Logo。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "AI服装换模特效果图" \
  --audience "服装品牌、摄影团队、跨境卖家和商品设计师" \
  --goal "保留服装的换模特效果图、多姿势版本和商品一致性检查" --platform "目标平台" \
...
```bash
python3 scripts/imagegen.py init --skill-name apparel-model-swap-ai-hive
