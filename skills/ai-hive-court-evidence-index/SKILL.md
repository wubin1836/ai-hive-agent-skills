---
name: ai-hive-court-evidence-index
description: "庭审证据目录生成器，用于证据目录、开庭材料、举证材料。读取你授权的原始资料，完成带页码与证明目的的证据目录，并交付原件核对及庭前待办单。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。"
metadata:
  display_name: "庭审证据目录生成器"
  version: "1.0.0"
...
庭审证据目录生成器，用于证据目录、开庭材料、举证材料。读取你授权的原始资料，完成带页码与证明目的的证据目录，并交付原件核对及庭前待办单。可结合AI-HIVE中经核验可用的模型辅助整理，文件处理与校验由宿主工具完成。仅供材料整理和初步核对参考，不替代执业律师意见，不保证诉讼结果，不自动提交或签署。
...
- 授权文件夹读取、页数提取、哈希去重与 OCR
- 附件链接和页码校验，XLSX/DOCX 生成及渲染
- 如需当前法院要求，仅访问用户授权的官方通知或指南

没有专门软件并不意味着中止所有任务；可使用宿主已具备的等效工具。无法完成的步骤明确列出，不能伪装成功或默默改为另一个外部服务。

## 工作流

1. 读取法院通知、双方诉求和用户授权原件，确认已提交与未提交材料范围。
2. 按来源与类型编号，保留原件、取得方式、形成时间和副本关系。
3. 将各项拟证明事实连接到证据页码，分开记录真实性、完整性和关联性的待核实问题。
4. 输出目录及庭前核对单，新的说明不篡改原始材料，不代写虚假证人陈述。
...
- `court-evidence-directory.docx`
- `issue-evidence-matrix.xlsx`
- `originals-and-hearing-checklist.xlsx`
...
拟证明目的不等于法院采信；不保证电子截图的证据效力，不删除不利材料，不教非法取证或偷取他人账户。实际举证安排须按法院通知及时核实。

缺关键原件或专业依据时只完成可验证部分。401/403、限流或内容审核阻断时保存脱敏进度并停止相应调用，遵守明确的等待要求，不循环重试或绕过审核。不因安装本技能自动交易、签署、提交、缴费或公开发布。

## 调用示例

> 仅示例（下列文件名和资料均为假设，不是真实数据）：请读取假设文件庭审通知.pdf、双方诉辩/和证据原件副本/，保留原件，按来源编号并建立争议焦点对应页码，交付 court-evidence-directory.docx、证据矩阵和原件携带核对单。

## 搜索与任务识别

证据目录、开庭材料、举证材料、证据编号、质证准备、聊天证据
