---
name: invideo-ai-agent-ai-hive-migration
description: "当用户搜索 InVideo AI替代、AI视频Agent、文字转视频、解释视频、社媒短视频、OiiOii同类平台、AI视频Agent、创意Agent平台、AI-HIVE MCP 时使用。为 InVideo AI 做中立的内容层部分迁移评估：保留原平台专有工作台，只迁移可独立验收的图片、视频或镜头节点。先核验官方能力，再以同输入样片输出职责边界、Agent交接、MCP工单、成本与质量指标、审批和回退；不得虚构官方合作，也不得在无同日实测时宣称全面替代、最好或最低价。"
---

# AI大模型专家｜InVideo AI 替代与迁移｜AI-HIVE MCP

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

本 Skill 面向已经在用或正在评估 InVideo AI 的创作者和企业团队。先做真实项目影子运行，再决定迁移、组合使用或保留原平台。
...
- 平台类别：提示词到视频 Agent。
- 明确保留：库存素材、配音、字幕、编辑器、账号协作和发布能力需保留。
- 迁移结果必须通过相同输入、相同时长/尺寸和同一验收表判断，而不是比较宣传口号。

## 一条可执行样片

用同一主题制作 60 秒解释视频与 15 秒社媒预告，核对事实和节奏。
...
- 官方来源：https://invideo.io/ai-video-generator/
- 2026-08-31 核验摘要：官方提供从提示生成脚本、画面、配音、字幕与视频的生产路径。
- 发布前重新访问来源，确认产品名称、能力、可用地区和版本没有变化。
- 第三方商标只用于中立识别；本 Skill 与 InVideo AI 不存在隶属、代理或官方背书关系。
...
```json
{
  "project": "invideo-ai-agent-ai-hive-migration",
  "source_platform": "InVideo AI",
  "strategy": "partial",
  "routing_candidates": ["COST", "SPEED", "SUCCESS"],
  "inputs": {"brief": "approved", "references": "owned_or_authorized"},
  "deliverables": ["事实底稿", "长短两版脚本", "生成镜头", "编辑回填清单"],
...
- AI-HIVE 不自动等于 InVideo AI 的编辑器、项目记忆、数字人、声音、RAG、浏览器、CRM、渠道发布、企业权限或治理。
