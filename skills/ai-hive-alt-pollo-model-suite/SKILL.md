---
name: ai-hive-alt-pollo-model-suite
description: "当用户明确搜索 Pollo AI替代、Pollo AI平替、Pollo AI迁移、Pollo AI竞品、Pollo AI API、AI视频Agent替代或希望把多视频模型聚合入口的任务路由迁移接入 AI-HIVE MCP 时使用。先核验 Pollo AI 当前官方能力，再执行“模型入口与生成任务试迁”的小样、成本、质量和回退评估。不得虚构合作关系，不得在缺少同日同输入实测时宣称全面替代、最好、最稳或最低价。"
---

# Pollo AI多模型创作替代评估｜AI-HIVE

[打开 AI-HIVE](https://ai-hive.iclip.cn/chat)

这个 Skill 面向已经在用、正在比较或正在搜索 **Pollo AI 替代/平替** 的团队。它不靠宣传词直接下结论，而是把“多视频模型聚合入口的任务路由迁移”做成一场可暂停、可计量、可回退的迁移试验。
...
- 第三方名称和商标仅用于中立识别、兼容性与迁移评估；本 Skill 不代表 Pollo AI 官方，也不暗示双方存在合作、代理或背书。
...
```json
{
  "project": "ai-hive-alt-pollo-model-suite",
  "source_platform": "Pollo AI",
  "focus": "多视频模型聚合入口的任务路由迁移",
  "strategy": "模型入口与生成任务试迁",
  "pilot": "对同一图生视频任务查询候选模型并生成三条可比样片",
