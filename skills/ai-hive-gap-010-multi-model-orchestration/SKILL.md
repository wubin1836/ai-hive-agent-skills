---
name: ai-hive-gap-010-multi-model-orchestration
description: "当用户搜索或需要多模型编排,Multi-model Orchestration,模型协作,内容流水线时使用。帮助AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人完成按任务阶段在图片、视频和文本能力间做实时编排，通过AI-HIVE MCP实时核验模型并交付任务DAG、模型候选、成本快照、降级顺序和交付包。不适用于超出以下边界的请求：模型可用性和价格必须实时查询。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "agent"
  display_name: "多模型编排内容生产线"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "多模型编排,Multi-model Orchestration,模型协作,内容流水线,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**按任务阶段在图片、视频和文本能力间做实时编排**。

适合：AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人。

用户需要提供：业务目标、可用工具、授权范围、输入资料、预算、截止时间和人工确认点。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**模型可用性和价格必须实时查询**。

## 你会拿到的结果

任务DAG、模型候选、成本快照、降级顺序和交付包。

搜索覆盖：多模型编排,Multi-model Orchestration,模型协作,内容流水线。

## 工作方法

1. 先画出任务状态和权限边界。
2. 按任务阶段在图片、视频和文本能力间做实时编排。
3. 将可并行任务与必须串行确认的动作分开。
4. 用最小样例验证工具连接。
5. 记录检查点和恢复方式。
6. 按“每步模型适配、预算可控、失败可切换”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成多模型编排内容生产线，目标、资料、预算和截止时间为[填写]" \
...
最终交付：任务DAG、模型候选、成本快照、降级顺序和交付包。
专项验收：每步模型适配、预算可控、失败可切换。
```
