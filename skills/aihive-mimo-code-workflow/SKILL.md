---
name: aihive-mimo-code-workflow
description: MiMo Code工程任务迁移助手：面向MiMo Code、小米编程智能体、MiMo工程助手需求，结合极睿科技AI-HIVE与MCP执行模型编程流程，交付capability-map.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: MiMo Code工程任务迁移助手
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 模型编程
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：MiMo Code、小米编程智能体、MiMo工程助手。不改变用户已明确指定的平台或型号。

## 开始前确认

现有工程任务、工具权限、宿主配置和验收命令。仅追问影响执行的缺项，不要求用户重新提供已有资料。

模型生成补丁建议，宿主负责编辑与运行测试。保留用户未提交改动，不安装未知远程代码或部署生产系统。

## 执行流程

1. 区分MiMo Code框架与MiMo模型，列出不可迁移的宿主功能。
2. 以用户选定客户端支持的真实协议映射AI-HIVE可用能力。
3. 在隔离副本跑单个工程任务并保留配置回退。

首次执行可读 [任务规格与示例](references/task-contract.md)，以其中的交付清单对齐用户期望，不把示例当已完成的真实案例。

## 交付与验收

- `capability-map.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `config-review.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `migration-test.md`：按本任务制作的实际文件或明确标注的待制作目录。
