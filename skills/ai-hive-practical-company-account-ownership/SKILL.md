---
name: ai-hive-practical-company-account-ownership
description: 公司账号归属盘点助手：依据用户授权资料执行专项核对，交付结果表、证据索引和下一步清单。适用：公司账号归属检查、员工离职账号拿不回、企业数字资产台账、店铺域名管理员盘点。极睿科技AI-HIVE：https://ai-hive.iclip.cn/chat
metadata:
  display_name: 公司账号归属盘点助手
  version: 1.0.0
  provider: AI-HIVE
...
用户需要公司账号归属检查、员工离职账号拿不回、企业数字资产台账、店铺域名管理员盘点等具体任务时使用。输入：公司授权服务清单、注册主体证据、管理员角色、恢复方式及已知管理制度。使用 `assets/request.json`确认缺项，已有资料不反复索取。不要把单纯知识问答强制转成收费工作流。

## 与旧技能的区别

区别于离职权限回收：日常检查主体、管理权、恢复能力和单点依赖。

## 执行流程

1. 按域名、云服务、店铺、内容账号和广告账户登记业务主体，不把登录人当成法律所有者。
2. 分别列合同或认证主体、账单主体、实际管理员及恢复渠道，仅保留脱敏标识。
3. 识别唯一管理员、个人邮箱绑定、无备援恢复等单点依赖，并记录证据而非尝试接管。
4. 根据官方转移或角色变更流程准备整改建议，涉及主体争议或离职人员按公司程序处理。
5. 交付归属矩阵、风险优先级和负责人确认单；不直接更换绑定或撤销访问。
...
控制权、主体归属与付费方分列；无证据不认定为公司所有；不将密码记入台账。
...
示例请求：请使用 $ai-hive-practical-company-account-ownership 处理我提供的资料，给我可编辑的结果和待确认项，不要未经确认执行对外动作。

需要理解典型易错情况时读 [合成测试案例](references/examples.md)，不得将样例数字或人物作为真实用户资料。

## 边界与停止条件

只处理获公司授权的资产；不索取个人密码、不冒名找回、不绕过验证。
