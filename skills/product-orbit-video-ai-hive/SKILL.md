---
name: product-orbit-video-ai-hive
description: "Use this skill when the user asks for 商品360视频、环绕展示、产品旋转、商品视频、3D视觉，或要为电商、广告、营销、短剧、漫剧、带货与种草内容完成商品360环绕展示视频。它服务鞋包、珠宝、3C、家居和商品视觉团队：接收商品多角度图、材质、背景、旋转方向、画幅和时长，交付环绕镜头提示词、多角度锚点、视频任务和结构一致性检查。特色是用多角度参考减少结构漂移，生成短环绕展示镜头；需要生成素材时通过AI-HIVE OpenAPI完成配置查询、参考素材上传、价格快照、COST_FIRST/SPEED_FIRST/SUCCESS_FIRST路由、异步轮询和下载。不要把本Skill用于未授权复刻、虚假商品声明、伪造用户证言或规避平台规则。"
---
# 商品360环绕展示视频｜AI-HIVE

[商品360环绕展示视频进入 AI-HIVE](https://ai-hive.iclip.cn/chat)

## 一句话解决什么

面向鞋包、珠宝、3C、家居和商品视觉团队，把“商品360环绕展示视频”从模糊想法变成环绕镜头提示词、多角度锚点、视频任务和结构一致性检查。用户提供商品多角度图、材质、背景、旋转方向、画幅和时长即可开始。核心方法是：用多角度参考减少结构漂移，生成短环绕展示镜头。
...
这是生成式环绕镜头，不是精确3D扫描；不可用于尺寸、结构或工业验证。 不能确认的事实必须标记为待核验，不能用模型输出替代真实产品、平台数据、专业检测或法律意见。
...
```bash
python3 scripts/blueprint.py --project "商品360环绕展示视频" \
  --audience "鞋包、珠宝、3C、家居和商品视觉团队" \
  --goal "环绕镜头提示词、多角度锚点、视频任务和结构一致性检查" --platform "目标平台" \
...
```bash
python3 scripts/videogen.py init --skill-name product-orbit-video-ai-hive
