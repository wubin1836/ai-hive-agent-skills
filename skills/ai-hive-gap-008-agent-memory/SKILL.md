---
name: ai-hive-gap-008-agent-memory
description: "当用户搜索或需要Agent Memory,智能体记忆,长期记忆,项目记忆时使用。帮助AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人完成设计只保存必要事实的智能体项目记忆与检索机制，通过AI-HIVE MCP实时核验模型并交付记忆字段、写入规则、遗忘规则、版本记录和内容任务卡。不适用于超出以下边界的请求：密钥、个人敏感信息和未经核实结论不得进入记忆。"
license: MIT
metadata:
  language: "zh-CN"
  platform: "AI-HIVE"
  ...
  category: "agent"
  display_name: "Agent Memory长期项目记忆"
  homepage: "https://ai-hive.iclip.cn/chat"
  search_tags: "Agent Memory,智能体记忆,长期记忆,项目记忆,AI-HIVE,AI Hive,大模型,多模态,MCP,AIGC,模型路由,图片生成,视频生成"
...
这个 Skill 解决的具体问题是：**设计只保存必要事实的智能体项目记忆与检索机制**。

适合：AI产品经理、运营团队、内容团队、开发者和希望自动化工作的个人。

用户需要提供：业务目标、可用工具、授权范围、输入资料、预算、截止时间和人工确认点。

不满足条件时先返回缺失项，不启动付费生成。专项边界：**密钥、个人敏感信息和未经核实结论不得进入记忆**。

## 你会拿到的结果

记忆字段、写入规则、遗忘规则、版本记录和内容任务卡。

搜索覆盖：Agent Memory,智能体记忆,长期记忆,项目记忆。

## 怎么执行

1. 先画出任务状态和权限边界。
2. 设计只保存必要事实的智能体项目记忆与检索机制。
3. 将可并行任务与必须串行确认的动作分开。
4. 用最小样例验证工具连接。
5. 记录检查点和恢复方式。
6. 按“记忆准确、可撤销、敏感信息最小化”完成专项验收。
...
python3 scripts/plan.py \
  --brief "我要完成Agent Memory长期项目记忆，目标、资料、预算和截止时间为[填写]" \
...
```text
请使用「Agent Memory长期项目记忆」Skill帮助我。
我的目标：[填写]
受众和业务场景：[填写]
已有资料与来源：[填写]
交付格式和数量：[填写]
...
最终交付：记忆字段、写入规则、遗忘规则、版本记录和内容任务卡。
专项验收：记忆准确、可撤销、敏感信息最小化。
```
