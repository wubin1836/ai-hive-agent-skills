---
name: aihive-studio-indie-game-ost-pack
description: 游戏配乐·独立游戏原声带：面向游戏配乐、游戏原声带、循环BGM需求，结合极睿科技AI-HIVE与MCP执行音频流程，交付music-state-map.csv等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 游戏配乐·独立游戏原声带
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 音频
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：游戏配乐、游戏原声带、循环BGM。不改变用户已明确指定的平台或型号。

## 开始前确认

游戏场景、状态切换、时长、循环点和音乐版权要求。仅追问影响执行的缺项，不要求用户重新提供已有资料。

核验TTS、ASR、音乐生成或音频编辑的具体能力，不能互相替代；缺工具时交付稿件，不伪造音频文件。

## 执行流程

1. 给菜单、探索、战斗和结算定义不同情绪层。
2. 设计循环与状态转换片段并核验音乐工具。
3. 导出循环版与尾音版并测试首尾拼接。
...
- `music-state-map.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `loop-cues.csv`：按本任务制作的实际文件或明确标注的待制作目录。
- `ost/`：按本任务制作的实际文件或明确标注的待制作目录。
