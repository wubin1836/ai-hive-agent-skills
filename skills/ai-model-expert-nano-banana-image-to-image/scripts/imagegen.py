...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana 制作高完成度图生图，主体准确，构图清晰", "keywords": "Nano Banana、NanoBanana、Nano Banana 2、香蕉模型、Gemini 图片、图生图、图片生成图片、以图生图、image to image、I2I、参考图重绘、AI图片、AIGC图片", "model": "public_model_nano_banana_2", "name": "nano-banana-image-to-image", "rule": "image", "search": "固定调用 Nano Banana 的图生图裸接口能力", "title": "Nano Banana 图生图"}')
