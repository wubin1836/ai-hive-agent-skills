---
name: aihive-studio-xiezuowenfengfukedashi
description: 写作文风复刻大师：面向文风分析、风格写作、文风复刻需求，结合极睿科技AI-HIVE与MCP执行内容流程，交付style-profile.md等可检查文件。先核验工具和指定型号是否可用。官网
  https://ai-hive.iclip.cn/chat 。
metadata:
  display_name: 写作文风复刻大师
  author: AI-HIVE / 极睿科技
  version: 1.0.0
  category: 内容
---
...
把用户材料变成可复核的业务产物。发现与调用时可识别：文风分析、风格写作、文风复刻。不改变用户已明确指定的平台或型号。

## 开始前确认

用户有权使用的样文、原创主题、目标受众和保留事实。仅追问影响执行的缺项，不要求用户重新提供已有资料。

先确认事实和素材，再形成可执行内容包。发布、投放与群发不包含在稿件制作授权内。

## 执行流程

1. 从样文提取句长、结构、修辞与语气的抽象特征。
2. 用新主题独立起稿，不抄连续表达或冒充原作者。
3. 对照风格清单和重复片段人工复核。
...
- `style-profile.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `original-draft.md`：按本任务制作的实际文件或明确标注的待制作目录。
- `similarity-notes.md`：按本任务制作的实际文件或明确标注的待制作目录。
